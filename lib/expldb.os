	...
	Если СписокФайлов.Количество() Тогда
		Для каждого элФайл Из СписокФайлов Цикл
			...
			Иначе
				Узел = Данные.НовыйСоседний(Узел, стрУзел);
			КонецЕсли;
			Данные.НовыйАтрибут(Узел, Новый Структура("Имя, Значение", "Размер", элФайл.Размер()));
			Данные.НовыйАтрибут(Узел, Новый Структура("Имя, Значение", "ВремяИзменения", элФайл.ПолучитьВремяИзменения()));
		КонецЦикла;
	КонецЕсли;
	Возврат родУзел;
КонецФункции

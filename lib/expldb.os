...
Функция СписокБаз(Данные, Параметр) Экспорт
	Узел = Данные.НовыйУзел(Новый Структура("Имя", "Узел"), Истина);
	родУзел = Узел;
	...
	Если СписокБаз.Количество() Тогда
		Для каждого БазаДанных Из СписокБаз Цикл
			стрУзел = Новый Структура("Имя, Значение", "Строка", БазаДанных.ИмяБезРасширения, Истина);
...
Функция СписокДанных(Данные, Параметр) Экспорт
	...
	Для н = 1 По Индекс.КоличествоСтрок() Цикл
		...
		Иначе
			Узел = Данные.НовыйСоседний(Узел, стрУзел, Истина);
		КонецЕсли;
		Данные.НовыйАтрибут(Узел, Новый Структура("Имя, Значение", "dataposition", нУзел.dataposition), Истина);
		Данные.НовыйАтрибут(Узел, Новый Структура("Имя, Значение", "date", Данные.УзелСвойство(нУзел, "date")), Истина);
	КонецЦикла;
	Возврат родУзел;
КонецФункции

...
Процедура ОбработатьОтвет(Действие, Данные, Свойства, Результат) Экспорт
	...
	ИначеЕсли Действие = "Элемент" Тогда

		Свойства.д.Номер.Значение = Результат.Строка;
		Результат.Удалить("Элемент");
		Свойства.Вставить("Элементы", Результат);
...
Функция Элемент_Свойства(Данные, оУзел) Экспорт
	...
	Запрос = Новый Структура("Библиотека, Данные, Параметры, Свойства, мДействие, cmd", ЭтотОбъект, Данные, Новый Структура("Позиция, База", Свойства.д.Позиция.Значение, Свойства.д.База.Значение), Свойства, "Элемент", "Морфология");
...
Функция Элемент_Модель(Данные, Свойства, Изменения) Экспорт
	...
	Если Свойства.д.Показать.Значение = "Да" Тогда
		Если Свойства.Свойство("Элементы") Тогда
			...
			зн = "";
			н = 1;
			Пока Свойства.Элементы.Свойство("эл_" + н, зн) Цикл
				...
				Если НЕ ЗначениеКнопка = Неопределено Тогда
					...
					ИначеЕсли ЗначениеКнопка = "token" Тогда
						...
						Если НЕ вТокен = сУзел Тогда
							Если НЕ вСвязь = "" Тогда // И НЕ вТокен.Значение = "" И НЕ сУзел.Значение = "" Тогда
								// добавить связь
								фУзел1 = Формы.с.Получить(ВРег(вТокен.Имя)).Дочерний;
								Пока НЕ фУзел1 = Неопределено Цикл // варианты форм
									Если вТокен.Значение = "" Тогда
										Если НЕ фУзел1.Соседний = Неопределено Тогда // не одна форма
											фУзел1 = Неопределено;
										КонецЕсли;
										Прервать;
									ИначеЕсли фУзел1.Значение = вТокен.Значение Тогда // форма известна
										Прервать;
									КонецЕсли;
								...
								Пока НЕ фУзел2 = Неопределено Цикл // варианты форм
									Если сУзел.Значение = "" Тогда
										Если НЕ фУзел2.Соседний = Неопределено Тогда // не одна форма
											фУзел2 = Неопределено;
										КонецЕсли;
										Прервать;
									ИначеЕсли фУзел2.Значение = сУзел.Значение Тогда // форма известна
										Прервать;
									КонецЕсли;
					...
					ИначеЕсли ЗначениеКнопка = "conf" ИЛИ ЗначениеКнопка = "excl" ИЛИ ЗначениеКнопка = "del"  ИЛИ ЗначениеКнопка = "delgr" Тогда
						...
						Запрос = Новый Структура("Библиотека, Данные, Параметры, Свойства, мДействие, cmd", ЭтотОбъект, Данные, Новый Структура("Слова", св), Свойства, "Связи", "Морфология");
						Данные.Процесс.НоваяЗадача(Запрос, "Служебный");
	...
	Если НЕ УзелСвойство(оУзел, "Обновить") = Ложь Тогда
		...
		Если Свойства.д.Разбор.Значение = "Нет" И Свойства.д.Открыть.Значение = "Да" Тогда
			Если Свойства.д.Свойство("Текст") Тогда
				...
				КонецЦикла;
				Если НЕ мсл = "" Тогда
					Запрос = Новый Структура("Библиотека, Данные, Параметры, Свойства, мДействие, cmd", ЭтотОбъект, Данные, Новый Структура("Слова", Сред(мсл, 2)), Свойства, "ФормыСлов", "Морфология");
					Данные.Процесс.НоваяЗадача(Запрос, "Служебный");
		...
		Если Свойства.д.Разбор.Значение = "Токены" Тогда
			...
			Если прГрамматики = Неопределено Тогда // проверим токены с дефисом
				ток1 = Свойства.д.Токены.Дочерний;
				Пока НЕ ток1 = Неопределено Цикл
					д = стрНайти(ток1.Имя, "-");
					Если НЕ д = 0 Тогда // есть дефис
						ток1Формы = Формы.с.Получить(ВРег(ток1.Имя));
						Если ток1Формы.Дочерний = Неопределено Тогда // нет формы с дефисом
			...
			Если НЕ Грамматики = "" Тогда
				Запрос = Новый Структура("Библиотека, Данные, Параметры, Свойства, мДействие, cmd", ЭтотОбъект, Данные, Новый Структура("Слова", Сред(Грамматики, 2)), Свойства, "Грамматики", "Морфология");
				Данные.Процесс.НоваяЗадача(Запрос, "Служебный");
			Иначе

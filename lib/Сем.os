...
Функция Предложение_Модель(Данные, Свойства, Изменения) Экспорт
	...
	Если НЕ УзелСвойство(оУзел, "Обновить") = Ложь Тогда
		...
		Вид = "";
		Если НЕ Свойства.д.Разбор.Значение = "Нет" И Свойства.д.Открыть.Значение = "Да" Тогда
	...
	Пока НЕ дУзел = Неопределено Цикл
		Если НЕ "" + дУзел.Имя = "" Тогда
			Если "" + дУзел.Значение = "" Тогда
				Связь = "<button id='_" + дУзел.Код + "' type='button' class='text-left btn1 btn-light' onclick=""addcmd(this,event); return false"" role='token'>" + дУзел.Имя + "</button>";

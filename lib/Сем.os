			...
			Если тСобытие = "ПриНажатии" Тогда
				...
				Если НЕ ЗначениеКнопка = Неопределено Тогда
					...
					ИначеЕсли ЗначениеКнопка = "token" ИЛИ ЗначениеКнопка = "optoken" Тогда
						...
						Если НЕ вТокен = сУзел Тогда
							Если НЕ вСвязь = "" Тогда
								...
								Данные.НовыйАтрибут(нУзел, ИмяЗначение(сУзел.код, п2.Код));
					...
					ИначеЕсли ЗначениеКнопка = "conf" ИЛИ ЗначениеКнопка = "excl" Тогда
						...
						Биграммы = сУзел.п.ток1имя + Символы.Таб + сУзел.п.ток1нач + Символы.Таб + сУзел.п.ток2имя + Символы.Таб + сУзел.п.ток2нач + Символы.Таб + ?(ЗначениеКнопка = "conf", "+", "-");
									...
									Если НЕ ток2Формы = Неопределено Тогда
										...
										Пока НЕ фУзел2 = Неопределено Цикл
											...
											Пока Истина Цикл
												...
												//Данные.НовыйАтрибут(нп, ИмяЗначение(ток2.код, п2.код));
												и2 = и2 + 1;
											КонецЦикла;
											фУзел2 = фУзел2.Соседний;
										КонецЦикла;
									...
									Если Лев(св.п.ток2гр, стрДлина(элРечи)) = элРечи Тогда
										Если св.п.ток2код = ток1.Код Тогда
											...
											Если р = "+" Тогда // корректная биграмма
												...
												//Данные.НовыйАтрибут(ток1, ИмяЗначение(ток2.Код, ток2.Имя));
										 	КонецЕсли;
									 	КонецЕсли;
									КонецЕсли;
									св = св.Соседний;
...
Функция ТокенВид(Данные, Свойства, Знач дУзел, мУзел = Неопределено, рУзел = Неопределено)
	...
	Пока НЕ дУзел = Неопределено Цикл
		...
		Если дУзел.Родитель = Свойства.д.Токены Тогда
			...
			Если (вТокен = "" И дУзел.Атрибут = Неопределено) ИЛИ вТокен = дУзел Тогда
			...
			Если НЕ дУзел.п.б = "-" Тогда // связь не некорректная
				...
				Если НЕ тУзел = Неопределено Тогда
					Если мУзел.Найти(тУзел) = Неопределено Тогда
						...
						Связь = "<button id='_" + дУзел.Код + "' type='button' class='text-left btn1 btn-secondary' onclick='addcmd(this,event); return false' role='rule_" + дУзел.п.п2код + "'>" + дУзел.Имя + "</button>";

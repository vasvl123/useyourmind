...
Процедура ОбработатьОтвет(Действие, Данные, Свойства, Результат) Экспорт
	...
	ИначеЕсли Действие = "Биграммы" Тогда
		Свойства.Вставить("Биграммы", Результат);
	...
	Если НЕ УзелСвойство(оУзел, "Обновить") = Ложь Тогда
									...
									Если НЕ ток2Формы = Неопределено Тогда
										...
										Пока НЕ фУзел2 = Неопределено Цикл
											...
											Пока Истина Цикл
												...
												Биграммы = Биграммы + Символы.ПС + фУзел1.Имя + Символы.Таб + ?(фУзел1.Атрибут = Неопределено, фУзел1.Имя, фУзел1.Атрибут.Имя) + Символы.Таб + фУзел2.Имя + Символы.Таб + ?(фУзел2.Атрибут = Неопределено, фУзел2.Имя, фУзел2.Атрибут.Имя);
		...
		Если Свойства.д.Разбор.Значение = "Привязать" Тогда
			...
			Для каждого элРечи Из чРечи Цикл
				...
				т1 = 1;
				Пока НЕ ток1 = Неопределено Цикл
					Если ток1.Атрибут = Неопределено Тогда
						т2 = 1;
						ток2 = Свойства.д.Токены.Дочерний;
						Пока НЕ ток2 = Неопределено Цикл
							Если НЕ ток1 = ток2 Тогда
								...
								Пока НЕ св = Неопределено Цикл
									пУзел = пДанные.ПолучитьУзел(св.Атрибут.Значение); // правило
									Если НЕ пУзел = Неопределено Тогда
										Если Лев(пУзел.Имя, стрДлина(элРечи)) = элРечи Тогда
											Если св.Атрибут.Имя = ток1.Код Тогда
		...
		Иначе
			...
			Если Верно Тогда
				...
				Пока НЕ тУзел = Неопределено Цикл // Проверить привязки
					Если тУзел.Код = дУзел.Атрибут.Имя Тогда // связанный токен
						аУзел = тУзел.Атрибут;
						Пока НЕ аУзел = Неопределено Цикл
							Если аУзел.Имя = дУзел.Родитель.Код Тогда // привязка соответствует
								Прервать;
							КонецЕсли;
							аУзел = аУзел.Соседний;
						КонецЦикла;
						Если НЕ аУзел = Неопределено Тогда
							Прервать;
						КонецЕсли;
					КонецЕсли;
					тУзел = тУзел.Соседний;

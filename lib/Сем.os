...
Процедура ОбработатьОтвет(Действие, Данные, Свойства, Результат) Экспорт
	...
	ИначеЕсли Действие = "Элемент" Тогда
		...
		Свойства.д.Номер.Значение = Результат.эл;
		Результат.Удалить("эл");
...
Функция Справочник_Свойства(Данные, оУзел) Экспорт
	...
	Запрос = Новый Структура("Библиотека, Данные, Слова, Свойства, мДействие, cmd", ЭтотОбъект, Данные, Свойства.д.Позиция.Значение, Свойства, "Элемент", "Морфология");
	...
	Если Свойства.д.Показать.Значение = "Да" Тогда
		Если Свойства.Свойство("Элементы") Тогда
			...
			Для каждого эл Из Свойства.Элементы Цикл
				д = Данные.НовыйСоседний(д, ИмяЗначение("О", "Сем.Справочник"), Истина);
				Данные.НовыйАтрибут(д, ИмяЗначение("Позиция", эл.Значение), Истина);
			КонецЦикла;

...
Процедура ОбработатьОтвет(Действие, Данные, Свойства, Результат) Экспорт
	...
	ИначеЕсли Действие = "Грамматики" Тогда
		...
		Для каждого б Из Результат Цикл
			...
			Свойства.Грамматики.Вставить(мб[0], мб[1]);
		КонецЦикла;

	ИначеЕсли Действие = "Биграммы" Тогда
		Если НЕ Свойства.Свойство("Биграммы") Тогда
			Свойства.Вставить("Биграммы", Новый Структура);
		КонецЕсли;
		Для каждого б Из Результат Цикл
			Свойства.Биграммы.Вставить(б.Ключ, б.Значение);
		КонецЦикла;
...
Функция Предложение_Модель(Данные, Свойства, Изменения) Экспорт
			...
			Если тСобытие = "ПриНажатии" Тогда
				...
				Если НЕ ЗначениеКнопка = Неопределено Тогда
					...
					ИначеЕсли ЗначениеКнопка = "conf" ИЛИ ЗначениеКнопка = "excl" Тогда
						...
						Биграммы = 	сУзел.п.ток1имя + Символы.Таб +
	...
	Если НЕ УзелСвойство(оУзел, "Обновить") = Ложь Тогда
		...
		Если Свойства.д.Разбор.Значение = "Токены" Тогда
							...
							Если НЕ ток1 = ток2 Тогда
								...
								Если НЕ ток2Формы = Неопределено Тогда
									...
									Пока НЕ фУзел2 = Неопределено Цикл
										...
										гр = фУзел1.Значение + Символы.Таб + фУзел2.Значение;

										Если прГрамматики = Неопределено Тогда
											Грамматики = Грамматики + Символы.ПС + гр;

										ИначеЕсли НЕ прГрамматики.Получить(гр) = Неопределено Тогда
											...
											нп = Данные.НовыйДочерний(ток1, ИмяЗначение(прГрамматики.Получить(гр), ток2.Имя), , Истина);
											нп.Вставить("п", Новый Структура); // параметры
											...
											нп.п.Вставить("б", "");
											Биграммы = Биграммы + Символы.ПС + нп.п.ток1имя + Символы.Таб + нп.п.ток1нач + Символы.Таб + нп.п.ток2имя + Символы.Таб + нп.п.ток2нач;
			...
			Если НЕ Биграммы = "" Тогда
				Запрос = Новый Структура("Библиотека, Данные, Слова, Свойства, cmd", ЭтотОбъект, Данные, Сред(Биграммы, 2), Свойства, "Биграммы");
				Данные.Процесс.НоваяЗадача(Запрос, "Служебный");
				Свойства.д.Разбор.Значение = "Биграммы";
			ИначеЕсли НЕ Грамматики = "" Тогда
				Запрос = Новый Структура("Библиотека, Данные, Слова, Свойства, cmd", ЭтотОбъект, Данные, Сред(Грамматики, 2), Свойства, "Грамматики");
				Данные.Процесс.НоваяЗадача(Запрос, "Служебный");
			Иначе
								...
								Пока НЕ св = Неопределено Цикл
									Если Лев(св.п.ток2гр, стрДлина(элРечи)) = элРечи Тогда
										Если св.п.ток2код = ток1.Код Тогда
											р = УзелСвойство(Биграммы, св.п.ток1имя + "_" + св.п.ток2имя);
											св.п.Вставить("б", р);
											Если р = "+" Тогда // корректная биграмма
												ток1.п.Вставить("ток2код", ток2.код);
...
Функция ТокенВид(Данные, Свойства, Знач дУзел, мУзел = Неопределено, рУзел = Неопределено)
	...
	Пока НЕ дУзел = Неопределено Цикл
		...
		Если дУзел.Родитель = Свойства.д.Токены Тогда
			...
			Если вТокен = "" Тогда
				Если дУзел.Свойство("п") Тогда
					Если дУзел.п.Свойство("ток2код") Тогда
		...
		Иначе
			...
			// КонецЕсли;
			// Если Верно Тогда
			Если НЕ дУзел.п.б = "-" Тогда // связь не некорректная
				тУзел = Данные.ПолучитьУзел(дУзел.п.ток2код);
				Если дУзел.п.б = "+" Тогда
				Иначе
					Если тУзел.п.Свойство("ток2код") Тогда
						Если НЕ тУзел.п.ток2код = дУзел.Код Тогда // не тот токен привязан
							тУзел = Неопределено;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				Если НЕ тУзел = Неопределено Тогда
					Если мУзел.Найти(тУзел) = Неопределено Тогда
						...
						Связь = "<button id='_" + дУзел.Код + "' type='button' class='text-left btn1 btn-secondary' onclick='addcmd(this,event); return false' role='optoken'>" + дУзел.Имя + "</button>";

...
Функция Элемент_Свойства(Данные, оУзел) Экспорт
	...
	Запрос = Новый Структура("Библиотека, Данные, Параметры, Свойства, cmd", ЭтотОбъект, Данные, Новый Структура("Позиция, База, Действие", Свойства.д.Позиция.Значение, Свойства.д.База.Значение, "Элемент"), Свойства, "Морфология");
			...
			Если тСобытие = "ПриНажатии" Тогда
				...
				Если НЕ ЗначениеКнопка = Неопределено Тогда
					...
					ИначеЕсли ЗначениеКнопка = "conf" ИЛИ ЗначениеКнопка = "excl" ИЛИ ЗначениеКнопка = "del"  ИЛИ ЗначениеКнопка = "delgr" Тогда
						...
						Запрос = Новый Структура("Библиотека, Данные, Параметры, Свойства, cmd", ЭтотОбъект, Данные, Новый Структура("Слова, Действие", св, "Связи"), Свойства, "Морфология");
						Данные.Процесс.НоваяЗадача(Запрос, "Служебный");
	...
	Если НЕ УзелСвойство(оУзел, "Обновить") = Ложь Тогда
		...
		Если Свойства.д.Разбор.Значение = "Нет" И Свойства.д.Открыть.Значение = "Да" Тогда
			Если Свойства.д.Свойство("Текст") Тогда
				...
				КонецЦикла;
				Если НЕ мсл = "" Тогда
					Запрос = Новый Структура("Библиотека, Данные, Параметры, Свойства, cmd", ЭтотОбъект, Данные, Новый Структура("Слова, Действие", Сред(мсл, 2), "ФормыСлов"), Свойства, "Морфология");
					Данные.Процесс.НоваяЗадача(Запрос, "Служебный");
		...
		Если Свойства.д.Разбор.Значение = "Токены" Тогда
			...
			Если НЕ Грамматики = "" Тогда
				Запрос = Новый Структура("Библиотека, Данные, Параметры, Свойства, cmd", ЭтотОбъект, Данные, Новый Структура("Слова, Действие", Сред(Грамматики, 2), "Грамматики"), Свойства, "Морфология");
				Данные.Процесс.НоваяЗадача(Запрос, "Служебный");
			Иначе

			...
			Если тСобытие = "ПриНажатии" Тогда
				...
				Если НЕ ЗначениеКнопка = Неопределено Тогда
					...
					ИначеЕсли ЗначениеКнопка = "conf" ИЛИ ЗначениеКнопка = "excl" ИЛИ ЗначениеКнопка = "del"  ИЛИ ЗначениеКнопка = "delgr" Тогда
						...
						Запрос = Новый Структура("Библиотека, Данные, Параметры, Свойства, вДействие, cmd", ЭтотОбъект, Данные, Новый Структура("Слова", св), Свойства, "Связи", "Морфология");
						Данные.Процесс.НоваяЗадача(Запрос, "Служебный");
	...
	Если НЕ УзелСвойство(оУзел, "Обновить") = Ложь Тогда
		...
		Если Свойства.д.Разбор.Значение = "Нет" И Свойства.д.Открыть.Значение = "Да" Тогда
			Если Свойства.д.Свойство("Текст") Тогда
				...
				КонецЦикла;
				Если НЕ мсл = "" Тогда
					Запрос = Новый Структура("Библиотека, Данные, Параметры, Свойства, вДействие, cmd", ЭтотОбъект, Данные, Новый Структура("Слова", Сред(мсл, 2)), Свойства, "ФормыСлов", "Морфология");
					Данные.Процесс.НоваяЗадача(Запрос, "Служебный");
		...
		Если Свойства.д.Разбор.Значение = "Токены" Тогда
			...
			Если НЕ Грамматики = "" Тогда
				Запрос = Новый Структура("Библиотека, Данные, Параметры, Свойства, вДействие, cmd", ЭтотОбъект, Данные, Новый Структура("Слова", Сред(Грамматики, 2)), Свойства, "Грамматики", "Морфология");
				Данные.Процесс.НоваяЗадача(Запрос, "Служебный");
			Иначе

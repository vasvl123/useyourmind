...
Функция СписокВТаблицу(Данные, Параметры) Экспорт
	...
	Список = Данные.УзелЗначение(Параметры.Список);
	нУзел = Неопределено;
	...
	Пока НЕ Список = Неопределено Цикл
		...
		Пока НЕ Атрибут = Неопределено Цикл
			...
			Если Параметры.Свойство(Атрибут.Имя, ПарШаблон) Тогда
				Определение = Данные.ОпределениеУзла(ЭтотУзел, ПарШаблон);
				Если НЕ Определение = Неопределено Тогда
					Узел.Значение = Неопределено;
					//дУзел = Данные.НовыйДочерний(Узел, Новый Структура("Имя, Значение", "Узел", "Строка"), Истина);
					//Данные.НовыйДочерний(дУзел, Новый Структура("Имя, Значение", "Ссылка", Список.Код), Истина);
					Аргументы = Новый Соответствие;
					//Аргументы.Вставить("Строка", "Строка");
					//Данные.НовыйСоседний(дУзел, Данные.Копия(Данные.Дочерний(Определение), дУзел, Аргументы), Истина);
				КонецЕсли;
			КонецЕсли;

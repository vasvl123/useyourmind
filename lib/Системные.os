...
Функция СписокВТаблицу(Данные, Параметры) Экспорт
	...
	КонецЦикла;
	Если РодительУзел = Неопределено Тогда
		Возврат Данные.НовыйУзел(Новый Структура("Имя, Значение", "Строка", ""), Истина)
	КонецЕсли;
...
Функция НовыйФайл(Данные, Параметры) Экспорт
	...
	Возврат Данные.НовыйУзел(Новый Структура("Имя, Значение", "Строка", стрСообщение), Истина);
КонецФункции
...
Функция СписокБаз(Данные, Параметры) Экспорт
	...
	Если СписокФайлов.Количество() Тогда
		Для каждого элФайл Из СписокФайлов Цикл
			...
			КонецЕсли;
			Атр = Данные.НовыйАтрибут(Узел, Новый Структура("Имя, Значение", "ИмяФайла", элФайл.ИмяБезРасширения), Истина);

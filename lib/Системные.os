...
Функция Вставить(Данные, Параметры) Экспорт
	Перем ИмяУзла;
	ЭтотУзел = Параметры.ЭтотУзел;
	ПутьУзел = Параметры.ПутьУзел;
	Значение = Параметры.Значение;
	УзелПоиск = ЭтотУзел;
	МассивПуть = СтрРазделить(ПутьУзел, ".");
	ПервыйНайден = Ложь;
	Для каждого элПуть Из МассивПуть Цикл
		Если НЕ ПервыйНайден Тогда
			УзелПоиск = Данные.ОпределениеУзла(ЭтотУзел, элПуть, Ложь);
			...
			КонецЕсли;
		Иначе
			УзелПоиск = Данные.НайтиУзел(УзелПоиск, элПуть);
			Если УзелПоиск = Неопределено Тогда
				Возврат Неопределено; // целевой узел не найден
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	Данные.УзелУстановитьЗначение(ЭтотУзел, УзелПоиск, Значение);
	Возврат Неопределено;
КонецФункции


Функция Модель(Данные, Параметры) Экспорт
	Перем ИмяУзла;
	ЭтотУзел = Параметры.ЭтотУзел;
	Модель = Параметры.Модель;
	Определение = Данные.ОпределениеМодели(ЭтотУзел, Модель);
	Если НЕ Определение = Неопределено Тогда
		Элемент = Данные.КопироватьВетку(Определение, Данные, ЭтотУзел, ЭтотУзел.Родитель, , Ложь, , Параметры);
		ЭтотУзел.Вставить("Дочерний", Элемент.Код);
	КонецЕсли;
	Возврат Неопределено;
КонецФункции

...
Функция ЗначениеПоляСтрокиТаблицы(Данные, Параметры) Экспорт
	ЭтотУзел = Параметры.ЭтотУзел;
	СтрокаТаблицы = Параметры.СтрокаТаблицы;
	ИмяПоля = Параметры.ИмяПоля;
	Возврат Данные.НовыйУзел(Новый Структура("Имя, Значение", "Строка", Данные.НайтиСоседний(Данные.Атрибут(СтрокаТаблицы), ИмяПоля).Значение), Истина);
КонецФункции // ЗначениеПоляСтрокиТаблицы()
...
Функция СписокВТаблицу(Данные, Параметры) Экспорт
	...
	Пока НЕ Список = Неопределено Цикл
		...
		Узел = Данные.НовыйДочерний(нУзел, Новый Структура("Имя, Значение", "Узел", "СтрокаТаблицы"), Истина);
		Данные.НовыйДочерний(Узел, Новый Структура("Имя, Значение", "Ссылка", Список.Код), Истина);
		...
		Пока НЕ Атрибут = Неопределено Цикл
			...
			Если Параметры.Свойство(Атрибут.Имя, ПарШаблон) Тогда
				...
				дУзел = Данные.НовыйДочерний(Узел, Новый Структура("Имя, Значение", "Узел", "ИмяПоля"), Истина);
				Данные.НовыйДочерний(дУзел, Новый Структура("Имя, Значение", "Строка", Атрибут.Имя), Истина);
				Аргументы = Новый Соответствие;
				Аргументы.Вставить("СтрокаТаблицы", "СтрокаТаблицы");
				Данные.НовыйСоседний(дУзел, Данные.Копия(ПарШаблон, дУзел, Аргументы), Истина);
			КонецЕсли;

...
Функция ИсточникДанных(Данные, Параметры) Экспорт
	Перем ЧислоСтрок, ПозицияДанных, СписокУсловий;
	ЭтотУзел = Параметры.ЭтотУзел;
	БазаДанных = Параметры.БазаДанных;
	Параметры.Свойство("ЧислоСтрок", ЧислоСтрок);
	Параметры.Свойство("СписокУсловий", СписокУсловий);
	Параметры.Свойство("ПозицияДанных", ПозицияДанных);
	Узел = Данные.НовыйУзел(Новый Структура("Имя", "Ссылка"), Истина);
	ЭтотУзел.Вставить("Дочерний", Узел.Код);
	// прежний дочерний еще нужно удалять
	Запрос = Новый Структура("nodeid, osdb, dataposition, СписокУсловий, ЧислоСтрок, cmd", Узел.Код, БазаДанных, ПозицияДанных, СписокУсловий, ЧислоСтрок, "dataquery");
...
Функция НоваяБаза(Данные, Параметры) Экспорт
	...
	Если НЕ "" + БазаДанных = "" Тогда
		Результат = Данные.Процесс.ВыполнитьЗапросКСерверуДанных("osdb" + Символы.Таб + БазаДанных);
...
Функция СписокДанных(Данные, Параметры) Экспорт
	...
	Индекс = Данные.Процесс.ВыполнитьЗапросКСерверуДанных("osdb" + Символы.Таб + БазаДанных);

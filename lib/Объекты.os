	...
	Если НЕ Изменения.Получить(Свойства.Родитель) = Истина Тогда
		Если Изменения.Получить(Свойства.д.Условие) = Истина Тогда
			Если Условие Тогда
				...
				ЗаполнитьЗначенияСвойств(Параметры, Свойства.д.Параметры);
				//Параметры.Вставить("taskid", Данные.Процесс.ПолучитьИД());
				Данные.Процесс.НоваяЗадача(Свойства, "Служебный");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	Возврат Изменения;

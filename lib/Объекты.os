...
Функция ИсточникДанных_Модель(Данные, Свойства, Изменения) Экспорт
	Если Данные.ЗначениеСвойства(Свойства.д.ЗапросДанных.д.Задача) = Данные.Пустой Тогда
...
Функция СтрокаТаблицы_Модель(Данные, Свойства, Изменения) Экспорт
	...
	Если Изменения.Получить(Свойства.Родитель) = Истина Тогда
		...
		Для каждого элПоле Из Поля Цикл
			Если элПоле.Ключ = "д" Тогда
				Продолжить;
			КонецЕсли;
			свПоле = Данные.СтруктураСвойств(элПоле.Значение);
			Шаблон = УзелСвойство(свПоле, "Шаблон");
			Если НЕ Шаблон = Неопределено Тогда
				стрУзел = Данные.КопироватьВетку(Данные.Дочерний(Шаблон), , Узел, Узел, , Ложь);
			Иначе
				Поле = элПоле.Ключ;
...
Функция Таблица_Модель(Данные, Свойства, Изменения) Экспорт
	...
	Если Изменения.Получить(Свойства.Родитель) = Истина Тогда
		Если УзелЗаголовок = Неопределено Тогда // создать заголовок
			...
			Для каждого элПоле Из Свойства.д.СписокПолей Цикл
				Если элПоле.Ключ = "д" Тогда
					Продолжить;
				КонецЕсли;
		...
		Если НЕ ИсточникСтрок = Неопределено Тогда
			...
			Пока НЕ ИсточникСтрок = Неопределено Цикл
				...
				Если Строки.Получить(ИмяСтроки) = Неопределено Тогда
					...
					Для каждого элСвойство Из Свойства.д.СвойстваСтроки Цикл
						Если элСвойство.Ключ = "д" Тогда
							Продолжить;
						КонецЕсли;
						Данные.КопироватьВетку(ЭлСвойство.Значение, , СвойстваСтроки, СвойстваСтроки, , Ложь);
					КонецЦикла;
					Данные.НовоеЗначениеУзла(СвойстваСтроки.Источник, ИмяЗначение("Указатель", ИсточникСтрок.Код), Истина);
					Данные.НовоеЗначениеУзла(СвойстваСтроки.Поля, ИмяЗначение("Указатель", Свойства.д.СписокПолей.Код), Истина);
				КонецЕсли;

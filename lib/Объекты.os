...
Функция Выполнить_Модель(Данные, Свойства, Изменения) Экспорт
	Если Изменения.Получить(Свойства.Условие) = Истина ИЛИ Изменения.Получить(Свойства) = Истина Тогда
...
Функция ИсточникДанных_Модель(Данные, Свойства, Изменения) Экспорт
	Если Свойства.ЗапросДанных.Задача.Значение = "" Тогда
		Запрос = Новый Структура("Данные, Узел, ЗапросДанных, cmd", Данные, Свойства.Записи, Данные.СвойстваВСтуктуру(Свойства.ЗапросДанных), "ЗапросДанных");
		ИдЗадачи = Данные.Процесс.НоваяЗадача(Запрос, "Служебный");
		Данные.НовоеЗначениеУзла(Свойства.ЗапросДанных.Задача, Новый Структура("Имя, Значение", "Строка", ИдЗадачи), Истина);
...
Функция Таблица_Модель(Данные, Свойства, Изменения) Экспорт
	...
	Если Изменения.Получить(Свойства.ИсточникСтрок) = Истина Тогда
		Содержимое = Данные.Соседний(Свойства.ЭтотУзел);
		Если Содержимое = Неопределено Тогда // создать объекты строк
			Сообщить("Строки!!");
		КонецЕсли;
	КонецЕсли;
	Возврат Изменения;

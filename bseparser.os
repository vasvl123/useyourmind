...
Функция Парсить(УРЛ, Знач Узел);
	...
	Пока Истина Цикл
		Рез = ВыполнитьHTTPЗапрос("https://slova.cc/enc/bse/" + УРЛ + ".html");
...
Функция ЗаписатьДанные()
	...
	ф.Записать(ОбъединитьПути(ТекущийКаталог(), "data/.files/inf11"));
	Сообщить("Запись");
...
Данные = Новый pagedata(ЭтотОбъект, "", "", "i11", "");
ф = Новый ТекстовыйДокумент;

Для н = 1969550 по 2064816 Цикл
	Парсить(н);
КонецЦикла;

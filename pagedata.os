		...
		Если СписокУзлов.Количество() Тогда
			Для каждого элУзел Из СписокУзлов Цикл
				Попытка
					...
					Если НЕ УзелСостояние(Узел, "Ошибка") = Неопределено Тогда
						Продолжить;
					КонецЕсли;

					Для каждого элСвязь Из элУзел.Значение Цикл
						Если элСвязь.Значение = ЭтотОбъект Тогда

							РодительУзел = элСвязь.Ключ.Родитель;
							Пока НЕ РодительУзел = Неопределено Цикл
								...
								ИначеЕсли РодительУзел.Имя = "Узел" ИЛИ РодительУзел.Имя = "У" Тогда
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	ИначеЕсли Имя = "Узел" ИЛИ Имя = "У" Тогда // объявление узла
		Если Узел = Корень Тогда
			Состояние = "<div id='" + Корень.Код + "'>" + Интерпретировать(Дочерний(Корень), , Ложь) + "</div>";
		КонецЕсли;
	...
	ИначеЕсли Имя = "Элемент" ИЛИ Имя = "Э" Тогда
		...
		Обновление = Обновление + Состояние;
...
Функция ПриСозданииОбъекта(обПроцесс, Текст = "", знБазаДанных = "", знИмяДанных = "", знПозицияДанных = "") Экспорт
	...
	Если НЕ Корень = Неопределено Тогда
		...
		КонецПопытки;
	Иначе
		Корень = НовыйУзел(Новый Структура("Имя, Значение, Старший, Родитель", "У", "Корень", Неопределено, Неопределено));
		Фронт = Корень;
	КонецЕсли;

...
// поиск узла вглубь
Функция НайтиУзел(Знач Узел, ЗначениеУзла, ИмяУзла = "") Экспорт
	...
	УзелДочерний = Дочерний(Узел);
	Если НЕ УзелДочерний = Неопределено Тогда
		Объявление = НайтиУзел(УзелДочерний, ЗначениеУзла, ИмяУзла);
	КонецЕсли;
	Если Объявление = Неопределено Тогда
		УзелСоседний = Соседний(Узел);
		Если НЕ УзелСоседний = Неопределено Тогда
			Объявление = НайтиУзел(УзелСоседний, ЗначениеУзла, ИмяУзла);
		КонецЕсли;
	КонецЕсли;
	Возврат Объявление;
...
Функция ПриСозданииОбъекта(обПроцесс, Текст = "", знБазаДанных = "", знИмяДанных = "", знПозицияДанных = "")

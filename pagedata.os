...
Функция СтруктуруВСтроку(Структ)
	...
	Для каждого Элемент Из Структ Цикл
		Если НЕ ТипЗнч(Элемент.Значение) = Тип("Структура") Тогда
...
Функция ЗначениеФункции(Знач Окружение, Знач СоставнаяФункция, Знач Аргумент)
	...
	Пока Выражение <> Неопределено Цикл
		Значения.Добавить(Интерпретировать(Окружение, Выражение));
...
Функция Интерпретировать(Знач Окружение, Знач Узел, Знач Применить = Ложь) Экспорт
	...
	Если Имя = "Объект" Тогда
		...
		ИначеЕсли Значение = "Пустой" Тогда
			Возврат Значение(Интерпретировать(Окружение, Соседний(Узел))) = Неопределено;
...
Функция ПриСозданииОбъекта(ДанныеИлиИмяФайлаДанных)
	Данные = Новый ТекстовыйДокумент;
	Если Лев(ДанныеИлиИмяФайлаДанных, 3) = "Код" Тогда
		Данные.УстановитьТекст(ДанныеИлиИмяФайлаДанных);
	Иначе
		Данные.Прочитать(ДанныеИлиИмяФайлаДанных);
	КонецЕсли;

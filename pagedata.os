		...
		Для каждого элИмяЭлемента Из ОбъектыОбновить Цикл
			Для каждого элОбъект Из Объекты Цикл
				Если элОбъект.Значение.Значение = элИмяЭлемента.Ключ Тогда
					РодительУзел = Родитель(элОбъект.Значение);
					ПервыйНайден = Ложь;
					Пока НЕ РодительУзел = Неопределено Цикл
						Если РодительУзел.Имя = "Узел" Тогда
							...
							Если НЕ ПервыйНайден Тогда
								УзелСостояниеЗначение(РодительУзел, "Обновить", Истина);
...
Функция Интерпретировать(Знач Окружение, Знач Узел, Знач Применить = Истина, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	ИначеЕсли Имя = "Загрузить" Тогда
		Значение = СтрЗаменить(Значение, "..", "");
		Библиотеки.Вставить(Значение, ЗагрузитьСценарий(ОбъединитьПути(ТекущийКаталог(), "lib", Значение + ".os")));
		Параметры = Дочерний(Узел);
		Аргументы = Лямбда(Окружение, Параметры);
	Иначе
		...
		Если Имя = "Узел" Тогда
			...
			Если УзелСостояние(Узел, "Обновить") = Ложь ИЛИ Обновить Тогда
			...
			УзелСостояниеЗначение(Узел, "Обновить", Ложь);

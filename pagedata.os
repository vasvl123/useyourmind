...
Функция ОбновитьПредставление(Узел = Неопределено) Экспорт
	...
	Попытка
		...
		Если НЕ Узел = Неопределено Тогда
			...
			Возврат ОбработатьОбъект(Узел);
		ИначеЕсли ОбъектыОбновить.Количество() Тогда
			НачалоЦикла = ТекущаяУниверсальнаяДатаВМиллисекундах();
			к = ОбъектыОбновить.Количество();
			Пока ОбъектыОбновить.Количество() И к > 0 И ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла < 100 Цикл
...
Функция ОбработатьОбъект(Знач Узел, Инициализация = Ложь) Экспорт
	...
	Если НЕ Свойства.Соседний = Неопределено Тогда
		Содержимое = Интерпретировать(Свойства.Соседний, , Ложь);
	КонецЕсли;

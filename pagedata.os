...
Функция ЗначениеСвойства(знач УзелСвойство, Имя = Неопределено) Экспорт
	...
	Если НЕ УзелСвойство = Неопределено Тогда
		...
		Если НЕ ДочернийУзел = Неопределено Тогда
			Значение = ПолучитьПредставление(ДочернийУзел, Ложь);
		Иначе
...
Функция ПолучитьСвойстваПоСсылке(Знач Узел, Путь = "", СоздатьСвязь = Ложь) Экспорт
	...
	Если ТипЗнч(Путь) = Тип("Строка") Тогда
		...
		Если Путь = "" Тогда
			...
			Если Путь = "" Тогда
				Если НЕ Узел.Дочерний = Неопределено Тогда
					Путь = "" + ПолучитьПредставление(Узел.Дочерний);
				КонецЕсли;
				Если Путь = "" Тогда
...
Функция ПолучитьПредставление(нУзел = Неопределено, Список = Ложь) Экспорт

	Если НЕ нУзел = Неопределено Тогда
		Сообщить("ПолучитьПредставление " + нУзел.Код);
		Очередь.Добавить(нУзел);
	Иначе
		Сообщить("ПолучитьПредставление очередь " + Очередь.Количество());
	КонецЕсли;
	...
	Пока Очередь.Количество() Цикл
		...
		Сообщить("" + Очередь.Количество() + " На очереди у " + Узел.Код);
		...
		Если Состояние = Неопределено Тогда
			...
			Сообщить("Вычислить " + Имя + " " + Значение);

			Если НЕ Имя = "div" Тогда
				...
				ИначеЕсли Имя = "Число" Тогда
					...
					Иначе
						...
						Если НЕ Узел.Дочерний = Неопределено Тогда
							...
							Если Состояние = Неопределено Тогда
								Состояние = ПолучитьПредставление(Узел.Дочерний);
						...
						Если НЕ Узел.Дочерний = Неопределено Тогда
							...
							Если НЕ УзелДочерний = Неопределено Тогда
								...
								Если сДочерний = Неопределено Тогда
									Состояние = ПолучитьПредставление(УзелДочерний, Ложь);
								КонецЕсли;
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
				...
				ИначеЕсли Имя = "Свойство" ИЛИ Имя = "Атрибут" ИЛИ Имя = "Первый" ИЛИ Имя = "Соседний" Тогда
					...
					Если НЕ Узел.Дочерний = Неопределено Тогда
						...
						Если Элемент = Неопределено Тогда
							Элемент = ПолучитьПредставление(Узел.Дочерний);
							// Очередь.Добавить(Узел.Дочерний);
							// Продолжить;
						КонецЕсли;
			...
			Если Состояние = Неопределено Тогда
				...
				Если НЕ Узел.Атрибут = Неопределено Тогда
					...
					Если сАтрибут = Неопределено Тогда
						...
						сАтрибут = ПолучитьПредставление(Узел.Атрибут, Ложь);
					КонецЕсли;
				КонецЕсли;
				...
				Если НЕ Узел.Дочерний = Неопределено Тогда
					сДочерний = Состояния.Получить(Узел.Дочерний);
					Если сДочерний = Неопределено Тогда
						сДочерний = ПолучитьПредставление(Узел.Дочерний, Ложь);
					КонецЕсли;
				КонецЕсли;
		...
		Очередь.Удалить(Очередь.Найти(Узел));

		Если Узел = нУзел И НЕ Список Тогда
			Возврат Состояние;
		КонецЕсли;
...
Функция ОбновитьУзел(Знач Узел) Экспорт
	...
	нСвойство = Узел;
	Пока НЕ оУзел = Неопределено Цикл
		Состояния.Удалить(оУзел);
		Если оУзел.Родитель.Имя = "Свойства." Тогда // нашли свойство
			нСвойство = оУзел;
		КонецЕсли;
		Если оУзел.Имя = "О" ИЛИ оУзел.Имя = "Объект" Тогда
			Сообщить("Обновить Объект " + оУзел.Код);
			Позиция = Очередь.Найти(оУзел);
			Если НЕ Позиция = Неопределено Тогда
				Очередь.Удалить(Позиция);
			КонецЕсли;
			Сообщить("Добавить обновить " + оУзел.Код);
			Очередь.Добавить(оУзел);
			Если НЕ Узел = оУзел Тогда
				...
				КонецЕсли;
				Сообщить("+Изменения " + нСвойство.Код);
				Изменения.Вставить(нСвойство, Истина);
			КонецЕсли;
			Процесс.ЗадачаОбновитьВыполнить = Истина; // продолжение
			Прервать;
		КонецЕсли;
...
Функция ПолучитьАргументы(Узел)
	...
	Пока НЕ Аргумент = Неопределено Цикл
		...
		Если Аргумент.Имя = "Аргумент" ИЛИ Аргумент.Имя = "А" Тогда
			Аргументы.Вставить(Аргумент.Значение, ПолучитьПредставление(Аргумент.Дочерний));
	...
	Если НЕ уСвойство = Неопределено Тогда
		...
		Для каждого Свойство Из Свойства Цикл
			...
			Если НЕ уЗначение = Неопределено Тогда
				Значение = ПолучитьПредставление(уСвойство.Соседний);
...
Функция ОбработатьОбъект(Знач Узел, Конструктор = Ложь) Экспорт
	...
	Если Изменения.Количество() Тогда
		...
		Если Рефлектор.МетодСуществует(Библиотека, ИмяФункции) Тогда
			...
			Для каждого св Из Изменения Цикл
				Связи = ВсеСвязи.Получить(св.Ключ);
				Если НЕ Связи = Неопределено Тогда
					Для каждого элУзел Из Связи Цикл
	...
	Пока НЕ УзелДочерний = Неопределено Цикл
		сДочерний = Состояния.Получить(УзелДочерний);
		Если сДочерний = Неопределено Тогда
			Позиция = Очередь.Найти(УзелДочерний);
			Если НЕ Позиция = Неопределено Тогда
				Очередь.Удалить(Позиция);
			КонецЕсли;
			Сообщить("Добавить вложенный " + УзелДочерний.Код);
			Очередь.Добавить(УзелДочерний);
	...
	Если Свойства.д.Свойство("Вид") Тогда // Стандартный вид
		ОбновитьСвойство = Ложь;
		УзелДочерний = Свойства.д.Вид.Дочерний;
		Пока НЕ УзелДочерний = Неопределено Цикл
			сДочерний = Состояния.Получить(УзелДочерний);
			Если сДочерний = Неопределено Тогда
				сДочерний = ПолучитьПредставление(УзелДочерний);
				Если сДочерний = Неопределено Тогда
					ОбновитьСвойство = Истина;
					Прервать;
				КонецЕсли;
			КонецЕсли;
			Состояние = Состояние + сДочерний;

...
Функция ОбработатьОбъект(Знач Узел, Инициализация = Ложь) Экспорт
	...
	Если Изменения = Неопределено Тогда
		...
		Если Рефлектор.МетодСуществует(Библиотека, ИмяФункции) Тогда
			...
			Если Результат = Неопределено Тогда // обработать объект сразу
			ИначеЕсли Результат = Истина ИЛИ Результат = Ложь Тогда // запустить повторно
			...
			Иначе // свойства по шаблону
				Если Свойства = Неопределено Тогда
					...
					СоздатьСвойства(Свойства, Результат);
					Узел.Вставить("Свойства", Свойства);
				Иначе
		...
		Если Узел = Корень Тогда
			Если Корень.Свойства.Свойство("д") Тогда
				уВерсии = УзелСвойство(Корень.Свойства.д, "Версии");

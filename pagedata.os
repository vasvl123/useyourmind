...
Функция ОбновитьПредставление() Экспорт
	...
	Если ОбъектыОбновить.Количество() Тогда
		...
		Для каждого элУзел Из УзлыОбновить Цикл
			...
			КонецЕсли;
			Представление = Представление + Интерпретировать(сОкружение, элУзел.Значение, , , Истина);
		КонецЦикла;
	Иначе
		Представление = Интерпретировать(Окружение, ПолучитьУзел("1"), , , Истина);
	КонецЕсли;
КонецФункции

Функция Интерпретировать1(Знач Окружение, Знач Узел, Знач Применить = Истина, ЭтоАтрибут = Ложь, НачальныйУзел = Ложь) Экспорт
...
Функция Интерпретировать(Знач Окружение, Знач Узел, Знач Применить = Истина, ЭтоАтрибут = Ложь, НачальныйУзел = Ложь) Экспорт
	...
	Иначе
		...
		Если Имя = "Узел" Тогда
			...
			Если Состояние = Неопределено Тогда
				Состояние = ПоказатьУзел(Узел, , Интерпретировать(Окружение, Дочерний(Узел)));

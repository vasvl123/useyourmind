...
Функция ОбновитьПредставление(Узел = Неопределено) Экспорт
	...
	Попытка
		...
		ИначеЕсли ОбъектыОбновить.Количество() Тогда
			...
			н = ОбъектыОбновить.Количество(); // сколько нужно обновить
			Пока ОбъектыОбновить.Количество() И н > 0 И ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла < 100 Цикл
				Показать = Истина;
				Узел = ОбъектыОбновить.Получить(0);
				рУзел = Узел.Родитель;
				Пока НЕ рУзел = Неопределено Цикл
					Если НЕ ОбъектыОбновить.Найти(рУзел) = Неопределено Тогда
						Показать = Ложь; // будет еще обновлен родитель
						Прервать;
					КонецЕсли;
...
Функция ОбработатьОбъект(Знач Узел, Инициализация = Ложь) Экспорт
	...
	//Сообщить("ОбработатьОбъект " + Узел.Код + " " + Узел.ТипОбъекта + " " + Узел.ИмяОбъекта);

	Свойства = УзелСвойство(Узел, "Свойства");
	Если Свойства = Неопределено Тогда
		Свойства = Узел.Дочерний;
		Если НЕ Свойства = Неопределено Тогда
			Если НЕ Свойства.Имя = "Свойства." Тогда // новый объект
				Свойства = Неопределено;
			КонецЕсли;
	...
	Если Изменения.Количество() ИЛИ НЕ ОбъектыОбновить.Найти(Узел) = Неопределено Тогда

		Пока НЕ ОбъектыОбновить.Найти(Узел) = Неопределено Цикл
			ОбъектыОбновить.Удалить(ОбъектыОбновить.Найти(Узел));
		КонецЦикла;
		...
		Если Рефлектор.МетодСуществует(Библиотека, ИмяФункции) Тогда
			...
			Если Обновить = Истина Тогда // запустить повторно
				...
				//ОбъектыОбновить.Вставить(Узел.Родитель, Истина);
			КонецЕсли;
	...
	кОбновить = ОбъектыОбновить.Количество();
	...
	Иначе
		Состояние = "<div id='_" + Узел.Код + "'>" + ОбъектВид(Узел, Свойства) + "</div>";
	КонецЕсли;
	...
	Если кОбновить = ОбъектыОбновить.Количество() Тогда
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	Пока НЕ СледУзел = Неопределено Цикл
		...
		ИначеЕсли Имя = "Объект" ИЛИ Имя = "О" Тогда // это объект
			// оптимизация
			Если ОбъектыОбновить.Найти(Узел) = Неопределено Тогда

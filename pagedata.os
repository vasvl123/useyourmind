...
Функция ОбновитьУзел(Знач Узел) Экспорт
	...
	Если НЕ Состояние = Неопределено Тогда
		...
		Пока НЕ РодительУзел = Неопределено Цикл
			Если РодительУзел = Корень Тогда
				УзлыОбновить.Вставить(Корень.Код, Корень);
				Прервать;
...
Функция УзелЗначение(Знач Узел) Экспорт
	...
	Если НЕ Узел = Неопределено Тогда
		...
		Если Состояние = Неопределено Тогда
			...
			УзелСостояниеЗначение(Узел, "ЗначениеУзла", Интерпретировать(Дочерний(Узел), , Ложь));
	...
	Для каждого элПуть Из МассивПуть Цикл
		Если НЕ ПервыйНайден Тогда
			...
			Если НЕ УзелПоиск = Неопределено Тогда
				ПервыйНайден = Истина;
				Если УзелСостояние(УзелПоиск, "ЗначениеУзла") = Неопределено Тогда
					УзелЗначение(УзелПоиск);
				КонецЕсли;
			Иначе
...
Функция ОбновитьДанные(Запрос) Экспорт
	Для каждого элПараметр Из Запрос Цикл
		УзелУстановитьЗначение(Корень, элПараметр.Ключ, элПараметр.Значение);
	КонецЦикла;

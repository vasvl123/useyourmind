...
Функция НайтиОбъект(Знач Узел, оПоз, оТип, оИмя)
	...
	Если оПоз = "р" Тогда // найти родителя
		Пока НЕ Узел = Неопределено Цикл
...
Функция СтруктураСвойств(УзелСвойства) Экспорт
	...
	// Если Свойства = Неопределено Тогда
		...
		Пока НЕ св = Неопределено Цикл
			...
			Если Прав(ИмяСвойства, 1) = "." Тогда // группа свойств
				ИмяСвойства = Лев(ИмяСвойства, СтрДлина(ИмяСвойства) - 1);
				Свойства.Вставить(ИмяСвойства, СтруктураСвойств(св));
			Иначе
...
Функция ПолучитьСвойствоПоСсылке(Знач Узел, Путь = "") Экспорт
	...
	оУзел = НайтиОбъект(Узел, оПоз, оТип, оИмя);
...
КонецФункции // ПолучитьСвойствоПоСсылке()
...
Функция ОбъектФорма(Узел, Свойства)
	Вид = "<script>var p = {id: '" + Узел.Код + "', fid: '" + Свойства.Форма.ЭтотУзел.Код + "'";
	св = Дочерний(Свойства.Форма.ЭтотУзел);
	Пока не св = Неопределено Цикл
...
Функция ОбработатьОбъект(Знач Узел) Экспорт
	...
	Сообщить("о. " + Узел.Код + " " + ТипИмя);

	Если УзелСвойство(Узел, "Содержимое") = Неопределено Тогда
 		ОбъектыОбновить.Вставить(Узел, Узел);
 	КонецЕсли;

	свУзел = Дочерний(Узел);
	Если НЕ свУзел = Неопределено Тогда
		Содержимое = Интерпретировать(свУзел, , Ложь);
	...
	Если Изменения.Количество() Тогда
		...
		Если НЕ Библиотека = Неопределено Тогда
			...
			Если Свойства = Неопределено Тогда
				...
				Свойства = СтруктураСвойств(УзелСвойства);
				Возврат Свойства;
			КонецЕсли;
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	Пока НЕ СледУзел = Неопределено Цикл
		...
		ИначеЕсли Имя = "Ссылка" Тогда
			Состояние = ПолучитьСвойствоПоСсылке(Узел);

		ИначеЕсли Имя = "Ключ" ИЛИ Имя = "К" Тогда

		ИначеЕсли Имя = "Значение" Тогда // значение свойства объекта
			Если Значение = "Содержимое" Тогда // содержимое объекта
				сУзел = Узел;
				Пока НЕ сУзел = Неопределено Цикл
					Если сУзел.Имя = "Свойства." Тогда
						Состояние = "" + УзелСвойство(сУзел.Родитель, "Содержимое");
						Прервать;
					КонецЕсли;
					сУзел = сУзел.Родитель;
				КонецЦикла;
			Иначе
				сУзел = ПолучитьСвойствоПоСсылке(Узел);
				Если НЕ сУзел = Неопределено Тогда
					Состояние = ЗначениеСвойства(сУзел);
				КонецЕсли;
			КонецЕсли;

...
// Получить копию
Функция Копия(Объявление, Применение, Аргументы = Неопределено) Экспорт
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	ИначеЕсли Имя = "Ссылка" ИЛИ Имя = "С" Тогда
		...
		Если Состояние = Неопределено Тогда // ссылка на определение узла
			Если НЕ "" + Значение = "" Тогда // указан код узла
				Состояние = ПолучитьУзел(Значение);
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли Имя = "Атрибут" Тогда
		Элемент = Интерпретировать(Дочерний(Узел));
		Состояние = НайтиСоседний(Атрибут(Элемент), Значение);

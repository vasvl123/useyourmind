...
Функция КопироватьВетку(Узел, Старший, Родитель, ЭтоАтрибут = Ложь, ПервыйВызов = Истина, Служебный = Истина) Экспорт
	...
	КопияУзел = НовыйУзел(Новый Структура("Имя, Значение, Старший, Родитель", ИмяУзла, ЗначениеУзла, Старший, Родитель), Служебный ИЛИ Служебный(Узел));

	Если НЕ ЭтоАтрибут Тогда

		УзелАтрибут = Атрибут(Узел);
		Если НЕ УзелАтрибут = Неопределено Тогда
			УзелАтрибут = КопироватьВетку(УзелАтрибут, КопияУзел, КопияУзел, Истина, Ложь, Служебный ИЛИ Служебный(УзелАтрибут));
	...
	Если НЕ УзелДочерний = Неопределено Тогда
		УзелДочерний = КопироватьВетку(УзелДочерний, КопияУзел, КопияУзел, , Ложь, Служебный ИЛИ Служебный(УзелДочерний));
	...
	Иначе
		...
		Если НЕ УзелСоседний = Неопределено Тогда
			УзелСоседний = КопироватьВетку(УзелСоседний, КопияУзел, Родитель, ЭтоАтрибут, Ложь, Служебный ИЛИ Служебный(УзелСоседний));
	...
	Если НЕ "" + ИмяУзла = "" Тогда // указано имя узла
		...
		Если НЕ Определение = Неопределено Тогда
			...
			Если НЕ Служебный(УзелЗначение) Тогда
				УзелЗначение = КопироватьВетку(УзелЗначение, Определение, Определение.Родитель, , Ложь);
			КонецЕсли;
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	ИначеЕсли (Имя = "Аргумент" ИЛИ Имя = "А") И этоАтрибут Тогда
		...
		Если Значение = "Значение" Тогда
			УзелДочерний = Дочерний(Узел);
			Если НЕ УзелДочерний = Неопределено Тогда
				Состояние = " value=""" + Интерпретировать(УзелДочерний) + """";
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли Имя = "Пусть" ИЛИ Имя = "П" Тогда
		УзелУстановитьЗначение(Узел, Дочерний(Узел));

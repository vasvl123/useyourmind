...
Функция ОбновитьОбъект(Знач Узел) Экспорт
	...
	Если НЕ Объекты = Неопределено Тогда
		УзлыОбновить.Вставить(Узел, Объекты);
...
Функция Копия(Объявление, Применение)
	...
	Возврат КопироватьВетку(Объявление, Применение, Применение.Родитель, Аргументы, , Ложь);

КонецФункции // Копия()


// Установка значения объекта узла
Функция УзелЗначение(Знач Узел)
	НачальныйУзел = Узел;
	ИмяЭлемента = Узел.Значение;

	Значение = Интерпретировать(Дочерний(Узел), , Ложь);
	Если Значение = Неопределено Тогда
		Значение = Пустой;
	КонецЕсли;

	// объявление объекта
	Узел = УзелСостояние(НачальныйУзел, "Определение");
	Если Узел = Неопределено Тогда
		Узел = ОпределениеУзла(НачальныйУзел, ИмяЭлемента, Ложь);
	КонецЕсли;

	УзелСостояниеЗначение(Узел, "ЗначениеУзла", Значение); // Новое значение

	ЭтоФронт = УзелСостояние(НачальныйУзел, "ЭтоФронт");
	Если ЭтоФронт Тогда
		Узел.Дочерний = НачальныйУзел.Дочерний;
	КонецЕсли;
	...
	Пока НЕ Узел = Неопределено Цикл
		Если Узел.Имя = "Элемент" ИЛИ Узел.Имя = "Э" Тогда
			Если "" + УзелСвойство(Узел, "Значение") = "Фронт" Тогда
				ЭтоФронт = Ложь;
			КонецЕсли;
		ИначеЕсли НЕ Узел = Родитель Тогда // прямой родитель не может быть определением
		...
		Для каждого элУзел Из УзлыОбновить Цикл
			...
			Для каждого элОбъект Из Объекты Цикл
				...
				Пока НЕ РодительУзел = Неопределено Цикл
					Если РодительУзел.Имя = "Элемент" ИЛИ РодительУзел.Имя = "Э" ИЛИ РодительУзел.Имя = "Узел" ИЛИ РодительУзел.Имя = "У" Тогда
		...
		Если СписокУзлов.Количество() Тогда
			Для каждого элУзел Из СписокУзлов Цикл
				...
				// Если НЕ РодительУзел = УзелЗначение Тогда // Источник обновлять не надо
					ПервыйНайден = Ложь;
					// Установить для обновления узлы которые нужно обновить
					Объекты = УзелСостояние(РодительУзел, "Объекты");
					Если НЕ Объекты = Неопределено Тогда
						УзлыОбновить.Вставить(РодительУзел, Объекты);
						УзелСостояниеЗначение(РодительУзел, "ЗначениеУзла", Неопределено);
					КонецЕсли;
					// Поиск элемента для обновления
					Пока НЕ РодительУзел = Неопределено Цикл
						Если РодительУзел.Имя = "Элемент" ИЛИ РодительУзел.Имя = "Э" Тогда
	...
	Если ИмяФункции.Количество() = 2 Тогда
		...
		Если НЕ Библиотека = Неопределено Тогда
			...
			Для каждого Метод Из ТаблицаМетодов Цикл
				...
				КонецЕсли
			КонецЦикла;
			Если ЗначениеФункции = Неопределено Тогда
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	ИначеЕсли Имя = "Ссылка" ИЛИ Имя = "С" Тогда
		...
		Если Состояние = Неопределено Тогда // ссылка на определение узла
	...
	ИначеЕсли Имя = "Функция" ИЛИ Имя = "Ф" Тогда
		...
		Если Элемент = Неопределено Тогда
			Если НЕ "" + Значение = "" Тогда // указано имя узла
				Определение = ВызватьФункцию(Узел, Значение);
				Если НЕ Определение = Неопределено Тогда
	...
	ИначеЕсли Имя = "Значение" ИЛИ Имя = "З" Тогда
		Если НЕ "" + Значение = "" Тогда // указано имя узла
			Если Дочерний(Узел) = Неопределено Тогда // значение не указано
				ОпределениеУзла = ОпределениеУзла(Узел, Значение);
				Если НЕ ОпределениеУзла = Неопределено Тогда
					...
					КонецЕсли;
				КонецЕсли;
			Иначе // назначить узлу значение
				УзелЗначение(Узел);
			КонецЕсли;
		Иначе
			Состояние = Интерпретировать(Дочерний(Узел));
		КонецЕсли;

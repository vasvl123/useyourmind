	...
	Пока НЕ РодительУзел = Неопределено Цикл
		Если РодительУзел.Имя = "Узел" ИЛИ РодительУзел.Имя = "У" ИЛИ РодительУзел.Имя = "Объект" ИЛИ РодительУзел.Имя = "О" Тогда
			...
			Если РодительУзел.Имя = "Объект" ИЛИ РодительУзел.Имя = "О" Тогда
				// изменения в объекте
				УзелСостояниеЗначение(РодительУзел, "Расширение", Неопределено);
				Показать = Истина;
			Иначе
...
Функция УзелЗначение(Знач Узел) Экспорт
	...
	Если Узел.Имя = "О" ИЛИ Узел.Имя = "Объект" Тогда
		Расширение = "" + ОбработатьОбъект(Узел);
	ИначеЕсли НЕ (ИмяУзла = "" ИЛИ Узел.Имя = "У" ИЛИ Узел.Имя = "Узел") Тогда // нужно найти определение узла
		Узел = ОпределениеУзла(Узел, ИмяУзла);
	КонецЕсли;

	Если НЕ Узел = Неопределено Тогда
		Состояние = УзелСостояние(Узел, "ЗначениеУзла"); // оптимизация
		Если Обновить ИЛИ Состояние = Неопределено ИЛИ Состояние = НетЗначения Тогда
...
Функция ДобавитьСвойство(Узел, стрСвойства) Экспорт
	УзелСвойства = Дочерний(Узел);
	Если НЕ УзелСвойства = Неопределено Тогда
		Если УзелСвойства.Имя = "Свойства" Тогда
			...
			Если НЕ Свойства = Неопределено Тогда
				...
				Иначе
					элСвойство = НовыйПоследний(Дочерний(УзелСвойства), стрСвойства, Истина);
...
Функция ДобавитьСобытие(Узел, Событие) Экспорт
	Пока НЕ (Узел.Имя = "О" ИЛИ Узел.Имя = "Объект") Цикл
		Узел = Узел.Родитель;
	КонецЦикла;
	События = Атрибут(Узел);
	Если НЕ События = Неопределено Тогда
		Пока НЕ События = Неопределено Цикл
			Если События.Имя = "События" Тогда
				Прервать;
			КонецЕсли;
			предАтр = События;
			События = Соседний(События);
		КонецЦикла;
		Если События = Неопределено Тогда
			События = НовыйСоседний(предАтр, Новый Структура("Имя, Значение", "События", ""), Истина);
		КонецЕсли;
	Иначе
		События = НовыйАтрибут(Узел, Новый Структура("Имя, Значение", "События", ""), Истина);
	КонецЕсли;
	НовыйДочерний(События, Новый Структура("Имя, Значение", Узел.Код, Событие), Истина);
...
Функция ОбработатьОбъект(Знач Узел, структЗадача = Неопределено) Экспорт
	...
	Если НЕ Библиотека = Неопределено Тогда
		...
		УзелВид = Библиотека.ОбработатьОбъект(ЭтотОбъект, Узел, ТипОбъекта, Аргументы, структЗадача);
		Если НЕ УзелВид = Неопределено Тогда
			дУзелВид = Дочерний(УзелВид);
			Если НЕ дУзелВид = Неопределено Тогда
				Состояние = Интерпретировать(дУзелВид, , Ложь);
			Иначе
				Состояние = "" + УзелСвойство(УзелВид, "Значение");
			КонецЕсли;
		КонецЕсли;
	Иначе
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	ИначеЕсли Имя = "Свойство" Тогда // получить значение свойства
		свУзел = Узел;
...
Функция Атрибут(Знач Узел) Экспорт
	...
	Если НЕ АтрибутУзел = Неопределено Тогда
		...
		УзелСостояниеЗначение(АтрибутУзел, "ЭтоАтрибут", Истина)
	КонецЕсли;
	Возврат АтрибутУзел;

...
Функция ОбработатьОбъект(Знач Узел, Конструктор = Ложь) Экспорт
	...
	Если Библиотека = Неопределено Тогда
		...
		Если НЕ Узел = Корень Тогда
			Если НЕ Корень.Свойства = Неопределено Тогда
				Если Корень.Свойства.Свойство("д") Тогда
					Версии = УзелСвойство(Корень.Свойства.д, "Версии");
					Если НЕ Версии = Неопределено Тогда
						уВерсия = УзелСвойство(Версии.д, ИмяБиблиотеки);
						Если НЕ уВерсия = Неопределено Тогда
							Версия = уВерсия.Значение;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	Если Изменения.Количество() Тогда
		...
		Если Рефлектор.МетодСуществует(Библиотека, ИмяФункции) Тогда
			...
			Если Рефлектор.ВызватьМетод(Библиотека, ИмяФункции, Параметры) = Ложь Тогда // запустить повторно
				ОбъектыОбновить.Вставить(Узел, Истина);
				Узел.Изменения.Вставить(Узел, Истина);

...
Функция ОбработатьОбъект(Знач Узел) Экспорт
	...
	Возврат "<div id=""_" + Узел.Код + """>" + Состояние + Содержимое + "</div>";
...
Функция ПолучитьУзел(Код, Старший = Неопределено) Экспорт
	...
	Если Узел.Свойство("Родитель") Тогда
		Если Узел.Имя = "Свойства." И (Узел.Родитель.Имя = "О" ИЛИ Узел.Родитель.Имя = "Объект") Тогда
			Если НЕ Узел.Свойство("д") Тогда
				Узел.Вставить("д", Новый Структура);
			КонецЕсли;
		ИначеЕсли Узел.Родитель.Свойство("д") Тогда
			УзелИмя = Узел.Имя;

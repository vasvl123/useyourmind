...
Функция ОбработатьОбъект(Знач Узел, Инициализация = Ложь) Экспорт
	...
	Если Изменения = Неопределено Тогда // инициализация
		...
		Для каждого Аргумент Из Аргументы Цикл
			Свойства.д.Вставить(Аргумент.Ключ, Аргумент.Значение);
		КонецЦикла;
...
Функция ПрочитатьВетку(Знач Узел)
	Если НЕ Узел = Неопределено Тогда
		ПрочитатьВетку(ПолучитьУзел(Узел.Атрибут));
		ПрочитатьВетку(ПолучитьУзел(Узел.Дочерний));
		ПрочитатьВетку(ПолучитьУзел(Узел.Соседний));
	КонецЕсли;
КонецФункции

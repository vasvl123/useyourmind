...
Функция СвойстваВСтуктуру(Свойства) Экспорт
	Результат = Новый Структура;
	Для каждого элСвойства Из Свойства Цикл
		Имя = элСвойства.Ключ;
		Узел = элСвойства.Значение;
		Значение = "";
		Если НЕ Узел.Свойство("Код") Тогда // это группа свойств
			Значение = СвойстваВСтуктуру(Узел);
		Иначе // это узел
			Если Узел.Значение = "" Тогда
				Дочерний = Дочерний(Узел);
				Если НЕ Дочерний = Неопределено Тогда
					Значение = Интерпретировать(Дочерний, , Ложь);
				КонецЕсли;
			Иначе
...
Функция НайтиОбъект(Знач Узел, Путь)
	...
	Если МассивПуть[0] = "р" Тогда // найти родителя
		Пока НЕ Узел = Неопределено Цикл
		...
		КонецЦикла;
		Возврат Узел;
	ИначеЕсли МассивПуть[0] = "д" Тогда // найти дочернего
		Узел = Дочерний(Узел);
	ИначеЕсли МассивПуть[0] = "с" Тогда // найти соседнего
		Узел = Дочерний(Узел.Родитель);
	КонецЕсли;

	Если НЕ МассивПуть.Количество() = 1 Тогда // не свой объект
		Пока НЕ Узел = Неопределено Цикл
			Если СравнитьИмя(Узел, оТип, оИмя) Тогда
				Прервать;
			КонецЕсли;
...
КонецФункции // НайтиОбъект(Узел, Путь)


Функция ПолучитьСвойства(УзелСвойства) Экспорт
	Свойства = УзелСостояние(УзелСвойства, "Свойства");
	Если Свойства = Неопределено Тогда
		Свойства = Новый Структура("ЭтотУзел", УзелСвойства);
		св = Дочерний(УзелСвойства);
		Пока НЕ св = Неопределено Цикл
			ИмяСвойства = св.Имя;
			Если Прав(ИмяСвойства, 1) = "." Тогда
				ИмяСвойства = Лев(ИмяСвойства, СтрДлина(ИмяСвойства) - 1);
				Свойства.Вставить(ИмяСвойства, ПолучитьСвойства(св));
			Иначе
				Свойства.Вставить(ИмяСвойства, св);
			КонецЕсли;
			св = Соседний(св);
		КонецЦикла;
		УзелСостояниеЗначение(УзелСвойства, "Свойства", Свойства); // оптимизация
		Если УзелСвойства.Родитель.Имя = "О" ИЛИ УзелСвойства.Родитель.Имя = "Объект" Тогда
			ОбъектыОбновить.Вставить(УзелСвойства, УзелСвойства.Родитель); // инициализация объекта
		КонецЕсли;
	КонецЕсли;
	Возврат Свойства;
...
Функция НайтиСвойствоОбъекта(Знач Узел, Путь)
	МассивПуть = СтрРазделить(Путь, " ");
	Свойство = Неопределено;
	Если НЕ Узел = Неопределено Тогда
		Свойство = ПолучитьСвойства(Дочерний(Узел));
		...
		Пока к < МассивСвойства.Количество() Цикл
			...
			Свойство = УзелСвойство(Свойство, ИмяСвойства);
	...
	Если НЕ Свойство = Неопределено Тогда
		Если Свойство.Свойство("ЭтотУзел") Тогда // это группа свойств
			Свойство = Свойство.ЭтотУзел;
		КонецЕсли;
	КонецЕсли;
	Возврат Свойство;
КонецФункции // НайтиСвойствоОбъекта()


Функция ЗначениеСвойстваОбъекта(Знач Узел, ПолучитьСсылку = Ложь) Экспорт
	...
	оУзел = НайтиОбъект(Узел, Путь);
	Если оУзел = Неопределено Тогда
		ВызватьИсключение "Объект не найден: " + Путь;
	КонецЕсли;

	Если Путь = "Содержимое" Тогда // это временное свойство
		Возврат "" + УзелСвойство(УзелСостояние(Дочерний(оУзел), "Свойства"), "Содержимое");
	КонецЕсли;

	Узел = НайтиСвойствоОбъекта(оУзел, Путь);

	Если Узел = Неопределено Тогда
		ВызватьИсключение "Свойство не найдено: " + Путь;
	КонецЕсли;
	...
	Если НЕ Узел = Неопределено Тогда
		Если ПолучитьСсылку Тогда
			Значение = Узел;
		Иначе
			Значение = ЗначениеСвойства(Узел);
		КонецЕсли;
		...
		Пока НЕ нОбъект = Неопределено Цикл
			Если нСвойство = НачальныйУзел И НЕ УзелСостояние(нОбъект.Родитель, "Свойства") = Неопределено Тогда // группа свойств
				Сообщить(нОбъект.Код);
				нСвойство = нОбъект;
			КонецЕсли;
		...
		Связи = УзелСостояние(Узел, "Связи"); // связанные свойства
		Если Связи = Неопределено Тогда
			Связи = Новый Соответствие;
...
Функция ДобавитьСобытие(Узел, стрСобытие) Экспорт
	УзелОбъект = Узел;
	Пока НЕ (УзелОбъект.Имя = "О" ИЛИ УзелОбъект.Имя = "Объект") Цикл
		УзелОбъект = УзелОбъект.Родитель;
	КонецЦикла;
	УзелСвойства = Дочерний(УзелОбъект); // потом переделать
	УзелСобытия = Дочерний(УзелСвойства);
	НовыйДочерний(УзелСобытия, стрСобытие, Истина);
	ОбъектыОбновить.Вставить(УзелСобытия, УзелОбъект);
КонецФункции // ДобавитьСобытие()


Функция ОбъектФорма(Узел, Свойства)
	Вид = "<script>var p = {id: '" + Узел.Код + "', fid: '" + Свойства.Форма.Код + "'";
	св = Дочерний(Свойства.Форма);
	Пока не св = Неопределено Цикл
...
Функция ОбъектВид(Узел, Свойства)
	Вид = "" + ЗначениеСвойства(Свойства, "Описание");
	Возврат Вид;
КонецФункции


Функция СоздатьСвойства(Узел, шСвойства)
	св = Неопределено;
	УзелСвойства = НовыйДочерний(Узел, Новый Структура("Имя, Значение", "Свойства", ""));
	к = -1;
	св = УзелСвойства;
	...
	Для каждого стр Из стрСвойства Цикл // парсер
		Если НЕ стр = "" Тогда
			...
			КонецЕсли;
			Если т > к Тогда
				св = НовыйДочерний(св, Новый Структура("Имя, Значение", Имя, "" + Зн));
				к = т;
			Иначе
				Пока т < к Цикл
					св = св.Родитель;
					к = к - 1;
				КонецЦикла;
				св = НовыйСоседний(св, Новый Структура("Имя, Значение", Имя, "" + Зн));
			КонецЕсли;
			// атрибуты
			ат = св;
			Пока п < м.Количество() Цикл
				...
				КонецЕсли;
				Если ат = св Тогда
					ат = НовыйАтрибут(ат, Новый Структура("Имя, Значение", Имя, "" + Зн));
				Иначе
					ат = НовыйСоседний(ат, Новый Структура("Имя, Значение", Имя, "" + Зн));
				КонецЕсли;
				п = п + 1;
			КонецЦикла;
		КонецЕсли;
...
Функция ОбновитьСвязи(Знач Узел) Экспорт
	...
	Если НЕ Связи = Неопределено Тогда
		Для каждого элУзел Из Связи Цикл
			Если элУзел.Значение = Узел Тогда // свои свойства не обновлять
				Продолжить;
			КонецЕсли;
			Сообщить("обн. " + элУзел.Ключ.Код);
			ОбъектыОбновить.Вставить(элУзел.Ключ, элУзел.Значение);
		КонецЦикла;
	КонецЕсли;
КонецФункции


Функция ОбработатьОбъект(Знач Узел) Экспорт

	Сообщить("о. " + Узел.Код);

	ТипИмя = Узел.Значение;

	Если ТипИмя = "" Тогда
		Возврат Ложь;
	КонецЕсли;

	мТипИмя = СтрРазделить(ТипИмя, " ");
	ТипОбъекта = мТипИмя[0];
	Состояние = "";
	...
	Если НЕ Библиотека = Неопределено Тогда
		...
		Если Свойства = Неопределено Тогда
			...
			КонецЕсли;
			УзелСвойства = СоздатьСвойства(Узел, шСвойства);
			Свойства = ПолучитьСвойства(УзелСвойства);
		КонецЕсли;

		Изменения = Новый Соответствие;
		Для каждого эл Из ОбъектыОбновить Цикл
			Если эл.Значение = Узел Тогда
				Изменения.Вставить(эл.Ключ, Истина);
			КонецЕсли;
		КонецЦикла;
		...
		Если Изменения.Количество() Тогда
			...
			Для каждого эл Из Изменения Цикл
				ОбъектыОбновить.Удалить(эл.Ключ);
			КонецЦикла;
			...
			Если Рефлектор.МетодСуществует(Библиотека, ИмяФункции) Тогда
				...
				Изменения = Рефлектор.ВызватьМетод(Библиотека, ИмяФункции, Параметры);
			КонецЕсли;
	...
	Если НЕ Свойства = Неопределено Тогда
		УзелВид = УзелСвойство(Свойства, "Вид");
		Если УзелВид = Неопределено Тогда // Стандартный вид
			Состояние = ОбъектВид(Узел, Свойства);
		Иначе
			дУзелВид = Дочерний(УзелВид);
			Если НЕ дУзелВид = Неопределено Тогда
				Свойства.Вставить("Содержимое", Содержимое);
				Содержимое = "";
				Состояние = "" + УзелСвойство(дУзелВид, "Значение");
				Если Состояние = "" Тогда
					Состояние = Интерпретировать(дУзелВид, , Ложь);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		Форма = УзелСвойство(Свойства, "Форма");
		Если НЕ Форма = Неопределено Тогда
			Состояние = Состояние + ОбъектФорма(Узел, Свойства);
		КонецЕсли;
	КонецЕсли;

	Состояние = Состояние + Содержимое;
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	Пока НЕ СледУзел = Неопределено Цикл
		...
		ИначеЕсли Имя = "Ссылка" Тогда
			Если НЕ "" + Значение = "" Тогда // указан путь к свойству объекта
				Состояние = ЗначениеСвойстваОбъекта(Узел, Истина);
			КонецЕсли;

		ИначеЕсли Имя = "Ключ" ИЛИ Имя = "К" Тогда

		ИначеЕсли Имя = "Значение" Тогда // значение свойства объекта
			Если НЕ "" + Значение = "" Тогда
				Состояние = ЗначениеСвойстваОбъекта(Узел);
			КонецЕсли;
...
Функция НовоеЗначениеУзла(Узел, СтруктураУзла, Служебный = Ложь) Экспорт
	ДочернийУзел = Дочерний(Узел);
	Если НЕ ДочернийУзел = Неопределено Тогда
		// прежний дочерний нужно удалять
		УдалитьУзел(ДочернийУзел, , Ложь);
	КонецЕсли;
	Возврат НовыйДочерний(Узел, СтруктураУзла, Служебный);
...
Функция УдалитьУзел(Узел, Совсем = Истина, НачальныйУзел = Истина) Экспорт
	...
	Если Совсем ИЛИ ЭтоСлужебный Тогда
		Если НЕ УзелСвойство(Узел, "Атрибут") = Неопределено Тогда
			УдалитьУзел(Атрибут(Узел), , Ложь);
		КонецЕсли;
		Если НЕ УзелСвойство(Узел, "Дочерний") = Неопределено Тогда
			УдалитьУзел(Дочерний(Узел), , Ложь);
		КонецЕсли;

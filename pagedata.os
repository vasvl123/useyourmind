...
Функция ОбновитьУзел(Знач Узел, НачальныйУзел = Истина) Экспорт

	Если НачальныйУзел И (Узел = Корень ИЛИ Узел.Имя = "Элемент" ИЛИ Узел.Имя = "Э") Тогда
		ОбновитьПредставлениеУзла(Корень);
			...
			Для каждого элУзел Из Связи Цикл
				...
				Пока НЕ РодительУзел = Неопределено Цикл
					Если РодительУзел.Имя = "Узел" ИЛИ РодительУзел.Имя = "У" Тогда
						...
						КонецЕсли;
						ОбновитьУзел(РодительУзел, Ложь);
					КонецЕсли;
...
Функция ПоказатьУзел(Знач Узел, Атрибуты = "", Дочерний = "", ЭтоАтрибут = Ложь) Экспорт
	...
	Если ЭтоАтрибут Тогда
		...
		Если АтрибутИмя = "ПриНажатии" Тогда
			Представление = Представление + " onclick=""addcmd(this); return false""";
		Иначе
...
// Создать копию ветки
Функция КопироватьВетку(Узел, Цель, Старший, Родитель, ЭтоАтрибут = Ложь, ПервыйВызов = Истина, Служебный = Истина, ПараметрыЗамены = Неопределено) Экспорт
...
Функция УзелЗначение(Знач Узел, Обновлять = Истина) Экспорт
	...
	Если НЕ (ИмяУзла = "" ИЛИ Узел.Имя = "У" ИЛИ Узел.Имя = "Узел") Тогда // нужно найти определение узла
		Узел = ОпределениеУзла(Узел, ИмяУзла, Обновлять);
	КонецЕсли;
...
Функция РасширениеУзла(Знач Узел)
	...
	Если НЕ Атр = Неопределено Тогда
		...
		Если Расширение = Неопределено Тогда
			...
			УзелСостояниеЗначение(Узел, "Расширение", Атр.Имя);

		КонецЕсли;

	Иначе
		УзелСостояниеЗначение(Узел, "Расширение", Ложь); // нет расширения
	КонецЕсли;
...
Функция ОбновитьДанные(Запрос) Экспорт
	Для каждого элПараметр Из Запрос Цикл
		...
		Если НЕ Определение = Неопределено Тогда
			УзелУстановитьЗначение(Определение, элПараметр.Значение);
		КонецЕсли;
	КонецЦикла;
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	ИначеЕсли Имя = "Строка" Тогда
		...
		КонецЕсли;
	ИначеЕсли (Имя = "Аргумент" ИЛИ Имя = "А") И этоАтрибут Тогда // потом убрать
	...
	ИначеЕсли Имя = "Значение" ИЛИ Имя = "З" Тогда
		Если НЕ "" + Значение = "" Тогда
			СвойствоОбновлять = НайтиАтрибут(Узел, "Обновлять", "Нет"); // если не нужно автоматически пересчитывать зависимости
			Состояние = УзелЗначение(Узел, СвойствоОбновлять = Неопределено);
		КонецЕсли;
	ИначеЕсли Имя = "Элемент" ИЛИ Имя = "Э" Тогда
		Состояние = ПоказатьУзел(Узел, "", УзелЗначение(Узел));
	ИначеЕсли Имя = "Пусть" ИЛИ Имя = "П" Тогда // потом убрать наверно
		Значение = Дочерний(Узел);
		Если НЕ Значение = Неопределено Тогда
			Значение = Интерпретировать(Значение, , Ложь);
			ВставитьЗначение(Узел, Значение);
		КонецЕсли;
	ИначеЕсли Имя = "Вставить" ИЛИ Имя = "В" Тогда // потом убрать наверно
		Значение = Дочерний(Узел);
		Если НЕ Значение = Неопределено Тогда
			ВставитьЗначение(Узел, Значение);
		КонецЕсли;

...
Функция ОбъектВид(Узел, Свойства)
	УзелОписание = УзелСвойство(Свойства, "Описание");
	Если УзелОписание = Неопределено Тогда
		Вид = Интерпретировать(Дочерний(Узел), , Ложь);
	Иначе
...
Функция ОбработатьОбъект(Знач Узел, Конструктор = Ложь) Экспорт
	...
	Сообщить("о. " + Узел.Код);
 	...
 	Если НЕ Узел.Свойство("Состояние") Тогда
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	Пока НЕ СледУзел = Неопределено Цикл
		...
		ИначеЕсли Имя = "Значение" Тогда // значение свойства объекта
			Если Значение = "Содержимое" Тогда
				Пока НЕ Узел = Неопределено Цикл
					Если Узел.Имя = "Свойства." Тогда
						Прервать;
					КонецЕсли;
					Узел = Узел.Родитель;
				КонецЦикла;
				Состояние = Интерпретировать(Узел, , Ложь);
			Иначе

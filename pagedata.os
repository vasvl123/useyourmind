...
Функция Родитель(Знач Узел) Экспорт
	Если НЕ ТипЗнч(Узел) = Тип("Структура") Тогда
		ВызватьИсключение "Неверный узел";
	КонецЕсли;
	Если Узел.Свойство("Родитель", Узел) Тогда
		Возврат ПолучитьУзел(Узел);
	КонецЕсли;
	УзелСтарший = Неопределено;
	Пока Узел.Свойство("Старший", УзелСтарший) Цикл
		УзелКод = Узел.Код;
		Узел = ПолучитьУзел(УзелСтарший);
		Если Узел.Свойство("Дочерний") Тогда
			Если Узел.Дочерний = УзелКод Тогда
				Возврат Узел;
			КонецЕсли;
		КонецЕсли
	КонецЦикла;

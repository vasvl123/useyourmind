...
Функция ЭлементОкружения(Знач СтаршийУзел, Знач ИмяЭлемента) Экспорт
	Перем Элемент;
	Пока Элемент = Неопределено И НЕ СтаршийУзел = Неопределено Цикл
		Окружение = УзелСостояние(СтаршийУзел, "Окружение");
...
Функция ОкружениеУзла(Узел) Экспорт
	Элементы = НовыйУзел(Новый Структура("Имя", "Элементы"), Истина);
...
Функция ОбновитьПредставление() Экспорт
	Представление = "";
	Если Обновить Тогда
		Представление = Интерпретировать(ПолучитьУзел("1"));
	КонецЕсли;

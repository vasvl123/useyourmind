...
Функция НайтиСвойствоОбъекта(Знач Узел, Путь)
	МассивПуть = СтрРазделить(Путь, " ");
	Если НЕ Узел = Неопределено Тогда
		Свойства = ПолучитьСвойства(Дочерний(Узел));
		Если НЕ Свойства = Неопределено Тогда
			...
			Пока к < МассивСвойства.Количество() Цикл
				Свойства = УзелСвойство(Свойства, МассивСвойства[к]);
				к = к + 1;
			КонецЦикла;
		Иначе
			Сообщить("а нет свойств");
		КонецЕсли;
	КонецЕсли;
	Возврат Свойства;
...
Функция ЗначениеСвойстваОбъекта(Знач Узел) Экспорт
	...
	Если НЕ Узел = Неопределено Тогда
		...
		КонецЕсли;
		Связи.Вставить(НачальныйУзел, нОбъект);
...
Функция ОбработатьОбъект(Знач Узел) Экспорт
	...
	Если НЕ Библиотека = Неопределено Тогда
		...
		Если Свойства = Неопределено Тогда
			...
			КонецЕсли;
			Свойства = СоздатьСвойства(Узел, шСвойства);
		КонецЕсли;

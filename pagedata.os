...
Функция СоздатьСвойства(Узел, стрСвойства)
	св = Неопределено;
	...
	Для каждого эл Из стрСвойства Цикл
		Если св = Неопределено Тогда
			св = НовыйДочерний(УзелСвойства, Новый Структура("Имя, Значение", эл.Ключ, "" + эл.Значение));
		Иначе
			св = НовыйСоседний(св, Новый Структура("Имя, Значение", эл.Ключ, "" + эл.Значение));
		КонецЕсли;
	...
	Если НЕ Библиотека = Неопределено Тогда
		...
		Если Свойства = Неопределено Тогда
			...
			Если Рефлектор.МетодСуществует(Библиотека, ИмяФункции) Тогда
				...
				стрСвойства = Рефлектор.ВызватьМетод(Библиотека, ИмяФункции, Параметры);
			КонецЕсли;
			Свойства = СоздатьСвойства(Узел, стрСвойства);
		КонецЕсли;

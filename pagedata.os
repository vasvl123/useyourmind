...
Функция ПолучитьПредставление(нУзел = Неопределено) Экспорт
	...
	Попытка
		...
		ИначеЕсли ОбъектыОбновить.Количество() Тогда
			Сообщить("ПолучитьПредставление ");
		...
		Пока Очередь.Количество() Цикл
			...
			Сообщить("На очереди у " + Узел.Код);
			...
			Если Имя = "Объект" ИЛИ Имя = "О" Тогда // это функциональный узел
	...
	Если НЕ уСвойство = Неопределено Тогда
		...
		Для каждого Свойство Из Свойства Цикл
			...
			Если НЕ уЗначение = Неопределено Тогда
				Значение = ПолучитьПредставление(уЗначение);
...
// 			Состояния.Удалить(элУзел.Ключ);
// 			Состояния.Удалить(элУзел.Значение);

...
Функция ОбъектЗначение(Окружение, Знач ИмяЭлемента, Знач Значение)
	Если НЕ Значение = Окружение.Элементы[ИмяЭлемента] Тогда
...
Функция ПоказатьУзел(Знач Окружение, Узел, ЭтоАтрибут = Ложь, Атрибуты = "", Дочерний = "") Экспорт
	...
	Иначе
		...
		Если Узел.Имя = "comment" Тогда
			...
			КонецЕсли;
		Иначе
			Представление = Представление + "<" + Узел.Имя + Атрибуты + " id=""" + "_" + Узел.Код + """>";
			Если Узел.Свойство("Значение") Тогда
				Представление = Представление + Узел.Значение;
			КонецЕсли;
...
Функция ОбновитьПредставление() Экспорт
	...
	Если ОбъектыОбновить.Количество() Тогда
			...
			Для каждого элОбъект Из Объекты Цикл
				Если элОбъект.Значение.Значение = элИмяЭлемента.Ключ Тогда
					...
					Пока НЕ РодительУзел = Неопределено Цикл
						Если УзелСостояние(РодительУзел, "Обновить") = Ложь Тогда
		...
		Для каждого элУзел Из УзлыОбновить Цикл
			Представление = Представление + Интерпретировать(Окружение, элУзел.Значение);
		КонецЦикла;
	Иначе
...
Функция Интерпретировать1(Знач Окружение, Знач Узел, Знач Применить = Истина, ЭтоАтрибут = Ложь) Экспорт
	Сообщить(Узел.Код + " Узел:" + Символы.Таб + СтруктуруВСтроку(Узел));
	Значение = Интерпретировать1(Окружение, Узел, Применить, ЭтоАтрибут);
...
Функция Интерпретировать(Знач Окружение, Знач Узел, Знач Применить = Истина, ЭтоАтрибут = Ложь) Экспорт
	...
	Иначе
		...
		Если НЕ Обновить Тогда
			Если УзелСостояние(Узел, "Обновить") = Ложь Тогда
				Возврат "";
			КонецЕсли;
		КонецЕсли;
		...
		Если НЕ ЭтоАтрибут Тогда
			...
			КонецЕсли;
			Состояние = ПоказатьУзел(Окружение, Узел, , ЗначениеУзелАтрибут, ЗначениеУзелДочерний);
		Иначе

...
Функция ОбновитьПредставление() Экспорт
	...
	Пока Циклов < 20 И УзлыОбновить.Количество() Цикл
				...
				Для каждого элСвязь Из элУзел.Значение Цикл
					Если элСвязь.Значение = ЭтотОбъект Тогда
						...
						Если Узел = Корень ИЛИ Узел.Имя = "Элемент" ИЛИ Узел.Имя = "Э" Тогда
							Состояния.Вставить(Узел.Код, Состояние);

						ИначеЕсли Узел.Имя = "Значение" ИЛИ Узел.Имя = "З" Тогда
	...
	Для каждого элСостояние Из Состояния Цикл
		Представление = Представление + элСостояние.Значение;
	КонецЦикла;

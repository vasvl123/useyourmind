	...
	Если Событие = "ПриНажатии" Тогда
		...
		Если НЕ АтрибутПриНажатии = Неопределено Тогда
			...
			Если НЕ уСвойство = Неопределено Тогда
				Свойство = ПолучитьСвойствоПоСсылке(уСвойство);
				уЗначение = Соседний(уСвойство);
				Если НЕ уЗначение = Неопределено Тогда
					сЗначение = ПолучитьСвойствоПоСсылке(уЗначение);
	...
	ИначеЕсли Событие = "ПриИзменении" Тогда
		...
		Если НЕ АтрибутЗначение = Неопределено Тогда
			...
			Если НЕ зСвойство = Неопределено Тогда
				Свойство = ПолучитьСвойствоПоСсылке(зСвойство);
				НовоеЗначениеУзла(Свойство, ИмяЗначение("Строка", Значение), Истина);
				Возврат Неопределено; // событие не создается
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;

	стрСобытие = ИмяЗначение("" + Процесс.ПолучитьИД(), Событие + " " + Узел.Код + " " + Значение);
...
Функция ОбработатьОбъект(Знач Узел, Конструктор = Ложь) Экспорт
	...
	Сообщить("ОбработатьОбъект " + Узел.Код + " " + ТипИмя);

	мТипИмя = СтрРазделить(ТипИмя, " ");
	ТипОбъекта = мТипИмя[0];
	ИмяОбъекта = ?(мТипИмя.Количество() > 1, мТипИмя[1], "");
	...
	Если Свойства = Неопределено Тогда // это новый объект
		...
		КонецЕсли;
		Свойства = НовыйДочерний(Узел, ИмяЗначение("Свойства.", ""));

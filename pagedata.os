...
Функция ОпределениеОбъекта(Знач Узел, Знач ИмяЭлемента, УзелШаблон = Ложь) Экспорт

	НачальныйУзел = Узел;
	Узел = Узел.Старший;
	Пока НЕ Узел = Неопределено Цикл
		Если УзелШаблон Тогда
	...
	Если Дочерний(Узел) = Неопределено Тогда
		Процесс.ЗаписатьСобытие("Интерпретатор", СтрШаблон("Неверное определение объекта %1", ИмяЭлемента), 3);
		Возврат Неопределено;
	КонецЕсли;
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	ИначеЕсли Имя = "Ссылка" ИЛИ Имя = "С" Тогда
		Определение = ОпределениеОбъекта(Узел, Узел.Значение, Истина);

	...
	Пока НЕ Узел = Неопределено Цикл
		Если Узел.Имя = "Перем" Тогда
			Если Узел.Значение = ИмяЭлемента Тогда
				Узел.Вставить("Дочерний", Значение.Код);
				// Установить для обновления узлы которые нужно обновить
				Объекты = УзелСостояние(Узел, "Объекты");
				Если НЕ Объекты = Неопределено Тогда
					УзлыОбновить.Вставить(Значение, Объекты);
...
Функция ОбъявлениеОбъекта(Знач Узел, Знач ИмяЭлемента, Отладка = Ложь) Экспорт
	НачальныйУзел = Узел;
	// Поиск объявления объекта
	Узел = Старший(Узел);
	Пока НЕ Узел = Неопределено Цикл
	...
	Если НЕ Отладка Тогда
		Если Узел.Имя = "Функция" Тогда
			ЗначениеОбъекта = ТелоФункции(Узел, НачальныйУзел);
		ИначеЕсли Узел.Имя = "Использовать" Тогда
	...
	Если Узел.Имя = "Функция" ИЛИ Узел.Имя = "Использовать" Тогда
		Если НЕ ЗначениеОбъекта = Неопределено Тогда
			НачальныйУзел.Вставить("Дочерний", ЗначениеОбъекта.Код);
			ЗначениеОбъекта.Вставить("Старший", НачальныйУзел.Код);
		КонецЕсли;
	Иначе
...
Функция ПолучитьЗначениеОбъекта(Знач Узел) Экспорт
	ЗначениеОбъекта = Дочерний(Узел);
	Если ЗначениеОбъекта = Неопределено Тогда
		ЗначениеОбъекта = ОбъявлениеОбъекта(Узел, Узел.Значение, Ложь);
		Если НЕ ЗначениеОбъекта = Неопределено Тогда
			Узел.Вставить("Дочерний", ЗначениеОбъекта.Код);
			ЗначениеОбъекта.Вставить("Старший", Узел.Код);
		КонецЕсли;
	КонецЕсли;
КонецФункции // ПолучитьЗначениеОбъекта()
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	Если Имя = "Объект" Тогда
		...
		Иначе
			...
			ЗначениеОбъекта = Дочерний(Узел);
			Если ЗначениеОбъекта = Неопределено Тогда
				ЗначениеОбъекта = ОбъявлениеОбъекта(Узел, Значение);
			КонецЕсли;
			Если НЕ ЗначениеОбъекта = Неопределено Тогда
...
Функция ПрочитатьВетку(Знач Узел, Знач СтаршийУзел = Неопределено)
	Если НЕ Узел = Неопределено Тогда
		...
		Если НЕ СтаршийУзел = Неопределено Тогда
			...
			Если НЕ УзелСтарший = СтаршийУзел.Код Тогда
				Сообщить("Узел " + Узел.Код + " неверный код старшего узла");
				Узел.Вставить("Старший", СтаршийУзел.Код);
			КонецЕсли;
		КонецЕсли;

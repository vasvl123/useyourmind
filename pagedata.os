...
Функция ЭлементОкружения(Знач Окружение, Знач ИмяЭлемента) Экспорт
	...
	Если Элемент = Неопределено Тогда
		ВызватьИсключение "ПоследнийУзел: " + ПоследнийУзел + " " + СтрШаблон("Неизвестный объект %1", ИмяЭлемента);
	КонецЕсли;
	Возврат Элемент;
...
Функция Сумма(Знач Окружение, Знач Аргумент)
	...
	Если Аргумент = Неопределено Тогда
		ВызватьИсключение "ПоследнийУзел: " + ПоследнийУзел + " " + "Ожидается аргумент";
	КонецЕсли;
...
Функция ЗначениеФункции(Знач Окружение, Знач Параметр, Знач Аргумент)
	...
	Если Выражение = Неопределено Тогда
		ВызватьИсключение "ПоследнийУзел: " + ПоследнийУзел + " " + "Ожидается тело функции";
	КонецЕсли;
	Если Параметр.Имя = "Список" Тогда
		Параметр = Дочерний(Параметр);
		Пока Параметр <> Неопределено Цикл
			Если Аргумент = Неопределено Тогда
				ВызватьИсключение "ПоследнийУзел: " + ПоследнийУзел + " " + "Недостаточно фактических параметров";
			КонецЕсли;
...
Функция Лямбда(Знач Окружение, Знач Параметр)
	...
	Если Параметр.Имя = "Список" Тогда
		...
		Если Не ПараметрыКорректны(Параметр) Тогда
			ВызватьИсключение "ПоследнийУзел: " + ПоследнийУзел + " " + "Ожидается имя параметра";
		КонецЕсли;
...
Функция ЗначениеВыраженияВыбор(Знач Окружение, Знач Список)
	...
	Если СписокКогда = Неопределено Тогда
		ВызватьИсключение "ПоследнийУзел: " + ПоследнийУзел + " " + "Ожидается условие";
	КонецЕсли;
	Пока СписокКогда <> Неопределено Цикл
		СписокТогда = Соседний(СписокКогда);
		Если СписокТогда = Неопределено Тогда
			ВызватьИсключение "ПоследнийУзел: " + ПоследнийУзел + " " + "Ожидается выражение";
		КонецЕсли;
	...
	КонецЦикла;
	ВызватьИсключение "ПоследнийУзел: " + ПоследнийУзел + " " + "Ни одно из условий не сработало!";
...
Функция ОбновитьПредставление(КодУзла = "1") Экспорт
	Если ОбъектыОбновить.Количество() Тогда
		...
		Представление = "";
	Иначе
		Представление = Интерпретировать(Окружение, ПолучитьУзел(КодУзла));
	КонецЕсли;
...
Функция Интерпретировать(Знач Окружение, Знач Узел, Знач Применить = Истина, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	Если НЕ ТипЗнч(Узел) = Тип("Структура") Тогда
		//Возврат Узел;
		ВызватьИсключение "ПоследнийУзел: " + ПоследнийУзел + " " + "Неверный узел: " + Узел;
	КонецЕсли;

	ПоследнийУзел = Узел.Код;
...
Функция Соседний(Знач Узел) Экспорт
	Если НЕ ТипЗнч(Узел) = Тип("Структура") Тогда
		ВызватьИсключение "ПоследнийУзел: " + ПоследнийУзел + " " + "Неверный узел";
	КонецЕсли;
...
Функция Родитель(Знач Узел, НайтиРодителя = Истина) Экспорт
	...
	КонецЦикла;
	//ВызватьИсключение "ПоследнийУзел: " + ПоследнийУзел + " " + "Родитель узла " + Узел.Код + " не найден";
	Возврат Неопределено;

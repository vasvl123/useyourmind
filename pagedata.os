	...
	Если Изменения.Количество() Тогда
		...
		Если НЕ Библиотека = Неопределено Тогда
			...
			Если Свойства = Неопределено Тогда // это новый объект
				...
				ОбъектыОбновить.Вставить(Узел, Истина);
				Возврат Свойства;
			КонецЕсли;
...
Функция ПолучитьУзел(Код, Старший = Неопределено) Экспорт
	...
	Если НЕ Старший = Неопределено Тогда
		...
		КонецЕсли;
		Если Прав(Узел.Имя, 1) = "." И (Родитель.Имя = "О" ИЛИ Родитель.Имя = "Объект" ИЛИ НЕ УзелСвойство(Родитель, "д") = Неопределено) Тогда
			Узел.Вставить("д", Новый Структура);
		ИначеЕсли НЕ УзелСвойство(Родитель, "д") = Неопределено Тогда
			Родитель.д.Вставить(Узел.Имя, Узел);
		КонецЕсли;
	КонецЕсли;
...
Функция НовыйУзел(Узел, Служебный = Ложь) Экспорт
	...
	Иначе
		...
		НовыйКод = "" + КодУзла;
	КонецЕсли;
	Если Прав(Узел.Имя, 1) = "." И (Узел.Родитель.Имя = "О" ИЛИ Узел. Родитель.Имя = "Объект" ИЛИ НЕ УзелСвойство(Узел.Родитель, "д") = Неопределено) Тогда
		Узел.Вставить("д", Новый Структура);
	ИначеЕсли НЕ УзелСвойство(Узел.Родитель, "д") = Неопределено Тогда
		Узел.Родитель.д.Вставить(Узел.Имя, Узел);
	КонецЕсли;
...
Функция ПриСозданииОбъекта(обПроцесс, Текст = "", знБазаДанных = "", знИмяДанных = "", знПозицияДанных = "") Экспорт
	...
	Если Количество = 0 Тогда
		Корень = НовыйУзел(Новый Структура("Имя, Значение, Родитель, Старший", "У", "Корень", Неопределено, Неопределено));
	Иначе

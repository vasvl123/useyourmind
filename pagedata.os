...
Функция ОбъектВид(Узел, Свойства)
	...
	Возврат "<div id=""_" + Узел.Код + """>" + Вид + Узел.Содержимое + "</div>";
КонецФункции
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	ИначеЕсли Имя = "Значение" ИЛИ Имя = "З" Тогда // значение свойства объекта
...
Функция НовоеЗначениеУзла(Знач Узел, Значение, Служебный = Ложь, Добавить = Ложь) Экспорт
	Если НЕ ТипЗнч(Значение) = Тип("Структура") ИЛИ НЕ Значение.Свойство("Имя") Тогда
		Значение = Новый Структура("Имя, Значение", Строка(ТипЗнч(Значение)), Значение);
	КонецЕсли;
	ДочернийУзел = Дочерний(Узел);
	Если НЕ ДочернийУзел = Неопределено Тогда
	...
	КонецЕсли;
	Если НЕ ДочернийУзел = Неопределено Тогда
		нУзел = НовыйСоседний(ДочернийУзел, Значение, Служебный);
	Иначе
		нУзел = НовыйДочерний(Узел, Значение, Служебный);
	КонецЕсли;
	ОбновитьУзел(Узел);
	Возврат нУзел;
...
Функция ПриСозданииОбъекта(обПроцесс, Текст = "", знБазаДанных = "", знИмяДанных = "", знПозицияДанных = "") Экспорт
	...
	Если Количество = 0 Тогда
		Корень = НовыйУзел(Новый Структура("Имя, Значение, Родитель, Старший", "О", "Корень", Неопределено, Неопределено));
	Иначе

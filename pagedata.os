...
Функция ОбъектыОбновитьДобавить(оУзел) Экспорт
	Если ОбъектыОбновить.Найти(оУзел) = Неопределено Тогда
	...
	Для каждого стр Из стрСвойства Цикл // парсер
		Если НЕ стр = "" Тогда
			...
			Если Имя = "" И Зн = "" Тогда
				Продолжить;
			КонецЕсли;
			Если Служебный = "Только" Тогда // только служебные
				Если НЕ Служ = Истина Тогда
					Продолжить;
				КонецЕсли;
				Если т = 0 Тогда
					Если Узел.д.Свойство(Имя) Тогда // такое свойство уже существует
						Продолжить;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			...
			Пока п < м.Количество() Цикл
				...
				д = "";
				н = СтрНайти(Зн, "|");
				Если н Тогда // дочернее значение атрибута
					д = Сред(Зн, н + 1);
					Зн = Лев(Зн, н - 1);
				КонецЕсли;
				Если ат = св Тогда
...
Функция ПостроитьСтруктуру(Узел)
	УзелИмя = Узел.Имя;
...
Функция ОбработатьОбъект(Знач Узел, Инициализация = Ложь) Экспорт
	...
	Если Свойства = Неопределено Тогда
		...
		Если НЕ Свойства = Неопределено Тогда
			Если Свойства.Имя = "Свойства." Тогда
	...
	Если Изменения = Неопределено Тогда // инициализация
		...
		Если Рефлектор.МетодСуществует(Библиотека, ИмяФункции) Тогда
			...
			Если Результат = Неопределено Тогда // обработать модель сразу
				Узел.Свойство("Свойства", Свойства);
			ИначеЕсли Результат = Истина ИЛИ Результат = Ложь Тогда // модель обработать позже
				Если Результат = Ложь Тогда // запустить инициализацию повторно
					Узел.Вставить("Изменения", Неопределено);
				КонецЕсли;
				Возврат Неопределено;
			Иначе // свойства по шаблону
				Если НЕ Свойства = Неопределено Тогда
		...
		Аргумент = Узел.Атрибут;
		Пока НЕ Аргумент = Неопределено Цикл
			Свойства.д.Вставить(Аргумент.Имя, Аргумент);
			Аргумент = Аргумент.Соседний;
		КонецЦикла;

		Если Узел = Корень Тогда
			уВерсии = УзелСвойство(Свойства.д, "Версии");
			Если НЕ уВерсии = Неопределено Тогда
				Для каждого эл Из уВерсии.д Цикл
	...
	//Если Изменения.Количество() Тогда
		...
		Если Рефлектор.МетодСуществует(Библиотека, ИмяФункции) Тогда
			...
			КонецЦикла;
			Если Обновить = Истина Тогда // запустить повторно
				ОбъектыОбновить.Добавить(Узел);
			КонецЕсли;
		КонецЕсли;
	//КонецЕсли;
	...
	Пока НЕ рУзел = Неопределено Цикл
		Если рУзел.Свойство("Свойства") Тогда // это объект
	...
	Иначе
		...
		Если НЕ Свойства.Соседний = Неопределено Тогда
...
Функция ВызватьФункцию(Знач Узел) Экспорт

	Если НЕ УзелСвойство(Узел, "Обновить") = Ложь Тогда

		ИмяФункции = УзелСвойство(Узел, "Значение");
		Если НЕ "" + ИмяФункции = "" Тогда
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	Пока НЕ СледУзел = Неопределено Цикл
		...
		ИначеЕсли Имя = "Объект" ИЛИ Имя = "О" Тогда // это объект
			// оптимизация
		...
		ИначеЕсли Имя = "Переменная" ИЛИ Имя = "П" Тогда // значение переменной объекта
			...
			Пока НЕ сУзел = Неопределено Цикл
				Если сУзел.Имя = "Свойства." Тогда
					...
					Если Значение = "Содержимое" И Состояние = Неопределено Тогда
						Состояние = Интерпретировать(сУзел, , Ложь);
...
Функция ПолучитьУзел(Код, Старший = Неопределено) Экспорт
	...
	Если НЕ Узел = Неопределено Тогда
		Возврат Узел;
	КонецЕсли;
	Если ТипЗнч(Код) = Тип("Структура") Тогда
		Возврат Код;
	КонецЕсли;
	...
	Для Каждого знСтр Из Стр Цикл
		...
		Иначе
			Если Узел = Неопределено Тогда
				Узел = Новый Структура("Код, Имя, Значение, Дочерний, Соседний, Атрибут", Код, "");
			КонецЕсли;
			Если Ключ = "И" Тогда
				Ключ = "Имя";
	...
	Если НЕ Узел = Неопределено Тогда
		...
		КонецЕсли;
		Если НЕ Узел.Соседний = Неопределено И НЕ Узел.Имя = "Свойства." Тогда
			Узел.Соседний = ПолучитьУзел(Узел.Соседний, Узел);
		КонецЕсли;
		Если НЕ Узел.Атрибут = Неопределено Тогда
...
Функция УдалитьРодителя(Дочерний) Экспорт
	...
	КонецЕсли;
	// Удалить узел
	УдалитьУзел(РодительУзел, , Истина);
...
Функция Служебный(Узел) Экспорт
	Если НЕ Узел = Неопределено Тогда
...
Функция УдалитьУзел(Знач Узел, Совсем = Истина, Цепочку = Ложь, НачальныйУзел = Истина) Экспорт

	УзелСоседний = Узел.Соседний;

	Если НачальныйУзел ИЛИ Цепочку Тогда

...
Функция НайтиОбъект(Знач Узел, Путь)
	...
	МассивПуть = СтрРазделить(Путь, "_");
	...
	Если НЕ МассивПуть[0] = "" Тогда // свой объект
		Пока НЕ Узел = Неопределено Цикл
			Если СравнитьИмя(Узел, МассивПуть[1], МассивПуть[2]) Тогда
				Прервать;
			КонецЕсли;
...
Функция ЗначениеСвойстваОбъекта(Знач Узел) Экспорт
	...
	Если НЕ (МассивПуть[0] = "" ИЛИ МассивПуть[0] = "р" ИЛИ МассивПуть[0] = "д" ИЛИ МассивПуть[0] = "с") Тогда // найти узел // потом убрать
		Возврат УзелЗначение(Узел);
	КонецЕсли;
...
Функция ДобавитьСобытие(Узел, стрСобытие) Экспорт
	...
	КонецЦикла;
	УзелСвойства = Дочерний(УзелОбъект);

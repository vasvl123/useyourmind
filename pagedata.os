...
Функция ОбработатьОбъект(Знач Узел, Конструктор = Ложь) Экспорт
	...
	Если Изменения.Количество() Тогда
		...
		Если Рефлектор.МетодСуществует(Библиотека, ИмяФункции) Тогда
			...
			Если Рефлектор.ВызватьМетод(Библиотека, ИмяФункции, Параметры) = Ложь Тогда
	...
	// 	Если УзелДочерний.Имя = "О" ИЛИ УзелДочерний.Имя = "Объект" Тогда
	// 		Содержимое = Содержимое + "<span id='_" + УзелДочерний.Код + "'></span>";
	// 		ОбъектыОбновить.Вставить(УзелДочерний, Истина);
	// 	Иначе
	// 		Содержимое = Содержимое + Интерпретировать(УзелДочерний);
	// 	КонецЕсли;
	...
	Состояние = "<span id='_" + Узел.Код + "'>" + Состояние + "</span>";

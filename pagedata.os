...
Функция СтруктуруВСтроку(Структ) Экспорт
	Если НЕ ТипЗнч(Структ) = Тип("Структура") Тогда
		Возврат Структ;
	КонецЕсли;
	Результат = "";
	Для каждого Элемент Из Структ Цикл
		Если НЕ ТипЗнч(Элемент.Значение) = Тип("Структура") И НЕ ("" + Элемент.Значение = "") И НЕ (Элемент.Ключ = "Код")  И НЕ (Элемент.Ключ = "Дочерний" И Лев(Элемент.Значение, 1) = "s") Тогда
...
Функция КопироватьВетку(Узел, Аргументы = Неопределено, ЭтоАтрибут = Ложь, ПервыйВызов = Истина)
	...
	КопияУзел = НовыйУзел(Новый Структура("Имя, Значение", ИмяУзла, ЗначениеУзла), Истина);

	Если НЕ ЭтоАтрибут Тогда

		УзелАтрибут = Атрибут(Узел);
		Если НЕ УзелАтрибут = Неопределено Тогда
			УзелАтрибут = КопироватьВетку(УзелАтрибут, Аргументы, Истина, Ложь);
			УзелАтрибут.Вставить("Старший", КопияУзел.Код);
			КопияУзел.Вставить("Атрибут", УзелАтрибут.Код);
		КонецЕсли;

		УзелДочерний = Дочерний(Узел);
		Если НЕ УзелДочерний = Неопределено Тогда
			УзелДочерний = КопироватьВетку(УзелДочерний, Аргументы, , Ложь);
			УзелДочерний.Вставить("Старший", КопияУзел.Код);
	...
	Иначе
		...
		Если НЕ УзелСоседний = Неопределено Тогда
			УзелСоседний = КопироватьВетку(УзелСоседний, Аргументы, ЭтоАтрибут, Ложь);
			УзелСоседний.Вставить("Старший", КопияУзел.Код);
...
Функция ТелоФункции(Объявление, Применение)
	...
	ИмяФункции = СтрРазделить(Объявление.Имя, ".");
		...
		Если НЕ Библиотека = Неопределено Тогда
			...
			Для каждого Метод Из ТаблицаМетодов Цикл
				Если Метод.Имя = ИмяФункции Тогда
					...
					Параметры.Добавить(Атрибут(Применение));
					ЗначениеОбъекта = Рефлектор.ВызватьМетод(Библиотека, ИмяФункции, Параметры);
					Прервать;
				КонецЕсли
			КонецЦикла;
	...
	Возврат КопироватьВетку(Дочерний(Объявление), Аргументы);
...
Функция ОбъектЗначение(Знач Узел)
	...
	Объекты = УзелСостояние(Объявление, "Объекты");
	Если НЕ Объекты = Неопределено Тогда
		// Соответствие со списком связанных узлов
		Объекты = Объекты[Значение];
		Если НЕ Объекты = Неопределено Тогда
...
Функция НайтиОбъект(Узел, ИмяЭлемента) Экспорт
...
Функция ОбъявлениеОбъекта(Знач Узел, Знач ИмяЭлемента) Экспорт
	...
	// Поиск объявления объекта
	Пока НЕ Узел = Неопределено Цикл
		Если Узел.Имя = "Пусть" ИЛИ Узел.Имя = "Перем" ИЛИ Узел.Имя = "Функция" Тогда
			Если Узел.Значение = ИмяЭлемента Тогда
	...
	Пока Циклов < 5 И УзлыОбновить.Количество() Цикл
		...
		Для каждого элУзел Из УзлыОбновить Цикл
			...
			Для каждого элОбъект Из Объекты Цикл
				...
				НовоеЗначениеУзла = элОбъект.Ключ;
				ПервыйНайден = Ложь;
				Пока НЕ РодительУзел = Неопределено Цикл
					Если РодительУзел.Имя = "Пусть" Тогда
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	Если Имя = "Объект" Тогда
		...
		Иначе
			...
			ЗначениеОбъекта = Дочерний(Узел);
			Если ЗначениеОбъекта = Неопределено Тогда
				ОбъявлениеОбъекта = ОбъявлениеОбъекта(Узел, Значение);
				Если НЕ ОбъявлениеОбъекта = Неопределено Тогда
					Если ОбъявлениеОбъекта.Имя = "Функция" Тогда
						ЗначениеОбъекта = ТелоФункции(ОбъявлениеОбъекта, Узел);
					Иначе
						ЗначениеОбъекта = КопироватьВетку(Дочерний(ОбъявлениеОбъекта));
					КонецЕсли;
					Узел.Вставить("Дочерний", ЗначениеОбъекта.Код);
					ЗначениеОбъекта.Вставить("Старший", Узел.Код);
				КонецЕсли;
			КонецЕсли;
			Состояние = Интерпретировать(ЗначениеОбъекта);

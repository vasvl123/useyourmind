...
Функция СравнитьИмя(Узел, Тип, Имя)
	Если Узел.Имя = "О" ИЛИ Узел.Имя = "Объект" Тогда
		...
		Если НЕ зн = "" Тогда
			м = СтрРазделить(зн, " ");
			Если (м[0] = Тип ИЛИ Тип = "*" ИЛИ (м[0] = Имя И Тип = Имя)) Тогда
				Если м.Количество() = 1 Тогда
					Возврат Истина;
				КонецЕсли;
				Если (м[2] = Имя ИЛИ Имя = "*") Тогда
					Возврат Истина;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция НайтиОбъект(Знач Узел, Путь)
	...
	Если МассивПуть[0] = "р" Тогда // найти родителя
		Пока НЕ Узел = Неопределено Цикл
			Если СравнитьИмя(Узел, МассивПуть[1], МассивПуть[2]) Тогда
				Прервать;
			КонецЕсли;
			Узел = Узел.Родитель;
		КонецЦикла;
...
Функция ЗначениеСвойстваОбъекта(Знач Узел) Экспорт
	...
	Пока НЕ Узел.Имя = "Свойства" Цикл // проверить что это свойство // потом убрать
		Если Узел.Имя = "У" ИЛИ Узел.Имя = "Узел" Тогда
			Возврат УзелЗначение(НачальныйУзел);
		КонецЕсли;
		Узел = Узел.Родитель;
	КонецЦикла;
	Узел = НачальныйУзел;

	Узел = НайтиСвойствоОбъекта(Узел, Путь);
...
Функция ОбработатьОбъект(Знач Узел, структЗадача = Неопределено) Экспорт
	...
	Если НЕ Библиотека = Неопределено Тогда
		...
		Состояние = "";
		Свойства = Библиотека.ОбработатьОбъект(ЭтотОбъект, Узел, ТипОбъекта, Атрибуты, структЗадача);
...
Функция Интерпретировать(Знач Узел, ЭтоАтрибут = Ложь, НачальныйУзел = Истина) Экспорт
	...
	Пока НЕ СледУзел = Неопределено Цикл
		...
		ИначеЕсли Имя = "Знач" Тогда // получить значение свойства
			свУзел = Узел;
			Пока НЕ Узел.Имя = "Свойства" Цикл
				Узел = Узел.Родитель;
				Если Узел = Неопределено Тогда
					ВызватьИсключение "Свойство не найдено: " + свУзел.Значение;
				КонецЕсли;
			КонецЦикла;
			Свойства = УзелСостояние(Узел, "Свойства");
			Если Свойства = Неопределено Тогда
				ВызватьИсключение "Свойство не найдено: " + свУзел.Значение;
			КонецЕсли;
			Свойство = УзелСвойство(Свойства, свУзел.Значение);
			Если Свойство = Неопределено Тогда
				ВызватьИсключение "Свойство не найдено: " + свУзел.Значение;
			КонецЕсли;
			дУзел = Дочерний(Свойство);
			Если НЕ дУзел = Неопределено Тогда
				Состояние = Интерпретировать(дУзел, , Ложь);
			Иначе
				Состояние = "" + УзелСвойство(Свойство, "Значение");
			КонецЕсли;
		...
		ИначеЕсли Имя = "Значение" ИЛИ Имя = "З" Тогда
			Если НЕ "" + Значение = "" Тогда
				Состояние = ЗначениеСвойстваОбъекта(Узел); // УзелЗначение(Узел);
			КонецЕсли;
...
Функция ПолучитьУзел(Код, Старший = Неопределено) Экспорт
	...
	Если НЕ Старший = Неопределено Тогда
		...
		Если УзелСвойство(Родитель, "Атрибут") = Узел.Код ИЛИ УзелСостояние(Старший, "ЭтоАтрибут") = Истина Тогда // в этом проблема отображения структуры атрибутов

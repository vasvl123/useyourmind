...
Функция ОбновитьДанные(Запрос) Экспорт
	Если НЕ УзелЗапрос = Неопределено Тогда
		Узел = УзелЗапрос;
		Для каждого элПараметр Из Запрос Цикл
			нстУзел = Новый Структура("Имя, Значение", "Пусть", элПараметр.Ключ);
			Если Узел.Значение = "Запрос" Тогда
				Узел = НовыйДочерний(Узел, нстУзел, Истина);
				УзелЗапрос.Вставить("Дочерний", Узел.Код);
			Иначе
				Узел = НовыйСоседний(Узел, нстУзел, Истина);
			КонецЕсли;
			нстУзел = Новый Структура("Имя, Значение", "Строка", элПараметр.Значение);
			НовыйДочерний(Узел, нстУзел, Истина)
		КонецЦикла;
		Если НЕ Узел = УзелЗапрос Тогда
			Интерпретировать(Дочерний(УзелЗапрос));
		КонецЕсли;
	КонецЕсли;
...
Функция ПриСозданииОбъекта(обПроцесс, ИмяФайлаДанных = Неопределено)
	...
	УзелЗапрос = НайтиУзел(Корень, "Запрос");

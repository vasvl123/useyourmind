...
Функция ВыполнитьЗадачу(структЗадача)
	...
	ИначеЕсли Команда = "ФормыСлов" Тогда
		...
		Если НЕ н = 0 Тогда
			...
			Для каждого г2 Из мф Цикл
				...
				сЛемма = ПолучитьЗначение(мг2[0]);
				нФорма = ПолучитьЗначение(мг2[1]);
				нЛемма = ПолучитьЗначение(мг2[2]);
	...
	ИначеЕсли Команда = "Связи" Тогда
		...
		//
		// 	ток1имя = мб[0];
		// 	ток1нач = НайтиЗначение(1, мб[1]);
		// 	ток1гр = ДобавитьЗначение(2, мб[2]);
		// 	свимя = НайтиЗначение(3, мб[3]);
		// 	ток2имя = мб[4];
		// 	ток2нач = НайтиЗначение(1, мб[5]);
		...
		Для каждого гр Из мсл Цикл
			...
			Если НЕ н = 0 Тогда // найдено совпадение по образцу
				...
				Для каждого г2 Из мф Цикл
					...
					м.Добавить(1);
					к = МассивИзСтроки(г2);
					Для каждого к1 Из к Цикл
						м.Добавить(к1);
					КонецЦикла;
					м.Добавить(1);
					м.Добавить(НайтиЗначение(МассивИзСтроки(Символ(1) + мгр[3])));
					м.Добавить(1);

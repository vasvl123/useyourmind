...
Функция ВыполнитьЗадачу(структЗадача)
	...
	ИначеЕсли Команда = "Грамматики" Тогда
		...
		Для каждого гр Из мСлова Цикл
			...
			нгр = ток1гр + Символы.Таб + ток2гр;

			н = НайтиЗначение(нгр + Символы.Таб);

			Если НЕ н = 0 Тогда // найдено совпадение по образцу
				мф = Новый Массив;
				ПолучитьЗначения(н, "", мф);
				Для каждого г2 Из мф Цикл
					Если сгр.Получить(нгр + Символы.Таб + г2) = Неопределено Тогда
						...
						структЗадача.Результат.Вставить("гр_" + структЗадача.Результат.Количество(), нгр + "_" + г2 + Символы.Таб + б);
						сгр.Вставить(нгр + Символы.Таб + г2, б);

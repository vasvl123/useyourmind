...
Функция ВыполнитьЗадачу(структЗадача)
	...
	ИначеЕсли Команда = "Грамматики" Тогда
		...
		Для каждого гр Из мсл Цикл
			...
			Если н = Неопределено Тогда
				н = Связи.НайтиЗначение(МассивИзСтроки(Символ(2) + мгр[0])); // найти грамматику
			...
			Если НЕ н = 0 Тогда // найдено совпадение по образцу
				...
				Если гр2 = Неопределено Тогда
					гр2 = Связи.НайтиЗначение(МассивИзСтроки(Символ(2) + мгр[1]));
				...
				Если НЕ гр2 = 0 Тогда
					...
					Для каждого мф Из мр Цикл
						...
						Если н = Неопределено Тогда
							н = Связи.НайтиЗначение(МассивИзСтроки(Символ(1) + мгр[2])); // найти связь
						...
						Если НЕ н = 0 Тогда
							...
							Если нф2 = Неопределено Тогда
								нф2 = Связи.НайтиЗначение(МассивИзСтроки(Символ(1) + мгр[3]));
							...
							Если НЕ нф2 = 0 Тогда
								...
								Для каждого мф1 Из мр1 Цикл
									Если мф[0] = мф1[0] Тогда
										мб = Связи.ПолучитьЗначения(мф1[1]); // что внутри
	...
	ИначеЕсли Команда = "Элемент" Тогда
		...
		поз = структЗадача.Запрос.Слова;

		Элемент = Связи.ПолучитьЭлемент(поз);
		структЗадача.Результат.Вставить("эл", Элемент.Значение);

		Если НЕ Элемент.Дочерний = 0 Тогда
			м = Связи.ПолучитьЗначения(Элемент.Дочерний);
			Для каждого эл Из м Цикл
				структЗадача.Результат.Вставить("эл_" + структЗадача.Результат.Количество(), эл[0]);
			КонецЦикла;
		КонецЕсли;

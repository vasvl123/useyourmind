	...
	ИначеЕсли Команда = "ФормыСлов" Тогда
		...
		Для к = 1 по СтрДлина(слф) Цикл
			...
			Пока Истина Цикл
				...
				Формы.Прочитать(б, 0, 2);
				ф1 = б.ПрочитатьЦелое16(0);
				Если ф1 = 1 Тогда
					Прервать;
		...
		Для каждого бигр Из мСлова Цикл
			...
			Если мб.Количество() = 8 Тогда // добавить или удалить
				...
				Если р = "+" Тогда // добавить корректную биграмму
					...
					б = Новый БуферДвоичныхДанных(10);
					...
					Для к = 1 по СтрДлина(гр) Цикл
						...
						Пока Истина Цикл
							...
							Связи.Прочитать(б, 0, 6);
							ф1 = б.ПрочитатьЦелое16(0);
							Если ф1 = 1 Тогда // последний
								Прервать;
							КонецЕсли;
							н = б.ПрочитатьЦелое32(2); // позиция следующего
							Если ф1 = 0 Тогда // удалено
								Продолжить;
							ИначеЕсли ф1 = ф Тогда // найден элемент
								н = б.ПрочитатьЦелое32(6); // позиция вложенного
								Прервать;
							КонецЕсли;
						КонецЦикла;
						Если ф1 = 1 Тогда

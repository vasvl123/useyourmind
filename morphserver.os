...
Функция ПолучитьЗначения(Знач н, ф, рез = "", мф = Неопределено, Первый = Истина)
	буф = Новый БуферДвоичныхДанных(16);
	Пока Истина Цикл
		Связи.Перейти(н, ПозицияВПотоке.Начало);
		Связи.Прочитать(буф, 0, 16);
		...
		Если Первый Тогда
			мф = Новый Массив;
			Возврат ПолучитьЗначения(вн, ф, рез, мф, Ложь);
		КонецЕсли;
		Если НЕ ф1 = 0 Тогда
			Если НЕ ф1 = ф Тогда
				р1 = ?(рез = "", "", рез + Символы.Таб) + ф1;
				Если НЕ вн = 0 Тогда // есть вложение
					ПолучитьЗначения(вн, ф, р1, мф, Ложь);
				Иначе
					мф.Добавить(р1);
				КонецЕсли;
			Иначе
...
Функция ПолучитьСтроку(н)
	...
	Пока Истина Цикл
		...
		н = буф.ПрочитатьЦелое32(12); // позиция родителья
...
Функция ВыполнитьЗадачу(структЗадача)
	...
	ИначеЕсли Команда = "ФормыСлов" Тогда
		...
		Если НЕ н = 0 Тогда
			...
			мф = ПолучитьЗначения(н, 1);

			Для каждого г2 Из мф Цикл
				мг2 = СтрРазделить(г2, Символы.Таб);
				сЛемма = ПолучитьСтроку(мг2[0]);
				нФорма = ПолучитьСтроку(мг2[1]);
				нЛемма = ПолучитьСтроку(мг2[2]);
	...
	ИначеЕсли Команда = "Связи" Тогда
		...
		Для каждого бигр Из мСлова Цикл
			...
			гр = МассивИзСтроки(Символ(2) + мб[2]);
			гр.Добавить(1);
			гр.Добавить(ток2гр);
			гр.Добавить(свимя);

			св = МассивИзСтроки(Символ(1) + мб[1]);
			св.Добавить(1);
			св.Добавить(ток2нач);
			св.Добавить(свимя);
			св.Добавить(КодСимвола(р));

			Если р = "+" Тогда // добавить грамматику
				ДобавитьЗначение(гр);
				ДобавитьЗначение(св);
				НайтиЗначение(св);
	...
	ИначеЕсли Команда = "Грамматики" Тогда
		...
		Для каждого гр Из мсл Цикл
			...
			м = МассивИзСтроки(Символ(2) + мгр[0]);
			м.Добавить(1);
			м.Добавить(НайтиЗначение(МассивИзСтроки(Символ(2) + мгр[1])));
			...
			Если НЕ н = 0 Тогда // найдено совпадение по образцу
				...
				Для каждого г2 Из мф Цикл
					...
					м = МассивИзСтроки(Символ(1) + мгр[2]);
					м.Добавить(1);
					м.Добавить(нф2);
					м.Добавить(Число(г2));
					...
					Если НЕ н = 0 Тогда // найдено совпадение по образцу
						св = ПолучитьЗначения(н, 1);
						Если св.Количество() Тогда
							б = Символ(св[0]);
						КонецЕсли;
					КонецЕсли;

					структЗадача.Результат.Вставить("гр_" + структЗадача.Результат.Количество(), гр + "_" + ПолучитьСтроку(г2) + Символы.Таб + б);

		...
		Иначе
			...
			Если м.Количество() = 1 Тогда // раскрыть дерево
				Пока м.Количество() = 1 Цикл
					Если Элемент.Дочерний = 0 Тогда
						Прервать;
					КонецЕсли;
					эл = Связи.ПолучитьЭлемент(Элемент.Дочерний);
					Если эл.Значение < 16 Тогда
						Прервать;
					КонецЕсли;
					поз = Элемент.Дочерний;
					Элемент = эл;
					м = Связи.ПолучитьЗначения(Элемент.Дочерний);
				КонецЦикла;
				структЗадача.Результат.Вставить("Элемент", Элемент);
			...
			Если ИмяДанных = "Связи" Тогда
				...
				Для каждого зэ Из зн Цикл
					...
					КонецЕсли;
				КонецЦикла;
				Если СтрНайти(тэ, "Позиция") Тогда
					з = Связи.ПолучитьСтроку(зэ);
				ИначеЕсли з = "" Тогда
					з = тэ;
				КонецЕсли;
			КонецЕсли;

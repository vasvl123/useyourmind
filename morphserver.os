...
Функция ВыполнитьЗадачу(структЗадача)
			...
			Пока Истина Цикл
				...
				Пока Истина Цикл
					...
					Если Связи.Получить(Число(фс[1])) = нф Тогда
						рез = ?(Рез = "", "", Символы.ПС) + слф + Символы.Таб + Леммы.Получить(Число(фс[1])) + Символы.Таб + нач + фс[0] + Символы.Таб + Леммы.Получить(Число(нф));
					КонецЕсли;
					м = м + 1;
				КонецЦикла;
				к = к + 1;
		...
		Иначе
			...
			Пока Истина Цикл
				...
				Если нф = Неопределено Тогда
					Прервать;
				КонецЕсли;
				Рез = ?(Рез = "", "", Символы.ПС) + слф + Символы.Таб + Леммы.Получить(Число(нф));
				м = м + 1;
			КонецЦикла;
	...
	структЗадача.Результат.Вставить(сл, рез);
				...
				Если ЕстьРезультат = Истина Тогда
					Попытка
						...
						Если НЕ Контроллер = Неопределено Тогда
							Если ПередатьДанные(Контроллер.Хост, Контроллер.Порт, структЗадача.Результат) = Неопределено Тогда
								Продолжить;
							КонецЕсли;
		...
		Для каждого зСоединение Из Соединения Цикл
			...
			Если Соединение.Статус = "Успех" Тогда
				...
				Если НЕ Запрос = Неопределено Тогда
					структЗадача = Новый Структура("ИдЗадачи, Запрос, Результат, ВремяНачало", ПолучитьИД(), Запрос, Неопределено, ТекущаяДата());

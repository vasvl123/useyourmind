...
Функция ПолучитьВсеЗначения(Знач н, Знач ф, мф)
	...
	Пока Истина Цикл
		...
		Если НЕ вн = 0 И НЕ ф1 = 9 Тогда // есть вложение
			ПолучитьВсеЗначения(вн, ф + Символ(ф1), мф);
		Иначе
		...
		КонецЕсли;
	КонецЦикла;
КонецФункции // ПолучитьВсеЗначения()
						...
						Для к = 1 по СтрДлина(гр) Цикл
							...
							Пока Истина Цикл
								...
								Если ф1 = ф Тогда  // найден элемент
									...
									н = нн;
									Прервать;
								ИначеЕсли нн = 0 Тогда // это последний
									Сообщить("АГА!");
									ОткрытьПоток(1);
									кн = Связи.Размер();

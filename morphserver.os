...
Функция ВыполнитьЗадачу(структЗадача)
	...
	ИначеЕсли Команда = "ФормыСлов" Тогда
		...
		н = НайтиЗначение(СтрокуВМассив(Символ(1) + слф + Символ(1)));
		...
		Если НЕ н = 0 Тогда
			...
			Для каждого г2 Из мф Цикл
				сЛемма = ПолучитьЗначение(КодСимвола(Символ(1) + Сред(г2, 2, 1));
				нФорма = ПолучитьЗначение(КодСимвола(Сред(г2, 3, 1)));
				нЛемма = ПолучитьЗначение(КодСимвола(Сред(г2, 4, 1)));
	...
	ИначеЕсли Команда = "Грамматики" Тогда
		...
		Для каждого гр Из мсл Цикл
			...
			ток1гр = НайтиЗначение(2, мгр[0]);
			ток2гр = НайтиЗначение(2, мгр[1]);

			н = НайтиЗначение(2, мгр[0] + ток1гр + Символы.Таб + ток2гр + Символы.Таб);
			...
			Если НЕ н = 0 Тогда // найдено совпадение по образцу
				...
				Для каждого г2 Из мф Цикл
					...
					ток1нач = НайтиЗначение(1, мгр[2]);
					ток2нач = НайтиЗначение(1, мгр[3]);

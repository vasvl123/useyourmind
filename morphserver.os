...
Функция ПолучитьЗначения(Знач н, к, мр = Неопределено, мф = Неопределено, Первый = Истина)
	буф = Новый БуферДвоичныхДанных(16);
	Если Первый Тогда
		Связи.Перейти(н, ПозицияВПотоке.Начало);
		Связи.Прочитать(буф, 0, 16);
	...
	Пока Истина Цикл
		...
		КонецЕсли;
		мф.Добавить(ф);
		Если НЕ к = 1 И НЕ вн = 0 Тогда // есть вложение
			ПолучитьЗначения(вн, к - 1, мр, мф, Ложь);
		КонецЕсли;
		н = сн;
	КонецЦикла;
	Возврат мр;
...
Функция ВыполнитьЗадачу(структЗадача)
	...
	ИначеЕсли Команда = "ФормыСлов" Тогда
		...
		Если НЕ н = 0 Тогда
			...
			мр = ПолучитьЗначения(н, 3);

			Для каждого мф Из мр Цикл
				Если мф.Количество() = 3 Тогда
					сЛемма = ПолучитьСтроку(мф[0]);
					нФорма = ПолучитьСтроку(мф[1]);
					нЛемма = ПолучитьСтроку(мф[2]);
	...
	ИначеЕсли Команда = "Грамматики" Тогда
		...
		Для каждого гр Из мсл Цикл
			...
			Если н = Неопределено Тогда
				...
				м.Добавить(1);
				н = НайтиЗначение(м); // найти грамматику
			...
			Если НЕ н = 0 Тогда // найдено совпадение по образцу
				...
				Если НЕ гр2 = 0 Тогда
					...
					Если мр = Неопределено Тогда
						мр = ПолучитьЗначения(н, 2);
						инд.Вставить(н, мр);
					КонецЕсли;

					Для каждого мф Из мр Цикл

						Если НЕ мф.Количество() = 2 Тогда
							Продолжить;
						КонецЕсли;
						...
						Если мф[0] = гр2 Тогда
							...
							Если НЕ н = 0 Тогда
								...
								Если НЕ нф2 = 0 Тогда
									...
									Если мр1 = Неопределено Тогда
										мр1 = ПолучитьЗначения(н, 3);
										инд.Вставить(н, мр1);
									КонецЕсли;

									Для каждого мф1 Из мр1 Цикл

										Если НЕ мф1.Количество() = 3 Тогда
											Продолжить;
										КонецЕсли;

										Если мф1[0] = нф2 И мф[1] = мф1[1] Тогда
											б = Символ(мф1[2]);
										КонецЕсли;
									КонецЦикла;
							...
							структЗадача.Результат.Вставить("гр_" + структЗадача.Результат.Количество(), гр + "_" + ПолучитьСтроку(мф[1]) + Символы.Таб + б);

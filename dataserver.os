...
Функция ВыполнитьЗадачу(структЗадача)
	...
	ИначеЕсли Команда = "ВнешниеДанные" Тогда
		...
		Контроллеры.Вставить(Запрос.База, структЗадача.Соединение);
		Возврат Ложь;

	ИначеЕсли Команда = "Элементы" Тогда

		кЗадача = Контроллеры.Получить(Запрос.Параметры.База);
		Если НЕ кЗадача = Неопределено Тогда // есть соединение с базой
			Если НЕ структЗадача.Свойство("Результат") Тогда // послать запрос
				кЗадача.Вставить("Результат", Новый Структура);
				кЗадача.Результат.Вставить("Элементы", Новый Массив);
			Иначе // взять результат
	...
	Пока НЕ ОстановитьСервер Цикл
		...
		Пока к > 0 И НЕ ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50 Цикл
			...
			Если структЗадача.Результат = Неопределено Тогда
				...
				ВыполнитьЗадачу = (структЗадача.Этап = "Выполнить");

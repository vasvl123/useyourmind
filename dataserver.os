...
Функция НовоеУсловиеОтбора(Условие = Неопределено, КлючИмя, Сравнение, КлючЗначение)
	Если Условие = Неопределено Тогда
		Условие = Новый Структура("СписокУсловий", Новый Соответствие);
	КонецЕсли;
	Условие.СписокУсловий.Вставить(КлючИмя, Новый Структура("КлючИмя, Сравнение, КлючЗначение", КлючИмя, Сравнение, КлючЗначение));
	Возврат Условие;
КонецФункции
...
Функция ПроверкаАвторизации(Параметры)
	...
	Если Параметры.Свойство("unm", Имя) Тогда
		...
		Если НЕ "" + Имя = "" Тогда
			...
			Профиль = Профили.НайтиЗаголовок(НовоеУсловиеОтбора(, "Имя", "=", Имя));
			СтатусСубъекта = "Неизвестный субъект";
			Если Профиль.Результат = "ЗаголовокНайден" Тогда
...
Функция ВыполнитьРегистрацию(Параметры)
	...
	Если Параметры.Свойство("unm", Имя) Тогда
		Если НЕ Имя = "" Тогда
			//Профиль = Профили.НайтиКлюч("Имя" + Символы.Таб + Имя + Символы.Таб);
			Профиль = Профили.НайтиЗаголовок(НовоеУсловиеОтбора(, "Имя", "=", Имя));
			Если Профиль.Результат = "ЗаголовокНеНайден" Тогда
	...
	Иначе
		...
		ИначеЕсли НЕ "" + БазаДанных = "" Тогда // чтение данных
			...
			Если НЕ "" + КлючИмя = "" Тогда // найти заголовок по значению ключа
				Ответ = ПолучитьДвоичныеДанныеИзСтроки(СтруктуруВСтроку(Данные.НайтиЗаголовок(НовоеУсловиеОтбора(, КлючИмя, "=", КлючЗначение))));

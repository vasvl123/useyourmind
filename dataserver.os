...
Функция РазобратьЗапросКлиента(ТекстЗапроса, Соединение)
	...
	Если Метод ="POST" Тогда
		...
		Если Содержимое = "text/plain;charset=UTF-8" Тогда // параметры строкой
			...
			Если НЕ Соединение.Статус = "Данные" Тогда
				Возврат Неопределено;
			КонецЕсли;
			POSTДанные = Соединение.ПолучитьСтроку();
		...
		ИначеЕсли Содержимое = "application/octet-stream" Тогда // двоичные данные
			...
			Если НЕ Соединение.Статус = "Данные" Тогда
				Возврат Неопределено;
			КонецЕсли;
			POSTДанные = ДвоичныеДанныеВСтруктуру(Соединение.ПолучитьДвоичныеДанные());
			Заголовок.Вставить("POSTData", POSTДанные);
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьКонтроллерИЗадачи(ИдКонтроллера)
	...
	Для каждого элЗадача Из Задачи Цикл
		...
		Если Задача.Запрос.Свойство("ИдПроцесса") Тогда
			Если Задача.Запрос.ИдПроцесса = ИдКонтроллера Тогда
				Задача.Этап = "Удалить";
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбработатьСоединения() Экспорт
	...
	TCPСерверК.ПриниматьЗаголовки = Истина;
			...
			Если Соединение.Статус = "Заголовки" ИЛИ Соединение.Статус = "Данные" Тогда
				...
				Если НЕ ТекстовыеДанныеВходящие = "" Тогда
					...
					Попытка
						...
						Если Запрос = Неопределено Тогда
							Если Соединение.Статус = "Ошибка" Тогда

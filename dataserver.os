	...
	Пока НЕ ОстановитьСервер Цикл
		...
		Пока к > 0 Цикл
			...
			Если Соединение.Статус = "Успех" Тогда
				...
				м = Новый Массив;
				Попытка
					Соединение.ТаймаутЧтения = 5;
					дд = Соединение.ПолучитьДвоичныеДанные();
					Пока НЕ дд.Размер() = 0 Цикл
						м.Добавить(дд);
						дд = Соединение.ПрочитатьДвоичныеДанные(); // дополучить остальное
					КонецЦикла;
				Исключение
					//Сообщить("dataserver: " + ОписаниеОшибки());
				КонецПопытки;
				...
				Попытка
					...
					ТекстовыеДанныеВходящие = ПолучитьСтрокуИзДвоичныхДанных(СоединитьДвоичныеДанные(м), "windows-1251");
				Исключение
					Сообщить("dataserver: " + ОписаниеОшибки());
				КонецПопытки;

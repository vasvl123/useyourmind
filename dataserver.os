...
Функция ВыполнитьЗадачу(структЗадача)
	...
	Если БазаДанных = "web" ИЛИ БазаДанных = "log" ИЛИ БазаДанных = "users" Тогда // системные базы
		Сис = ОбъединитьПути("data", "sys");
		...
		Попытка
			Если Сис = "sys" ИЛИ НЕ Суб = "" Тогда // контроль прав
				...
				Иначе // записать в файл
						ИмяФайлаДанных = ОбъединитьПути(ТекущийКаталог(), Сис, Запрос.Заголовок.ИмяДанных + ".sd");
		...
		ИначеЕсли Запрос.ЗапросДанных.Команда = "СписокБаз" ИЛИ Запрос.ЗапросДанных.Команда = "СписокФайлов" Тогда
			Если НЕ Запрос.Свойство("СписокФайлов") Тогда
				...
				Если Запрос.ЗапросДанных.Команда = "СписокБаз" Тогда
					// базы пользователя
					СписокФайлов = НайтиФайлы(ОбъединитьПути(ТекущийКаталог(), Сис), "*.sdb", Ложь);
				Иначе // СписокФайлов
					СписокФайлов = НайтиФайлы(ОбъединитьПути(ТекущийКаталог(), Сис), "*.sd", Ложь);
				КонецЕсли;
		...
		Иначе // прочитать из файла
			Попытка
				...
				Если НЕ СтрНайти(Запрос.ИмяДанных, "..") Тогда
					ИмяФайлаДанных = ОбъединитьПути(ТекущийКаталог(), Сис, ИмяДанных + ".sd");
					Файл = Новый Файл(ИмяФайлаДанных);
					Если НЕ Файл.Существует() Тогда // public
						ИмяФайлаДанных = ОбъединитьПути(ТекущийКаталог(), "data", ИмяДанных + ".sd");
						Файл = Новый Файл(ИмяФайлаДанных);
					КонецЕсли;
					Если Файл.Существует() Тогда
						...
						структЗадача.Вставить("Ответ", "Успешно");
					Иначе
						структЗадача.Вставить("Ответ", "НетФайла");
					КонецЕсли;
				Иначе

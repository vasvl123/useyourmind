...
Функция ВыполнитьЗадачу(структЗадача)
	...
	Если НЕ ИдПроцесса = Неопределено Тогда
		...
		Если НЕ Контроллер = Неопределено Тогда
			...
			Если НЕ Профиль = Неопределено Тогда
				...
				Сис = ОбъединитьПути("data", Суб);
				Кат = ОбъединитьПути(ТекущийКаталог(), Сис);
				Файл = Новый Файл(Кат);
	...
	Если ИмяДанных = "login" ИЛИ ИмяДанных = "register" ИЛИ ИмяДанных = "log" ИЛИ ИмяДанных = "data" ИЛИ ИмяДанных = "expldb" // системные файлы
		ИЛИ БазаДанных = "web" ИЛИ БазаДанных = "log" ИЛИ БазаДанных = "users" Тогда // системные базы
			Сис = "sys";
	ИначеЕсли БазаДанных = "public" Тогда
		Сис = "";
	КонецЕсли;
	...
	Если НЕ структЗадача.Свойство("Данные", Данные) Тогда
		Если НЕ "" + БазаДанных = "" Тогда // имя контейнера указано
			...
			Если Данные = Неопределено Тогда // открыть контейнер
				Данные = Новый dbaccess(ОбъединитьПути(ТекущийКаталог(), "data", Сис), БазаДанных);
		...
		Попытка
			Если Сис = "sys" ИЛИ НЕ Суб = "" Тогда // контроль прав
				...
				Иначе // записать в файл
						ИмяФайлаДанных = ОбъединитьПути(ТекущийКаталог(), "data", Сис, Запрос.Заголовок.ИмяДанных + ".sd");
		...
		ИначеЕсли Запрос.ЗапросДанных.Команда = "СписокБаз" ИЛИ Запрос.ЗапросДанных.Команда = "СписокФайлов" Тогда
			Если НЕ Запрос.Свойство("СписокФайлов") Тогда
				...
				Если Запрос.ЗапросДанных.Команда = "СписокБаз" Тогда
					Если НЕ Сис = "" Тогда // добавить public.sdb
						СписокФайлов = НайтиФайлы(ОбъединитьПути(ТекущийКаталог(), "data"), "public.sdb", Ложь);
						Если СписокФайлов.Количество() Тогда
							элФайл = СписокФайлов[0];
					...
					КонецЕсли;
					// базы пользователя
					СписокФайлов = НайтиФайлы(ОбъединитьПути(ТекущийКаталог(), "data", Сис), "*.sdb", Ложь);
				Иначе // СписокФайлов
					СписокФайлов = НайтиФайлы(ОбъединитьПути(ТекущийКаталог(), "data", Сис), "*.sd", Ложь);
				КонецЕсли;
				Если СписокФайлов.Количество() Тогда
	...
	ИначеЕсли Команда = "ПолучитьДанные" Тогда
		...
		Если НЕ "" + БазаДанных = "" Тогда // чтение данных из контейнера
			...
			ИначеЕсли НЕ "" + ИмяДанных = "" Тогда // найти по имени данных
				Результат = Данные.НайтиКлюч(ИмяДанных);
				Если Результат.Результат = "ЗаголовокНайден" Тогда
					структЗадача.Вставить("Результат", Результат.Заголовок);
				Иначе
					структЗадача.Вставить("Результат", ПолучитьДвоичныеДанныеИзСтроки("И" + Символы.Таб + "H1" + Символы.Таб + "З" + Символы.Таб + "404"));
				КонецЕсли;
		...
		Иначе // прочитать из файла
			Попытка
				...
				Если НЕ СтрНайти(Запрос.ИмяДанных, "..") Тогда
					ИмяФайлаДанных = ОбъединитьПути(ТекущийКаталог(), "data", Сис, ИмяДанных + ".sd");

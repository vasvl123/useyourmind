...
Функция ВыполнитьЗадачу(структЗадача)
	...
	ИначеЕсли Команда = "ПолучитьДанные" И НЕ "" + БазаДанных = "" Тогда // чтение данных
	...
	Пока НЕ ОстановитьСервер Цикл
		...
		Если НЕ Соединение = Неопределено Тогда
			...
			Если НЕ Запрос = "" Тогда
				структЗадача = Новый Структура("ИдЗадачи, Этап, Запрос, Результат, ВремяНачало", ПолучитьИД(), "ВыполнитьЗадачу", Запрос, Неопределено, ТекущаяДата());

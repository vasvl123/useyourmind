...
Для н = 1 по Словарь.КоличествоСтрок() Цикл
	...
	Иначе
		...
		Если нФорма = Неопределено Тогда
			...
			нЛемма = СокрЛП(мс[1]);
			к = 0;
			Пока НЕ НачальныеФормы.Получить(СокрЛП(мс[0]) + "_" + к) = Неопределено Цикл
				к = к + 1;
			КонецЦикла;
			НачальныеФормы.Вставить(СокрЛП(мс[0]) + "_" + к, нЛемма);
		Иначе
			Леммы.Вставить(СокрЛП(мс[1]) + Символы.Таб + нЛемма);
			сФорма = СокрЛП(мс[0]);
			к = 0;
			Пока Истина Цикл
				к = к + 1;
			...
			Ок = Сред(сФорма, к);
			нОк = Сред(нФорма, к);
			пр = Ок + Символы.Таб + нОк + Символы.Таб + СокрЛП(мс[1]);
			Правила.Вставить(пр);
		КонецЕсли;
	КонецЕсли;
...
Для н = 1 по ф.КоличествоСтрок() Цикл
	...
	фс = Леммы.Получить(СокрЛП(м[0]));
	Если фс = Неопределено Тогда
		фс = Леммы.Количество();
		Леммы.Вставить(СокрЛП(м[0]), фс);
	КонецЕсли;
	нф = Леммы.Получить(СокрЛП(м[1]));
	Если нф = Неопределено Тогда
		нф = Леммы.Количество();
		Леммы.Вставить(СокрЛП(м[1]), нф);
	КонецЕсли;
	к = 0;
...
Для н = 1 по ф.КоличествоСтрок() Цикл
	...
	м = СтрРазделить(стр, Символы.Таб);
	нф = Леммы.Получить(СокрЛП(м[1]));
	НачальныеФормы.Вставить(СокрЛП(м[0]), нф);
КонецЦикла;
ф = Новый ТекстовыйДокумент;
Для каждого эл Из НачальныеФормы Цикл
	ф.ДобавитьСтроку(эл.Ключ);
...
Для н = 1 по ф.КоличествоСтрок() Цикл
	стр = ф.ПолучитьСтроку(н);
	м = СтрРазделить(стр, Символы.Таб);
	фс = Леммы.Получить(СокрЛП(м[2]));
	к = 0;
	Пока НЕ Правила.Получить(СокрЛП(м[0]) + "_" + к) = Неопределено Цикл
		к = к + 1;
	КонецЦикла;
	Правила.Вставить(СокрЛП(м[0]) + "_" + к, СокрЛП(м[1]) + Символы.Таб + фс);
КонецЦикла;
ф = Новый ТекстовыйДокумент;
Для каждого эл Из Правила Цикл
	ф.ДобавитьСтроку(эл.Ключ);

...
namespace onesharp
{
    public class pagedata : Onesharp
    {
        ...
        public object ОбработатьОбъект(Узел Узел, bool Инициализация = false)
        {
            ...
            Сообщить(ИмяДанных + " " + Узел.Код + " " + Узел.ТипОбъекта);
            ...
            else
            {
                ...
                if (Содержимое == Неопределено)
                {
                    ...
                    if (Свойства.Свойство("_Соседний"))
                    {
                        var Код = Свойства["_Соседний"] as string;
                        Свойства.Удалить("_Соседний");
                ...
                { // значение переменной объекта
                    ...
                    while (!(сУзел == Неопределено))
                    {
                        if (сУзел.Имя == "Свойства.")
                        {
                            ...
                            {
                                if (сУзел.Свойство("_Соседний"))
                                {
                                    var Код = сУзел["_Соседний"] as string;
                                    сУзел.Удалить("_Соседний");
                                    Сообщить("ПолучитьУзел " + Код);
        ...
        public Узел НовыйУзел(Узел Узел, bool Служебный = false)
        {
            ...
            if (НовыйКод == null)
            {
                НовыйКод = Служебный ? "s" : "n" + сКоличество;
        ...
        public pagedata(Ishowdata обПроцесс, string знИстДанных, string знИмяДанных, string знКорень) : base("pagedata")
        {
            ...
            Корень = НовыйУзел(new Узел("Код, Имя, Значение", знКорень, "О", "Корень"));

			...
			if (!(Изменения.Получить(Свойства.Родитель) as bool? == Истина))
			{
                if (Изменения.Получить(Свойства.д("Условие")) as bool? == Истина)
				{
					if (Условие == Истина)
					{
                        ...
                        var пУзел = Свойства.д("Параметры").Дочерний;
                        while (пУзел != null)
						{
                            Параметры.Вставить(пУзел.Имя, Данные.ЗначениеУзла(пУзел));
		...
		public void Таблица_Модель(pagedata Данные, Узел Свойства, Соответствие Изменения)
		{
			...
			if (Инициализация)
			{
				...
				{ // создать заголовок
                    ...
                    var Поле = Свойства.д("СписокПолей").Дочерний;
                    while (Поле != null)
					{
					...
					while (!(ИсточникСтрок == Неопределено))
					{
						...
						if (оСтрока == Неопределено)
						{
							...
							if (Свойства.д("СвойстваСтроки") != null)
							{
								...
								{
									var свУзел = Данные.КопироватьВетку(элСвойство, Данные.ЭтотОбъект, стСвойстваСтроки, СвойстваСтроки);
			...
			if (Инициализация || ИсточникИзменен)
			{
				...
				if (!(Источник.Значение == Данные.Пустой))
				{
					...
					while (элПоле != null)
                    {
						...
						{
							var Поле = Стр.Заменить(элПоле.Имя, ".", "");
							var ПолеЗначение = ((Структура)Источник.Значение)[Поле] as string;
		...
		public void Загрузить_Модель(pagedata Данные, Узел Свойства, Соответствие Изменения)
		{
			...
			if (Изменения.Получить(Свойства.д("События")) as bool? == Истина)
			{
				...
				if (!(дУзел == Неопределено))
				{
					...
					{
						if (дУзел.п.Свойство("filename", out var _filename))
						{
							...
							Данные.Процесс.ПередатьДанныеД(Структура.Новый("ИстДанных, БазаДанных, Заголовок, Команда, дДанные, неОбратныйЗапрос", Данные.Процесс.ПолучитьСубъект(), "inbox", Структура.Новый("ИмяДанных, ТипДанных", Свойства.д("Файл").Значение, 2), "ЗаписатьДанные", дУзел.п["fl"], Неопределено));
						}

        ...
        void ОбработатьЗадачи()
        {
            ...
            while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50))
            {
                ...
                if (Задача.Этап == "Обработка")
                {
                    ...
                    {
                        if (НачалоЦикла - Задача.ВремяНачало > 5 * 1000)
                        {
                ...
                if (Задача.Этап == "Обработка")
                {
                    ...
                    if (Задача.Свойство("upddata")) 
                    {
                        ...
                        if (!(структКонтроллер == Неопределено))
                        {
                            var Результат = структКонтроллер["Результат"] as Массив;
                            if (Результат != Неопределено)
                            {
                                ...
                                if (!Задача.Свойство("Заголовки"))
                                {
                                    ...
                                    дРезультат.Добавить(ПолучитьДвоичныеДанныеИзСтроки("Connection: keep-alive" + ПС));
                                ...
                                foreach (ДвоичныеДанные Кусок in Результат) {
                                    Сообщить(Кусок.Размер().ToString("X") + ПС);
                                ...
                                структКонтроллер.Вставить("Результат", Неопределено);
                        ...
                        if (!(КонтроллерЗапрос == Неопределено))
                        {
                            if (КонтроллерЗапрос.Свойство("procid"))
                            {
                                ...
                                if (КонтроллерЗапрос.Свойство("cmd"))
                                {
                                    ...
                                    if (cmd == "upddata")
                                    {
                                        var р = структКонтроллер["Результат"] as Массив;
                                        if (р == Неопределено) р = Новый.Массив();
                                        р.Добавить(КонтроллерЗапрос["Результат"]);
                                        структКонтроллер.Вставить("Результат", р);
                                    }

...
namespace webserver
{
    class webserver : useyourmind
    {
        ...
        void РазобратьДанныеЗапроса(Задача Задача)
        {
            ...
            if (Содержимое != null)
            {
                ...
                else if (Лев(Содержимое, 24)  == "application/octet-stream")
                {
                    ...
                    var POSTДанные = "" + Задача.Заголовок.Получить("params");
                    if (Стр.Длина(POSTДанные) > 0)
                    {
                        POSTДанные = POSTДанные + "&";
                        while (Найти(POSTДанные, "&") > 0)
                    ...
                    POSTСтруктура.Вставить("data", Задача.Соединение.ПолучитьДвоичныеДанные());
                }
            }
        ...
        void ОбработатьЗадачи()
        {
            ...
            while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50))
            {
                ...
                try
                {
                    ...
                    {
                        if (Задача.Соединение.Статус == "Заголовки" || Задача.Соединение.Статус == "Данные")
                        {
                            if ((int)Число(Задача.Заголовок.Получить("Content-Length")) == Задача.Соединение.Длина)
                            {
                                ...
                                Задача.Соединение.ДочитатьДанные();
                        }
                    ...
                    if (Задача.Этап == "Новая")
                    {
                        ...
                        if (ИмяКонтроллера == "i")
                        {
                            Задача.ПараметрыЗапроса.Вставить("data", Задача.ПараметрыЗапроса["ИмяМетода"]);
                            Задача.ПараметрыЗапроса.Вставить("sdb", "inbox");
                            Задача.ПараметрыЗапроса.Вставить("sdata", "");
                            Задача.ПараметрыЗапроса.Вставить("cmd", "file");

                            ИмяКонтроллера = "procid";
                            Задача.ПараметрыЗапроса["ИмяМетода"] = "1";
                        }

                        if (ИмяКонтроллера == "ext") // внешний контроллер
                        {
                            Структура ПараметрыЗапроса = Задача.ПараметрыЗапроса;
                            if (!ПараметрыЗапроса.Свойство("data")) // ожидает задачи
                            {
                            ...
                            {
                                var стрРез = Новый.Структура("ИдЗадачи, РезультатДанные", ПараметрыЗапроса["taskid"], Новый.Структура("дДанные", ПараметрыЗапроса["data"] as ДвоичныеДанные));
                                var структКонтроллер = Контроллеры.Получить(ПараметрыЗапроса["procid"]) as Контроллер;
        ...
        void ОбработатьСоединения()
        {
            ...
            Сообщить("Процесс webserver завершен.");

            ...
            try
            {
                ...
                try
                {
                    ...
                    if (Задача.Свойство("upddata"))
                    {
                        ...
                        {
                            мДанные.Добавить(ПолучитьДвоичныеДанныеИзСтроки("HTTP/1.1 200 OK" + ПС + "Connection:keep-alive" + ПС + "Content-Type:text/event-stream; charset=utf-8" + ПС + "Cache-Control:no-cache" + ПС + "X-Accel-Buffering:no" + ПС + ПС + ПС));

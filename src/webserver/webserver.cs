...
namespace onesharp
{
    class webserver : Onesharp
    {
        ...
        Структура ext; // внешний контроллер
            ...
            while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50))
            {
                ...
                if (Задача.с.Этап == "Новая")
                {
                    ...
                    if (ИмяКонтроллера == "ext") // внешний контроллер
                    {
                        ...
                        {
                            ЛогСообщить("Подключен внешний контроллер");
                            ext = Задача;
                            Задача.с.Этап = "Обработка";
                        }
                        else // получен ответ
                        {
                            ...
                            var стрРез = Новый.Структура("taskid, Данные", par[2], ПараметрыЗапроса.с.Данные as ДвоичныеДанные);
                            var структКонтроллер = Контроллеры.Получить(par[1]) as Структура;
                            if (!(структКонтроллер == Неопределено))
                            {
                                if (!(Функции.ПередатьДанные(структКонтроллер.с.Хост, структКонтроллер.с.Порт, Задача.с.ПараметрыЗапроса) == Неопределено))
                                {
                                    Задача.с.Этап = "Вернуть";
                                }
                            }
                            ...
                            if (КонтроллерЗапрос.Свойство("procid"))
                            {
                                ...
                                if (КонтроллерЗапрос.Свойство("cmd"))
                                {
                                    ...
                                    else if (КонтроллерЗапрос.с.cmd == "ext") // задача для внешнего контроллера
                                    {
                                        if (ext != null)
                                        {
                                            ext.с.Результат = КонтроллерЗапрос.с.Параметры;
                                        }
                                    }
                                }

    ...
    class webserver : Onesharp
    {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (Задача.Свойство("upddata"))
                    {
                        ...
                        foreach (ДвоичныеДанные дд in Задача.Результат as Массив)
                        {
                            мДанные.Добавить(дд);
                            мДанные.Добавить(ПолучитьДвоичныеДанныеИзСтроки(ПС + ПС + ПС));
                        }

                        Сообщить("webserver: upddata " + Задача.ИдЗадачи);
                    ...
                    ((TCPСоединение)Задача.Соединение).ОтправитьДвоичныеДанныеАсинхронно(СоединитьДвоичныеДанные(мДанные));
        ...
        void ОбработатьЗадачи()
        {
            ...
            while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50))
            {
                ...
                if (Задача.Этап == "Новая")
                {
                    ...
                    if (ИмяКонтроллера == "ext") // внешний контроллер
                    {
                        ...
                        else // получен ответ
                        {
                            ...
                            {
                                if (!(Функции.ПередатьДанные(структКонтроллер.Хост, структКонтроллер.Порт, стрРез) == Неопределено))
                                {
                                    Задача.Результат = "";
                                    Задача.Этап = "Обработка";
                                }
                    ...
                    else if (ИмяКонтроллера == "upddata")
                    {
                        ...
                        Задача.ИдКонтроллера = Задача.ПараметрыЗапроса["ИмяМетода"] as string;
                        var структКонтроллер = Контроллеры.Получить(Задача.ИдКонтроллера) as Контроллер;
                        if (!(структКонтроллер == Неопределено))
                        {
                            Задача.структКонтроллер = структКонтроллер;
                            var ПараметрыЗапроса = Новый.Структура("taskid, cmd", Задача.ИдЗадачи, "upddata");
                            if (Функции.ПередатьДанные(Задача.структКонтроллер.Хост, Задача.структКонтроллер.Порт, ПараметрыЗапроса) == Неопределено)
                            {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (Контроллеры.Получить(ПарИдКонтроллера) == Неопределено)
                            {
                                if (Функции.ПередатьДанные((string)Параметры["Хост"], (int)Параметры["ПортС"], Структура.Новый("procid, cmd", ПарИдКонтроллера, "startproc")) != Неопределено)
                                {
                ...
                {
                    if (!(Функции.ПередатьДанные(Задача.структКонтроллер.Хост, Задача.структКонтроллер.Порт, Задача.ПараметрыЗапроса) == Неопределено))
                    {
                        Задача.Этап = "Обработка";
                    }
                }
                    ...
                    if (!(Задача.Соединение == Неопределено))
                    {
                        if (НачалоЦикла - Задача.ВремяНачало > 30 * 1000)
                        {
                            ...
                            if (!(Задача.Соединение.Активно))
                            {
                                ...
                                if (!(структКонтроллер == Неопределено))
                                {
                                    var ПараметрыЗапроса = Структура.Новый("ИдЗадачи, cmd", Задача.ИдЗадачи, "taskend");
                                    Функции.ПередатьДанные(структКонтроллер.Хост, структКонтроллер.Порт, ПараметрыЗапроса);
                                }
                                Задача.Этап = "Завершить";
                            }
                        }
        ...
        void ОбработатьСоединения()
        {
            ...
            while (!(ОстановитьСервер))
            {
                            ...
                            if (КонтроллерЗапрос.Свойство("procid"))
                            {
                                    ...
                                    if (КонтроллерЗапрос.Свойство("taskid"))
                                {
                                    ...
                                    if (!(Задача == Неопределено))
                                    {
                                        if (Задача.Свойство("upddata"))
                                        {
                                            var р = КонтроллерЗапрос["Результат"] as Массив;
                                            if (р == Неопределено) р = Новый.Массив();
                                            р.Добавить(КонтроллерЗапрос["Результат"]);
                                            Задача.Результат = р;
                                        }
                                        else
                                        {
                                            ...
                                            {
                                                Задача.Вставить("ContentType", КонтроллерЗапрос["ContentType"] as string);
                                            }
                ...
                {
                    Функции.ПередатьДанные((string)Параметры["Хост"], (int)Параметры["ПортД"], (Структура)Сообщения.Получить(0));
                    Сообщения.Удалить(0);
                }

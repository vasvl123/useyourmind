        ...
        void ОбработатьЗадачи()
        {
            ...
            while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50))
            {
                ...
                if (Задача.Этап == "Данные")
                {
                    ...
                    {
                        if ((int)Число(Задача.Заголовок.Получить("Content-Length")) == Задача.Соединение.Длина || !Задача.Соединение.ДочитатьДанные())
                        {
                ...
                if (Задача.Этап == "Новая")
                {
                    ...
                    if (ИмяКонтроллера == "ext") // внешний контроллер
                    {
                        ...
                        else // получен ответ
                        {
                            ...
                            var стрРез = Новый.Структура("ИдЗадачи, РезультатДанные", par[2], Новый.Структура("дДанные", ПараметрыЗапроса["Данные"] as ДвоичныеДанные));

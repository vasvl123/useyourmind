...
namespace onesharp.lib
{
    public class Сем : Onesharp
    {
        ...
        public void Элемент_Модель(pagedata Данные, Узел Свойства, Соответствие Изменения)
        {
            ...
            // обработать события
            if (Изменения.Получить(Свойства.дс.События) == Истина)
            {
            ...
            if (Свойства.дс.Показать.Значение == "Да")
            {
                if (Свойства.п.Свойство("Элементы"))
                {
                    ...
                    foreach (int зн in Свойства.п.с.Элементы as Массив)
                    {
                        ...
                        Данные.НовыйАтрибут(д, ИмяЗначение("База", Свойства.дс.База.Значение), Истина);
                        н = н + 1;
                    }
        ...
        public void Предложение_Модель(pagedata Данные, Узел Свойства, Соответствие Изменения)
        {
            ...
            if (!(оУзел.п["Обновить"] as bool? == Ложь))
            {
                ...
                {
                    if (Свойства.д("Текст") != null)
                    {
                        var служ = Данные.Служебный(Формы);
                        var пр = Данные.ЗначениеУзла(Свойства.дс.Текст);
                ...
                Свойства.дс.Вид.с.Значение = "<p>" + Вид + "</p>";

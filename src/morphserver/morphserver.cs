    ...
    class morphserver : Onesharp
    {
        ...
        bool ВыполнитьЗадачу(Структура структЗадача)
        {
            ...
            else if ((string)Команда == "Связи")
            {
                ...
                Индекс.Вставить(0, 0);
                Индекс.Вставить(1, 4);
                var нТок2 = Связи.ДобавитьЗначение(Индекс, нТок1);
                ...
                { // добавить грамматику и связь
                    ...
                    Связи.ДобавитьЗначение(св, нТок2);
                ...
                { // удалить связь
                    ...
                    var н = Связи.НайтиЗначение(св, нТок2);
                    if (!(н == 0))
                    {
                        Связи.УдалитьЗначение(н);
                    }
                    св[св.Количество() - 1] = КодСимвола("+");
                    н = Связи.НайтиЗначение(св, нТок2);
                    if (!(н == 0))
                    {
                        Связи.УдалитьЗначение(н);
                    }
            ...
            else if ((string)Команда == "Грамматики")
            {
                ...
                Массив мсл = структЗадача.с.Запрос.с.Параметры.с.Слова;

                var гр = Новый.Массив();

                структЗадача.с.Результат.Вставить("гр", гр);

                var инд = Соответствие.Новый(); // индекс

                foreach (Структура сл in мсл)
                {
                    string сф1 = сл.с.сл;
                    var Индекс = инд.Получить(сф1) as Массив;

                    if (Индекс == null) {
                        var ин = ПолучитьЗначения(сл.с.ф as Массив);
                        Индекс = Новый.Массив(index.Индекс.Получить(ин.Arr, 100));
                        Индекс.Вставить(0, 4);
                        инд.Вставить(сф1, Индекс);
                    }

                    var нТок1 = Связи.НайтиЗначение(Индекс);

                    if (!(нТок1 == 0)) {

                        foreach (Структура сл2 in мсл)
                        {
                            string сф2 = сл2.с.сл;
                            if (сф1 == сф2) continue;
                            var Индекс2 = инд.Получить(сф2) as Массив;
                            if (Индекс2 == null)
                            {
                                var ин2 = ПолучитьЗначения(сл2.с.ф as Массив);
                                Индекс2 = Новый.Массив(index.Индекс.Получить(ин2.Arr, 100));
                                Индекс2.Вставить(0, 4);
                                инд.Вставить(сф2, Индекс2);
                            }

                            var нТок2 = Связи.НайтиЗначение(Индекс2, нТок1);

                            if (!(нТок2 == 0))

                            {
                            
                                var мр1 = инд.Получить(нТок2) as Массив;
                                if (мр1 is null)
                                {
                                    мр1 = Связи.ПолучитьВложенныеЗначения(нТок2);
                                    инд.Вставить(нТок2, мр1);
                                }

                                foreach (Массив мф1 in мр1)
                                {
                                    var мр2 = Связи.ПолучитьВложенныеЗначения((int)мф1[1]); // свимя
                                    foreach (Массив мф2 in мр2)
                                    {
                                        var мб = Связи.ПолучитьВложенныеЗначения((int)мф2[1]); // б
                                        var св = Связи.ПолучитьСтроку((int)мф2[0]);
                                        foreach (Массив б1 in мб)
                                        {
                                            var б = Символ((int)б1[0]);
                                            гр.Добавить(Новый.Структура("сф1, сф2, свимя, б", сф1, сф2, св, б));
                                        }
                                    }
                                }

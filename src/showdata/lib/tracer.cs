    ...
    class tracer : Onesharp
    {
        ...
        string Рендер(Кадр Кадр, int Высота, int Ширина, int _верт, int _гор, int _выс, int _шир)
        {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                рез = "<img src='data:image/bmp;base64," + Convert.ToBase64String(ms.GetBuffer()) + "' style='width:32px;height:32px;'>";
            }
        ...
        public void Страница_Модель(pagedata Данные, Узел Свойства, Соответствие Изменения)
        {
            ...
            if (!(УзелСвойство(оУзел, "Обновить") as bool? == Ложь))
            {
                ...
                if (Свойства.д.Новая.Значение == "Да")
                {
                    ...
                    for (int верт = 0; верт < 6; верт++)
                    {

                        for (int гор = 0; гор < 3; гор++)
                        {

                            var вп = Данные.НовыйДочерний(п, ИмяЗначение("О", "tracer.Кадр"), Истина, Истина);
                            var свп = Данные.ОбработатьОбъект(вп, Истина) as Узел;

                            if (ЗначениеЗаполнено(Свойства.д.Ид1.Значение))
                            { 
                                свп.д.Ид1.Значение = Свойства.д.Ид1.Значение;
                                свп.д.Ид2.Значение = Данные.Процесс.ПолучитьИД();
                            }
        ...
        public void Пиксель_Модель(pagedata Данные, Узел Свойства, Соответствие Изменения)
        {
            ...
            if (!(УзелСвойство(оУзел, "Обновить") as bool? == Ложь))
            {
                ...
                int _шир = Свойства.д.Выс.Значение;
                ...
                else if (Дальше != "")
                {
                    ...
                    {
                        var Камера = Новый_Вектор(0, 0, 0);
                        var Напр_Z = -_выс / (2 * Tan(Кадр.УголЗрения / 2));
                        var Напр_Y = -(_верт + 0.5) + _выс / 2;
                        var Напр_X = (_гор + 0.5) - _шир / 2;
                        var Направление = Вектор_Нормализация(new Вектор(Напр_X, Напр_Y, Напр_Z));
                        var цв = ТрассироватьЛуч(Кадр, Камера, Направление);
                        var цвзн = "rgb(" + Норм_Цвет(цв.x) + ", " + Норм_Цвет(цв.y) + ", " + Норм_Цвет(цв.z) + ")";
                        Свойства.д.Вид.Значение = "<div style='width:" + 128 / _шир + "px; height:" + 128 / _выс + "px; background-color: " + цвзн + "'></div>";
                    }
                    else
                    {
                        Свойства.д.Вид.Значение = Рендер(Кадр, 64, 64, _верт * 16, _гор * 16, 16, 16);
                        Дальше = "";
                    }
        ...
        public void Кадр_Модель(pagedata Данные, Узел Свойства, Соответствие Изменения)
        {
                ...
                if (!(дУзел is null))
                {
                    ...
                    if (тСобытие == "ПриНажатии")
                    {
                        ...
                        if (!(ЗначениеКнопка is null))
                        {
                            ...
                            {
                                var Параметры = Новый.Структура("Данные, Свойства, data, templ, cmd", Данные, Свойства, "" + Свойства.д.Ид1.Значение, "tracer2", "newtab");
                    ...
                    { 
                        ...
                        var _оУзел = _Данные.ПолучитьУзел("3");
                        var _Свойства = _Данные.ОбработатьОбъект(_оУзел, Истина) as Узел;;
                        _Свойства.д.Ид1.Значение = Свойства.д.Ид1.Значение;
                    }
                    else
                    { // изменение полей
                    }
            ...
            if (!(УзелСвойство(оУзел, "Обновить") as bool? == Ложь))
            {
                ...
                {
                    var Ид1 = Свойства.д.Ид1.Значение;

                    if (!ЗначениеЗаполнено(Ид1))
                    {
                        Ид1 = Цел(Данные.Процесс.ПолучитьИД());
                        Свойства.д.Ид1.Значение = Ид1;
                    }
                    ...
                    var Ид2 = Свойства.д.Ид2.Значение;

                    if (ЗначениеЗаполнено(Ид2))
                    {
                        г = new ГСЧ(Ид2);
                        
                        if (г.Ч() > 0.9) УголЗрения *= г.Ч();

                        if (г.Ч() > 0.9) ЦветФона.x *= г.Ч();
                        if (г.Ч() > 0.9) ЦветФона.y *= г.Ч();
                        if (г.Ч() > 0.9) ЦветФона.z *= г.Ч();
                        
                        for (var n = 0; n < Сферы.Количество(); n++) 
                        {
                            var сф = (Сфера)Сферы[n];

                            if (г.Ч() > 0.9) сф.Центр.x *= г.Ч();
                            if (г.Ч() > 0.9) сф.Центр.y *= г.Ч();
                            if (г.Ч() > 0.9) сф.Центр.z *= г.Ч();
                            
                            if (г.Ч() > 0.9) сф.Радиус *= г.Ч();
                              
                            if (г.Ч() > 0.9) сф.Материал.Альб0 *= г.Ч();
                            if (г.Ч() > 0.9) сф.Материал.Альб1 *= г.Ч();
                            if (г.Ч() > 0.9) сф.Материал.Альб2 *= г.Ч();
                            if (г.Ч() > 0.9) сф.Материал.Альб3 *= г.Ч();
                            if (г.Ч() > 0.9) сф.Материал.КПрелом *= г.Ч();
                            if (г.Ч() > 0.9) сф.Материал.ЭкспОтраж *= г.Ч();
                            
                            if (г.Ч() > 0.9) сф.Материал.Цвет.x *= г.Ч();
                            if (г.Ч() > 0.9) сф.Материал.Цвет.y *= г.Ч();
                            if (г.Ч() > 0.9) сф.Материал.Цвет.z *= г.Ч();
                    ...
                    свп.д.Шир.Значение = 1;
                    свп.д.Выс.Значение = 1;

                    свп.д.Кадр.Значение = new Кадр(ЦветФона, Сферы, Освещение, Ширина, Высота, УголЗрения, ГЛУБИНА_РЕКУРСИИ);
                }

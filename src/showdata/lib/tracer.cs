...
namespace onesharp.lib
{
    class tracer : Onesharp
    {
        ...
        bool НайтиПересечения(Структура Исх, Структура Напр, ref Структура Точка, ref Структура Нормаль, ref Структура Материал)
        {
            double Расст = 0;
        ...
        Структура ТрассироватьЛуч(Структура Исх, Структура Напр, int Глубина = 0)
        {
            ...
            if (Глубина > ГЛУБИНА_РЕКУРСИИ || !(НайтиПересечения(Исх, Напр, ref Точка, ref Нормаль, ref Материал)))
            {
                return ЦветФона;
            }
            ...
            foreach (Структура Свет in Освещение)
            {
                ...
                if (НайтиПересечения(ТеньИсх, НапрКСвету, ref ТеньТочка, ref ТеньНормаль, ref ВремМатериал) && Вектор_КвНормы(Вектор_Разность(ТеньТочка, ТеньИсх)) < РасстДоСвета)
                {
                    continue;
                }

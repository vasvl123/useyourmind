...
namespace onesharp.lib
{
    ...
    class tracer : Onesharp
    {
        ...
        string Рендер(Кадр Кадр, int Высота, int Ширина, int _верт, int _гор)
        {
            var рез = "";
            var Камера = Новый_Вектор(0, 0, 0);
            var Напр_Z = - Высота / (2 * Tan(Кадр.УголЗрения / 2));
            for (int верт = _верт; верт <= _верт + 7; верт++)
            {
                рез = рез + "<div style='display: flex'>";

                var Напр_Y = - (верт + 0.5) + Высота / 2;
                for (int гор = _гор; гор <= _гор + 7; гор++)
                {
                    ...
                    рез = рез + "<div style='width:3px; height:3px; background-color: " + цвзн + "'></div>";
                }
                рез = рез + "</div>";
            }
            return рез;
            ...
            if (!(УзелСвойство(оУзел, "Обновить") as bool? == Ложь))
            {
                ...
                else if (Дальше != "")
                {
                    if (_шир < 8)
                    {
                        ...
                        var цв = ТрассироватьЛуч(Кадр, Камера, Направление);
                        var цвзн = "rgb(" + Норм_Цвет(цв.x) + ", " + Норм_Цвет(цв.y) + ", " + Норм_Цвет(цв.z) + ")";
                        Свойства.д.Вид.Значение = "<div style='width:" + 3 * 64 / _шир + "px; height:" + 3 * 64 / _выс + "px; background-color: " + цвзн + "'></div>";
                    }
                    else
                    {
                        Свойства.д.Вид.Значение = Рендер(Кадр, 64, 64, _верт * 8, _гор * 8);
                        Дальше = "";
                    }
                ...
                if (!(дУзел is null))
                {
                    ...
                    if (тСобытие == "ПриНажатии")
                    {
                        ...
                        if (!(ЗначениеКнопка is null))
                        {
                            ...
                            {
                                var Параметры = Новый.Структура("Данные, Свойства, data, templ, cmd", Данные, Свойства, "Шаг 2", "tracer2", "newtab");

                    ...
                    if (тСобытие == "ПриНажатии")
                    {
                        ...
                        if (!(ЗначениеКнопка is null))
                        {
                            if (ЗначениеКнопка == "sel")
                            {
                                ...
                                var Параметры = Новый.Структура("Данные, Свойства, data, templ, cmd", Данные, Свойства, "" + м[м.Количество()-1], "tracer2", "newtab");
            ...
            if (!(УзелСвойство(оУзел, "Обновить") as bool? == Ложь))
            {
                ...
                if (Свойства.д.Новый.Значение == "Да")
                {
                    ...
                    for (var н = 1; н < Ид.Количество(); н++)
                    {
                        ...
                        if (г.Ч() > 0.8) УголЗрения *= г.Ч();

                        if (г.Ч() > 0.8) ЦветФона.x *= г.Ч();
                        if (г.Ч() > 0.8) ЦветФона.y *= г.Ч();
                        if (г.Ч() > 0.8) ЦветФона.z *= г.Ч();
                        
                        for (var n = 0; n < Сферы.Количество(); n++) 
                        {
                            var сф = (Сфера)Сферы[n];

                            if (г.Ч() > 0.8) сф.Центр.x *= г.Ч();
                            if (г.Ч() > 0.8) сф.Центр.y *= г.Ч();
                            if (г.Ч() > 0.8) сф.Центр.z *= г.Ч();
                            
                            if (г.Ч() > 0.8) сф.Радиус *= г.Ч();
                              
                            if (г.Ч() > 0.8) сф.Материал.Альб0 *= г.Ч();
                            if (г.Ч() > 0.8) сф.Материал.Альб1 *= г.Ч();
                            if (г.Ч() > 0.8) сф.Материал.Альб2 *= г.Ч();
                            if (г.Ч() > 0.8) сф.Материал.Альб3 *= г.Ч();
                            if (г.Ч() > 0.8) сф.Материал.КПрелом *= г.Ч();
                            if (г.Ч() > 0.8) сф.Материал.ЭкспОтраж *= г.Ч();
                            
                            if (г.Ч() > 0.8) сф.Материал.Цвет.x *= г.Ч();
                            if (г.Ч() > 0.8) сф.Материал.Цвет.y *= г.Ч();
                            if (г.Ч() > 0.8) сф.Материал.Цвет.z *= г.Ч();

...
namespace onesharp.lib
{
    class tracer : Onesharp
    {
        ...
        string Рендер(int Высота, int Ширина, int _верт, int _гор)
        {
            var рез = "";
            ...
            for (int верт = _верт; верт <= _верт + 7; верт++)
            {
                ...
                for (int гор = _гор; гор <= _гор + 7; гор++)
                {
                    ...
                    рез = рез + "<div style='width:4px; height:4px; background-color: " + цвзн + "'></div>";
                }
                рез = рез + "</div>";
            }
            return рез;
            ...
            if (!(УзелСвойство(оУзел, "Обновить") as bool? == Ложь))
            {
                ...
                else if (Дальше != "")
                {
                    if (_шир < 8) 
                    {
                        ...
                        var цв = ТрассироватьЛуч(Камера, Направление);
                        var цвзн = "rgb(" + Норм_Цвет(цв.x) + ", " + Норм_Цвет(цв.y) + ", " + Норм_Цвет(цв.z) + ")";
                        Свойства.д.Вид.Значение = "<div style='width:" + 4 * 64 / _шир + "px; height:" + 4 * 64 / _выс + "px; background-color: " + цвзн + "'></div>";
                    }
                    else
                    {
                        Свойства.д.Вид.Значение = Рендер(64, 64, _верт * 8, _гор * 8);
                        Дальше = "";
                    }

...
namespace onesharp.lib
{
    class tracer : Onesharp
    {
        ...
        public void Пиксель_Модель(pagedata Данные, Узел Свойства, Соответствие Изменения)
        {
            ...
            {
                if (ЗначениеЗаполнено(Свойства.д.Вид.Значение))
                {
                    var Камера = Новый_Вектор(0, 0, 0);
                    var Напр_Z = - Высота / (2 * Tan(УголЗрения / 2));
                    var верт = Свойства.д.Вер.Значение;
                    var Напр_Y = - (верт + 0.5) + Высота / 2;
                    var гор = Свойства.д.Гор.Значение;
                    var Напр_X = (гор + 0.5) - Ширина / 2;
                    var Направление = Вектор_Нормализация(new Вектор(Напр_X, Напр_Y, Напр_Z));
                    var цв = ТрассироватьЛуч(Камера, Направление);
                    Свойства.д.Цвет.Значение = "rgb(" + Норм_Цвет(цв.x) + ", " + Норм_Цвет(цв.y) + ", " + Норм_Цвет(цв.z) + ")";
                }
                else
                {
                    Данные.ОбъектыОбновить.Добавить(оУзел);
                }
    
                Свойства.д.Вид.Значение = "<text style='color: " + Свойства.д.Цвет.Значение + "'>@</text>";
            }            
        }
        ...
        public void Кадр_Модель(pagedata Данные, Узел Свойства, Соответствие Изменения)
        {
            ...
            if (!(УзелСвойство(оУзел, "Обновить") as bool? == Ложь))
            {
                ...
                for (int верт = 0; верт <= Высота - 1; верт++)
                {
                    for (int гор = 0; гор <= Ширина - 1; гор++)
                    {
                        п = Данные.НовыйСоседний(п, ИмяЗначение("О", "tracer.Пиксель"), Истина);
                        var свп = Данные.ОбработатьОбъект(п, Истина) as Узел;
                        свп.д.Гор.Значение = гор;
                        свп.д.Вер.Значение = верт;
                        свп.д.Цвет.Значение = "rgb(" + Норм_Цвет(ЦветФона.x) + ", " + Норм_Цвет(ЦветФона.y) + ", " + Норм_Цвет(ЦветФона.z) + ")";
                    }
                    
                    п = Данные.НовыйСоседний(п, ИмяЗначение("br"), Истина);

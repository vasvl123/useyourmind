				...
				if (!(дУзел == Неопределено))
				{
					...
					if (мСобытие[0] == "ПриОтправке")
					{
						if (дУзел.с.Параметры.Свойство("filename"))
						{
							...
							Данные.Процесс.ПередатьДанныеД(Структура.Новый("ИстДанных, БазаДанных, Заголовок, Команда, дДанные, неОбратныйЗапрос", Данные.Процесс.Субъект, "inbox", Структура.Новый("ИмяДанных, ТипДанных", Свойства.д.с.Файл.Значение, 2), "ЗаписатьДанные", дУзел.с.Параметры.с.fl, Неопределено));
						}

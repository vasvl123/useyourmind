    ...
    class pagedata : Onesharp
    {
        ...
        string СтруктуруВСтроку(object знСтруктура)
        {
            ...
            foreach (КлючИЗначение Элемент in (Структура)знСтруктура)
            {
                ...
                {
                    Ключ = "З";
                    Значение = Стр.Заменить((string)Значение, Символы.Таб, "#x9");
                    Значение = Стр.Заменить((string)Значение, Символы.ПС, "#xA");
                    Значение = Стр.Заменить((string)Значение, Символы.ВК, "#xD");
        ...
        public object ЗначениеСвойства(Структура знУзелСвойство, string Имя = null)
        {
            ...
            {
                var ДочернийУзел = знУзелСвойство.с.Дочерний;
        ...
        Массив ПолучитьСвойстваПоСсылке(Узел Узел, object _Путь = null, bool СоздатьСвязь = false)
        {
            ...
            if (Путь is string)
            {
                ...
                {
                    var ИмяСвойства = МассивПуть[МассивПуть.Length - 1];
                    Узел = (Узел)СвойствоОбъекта(оУзел, ИмяСвойства);
                    if (Узел == Неопределено)
                    {
                ...
                if (!(стр == ""))
                {
                    ...
                    while (п < м.Length)
                    {
                        ...
                        else if (Имя == "")
                        {
                            if (т == 0)
                            {
                                Служ = (зСлужебный is bool == Истина);
                            }
        ...
        public object ОбработатьОбъект(Узел Узел, bool Инициализация = false)
        {
            ...
            if (_Рефлектор.МетодСуществует(Библиотека, ИмяФункции))
            {
                ...
                var Обновить = (_Рефлектор.ВызватьМетод(Библиотека, ИмяФункции, Параметры) as bool? == true);
            ...
            while (!(СледУзел == Неопределено))
            {
                ...
                { // это оператор
                    ...
                    if (!("" + Значение == ""))
                    {
                        ...
                        Состояние = _Рефлектор.ВызватьМетод(Библиотека, "Оператор_" + ВРег(ИмяФункции), Параметры);
                    }
        ...
        public void ЗагрузитьHTML(Узел Узел, Массив СтруктураHTML, int НомерЭлемента)
        {

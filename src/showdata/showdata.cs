...
namespace onesharp
{
    public class showdata : useyourmind, Ishowdata
    {
        ...
        object ПоказатьВкладки()
        {
            ...
            var Заголовок = "";
            if (ТекущиеДанные != null) Заголовок = ТекущиеДанные["ЗаголовокСтраницы"] as string;

            var Режим = "";
            if (ТекущаяВкладка != null) Режим = Вкладки.Получить<Структура>(ТекущаяВкладка)["Режим"] as string;

            var ПараметрыШаблона = Структура.Новый();
            ПараметрыШаблона.Вставить("ПараметрВкладки", Текст);
            ПараметрыШаблона.Вставить("ПараметрЗаголовокСтраницы", "" + Заголовок);
        ...
        {
            var ИстДанных = "" + структЗадача.Запрос["sdata"];
            var БазаДанных = "" + структЗадача.Запрос["sdb"];
            var ИмяДанных = "" + структЗадача.Запрос["data"];
            var ПозицияДанных = "" + структЗадача.Запрос["datapos"];
            var ИмяШаблона = "" + структЗадача.Запрос["templ"];
            var ПутьДанные = ИстДанных + "/" + ИмяДанных;
        ...
        Вкладка НоваяВкладка(Задача структЗадача)
        {
            ...
            else if (_Данные is ДвоичныеДанные)
            {
                var Режим = "image";
                var ТипВкладки = "win";
                var ИмяДанных = структЗадача.Запрос["data"] as string;
                ...
                ТекущиеДанные.Вставить("ПозицияДанных", "" + структЗадача.Запрос["datapos"]);
            }
            else
            {
                ...
                var Режим = "" + структЗадача.Запрос["mode"];
                if (Режим == "")
                {
                    Режим = "view";
                }
                var ТипВкладки = "" + структЗадача.Запрос["type"];
                if (ТипВкладки == "")
                {
                    ТипВкладки = "data";
                }
                var ИдУзла = "" + структЗадача.Запрос["nodeid"];
                if (ИдУзла == "")
                {
                ...
                if (!(свУзел == Неопределено))
                {
                    ...
                    {
                        var Свойства = свУзел;
                        if (Свойства.д("Заголовок") != null)
                        {
                            ЗаголовокСтраницы = Свойства.д("Заголовок").Значение.ToString();
                        }
                        if (Свойства.д("Меню") != null)
                        {
        ...
        string ПоказатьСтруктуруУзла(Вкладка Вкладка, Узел Узел, bool ЭтоАтрибут = false, string Атрибуты = "", string Дочерний = "")
        {
            ...
            var УзелЗначение = "" + Узел.Значение;
                ...
                if (!(Узел.Соседний == Неопределено))
                {
                    ...
                    { // свойства объекта
                        ...
                        { // прочитать вложение объекта
                            Узел.Соседний = Данные.ПолучитьУзел(Узел._Соседний, Узел);
                        }
                    }
        ...
        string Диалог(Вкладка Вкладка, Узел Узел)
        {
            ...
            var УзелЗначение = "";
            if (Узел.Значение != null)
            {
                УзелЗначение = Узел.Значение.ToString();
            }

            var ЭтоАтрибут = (Узел.ЭтоАтрибут);
        ...
        object ВыполнитьДействия(Задача структЗадача)
        {
            ...
            var Действие = структЗадача["Действие"] as string;
            ...
            else if (Действие == "ПолучитьУзел")
            {
                if (!(Запрос.Свойство("РезультатДанные")))
                { // запросить данные
                    var стрЗапрос = Структура.Новый("ИстДанных, ИмяДанных, УзелКод, СтаршийКод, Команда", Запрос.Данные.ИстДанных, Запрос.Данные.ИмяДанных, Запрос["УзелКод"], Запрос["СтаршийКод"], "ПолучитьУзел");
                ...
                { // получен ответ дата-сервера
                    ...
                    if (Ответ == "ЕстьУзел")
                    {
                        var _Данные = структЗадача.Запрос.Данные as pagedata;
                        var сРезультат = Запрос.РезультатДанные["Результат"] as Структура;
                        var дДанные = сРезультат["Данные"] as ДвоичныеДанные;
                        var нУзел = _Данные.ДвоичныеДанныеВУзел(дДанные) as Узел;
                        if ((нУзел.Старший != null) && ("" + нУзел.Старший._Соседний == нУзел.Код))
                        {
                            нУзел.Старший.Соседний = нУзел;
                            //нУзел.Старший._Соседний = null;
                        };

                        _Данные.ОбъектыОбновить.Добавить(нУзел.Родитель);
                    }
            ...
            { // загрузить данные с дата-сервера
                ...
                { // получен ответ дата-сервера
                    if (структЗадача.Данные == null)
                    {
                        ...
                        {
                            var сРезультат = Запрос.РезультатДанные["Результат"] as Структура;
                            var ТипДанных = "" + сРезультат["ТипДанных"];
                            if (ТипДанных != "2") // не файл
                            {
                                var дДанные = сРезультат["Данные"] as ДвоичныеДанные;
                                var сДанные = new pagedata(this, сРезультат["ИстДанных"] as string, сРезультат["ИмяДанных"] as string);
                                сДанные.ДвоичныеДанныеВУзел(дДанные);
                                структЗадача.Данные = сДанные;
                            }
                            else
                            {
                                структЗадача.Данные = сРезультат["Данные"];
                            }
                        }
                        else
                        {
                        ...
                        ВсеДанные.Вставить(ПутьДанные, структЗадача.Данные);
                        if (Запрос.ИстДанных == "doc")
                        {
                            ((pagedata)структЗадача.Данные).Корень.Дочерний.Дочерний.Значение = Запрос.ИмяДанных;
                        }
                    ...
                    { // обновление данных
                        ...
                        if (Ответ == "ЕстьДанные")
                        {
                            var _Данные = структЗадача.Данные as pagedata;
                            var сРезультат = Запрос.РезультатДанные["Результат"] as Структура;
                            var мДанные = сРезультат["Данные"] as Массив;
                            foreach (ДвоичныеДанные дДанные in мДанные)
                                _Данные.ОбъектыОбновить.Добавить(_Данные.ДвоичныеДанныеВУзел(дДанные));
            ...
            else if (Действие == "ЗапросДанных")
            {
                ...
                {
                    if (Запрос.ЗапросДанных["Команда"] as string == "ЗагруженныеДанные")
                    {
                        var Позиция = 1;
                ...
                if (!(Запрос.Свойство("РезультатДанные")))
                {
                    ...
                    var ИстДанных = "" + Запрос.ЗапросДанных["ИстДанных"];
                    if (ИстДанных == "")
                    {
                        ИстДанных = Субъект;
                    }
                    стрЗапрос.Вставить("ИстДанных", ИстДанных);
                    стрЗапрос.Вставить("БазаДанных", Запрос.ЗапросДанных["БазаДанных"]);
                    стрЗапрос.Вставить("ИмяДанных", Запрос.ЗапросДанных["ИмяДанных"]);
                    стрЗапрос.Вставить("ПозицияДанных", Запрос.ЗапросДанных["ПозицияДанных"]);
                    стрЗапрос.Вставить("ОбратныйЗапрос", Структура.Новый("ИдЗадачи", структЗадача.ИдЗадачи));
                ...
                { // получен ответ дата-сервера
                    ...
                    var ЗапросДанные = РезультатДанные["Результат"] as Структура;
                    if (РезультатДанные["Ответ"] as string == "ОшибкаПотокаДанных")
                    {
                        // сообщить об ошибке
                        return Истина;
                    }
                    ...
                    {
                        var КонечнаяПозиция = "" + ЗапросДанные["ПозицияДанных"];
                        if (!(КонечнаяПозиция == ""))
                        {
                        ...
                        while (!(ЗапросДанные == Неопределено))
                        {
                            ...
                            ЗапросДанные = ЗапросДанные["Соседний"] as Структура;
                    ...
                    if (РезультатДанные["Ответ"] as string == "ЗапросЗавершен")
                    { // все данные получены
                        return Истина;
                    }
                }
            ...
            { // запрос к внешним данным
                ...
                if (!(Запрос.Свойство("РезультатДанные")))
                {
                    string Пар = "par=" + "&procid=" + procid + "&taskid=" + структЗадача.ИдЗадачи;
            ...
            { // запрос к внешним данным
                ...
                { // получен ответ сервера
                    var РезультатДанные = Запрос.РезультатДанные;
                    ((Сем)Запрос.Библиотека).ОбработатьОтвет(Запрос.Параметры["Действие"] as string, Запрос.Данные, Запрос.Свойства, РезультатДанные["Результат"] as Структура);
                    Запрос.РезультатДанные = null;
                    return Истина;
                }
                return Ложь;
            ...
            else if (Действие == Команда["ОбновитьСтраницу"] as string)
            {
                ...
                var Источник = структЗадача.Запрос["ИмяКонтроллера"] as string;
                var ИмяДанных = структЗадача.Запрос["ИмяМетода"] as string;
                if (!(Источник == "procid"))
                {
                    if (ИмяДанных == "")
                    {
                ...
                if (!(ТекущиеДанные == Неопределено))
                {
                    ...
                    Меню = "" + ТекущиеДанные["Меню"];
                    Заголовок = "" + ТекущиеДанные["ЗаголовокСтраницы"];
                }
            ...
            else if (Действие == Команда["СохранитьДанные"] as string)
            {
                if (!(Запрос.Свойство("РезультатДанные")))
                {
                    ...
                    if (!(Вкладка == Неопределено))
                    {
                        ...
                        var дДанные = _Данные.УзелВДвоичныеДанные(_Данные.ПолучитьУзел(Вкладка.ИдУзла));
                        var стрЗапрос = Структура.Новый("ИстДанных, ИмяДанных, дДанные, Команда", _Данные.ИстДанных, _Данные.ИмяДанных, дДанные, "СохранитьУзел");
                        стрЗапрос.Вставить("ОбратныйЗапрос", Структура.Новый("ИдЗадачи", структЗадача.ИдЗадачи));
                        ПередатьДанные(Параметры.Хост, Параметры.ПортД, стрЗапрос);
                ...
                { // получен ответ дата-сервера
                    ...
                    if (!(Вкладка == Неопределено))
                    {
                        ...
                        if (Ответ == "Успешно")
                        {
                            var НовыеНомера = Запрос.РезультатДанные.Получить<Структура>("Результат").Получить<Соответствие>("НовыеНомера");
                            foreach (КлючИЗначение нНомер in НовыеНомера)
                            {
                                var нУзел = _Данные.ПолучитьУзел(нНомер.Ключ);
                                _Данные.Узлы.Удалить(нНомер.Ключ);
                                нУзел.Код = нНомер.Значение as string;
                                _Данные.Узлы.Вставить(нУзел.Код, нУзел);
                            }
            ...
            {
                var стрСообщение = "";
                var БазаДанных = Запрос["sdb"] as string;
                if (!("" + БазаДанных == ""))
                {
                    if (!(Запрос.Свойство("РезультатДанные")))
                    ...
                    { // получен ответ дата-сервера
                        var кСооб = 0;
                        if (Запрос.РезультатДанные["Ответ"] as string == "Успешно")
                        {
                        ...
                        {
                            // Сообщить об ошибке
                            стрСообщение = Запрос.РезультатДанные["Ответ"] as string;
                            кСооб = 3;
                        }
            ...
            var datascrolled = "" + Запрос["datascrolled"];
            ...
            var winscrolled = "" + Запрос["winscrolled"];
            ...
            if (Действие == Команда["НоваяВкладка"] as string)
            {
                ...
                if (структЗадача.Запрос.Свойство("Данные")) // вернуть результат
                {
                    ...
                    Узел _События = структЗадача.Запрос.Свойства.д("События");
            ...
            var ИдУзла = структЗадача.Запрос["nodeid"] as string;
            ...
            if (Действие == Команда["Авторизация"] as string || Действие == Команда["Регистрация"] as string)
            {
                ...
                { // получен ответ дата-сервера
                    ...
                    Структура ЗапросДанные = РезультатДанные["Результат"] as Структура;
                    if (РезультатДанные["Ответ"] as string == "ОшибкаПотокаДанных")
                    {
                        // сообщить об ошибке
                        return Истина;
                    ...
                    if (ЗапросДанные["ПрошелАвторизацию"] as string == "True")
                    {
                        Субъект = ЗапросДанные["unm"] as string;
                        ИзменитьАдрес();
            ...
            {
                var ЗначениеКнопка = Запрос["role"];
            ...
            {
                РезультатОбновить = РезультатОбновить + ПоказатьМенюИнструменты(Вкладка, Узел, Запрос["attr"] as string == "1");
            ...
            else if (Действие == Команда["ОбновитьУзел"] as string)
            {
                ...
                {
                    if (рУзел.Имя == "О" || рУзел.Имя == "Объект")
                    {
            ...
            else if (Действие == Команда["СвойстваУзла"] as string)
            {
                ...
                bool ПоказатьСвойство = (Запрос["show"] as string == "True");
                ...
                var запрСвойства = Запрос["prop"] as string;
                if (!("" + запрСвойства == ""))
            ...
            else if (Действие == Команда["РедактироватьУзел"] as string)
            {
                ...
                ТекущееОкно = НоваяСтруктураВкладка(Данные, "win", Данные.ИмяДанных + "  " + Узел.Имя + ":" + СтрЭкранироватьРазметку("" + Узел.Значение, 20), "struct", Узел.Код);
            ...
            else if (Действие == Команда["ЗагрузитьДерево"] as string)
            {
                ...
                var СтруктураЗагрузить = Запрос["enc_value"] as string;
                if (СтруктураЗагрузить is null)
                {
                    ТекущееОкно = НоваяСтруктураВкладка(Данные, "win", Данные.ИмяДанных + " " + Узел.Имя + ":" + СтрЭкранироватьРазметку("" + Узел.Значение, 20) + " import", "struct", Узел.Код);
            ...
            else if (Действие == Команда["ЗагрузитьHTML"] as string)
            {
                ...
                var _СтруктураHTML = Запрос["enc_value"] as string;
            ...
            else if (Действие == Команда["НайтиУзел"] as string)
            {
                ...
                var НомерУзла = "" + Запрос["nodenumber"];
                var ЗначениеУзла = "" + Запрос["nodevalue"];
            ...
            else if (Действие == Команда["ВставитьАтрибут"] as string)
            {
                if (!(Буфер == Неопределено))
                {
                    ...
                    НовыйУзел.Атрибут = null;
                    //НовыйУзел.Вставить("Дочерний", Неопределено);
                    var УзелАтрибут = Узел.Атрибут;
                    Узел.Атрибут = НовыйУзел;
                    if (!(УзелАтрибут == Неопределено))
                    {
                        НовыйУзел.Соседний = УзелАтрибут;
                        УзелАтрибут.Старший = НовыйУзел;
                    }
                }
                ОбновитьСостояние(Вкладка, Узел, "ОбновитьУзел", Истина, Истина);
            ...
            else if (Действие == Команда["ВставитьДочерний"] as string)
            {
                if (!(Буфер == Неопределено))
                {
                    ...
                    Узел.Дочерний = НовыйУзел;
                    if (!(УзелДочерний == Неопределено))
                    {
                        НовыйУзел.Соседний = УзелДочерний;
                        УзелДочерний.Старший = НовыйУзел;
                    }
                }
            ...
            else if (Действие == Команда["ВставитьСоседний"] as string)
            {
                ...
                if (!(Буфер == Неопределено))
                {
                    ...
                    Узел.Соседний = НовыйУзел;
                    if (!(УзелСоседний == Неопределено))
                    {
                        НовыйУзел.Соседний = УзелСоседний;
                        УзелСоседний.Старший = НовыйУзел;
                    }
                }
                ОбновитьСостояние(Вкладка, НовыйУзел.Родитель, "ОбновитьУзел", Истина, Истина);
            ...
            else if (Действие == Команда["ЗначениеУзла"] as string)
            {
                ...
                Узел.п.Удалить("Содержимое");
                Узел.п.Удалить("Состояние");
                ТекущиеДанные = НоваяСтруктураВкладка(Вкладка.Данные, "win", Данные.ИмяДанных + " Значение " + " " + Узел.Имя + " : " + СтрЭкранироватьРазметку("" + Узел.Значение, 20), "view", Узел.Код);
        ...
        public Структура НоваяЗадача(Структура Запрос, string Тип = "Запрос", object ЗадачаВладелец = null)
        {
            ...
            else
            {
                ...
                Сообщить("Новая задача " + Тип + "=" + структЗадача.ИдЗадачи + " " + структЗадача.Запрос["cmd"]);
            ...
            while (Истина)
            {
                ...
                while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 100))
                {
                    ...
                    if (структЗадача.Этап == "УдалитьЗадачу")
                    {
                        ...
                        Сообщить("Завершил задачу=" + структЗадача.ИдЗадачи + " " + структЗадача.Запрос["cmd"] + " время=" + (ТекущаяУниверсальнаяДатаВМиллисекундах() - структЗадача.ВремяНачало));
                        continue;
                    }

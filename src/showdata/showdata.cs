...
namespace onesharp
{
    class showdata : Onesharp
    {
        ...
        public TCPСоединение ПередатьДанные(string Хост, int Порт, Структура стрДанные)
        {
            ...
            return Функции.ПередатьДанные(Хост, Порт, стрДанные);
        }
        ...
        Вкладка НоваяВкладка(Задача структЗадача)
        {
            ...
            else if (_Данные is ДвоичныеДанные)
            {
                ...
                ТекущиеДанные.Вставить("ИстДанных", структЗадача.Запрос.с.sdata);
                ТекущиеДанные.Вставить("БазаДанных", структЗадача.Запрос.с.sdb);
        ...
        object СформироватьОтвет()
        {
            ...
            if (ОбновитьВкладки == Истина)
            {
                ...
                var Режим = УзелСвойство(Вкладки.Получить(ТекущаяВкладка) as Вкладка, "Режим") as string;
                if (Режим == "design")
                {
            ...
            // Если Команд > 0 Тогда
            // 	ОбщийРезультат = ОбщийРезультат + ПолучитьОбласть("ОбластьСтатус", Структура.Новый("ПараметрСтатусСообщение", "" + Цел(100*(ТекущаяУниверсальнаяДатаВМиллисекундах() - ВремяНачало))/100 + " s " + Команд + " c"));
            // КонецЕсли;
            return Ответ;
        ...
        object ВыполнитьДействия(Задача структЗадача)
        {
            ...
            { // загрузить данные с дата-сервера
                ...
                { // запросить данные
                    var ИстДанных = Запрос.с.ИстДанных;
                    var БазаДанных = Запрос.с.БазаДанных;
                    var ИмяДанных = Запрос.с.ИмяДанных;
                    var ПозицияДанных = Запрос.с.ПозицияДанных;
                    if (ИстДанных == "doc")
                    { // шаблон документации
                        ИстДанных = "";
                        ИмяДанных = "doc";
                    }
                    else if (!(Запрос.с.ИмяШаблона == ""))
                    {
                        БазаДанных = "";
                        ИмяДанных = Запрос.с.ИмяШаблона;
                        ПозицияДанных = "";
                    }
                ...
                if (!(Запрос.с.РезультатДанные == Неопределено))
                { // получен ответ дата-сервера

                    object _Данные = null;
                    Сообщить(Запрос.с.РезультатДанные.с.Ответ);
                    string Текст = null;
                    if (Запрос.с.РезультатДанные.с.Ответ == "Успешно")
                    {
                        var ТипДанных = "" + УзелСвойство(Запрос.с.РезультатДанные.с.Результат, "ТипДанных");
                        if (ТипДанных == "2")
                        { // это файл
                            _Данные = Запрос.с.РезультатДанные.с.Результат.с.Данные;
                        }
                        else
                        {
                            Текст = ПолучитьСтрокуИзДвоичныхДанных(Запрос.с.РезультатДанные.с.Результат.с.Данные);
                        }
                    }
                    else
                    {
                    ...
                    if (!(Текст is null))
                    {
                        _Данные = new pagedata(this, Текст, Запрос.с.БазаДанных, Запрос.с.ИмяДанных, Запрос.с.ПозицияДанных);
                    }
                    var ПутьДанные = Запрос.с.ИстДанных + "/" + Запрос.с.БазаДанных + "/" + Запрос.с.ИмяДанных + "/" + Запрос.с.ПозицияДанных;
                    ВсеДанные.Вставить(ПутьДанные, _Данные);
                    if (Запрос.с.ИстДанных == "doc")
                    {
                        ((pagedata)_Данные).Корень.Дочерний.Дочерний.Вставить("Значение", Запрос.с.ИмяДанных);
                    }
            ...
            else if (Действие == "ЗапросДанных")
            {
                ...
                {
                    if (Запрос.с.ЗапросДанных.с.Команда == "ЗагруженныеДанные")
                    {
                        var Позиция = 1;
                        foreach (КлючИЗначение элОбъектДанных in ВсеДанные)
                        {
                            ...
                            Запрос.с.Данные.НовоеЗначениеУзла(Запрос.с.Свойства.д.Результат, ЗапросДанные, Истина, Истина);
                            Позиция = Позиция + 1;
                        }
                ...
                if (!(Запрос.Свойство("РезультатДанные")))
                {
                    var стрЗапрос = Структура.Новый("ЗапросДанных, Команда", Запрос.с.ЗапросДанных, "ЗапросДанных");
                    var ИстДанных = "" + УзелСвойство(Запрос.с.ЗапросДанных, "ИстДанных");
                    if (ИстДанных == "")
                    {
                        ИстДанных = Субъект;
                    }
                    стрЗапрос.Вставить("ИстДанных", ИстДанных);
                    стрЗапрос.Вставить("БазаДанных", УзелСвойство(Запрос.с.ЗапросДанных, "БазаДанных"));
                    стрЗапрос.Вставить("ИмяДанных", УзелСвойство(Запрос.с.ЗапросДанных, "ИмяДанных"));
                    стрЗапрос.Вставить("ПозицияДанных", УзелСвойство(Запрос.с.ЗапросДанных, "ПозицияДанных"));
                    стрЗапрос.Вставить("ОбратныйЗапрос", Структура.Новый("ИдЗадачи", структЗадача.ИдЗадачи));
                ...
                else if (!(Запрос.с.РезультатДанные == Неопределено))
                { // получен ответ дата-сервера
                    var РезультатДанные = Запрос.с.РезультатДанные;
                    ...
                    else if (ЗапросДанные.ЗаголовокНайден as string == "True")
                    {
                        var КонечнаяПозиция = "" + УзелСвойство(ЗапросДанные, "ПозицияДанных");
                        if (!(КонечнаяПозиция == ""))
                        {
                            Запрос.с.Свойства.д.ЗапросДанных.д.КонечнаяПозиция.Значение = КонечнаяПозиция;
                        }
                        while (!(ЗапросДанные == Неопределено))
                        {
                            ...
                            Запрос.с.Данные.НовоеЗначениеУзла(Запрос.с.Свойства.д.Результат, Запись, Истина, Истина); // Добавить запись в источник данных
                        }
                        Запрос.с.РезультатДанные = null;
                    }
            ...
            { // запрос к внешним данным
                ...
                if (!(Запрос.Свойство("РезультатДанные")))
                {
                    string Пар = "" + Запрос.с.Параметры + ";" + procid + ";" + структЗадача.с.ИдЗадачи;
                ...
                { // получен ответ сервера
                    ДвоичныеДанные РезультатДанные = Запрос.с.РезультатДанные;
                    var Рез = ПолучитьСтрокуИзДвоичныхДанных(РезультатДанные);
                    Запрос.с.Библиотека.ОбработатьОтвет(Действие, Запрос.с.Данные, Запрос.с.Свойства, Рез);
                    Запрос.с.РезультатДанные = Неопределено;
                    return Истина;
                }
                return Ложь;
            }
            ...
            { // запрос к внешним данным
                ...
                if (!(Запрос.Свойство("РезультатДанные")))
                {
                    var стрЗапрос = Структура.Новый("ОбратныйЗапрос, Параметры, cmd", Структура.Новый("ИдЗадачи, Хост, Порт", структЗадача.ИдЗадачи, Параметры.Хост, Порт), Запрос.с.Параметры, Действие) as Структура;
                ...
                { // получен ответ сервера
                    var РезультатДанные = Запрос.с.РезультатДанные;
                    Запрос.с.Библиотека.ОбработатьОтвет(Запрос.с.Параметры.Действие, Запрос.с.Данные, Запрос.с.Свойства, РезультатДанные.Результат);
                    Запрос.с.РезультатДанные = Неопределено;
                    return Истина;
                }
                return Ложь;
            ...
            {
                ПередатьДанныеД(Структура.Новый("ИстДанных, БазаДанных, Заголовок, Команда, дДанные, неОбратныйЗапрос", Субъект, "inbox", Структура.Новый("ИмяДанных, ТипДанных", Запрос.с.filename, 2), "ЗаписатьДанные", Запрос.с.data, Неопределено));
                return Истина;

            }
            else if (Действие == "redirect")
            {
                ОбщийРезультат = ОбщийРезультат + ПолучитьОбласть("ОбластьПеренаправить", Структура.Новый("ПараметрПуть, ПараметрПауза, ПараметрТекст", Запрос.с.url, 0, ""));
                return Истина;

            }
            else if (Действие == "ЗавершитьЗадачу")
            {
                var сЗадача = Задачи.Получить(Запрос.с.сЗадача) as Задача;
            ...
            else if (Действие == Команда["ОстановитьСервер"] as string)
            {
                ...
                ОбщийРезультат = ОбщийРезультат + ПолучитьОбласть("ОбластьПеренаправить", Структура.Новый("ПараметрПуть, ПараметрПауза, ПараметрТекст", "/", 1000, ""));
            ...
            {
                ОбщийРезультат = "<div id='container' class='container-fluid data'><h3>завершение ...</h3></div>";
                ОбщийРезультат = ОбщийРезультат + ПолучитьОбласть("ОбластьПеренаправить", Структура.Новый("ПараметрПуть, ПараметрПауза, ПараметрТекст", "/", 1500, ""));
            ...
            {
                ЗадачаОбновить = структЗадача.ИдЗадачи;
                return Ложь;
            ...
            else if (Действие == Команда["ОбновитьСтраницу"] as string)
            {
                ...
                if (Меню == "")
                {
                    if (!(Субъект == "") || Локальный)
                    {
                        ...
                        Скрипты = ПолучитьОбласть("ОбластьПанельМенюСкрипт");
                    }
                    else    
            ...
            else if (Действие == Команда["СохранитьДанные"] as string)
            {
                if (!(Запрос.Свойство("РезультатДанные")))
                {
                    ...
                    if (!(Вкладка == Неопределено))
                    {
                        ...
                        {
                            foreach (КлючИЗначение св in Запрос.с.Заголовок)
                            {
                                Заголовок.Вставить((string)св.Ключ, св.Значение);
                            }
                        }
                        ПередатьДанные(Параметры.Хост, Параметры.ПортД, Структура.Новый("ИстДанных, БазаДанных, Заголовок, Команда, дДанные, ОбратныйЗапрос", Субъект, _Данные.БазаДанных, Заголовок, "ЗаписатьДанные", ПолучитьДвоичныеДанныеИзСтроки(_Данные.СохранитьДанные()), ОбратныйЗапрос));
                        Запрос.Вставить("РезультатДанные", Неопределено);
                    }
                }
                else if (!(Запрос.с.РезультатДанные == Неопределено))
                { // получен ответ дата-сервера
                    ...
                    Вкладка = Вкладки.Получить(Запрос.с.ТекущаяВкладка) as Вкладка;
                    if (!(Вкладка == Неопределено))
                    {
                        var _Данные = Вкладка.Данные as pagedata;
                        var Ответ = Запрос.с.РезультатДанные.с.Ответ;
                        var ОтветСтатус = 2;
                        if (Ответ == "Успешно")
                        {
                            if (Запрос.Свойство("События"))
                            {
                                _Данные.ДобавитьСобытие(Запрос.с.События, "savedata");
                            }
                            _Данные.ПозицияДанных = Запрос.с.РезультатДанные.Результат;
                            Ответ = "Данные сохранены";
                            ОтветСтатус = 1;
                        }
                        ЗаписатьСобытие(_Данные.БазаДанных + "/" + _Данные.ИмяДанных + "/" + _Данные.ПозицияДанных, Ответ, ОтветСтатус);
                    }
                    return Истина;
                }
                return Ложь;
            ...
            else if (Действие == Команда["НоваяБаза"] as string)
            {
                ...
                if (!("" + БазаДанных == ""))
                {
                    ...
                    { // получен ответ дата-сервера
                        ...
                        {
                            // Сообщить об ошибке
                            стрСообщение = Запрос.с.РезультатДанные.с.Ответ;
                            кСооб = 3;
                        }
            ...
            if (Действие == Команда["НоваяВкладка"] as string)
            {
                ...
                {
                    pagedata _Данные = структЗадача.Запрос.с.Данные;
                    Узел _События = структЗадача.Запрос.с.Свойства.д.События;
            ...
            if (Действие == Команда["Авторизация"] as string || Действие == Команда["Регистрация"] as string)
            {
                ...
                { // получен ответ дата-сервера
                    Структура РезультатДанные = Запрос.с.РезультатДанные;
                    ...
                    if (ЗапросДанные.с.ПрошелАвторизацию == "True")
                    {
                        ...
                        Скрипты = Скрипты + ПолучитьОбласть("ОбластьПанельМенюСкрипт");
                    }
                    Данные.ОбновитьУзел(Узел);
                    return Истина;
                }
            ...
            {
                ОбщийРезультат = ОбщийРезультат + ПоказатьМенюИнструменты(Вкладка, Узел, УзелСвойство(Запрос, "attr") as string == "1");
            ...
            else if (Действие == Команда["НовоеЗначениеУзла"] as string)
            {
                ...
                {
                    Данные.ОбновитьУзел(Узел);
                    ОбновитьСостояние(Вкладка, Узел, "НовоеЗначениеУзла", Запрос.с.valuedit);
                }
                else
                {
            ...
            else if (Действие == Команда["СвойстваУзла"] as string)
            {
                ...
                { // установить значения свойств формы объекта
                    var фУзел = Данные.ПолучитьУзел(Запрос.с.propid);
            ...
            else if (Действие == Команда["НовоеИмяУзла"] as string)
            {
                ...
                if (Запрос.Свойство("valuedit"))
                {
                    ОбновитьСостояние(Вкладка, Узел, "НовоеИмяУзла", Запрос.с.valuedit);
                }
                else
                {
            ...
            else if (Действие == Команда["СохранитьУзел"] as string)
            {
                ...
                // 	ОбновитьСостояние(Вкладка, Узел.Родитель, "ОбновитьУзел", Истина);
                // КонецЕсли;
                ОбновитьСостояние(Вкладка, Узел, "НовоеИмяУзла", Запрос.с.name);
                ОбновитьСостояние(Вкладка, Узел, "НовоеЗначениеУзла", Запрос.с.value);
                структЗадача.Вставить("Действие", Команда["ЗакрытьВкладку"]);
                return Неопределено;
        ...
        void ИзменитьАдрес()
        {
            ОбщийРезультат = ОбщийРезультат + ПолучитьОбласть("ОбластьИзменитьАдрес", Структура.Новый("ПараметрПуть", "/procid/" + procid));
        } // ИзменитьАдрес()
        ...
        public Структура НоваяЗадача(Структура Запрос, string Тип = "Запрос", object ЗадачаВладелец = null)
        {
            ...
            {
                var Задача = Задачи.Получить(ИдЗадачи) as Задача;
                if (!(Задача == Неопределено))
                {
                    if (Запрос.Свойство("РезультатДанные"))
                    {
                        ...
                        Задача.Запрос.Вставить("РезультатДанные", Запрос.с.РезультатДанные);
                    }
                    else if (Запрос.с.cmd == "taskend")
                    {
            ...
            else
            {
                ...
                структЗадача = Задача.Новый("ИдЗадачи, Тип, Этап, Запрос, Действие, Содержимое, Результат, ВремяНачало, ЗадачаВладелец", Строка(ИдЗадачи), Тип, "ВыполнитьЗадачу", Запрос, Неопределено, "", "", ТекущаяУниверсальнаяДатаВМиллисекундах(), ЗадачаВладелец);
                var Действие = Неопределено;
                структЗадача.Запрос.Свойство("cmd", out Действие);
                структЗадача.Действие = Действие as string;
                if (Действие as string != "upddata") ВремяНачало = ТекущаяУниверсальнаяДатаВМиллисекундах();
        ...
        void ОбработатьСоединения()
        {
            ...
            while (Истина)
            {
                ...
                if (!(Локальный))
                {
                    ...
                    { // новый и общий процессы не завершаем
                        ...
                        { // бездействие
                            ...
                            ОбщийРезультат = ПолучитьОбласть("ОбластьВыход");
                        }
                    }
                }
                ...
                if (РезультатОбновить == "" && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - ВремяНачало > 60000))
                {
                    // обработать данные
                ...
                if (ЗадачаОбновить != null && РезультатОбновить != "")
                {
                    var Результат = ПолучитьДвоичныеДанныеИзСтроки("data: " + КодироватьСтроку(РезультатОбновить, СпособКодированияСтроки.КодировкаURL));
                    РезультатОбновить = "";
                    if (!(ПередатьДанные(Параметры.Хост, Параметры.ПортВ, Структура.Новый("procid, taskid, Результат", procid, ЗадачаОбновить, Результат)) == Неопределено))
                    {
                        if (procid == "1" && !Локальный) 
                            ЗадачаОбновить = null;
                        else
                            РезультатОбновить = "";
    
                        Сообщить("upddata " + ЗадачаОбновить);
                    }
                }
                ...
                while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 100))
                {
                    ...
                    if (структЗадача.Этап == "ЕстьРезультат")
                    {
                        ...
                        {
                            if (структЗадача.Результат as string == "")
                            {
                                структЗадача.Результат = РезультатОбновить;
                                РезультатОбновить = "";
                            }

                            if (!(ПередатьДанные(Параметры.Хост, Параметры.ПортВ, Структура.Новый("procid, taskid, ContentType, Результат", procid, структЗадача.Запрос.с.taskid, структЗадача.Содержимое, структЗадача.Результат)) == Неопределено))
                            {
                                структЗадача.Этап = "УдалитьЗадачу";
                            }
                        }
                ...
                while (к > 0)
                {
                    ...
                    if (Соединение.Статус == "Данные")
                    {
                        ...
                        try
                        {
                            ...
                            Запрос = Функции.ДвоичныеДанныеВСтруктуру(дд);
                            Таймаут = 5;
                        }
                        catch (Exception e)
                        {

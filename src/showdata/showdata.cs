...
namespace onesharp
{
    public class showdata : useyourmind, Ishowdata
    {
        ...
        object ПоказатьВкладки()
        {
            ...
            ПараметрыШаблона.Вставить("ПараметрРежим", Вкладки.Получить<Структура>(ТекущаяВкладка)["Режим"]);
        ...
        object ВыполнитьДействия(Задача структЗадача)
        {
                ...
                { // получен ответ дата-сервера
                    ...
                    else if (ЗапросДанные.с.ЗаголовокНайден as string == "True")
                    {
                        ...
                        while (!(ЗапросДанные == Неопределено))
                        {
                            ...
                            ЗапросДанные = ЗапросДанные.Соседний;
            ...
            else if (Действие == Команда["РедактироватьУзел"] as string)
            {
                ...
                ТекущееОкно = НоваяСтруктураВкладка(Данные, "win", Данные.ИмяДанных + "  " + Узел.Имя + ":" + СтрЭкранироватьРазметку(Узел.Значение as string, 20), "struct", Узел.Код);
            ...
            else if (Действие == Команда["ЗагрузитьДерево"] as string)
            {
                ...
                {
                    ТекущееОкно = НоваяСтруктураВкладка(Данные, "win", Данные.ИмяДанных + " " + Узел.Имя + ":" + СтрЭкранироватьРазметку(Узел.Значение as string, 20) + " import", "struct", Узел.Код);

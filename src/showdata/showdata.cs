    ...
    class showdata : useyourmind
    {
        ...
        string ПоказатьСтруктуруУзла(Вкладка Вкладка, Узел Узел, bool ЭтоАтрибут = false, string Атрибуты = "", string Дочерний = "")
        {
            ...
            else
            {
                ...
                if (РедактироватьИмя == Истина)
                {
                    ...
                    РезультатОбновить = РезультатОбновить + ПолучитьОбласть("ОбластьСкриптИзменитьИмяЗначение", ПараметрыШаблона);
        ...
        string Диалог(Вкладка Вкладка, Узел Узел)
        {
            ...
            if (УзелСостояние(Вкладка, Узел, "РедактироватьУзел") as bool? == Истина)
            {
                ...
                ПараметрыШаблона.Вставить("ПараметрЭтоАтрибут", ЭтоАтрибут ? 1 : 0);
                РезультатОбновить = РезультатОбновить + ПолучитьОбласть("ОбластьСкриптРедактироватьУзел", ПараметрыШаблона);
            ...
            if (УзелСостояние(Вкладка, Узел, "НайтиУзел") as bool? == Истина)
            {
                ...
                ПараметрыШаблона.Вставить("ПараметрВкладка", "" + Вкладка.ИдВкладки);
                ПараметрыШаблона.Вставить("ПараметрИдУзла", "" + Вкладка.ИдВкладки + "_" + Узел.Код);
                РезультатОбновить = РезультатОбновить + ПолучитьОбласть("ОбластьСкриптНайтиУзел", ПараметрыШаблона);
        ...
        object ВыполнитьДействия(Задача структЗадача)
        {
            ...
            {
                РезультатОбновить = РезультатОбновить + ПолучитьОбласть("ОбластьПеренаправить", Структура.Новый("ПараметрПуть, ПараметрПауза, ПараметрТекст", Запрос.с.url, 0, ""));
                return Истина;
            ...
            {
                if (РезультатОбновить != "" || структЗадача.Результат != Неопределено)
                {
                    if (РезультатОбновить != "" && структЗадача.Результат == Неопределено)
                    {
                        структЗадача.Результат = РезультатОбновить + "<end/>";
                        РезультатОбновить = "";
                    }
            ...
            else if (Действие == Команда["ОбновитьСтраницу"] as string)
            {
                ...
                if (Меню == "")
                {
                    if (!(Субъект == "") || Локальный)
                    {
                        ...
                        РезультатОбновить = ПолучитьОбласть("ОбластьПанельМенюСкрипт");
                    }
                    else    
            ...
            if (Действие == Команда["Авторизация"] as string || Действие == Команда["Регистрация"] as string)
            {
                ...
                { // получен ответ дата-сервера
                    ...
                    if (ЗапросДанные.с.ПрошелАвторизацию == "True")
                    {
                        ...
                        РезультатОбновить = РезультатОбновить + ПолучитьОбласть("ОбластьПанельМеню") + ПолучитьОбласть("ОбластьПанельМенюСкрипт");
                    }
                    Данные.ОбновитьУзел(Узел);
                    return Истина;
                }
            ...
            while (Истина)
            {
                ...
                if (ОбновитьВкладки == Истина)
                {
                    ...
                    if (ТекущаяВкладка != null)
                    {
                        ...
                        ПараметрыШаблона.Вставить("ПараметрТекущаяВкладка", ТекущаяВкладка);
                        РезультатОбновить = РезультатОбновить + ПолучитьОбласть("ОбластьСкрипт", ПараметрыШаблона);
                    }

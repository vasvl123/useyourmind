    ...
    class showdata : useyourmind
    {
        ...
        object ВыполнитьДействия(Задача структЗадача)
        {
            ...
            else if (Действие == "ЗапросДанных")
            {
                ...
                if (Запрос.Свойство("ЗапросДанных"))
                {
                    if (Запрос.с.ЗапросДанных.с.Команда == "ЗагруженныеДанные")
                    {
                        ...
                        foreach (КлючИЗначение элОбъектДанных in ВсеДанные)
                        {
                            ...
                            Запрос.с.Данные.НовоеЗначениеУзла(Запрос.с.Свойства.д.с.Результат, ЗапросДанные, Истина, Истина);
                            Позиция = Позиция + 1;
                        }
                ...
                { // получен ответ дата-сервера
                    ...
                    else if (ЗапросДанные.с.ЗаголовокНайден as string == "True")
                    {
                        ...
                        if (!(КонечнаяПозиция == ""))
                        {
                            Запрос.с.Свойства.д.с.ЗапросДанных.д.с.КонечнаяПозиция.Значение = КонечнаяПозиция;
                        }
                        while (!(ЗапросДанные == Неопределено))
                        {
                            ...
                            Запрос.с.Данные.НовоеЗначениеУзла(Запрос.с.Свойства.д.с.Результат, Запись, Истина, Истина); // Добавить запись в источник данных
                        }
            ...
            { // запрос к внешним данным
                ...
                { // получен ответ сервера
                    var РезультатДанные = Запрос.с.РезультатДанные;
                    Запрос.с.Библиотека.ОбработатьОтвет(Запрос.с.Параметры.с.Действие, Запрос.с.Данные, Запрос.с.Свойства, РезультатДанные.с.Результат);
                    Запрос.с.РезультатДанные = Неопределено;
                    return Истина;
                }
                return Ложь;
            ...
            if (Действие == Команда["НоваяВкладка"] as string)
            {
                ...
                if (структЗадача.Запрос.Свойство("Данные")) // вернуть результат
                {
                    ...
                    Узел _События = структЗадача.Запрос.с.Свойства.д.с.События;

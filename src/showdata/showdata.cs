        ...
        object ВыполнитьДействия(Задача структЗадача)
        {
            ...
            { // загрузить данные с дата-сервера
                ...
                { // получен ответ дата-сервера
                    ...
                    object _Данные = null;
                    Сообщить(Запрос.РезультатДанные.с.Ответ);
                    string Текст = null;
                    if (Запрос.РезультатДанные.с.Ответ == "Успешно")
                    {
                        var ТипДанных = "" + УзелСвойство(Запрос.РезультатДанные.с.Результат, "ТипДанных");
                        if (ТипДанных == "2")
                        { // это файл
                            _Данные = Запрос.РезультатДанные.с.Результат.с.Данные;
                        }
                        else
                        {
                            Текст = ПолучитьСтрокуИзДвоичныхДанных(Запрос.РезультатДанные.с.Результат.с.Данные);
                        }
                    }
                    else
                    {
                    ...
                    if (!(Текст is null))
                    {
                        _Данные = new pagedata(this, Текст, Запрос.БазаДанных, Запрос.ИмяДанных, Запрос.ПозицияДанных);
                    }
            ...
            { // запрос к внешним данным
                ...
                { // получен ответ сервера
                    var РезультатДанные = Запрос.Получить<ДвоичныеДанные>("РезультатДанные");
                    var Рез = ПолучитьСтрокуИзДвоичныхДанных(РезультатДанные);
                    Запрос.с.Библиотека.ОбработатьОтвет(Действие, Запрос.Данные, Запрос.Свойства, Рез);
                    Запрос.РезультатДанные = null;
                    return Истина;
                }
                return Ложь;

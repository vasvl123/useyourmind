    ...
    class showdata : useyourmind
    {
        ...
        void ОбработатьСоединения()
        {
            ...
            while (Истина)
            {
                ...
                if (РезультатОбновить != "")
                {
                    if (!(ПередатьДанные(Параметры.Хост, Параметры.ПортВ, Структура.Новый("procid, Результат, cmd", procid, ПолучитьДвоичныеДанныеИзСтроки(РезультатОбновить), "upddata")) == Неопределено))
                    {
                        РезультатОбновить = "";

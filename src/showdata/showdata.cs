...
namespace onesharp
{
    class showdata : useyourmind
    {
        ...
        object ВыполнитьДействия(Задача структЗадача)
        {
            ...
            {
                if (upddata == null) upddata = структЗадача;
                if (upddata != структЗадача) upddata.Этап = "УдалитьЗадачу";
                
                if (РезультатОбновить != "")
                {
                    var Результат = ПолучитьДвоичныеДанныеИзСтроки(РезультатОбновить + "<end/>");
                    if (!(ПередатьДанные(Параметры.Хост, Параметры.ПортВ, Структура.Новый("procid, taskid, ContentType, Результат", procid, структЗадача.Запрос.с.taskid, структЗадача.Содержимое, Результат)) == Неопределено))
                    {
                        РезультатОбновить = "";
                    }
                }

    ...
    public class treedata : pagedata
    {
        ...
        public int СохранитьУзел(Узел элУзел, Соответствие НовыеНомера)
        {
            ...
            if (элУзел.Изменения)
            {
                ...
                _Узел._Имя = Данные.НайтиСтроку(элУзел.Имя, Истина);
                _Узел._Значение = Данные.НайтиСтроку("" + элУзел.Значение, Истина);
        ...
        public Узел _ПолучитьУзел(ТекстовыйДокумент _Данные, object Код, Узел Старший = null)
        {
            ...
            try
            {
                //if (Лев(Код as string, 1) == "n") Код = Сред(Код as string, 2);
            ...
            foreach (string _знСтр in мСтр)
            {
                ...
                else
                {
                    ...
                    else if (Ключ == "З")
                    {
                        ...
                        //if (Узел.Имя == "С" || Узел.Имя == "Ссылка" || Узел.Имя == "З" || Узел.Имя == "Значение")
                            //if (Лев(знСтр, 2) == "у ")
                                //знСтр = Лев(знСтр, 2) + "n" + Сред(знСтр, 3);
        ...
        public treedata(Ishowdata обПроцесс, string знИстДанных, string знИмяДанных, bool Новый = false) : base(обПроцесс, знИстДанных,  знИмяДанных)
        {

            if (!Новый) ОткрытьДанные();

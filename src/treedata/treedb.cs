...
namespace onesharp
{
    ...
    public class treedb : Onesharp
    {
        ...
        public int СвойствоСтроки(int н, int стрСвойство, int имяСвойство, int? знСвойство = null)
        {
            var буф = БуферДвоичныхДанных.Новый(20);
            long пн = 0;
            long нн = н * 20;
            ...
            do
            {
                if (нн == 0)
                {
                    if (знСвойство == null) return 0;
                    
                    var буф1 = БуферДвоичныхДанных.Новый(20);
                    буф1.ЗаписатьЦелое32(8, (int)знСвойство); // значение свойства
                    буф1.ЗаписатьЦелое32(12, имяСвойство); // имя свойства
                    буф1.ЗаписатьЦелое32(16, стрСвойство); // указ. строки
                ...
                пу = буф.ПрочитатьЦелое32(12);

                пн = нн;

                if ((пу == имяСвойство || имяСвойство == 0) && знСвойство == null && нн != н * 20) 
                {
                    var зс = буф.ПрочитатьЦелое32(16);
                    if (зс == стрСвойство) break;
                }
        ...
        public int НайтиСтроку(string знСтрока, bool Создать = false)
        {
            ...
            foreach (var сл in мСтр)
            { 
                var тс = (int)(НайтиСлово(сл, Истина) / 20);
                ...
                do
                {
                    ...
                    else if (нн == 0)
                    {
                        ...
                        var буф1 = БуферДвоичныхДанных.Новый(20);
                        буф1.ЗаписатьЦелое32(12, тс); // тек слово.
                        буф1.ЗаписатьЦелое32(16, вн); // пред. указатель
                    ...
                    зс = буф.ПрочитатьЦелое32(12);

                    if (зс == тс)
                    {
                        var пу = буф.ПрочитатьЦелое32(16);
                        if (пу == вн) break;
                    }

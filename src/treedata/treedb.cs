...
namespace onesharp
{
    public class treedb : Onesharp
    {
        ...
        public int ЗаписатьУзел(Узел Узел)
        {
            ...
            {
                var сКод = (int)Число(Узел.Код);
                var буф1 = БуферДвоичныхДанных.Новый(4);
                буф1.ЗаписатьЦелое32(0, Код);
            ...
            буф.ЗаписатьЦелое32(16, пИмя);
            буф.ЗаписатьЦелое32(20, пЗначение);
        ...
        public Узел ПрочитатьУзел(long н = 0, Узел пУзел = null)
        {
            ...
            if (Версия != 0 && пУзел == null) // новая версия
            ...
            var зИмя = буф.ПрочитатьЦелое32(16);
            нУзел.Имя = ПолучитьСтроку(зИмя);
            var зЗначение = буф.ПрочитатьЦелое32(20);
            ...
            if (!(ПотокДанных.Размер() == 0))
            {
                ...
                while (Истина)
                {
                    ...
                    while (Истина)
                    {
                        ...
                        if (ф1 == ф || ф == 0) // 0 нужен для добавления значения к существующему
                        { // найден элемент
                            рн = н;
                            к = к + 1;
                            if (!(к > сгр))
        ...
        public string ПолучитьСтроку(int нз, int кл = 1)
        {
            if (нз == 0) return "";
            
            var м2 = Новый.Массив();
            var мСтр = ПолучитьМассив(нз);
            if ((int)мСтр[0] == кл) // слово
                м2.Добавить(мСтр);
            else // предложение
            {

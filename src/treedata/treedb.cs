...
namespace onesharp
{
    ...
    public class treedb : Onesharp
    {
        ...
        public Соответствие ПолучитьЗначенияСвойства(int н)
        {
            var м = Соответствие.Новый();

            if (н == 0) return м;

            var у = ПрочитатьУзел(н);
            у = ПрочитатьУзел(у._Атрибут);

            while (true)
            {
                м.Добавить(у._Имя, Новый.Структура("Тип, Имя, Поз", "П", ПрочитатьСтроку(у._Имя), у._Имя));
                break;
        ...
        public Соответствие ПолучитьСвойства(int нн, int н, Соответствие м = null)
        {
            if (м == null)
            {
                м = Соответствие.Новый();
                var пу = ПрочитатьУзел(н);
                if (пу._Атрибут != 0) 
                        м.Добавить(пу._Атрибут, Новый.Структура("Тип, Имя, Поз", "С", ПрочитатьСтроку(пу._Атрибут), пу._Атрибут));
                
                //while (пу._Значение != 0)
                //{
                //    пу = ПрочитатьУзел(пу._Значение);
                //    м.Добавить(пу._Атрибут, Новый.Структура("Тип, Имя, Поз", "С", ПрочитатьСтроку(пу._Имя), пу._Имя));
                //}
            }
            ...
            if (у._Значение == нн)
                м.Добавить(у._Код, Новый.Структура("Тип, Имя, Поз", "О", ПрочитатьСтроку(у._Имя), у._Код));

            if (у._Дочерний != 0) м = ПолучитьСвойства(нн, у._Дочерний, м);
            if (у._Соседний != 0) м = ПолучитьСвойства(нн, у._Соседний, м);

            return м;
        }

        public Соответствие ПолучитьСлова(int нн, int н, string с = "", Соответствие м = null)
        {
            var у = ПрочитатьУзел(н);
            ...
            if (у._Имя == нн)
            {
                м.Добавить(у._Код, Новый.Структура("Тип, Имя, Поз", "П", ПрочитатьСтроку(у._Код), у._Код));
            }

            if (у._Дочерний != 0) м = ПолучитьСлова(нн, у._Дочерний, с, м);
            if (у._Соседний != 0) м = ПолучитьСлова(нн, у._Соседний, с, м);

            return м;
        }


        public Соответствие ПолучитьЭлементы(int н, string с = "", Соответствие м = null)
        {
            var у = ПрочитатьУзел(н);
            ...
            if (у._Имя != 0)
            {
                if (у._Дочерний != 0 || у._Соседний != 0)
                {
                    if (с != "")
                    {
                        м.Добавить(у._Код, Новый.Структура("Тип, Имя, Поз", "Б", с, у._Код));
                        return м;
                    }
                }
                if (у._Атрибут != 0)
            ...
            {
                м.Добавить(у._Код, Новый.Структура("Тип, Имя, Поз", "С", с, у._Код));
        ...
        public int СвойствоСтроки(int н, int знСтарший, int знСвойство, int? знЗначение = null)
        {
            ...
            do
            {
                if (нн == 0)
                {
                    if (знЗначение == null) return 0;
                    
                    var буф1 = БуферДвоичныхДанных.Новый(20);
                    буф1.ЗаписатьЦелое32(8, (int)знЗначение); // значение свойства
                    буф1.ЗаписатьЦелое32(12, знСвойство); // имя свойства
                    буф1.ЗаписатьЦелое32(16, знСтарший); // указ. строки
                    ...
                    if ((пу & 0x000000ff) > (знСтарший & 0x000000ff))
                ...
                пу = буф.ПрочитатьЦелое32(16);

                пн = нн;

                {
                    if (пу == знСтарший && знЗначение == null && нн != н * 20) 
                    {
                        if (знСвойство == 0) break;
                        var зс = буф.ПрочитатьЦелое32(12);
                        if (зс == знСвойство) break;
                    }
                }
        ...
        public long НайтиСлово(string слово, bool Создать = false)
        {
        ...
        public int НайтиСтроку(string знСтрока, bool Создать = false)
        {
...

            {
                ...
                for (var нс = 1; ; нс++)
                {
                    ...
                    var пус = узн;
                    узн = (Стр.Найти(сСимв, сс) != 0) || (узн && сс == "-");
    
                    if (!узн && пус)
                    {
                        var пар = Новый.Структура("сл, зн", НайтиСлово(сл, Истина), НайтиСлово(зн, Истина));
                        атр.Добавить(пар);
                        сл = "";
                        зн = "";
                    }
            ...
            {
                long нн = (long)((Структура)атр[н])["сл"];
                int знн = (int)((long)((Структура)атр[н])["зн"]) / 20;

                long пу = 0;                
                long пн = 0;

                var текСлово = (int)(нн / 20);
                ...
                do
                {
                    if (нн == 0)
                    {
                        ...
                        var буф1 = БуферДвоичныхДанных.Новый(20);
                        буф1.ЗаписатьЦелое32(8, знн); // знаки
                        буф1.ЗаписатьЦелое32(12, текСлово); // тек слово.
                        буф1.ЗаписатьЦелое32(16, (int)(предУказ / 20)); // пред. указатель
                        ...
                        if ((пу & 0x000000ff) > (предУказ & 0x000000ff))
                            буф.ЗаписатьЦелое32(0, (int)(нн / 20));
                        else
                            буф.ЗаписатьЦелое32(4, (int)(нн / 20));
                    ...
                    пу = буф.ПрочитатьЦелое32(16) * 20;

                    if (пу == предУказ)
                    {
                        var зн = буф.ПрочитатьЦелое32(8);
                        if (зн == знн)
                        {
                            var зс = буф.ПрочитатьЦелое32(12);
                            if (зс == текСлово) break;
                        }
                    }
        ...
        public string ПрочитатьСтроку(int н)
        {
            ...
            do
            {
                ...
                нс = буф.ПрочитатьЦелое32(8) * 20; // знаки

                var сл = "";
                while (нс != 0)
                {
                    ПотокДанных.Перейти(нс, ПозицияВПотоке.Начало);
                    ПотокДанных.Прочитать(буф1, 0, 20);
                    var с = буф1.ПрочитатьЦелое32(12);
                    сл = Символ(с) + сл;
                    нс = буф1.ПрочитатьЦелое32(16) * 20;
                }
                зСтр = сл + зСтр;

                нс = буф.ПрочитатьЦелое32(12) * 20; // слово

...
namespace onesharp
{
    ...
    public class treedb : Onesharp
    {
        ...
        public Массив ПолучитьЭлементы(int н, Массив м = null, int нн = 0)
        {
            ...
            {
                if (у._Атрибут != 0 && у._Значение == нн)
                {
        ...
        public _Узел НайтиСтроку(string знСтрока, bool Создать = false)
        {
            ...
            while (нс <= дСтр)
            {
                ...
                if (Стр.Найти(сСимв, сс) == 0)
                {

                    дКод = 21;
                    ОткрытьПотокДанных();
                    ПотокДанных.Перейти(21, ПозицияВПотоке.Начало);
                    ПотокДанных.Прочитать(буф, 0, 20);
                    ...
                    do
                    {
                        ...
                        сс = Сред(знСтрока, нс, 1);
                        с = КодСимвола(сс);

                    }
                    while (Стр.Найти(сСимв, сс) == 0);

                    ан = буф.ПрочитатьЦелое32(8);
                    адКод = дКод;
                }

                // создать ссылку на предыдущее слово
                    
                var буф2 = БуферДвоичныхДанных.Новый(20);

                if (ан == 0)
                {
                    if (!Создать) return Узел;

                    буф2.ЗаписатьЦелое32(8, 0);
                    буф2.ЗаписатьЦелое32(16, дКод);

                    ОткрытьПотокДанных(Истина);
                    ан = (int)ПотокДанных.Размер();
                    ПотокДанных.Перейти(0, ПозицияВПотоке.Конец);
                    ПотокДанных.Записать(буф2, 0, 20);
                ...
                else
                {
                    ...
                    ПотокДанных.Прочитать(буф2, 0, 20);
                }

                var _Атрибут = буф2.ПрочитатьЦелое32(8);

                while (!(_Атрибут == пКод && буф2.ПрочитатьЦелое32(12) == с))
                {
                    var пн = ан;
                    
                    if (_Атрибут > пКод)
                        ан = буф2.ПрочитатьЦелое32(0);
                    else
                        ан = буф2.ПрочитатьЦелое32(4);
                    ...
                    if (ан == 0)
                    {
                        ...
                        if (_Атрибут > пКод)
                            буф2.ЗаписатьЦелое32(0, ан);
                        else
                            буф2.ЗаписатьЦелое32(4, ан);
                    ...
                    ПотокДанных.Перейти(ан, ПозицияВПотоке.Начало);
                    ПотокДанных.Прочитать(буф2, 0, 20);
                    _Атрибут = буф2.ПрочитатьЦелое32(8);
        ...
        public string ПрочитатьСтроку(int нз)
        {
            ...
            do
            {
                ...
                var с = буф.ПрочитатьЦелое32(12);
                if (с != 0) сл = сл + Символ(с);
                зСтр = сл + зСтр;

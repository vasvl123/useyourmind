...
namespace onesharp
{
    public class treedata : pagedata
    {
        ...
        public int СохранитьУзел(Узел элУзел, Соответствие НовыеНомера)
        {
            ...
            if (элУзел.Изменения)
            {
                if (элУзел.Атрибут != Неопределено) СохранитьУзел(элУзел.Атрибут, НовыеНомера);
                if (элУзел.Дочерний != Неопределено) СохранитьУзел(элУзел.Дочерний, НовыеНомера);
                if (элУзел.Соседний != Неопределено) СохранитьУзел(элУзел.Соседний, НовыеНомера);

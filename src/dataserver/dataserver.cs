...
namespace onesharp
{
    class dataserver : useyourmind, Ishowdata
    {
        ...
        object ПолучитьДанные<T>(string ИстДанных, string ИмяДанных, bool Новый = false)
        {
            ...
            if (Данные == Неопределено)
            {
                ...
                {
                    var ИмяФайлаДанных = ОбъединитьПути(Сис, ИстДанных, ИмяДанных + ".tdb");
                    var файл = Файл.Новый(ИмяФайлаДанных);
                    if (файл.Существует())
                    {
                        Данные = new treedata(this, ИстДанных, ИмяДанных, false);
                    }
                    else // старый формат
                    {
                        ИмяФайлаДанных = ОбъединитьПути(Сис, ИстДанных, ИмяДанных + ".sd");
                        ...
                        if (файл.Существует())
                        {
                            ...
                            Данные = new treedata(this, ИстДанных, ИмяДанных, true);
                    }
                    Источники.Вставить(ИмяДанных, Данные);
                }
            }
            ...
            foreach (КлючИЗначение элИсточник in ВсеДанные)
            {
                foreach (КлючИЗначение элДанные in элИсточник.Значение as Соответствие)
                {
                    if (элДанные.Значение is treedata)
                    {
                        ...
                        while (Данные.ОбъектыОбновить.Количество() != 0)
                        {
                            if (ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50) break;

                            var Узел = Данные.ОбъектыОбновить.Получить(0) as Узел;
                            Данные.ОбъектыОбновить.Удалить(0);

                            var Результат = Данные.ОбработатьОбъект(Узел);
                            if (Результат == Неопределено)
                            {
                                Данные.ОбъектыОбновить.Добавить(Узел);
                            }
                            else
                            {
                                ДвоичныеДанные дДанные = null;
                                foreach (Задача стрЗадача in мЗадачи)
                                {
                                    if (стрЗадача.Запрос.Команда == "ПолучитьДанные")
                                    {
                                        if (стрЗадача.Данные == Данные)
                                        {
                                            if (стрЗадача.Узлы == null) стрЗадача.Узлы = Новый.Массив();
                                            if (дДанные == null) дДанные = Данные.УзелВДвоичныеДанные(Узел);
                                            стрЗадача.Узлы.Добавить(дДанные);
                                            стрЗадача.Этап = "Выполнить";
                                        }
                                    }
                                }
                            }
        ...
        bool? ВыполнитьЗадачу(Задача структЗадача)
        {
            ...
            if (Команда == "ПолучитьУзел")
            {
                var _Данные = ПолучитьДанные<treedata>(ИстДанных, ИмяДанных) as treedata;
                if (_Данные != null)
                {
                    структЗадача.Вставить("Ответ", "ЕстьУзел");
                    var ст = _Данные.ПолучитьУзел(Запрос["СтаршийКод"]);
                    структЗадача.Результат = Новый.Структура("Данные", _Данные.УзелВДвоичныеДанные(_Данные.ПолучитьУзел(Запрос["УзелКод"], ст)));
                    return Истина;
                }
            ...
            if (Команда == "СохранитьУзел")
            {
                var _Данные = ПолучитьДанные<treedata>(ИстДанных, ИмяДанных) as treedata;
                if (_Данные != null)
                {
                    var Узел = _Данные.ДвоичныеДанныеВУзел(Запрос["дДанные"] as ДвоичныеДанные) as Узел;
                    var НовыеНомера = Новый.Соответствие();
                    _Данные.ОткрытьДанные();
                    _Данные.СохранитьУзел(Узел, НовыеНомера);
                    //_Данные.ОбъектыОбновить.Добавить(Узел);
                    структЗадача.Вставить("Ответ", "Успешно");
                    структЗадача.Результат = Новый.Структура("НовыеНомера", НовыеНомера);
                }
                else
                {
            ...
            if (Команда == "ПолучитьДанные")
            {
                if (структЗадача.Данные == null)
                {
                    ...
                    { // чтение данных из файла
                        try
                        {
                            ...
                            {
                                var _Данные = ПолучитьДанные<treedata>((ИстДанных == "") ? Субъект : ИстДанных, ИмяДанных) as treedata;
                                if (_Данные == null) // public?
                                    _Данные = ПолучитьДанные<treedata>("public", ИмяДанных) as treedata;
                                if (_Данные == null) // новый
                                    _Данные = ПолучитьДанные<treedata>(ИстДанных, ИмяДанных, Истина) as treedata;

                                структЗадача.Вставить("Данные", _Данные);
                                структЗадача.Вставить("Ответ", "ЕстьДанные");
                                структЗадача.Результат = Новый.Структура("Данные, ИстДанных, ИмяДанных", _Данные.УзелВДвоичныеДанные(_Данные.Корень), _Данные.ИстДанных, _Данные.ИмяДанных);
                                return Истина;
                ...
                {
                    if (структЗадача.Узлы != null) // передать обновление узлов
                    {
                        структЗадача.Ответ = "ЕстьДанные";
                        структЗадача.Результат = Новый.Структура("Данные", структЗадача.Узлы);
                        структЗадача.Узлы = null;
                        return Истина;
                    }
            ...
            { // запись заголовка
                ...
                { // системные базы
                    Данные = ПолучитьДанные<dbaccess>("sys", БазаДанных) as dbaccess;
                }
                ...
                {
                    Данные = ПолучитьДанные<dbaccess>(ИстДанных, "inbox") as dbaccess;
                }
            ...
            { // запись данных
                try
                {
                    ...
                    { // имя контейнера указано
                        ...
                        {
                            Данные = ПолучитьДанные<dbaccess>(Субъект, "inbox") as dbaccess;
                        }
                        else
                        {
                            Данные = ПолучитьДанные<dbaccess>(ИстДанных, "inbox") as dbaccess;
            ...
            { // выбрать данные по запросу
                ...
                { // выбрать данные по запросу
                    ...
                    else
                    {
                        ...
                        {
                            Данные = ПолучитьДанные<dbaccess>("public", "public") as dbaccess;
                        }
                        else
                        {
                            Данные = ПолучитьДанные<dbaccess>(ИстДанных, "public") as dbaccess;
                        }
                        // Иначе
                        // 	структЗадача.Вставить("Результат", "");
                        // 	структЗадача.Вставить("Ответ", "Нет прав");
            ...
            else if (Команда == "ПолучитьДанные")
            {
                ...
                ПозицияДанных = "" + Строка(Запрос["ПозицияДанных"]);
                ...
                { // найти по имени данных
                    ...
                    {
                        ЗапросДанных.с.Заголовок.Вставить("Данные", Данные.ПолучитьДанные(Число(ЗапросДанных.с.Заголовок.с.ПозицияДанных)));
        ...
        void ОбработатьСоединения()
        {
                ...
                while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50))
                {
                    ...
                    if (структЗадача.Этап == "Выполнить")
                    {
                        ...
                        if (ЕстьРезультат != null)
                        {
                            ...
                            if (Строка(структЗадача.Ответ) == "ЕстьДанные")
                            {
                                структЗадача.Этап = "Ожидание";
                            }
                            else if (Строка(структЗадача.Ответ) == "ЗапросПриостановлен")
                            {
                                структЗадача.Этап = "Приостановлен";
                            }
                            else структЗадача.Этап = "Удалить";
            ...
            Сообщить("Процесс dataserver завершен.");

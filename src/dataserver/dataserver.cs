    ...
    class dataserver : useyourmind, Ishowdata
    {
        ...
        T ПолучитьДанные<T>(string ИстДанных, string ИмяДанных)
        {
            ...
            if (Данные == Неопределено)
            {
                ...
                if (typeof(T) == typeof(pagedata))
                {
                    var ИмяФайлаДанных = ОбъединитьПути(Сис, ИмяДанных + ".sd");
                    var дДанные = new ДвоичныеДанные(ИмяФайлаДанных);
                    Данные = new pagedata(this, ПолучитьСтрокуИзДвоичныхДанных(дДанные), "", ИмяДанных);
                    Источники.Вставить(ИмяДанных, Данные);
                }
                if (typeof(T) == typeof(treedata))
                {
                    Данные = new treedata(this, ОбъединитьПути(Сис, ИстДанных), ИмяДанных);
                    Источники.Вставить(ИмяДанных, Данные);
                }
            }
            return (T)Данные;
        ...
        bool ОбработатьДанные(Задача структЗадача = null)
        {
            ...
            if (структЗадача != null)
            {
                ...
                var ИмяДанных = Запрос["data"] as string;
                var Истданных = Запрос["sdata"] as string;

                var Данные = ПолучитьДанные<treedata>(Истданных, ИмяДанных);

                if (Данные != null)
                {
                    if (Действие == "newtab") // открыть данные
                    {
                        var ИдПроцесса = Запрос["ИмяМетода"] as string;
                ...
                while (к > 0)
                {
                    ...
                    if (Данные.Представление != "")
                    {
                        ...
                        foreach(КлючИЗначение элПроцесс in Процессы)
                        {
                            ...
                            if (!(Контроллер == Неопределено))
                            {
                                var Запрос = Структура.Новый("ИстДанные, ИмяДанные, Представление, cmd", Данные.ИстДанных, Данные.ИмяДанных, Данные.Представление, "ДанныеОбновить");

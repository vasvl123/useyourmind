    ...
    class dataserver : Onesharp
    {
        ...
        object ВыполнитьЗадачу(Структура структЗадача)
        {
            ...
            { // завершить существующую задачу
                foreach (КлючИЗначение элЗадача in Задачи)
                {
                    ...
                    {
                        if (стрЗадача.с.Запрос.ОбратныйЗапрос.ИдЗадачи == Запрос.сЗадача)
                        {
            ...
            { // выбрать данные по запросу
                ...
                { // выбрать данные по запросу
                    ...
                    if (Запрос.ЗапросДанных.ЗаголовокНайден == Истина || структЗадача.с.Ответ == "ЗапросЗавершен" || структЗадача.с.Ответ == "ЗапросПриостановлен")
                    {
            ...
            else if (Команда == "ВнешниеДанные")
            {
                ...
                var кЗадача = Контроллеры.Получить(Запрос.Параметры.База) as Структура;
                if (!(кЗадача == Неопределено))
                { // есть соединение с базой
                    if (кЗадача.с.Результат == Неопределено)
                    { // послать запрос
                        Запрос.Параметры.Вставить("ИдЗадачи", структЗадача.с.ИдЗадачи);
                        кЗадача.с.Результат = Функции.СтруктуруВДвоичныеДанные(Запрос.Параметры);
                        структЗадача.с.Этап = "Приостановить";
                    }
                }
        ...
        void УдалитьКонтроллерИЗадачи(string ИдКонтроллера)
        {
            ...
            foreach (КлючИЗначение элЗадача in Задачи)
            {
                ...
                {
                    if (Задача.с.Запрос.ИдПроцесса == ИдКонтроллера)
                    {
            ...
            while (!(ОстановитьСервер))
            {
                ...
                while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50))
                {
                    ...
                    if (!((bool)ЕстьРезультат))
                    {
                        ...
                        if (структЗадача.с.Ответ == "ЗапросПриостановлен")
                        {
                            ...
                            {
                                if (структЗадача.с.Запрос.Команда == "ЗапросДанных")
                                {
                                    if (структЗадача.с.Запрос.ЗапросДанных.Команда == "СписокФайлов")
                                    {
                                        if (ОбновитьСписокФайлов > структЗадача.с.ВремяСоздания)
                                        {
                                            структЗадача.с.Запрос.ЗапросДанных.Позиция = 0;
                                            структЗадача.с.Запрос.Удалить("СписокФайлов");
                                            структЗадача.с.ВремяСоздания = ТекущаяДата();
                                            Выполнить = Истина;
                                        }
                                    }
                                    if (структЗадача.с.Запрос.ЗапросДанных.Команда == "СписокБаз")
                                    {
                                    ...
                                    if (структЗадача.с.Запрос.ЗапросДанных.Команда == "НайтиЗаголовок")
                                    {
                                        var Данные = (dbaccess)ВсеДанные.Получить(структЗадача.с.Данные);
                                        if (Данные.ВремяИзменения > структЗадача.с.ВремяСоздания)
                                        {
                                            структЗадача.с.Запрос.ЗапросДанных.ПоследняяПозиция = 0;
                                            структЗадача.с.Запрос.ЗапросДанных.Удалить("ПозицияДанных");
                                            структЗадача.с.ВремяСоздания = ТекущаяДата();
                                            Выполнить = Истина;
                                        }
                                    }
                    ...
                    if ((bool)ЕстьРезультат == Истина)
                    {
                        try
                        {
                            ...
                            { // возвращаем результат
                                ...
                                var Контроллер = Контроллеры.Получить(структЗадача.с.Запрос.ИдПроцесса) as Структура;
                                if (!(Контроллер == Неопределено))
                                {
                                    ОбратныйЗапрос.Вставить("РезультатДанные", Структура.Новый("Ответ, Результат", структЗадача.с.Ответ, структЗадача.с.Результат));
                                    структЗадача.Вставить("Соединение", Функции.ПередатьДанные(Контроллер.с.Хост, Контроллер.с.Порт, ОбратныйЗапрос));
                    ...
                    {
                        if (!(структЗадача.с.Соединение.Статус == "Занят"))
                        {
                ...
                while (к > 0)
                {
                    ...
                    if (Соединение.Статус == "Данные")
                    {
                        ...
                        {
                            Запрос = null;
                            Запрос = Функции.ДвоичныеДанныеВСтруктуру(Соединение.ПолучитьДвоичныеДанные()) as Структура;
                        }
                        catch (Exception e)
                        {

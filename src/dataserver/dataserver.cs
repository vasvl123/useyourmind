...
namespace onesharp
{
    class dataserver : useyourmind, Ishowdata
    {
        ...
        T ПолучитьДанные<T>(string ИстДанных, string ИмяДанных)
        {
            ...
            {
                Данные = (pagedata)ВсеДанные.Получить(ИстДанных + "/" + ИмяДанных);
                if (Данные == Неопределено)
                {
            ...
            {
                Данные = (treedata)ВсеДанные.Получить(ИстДанных + "/" + ИмяДанных);
                if (Данные == Неопределено)
                {
        ...
        bool ОбработатьДанные(Задача структЗадача) 
        {
            return false;
        }
        ...
        void ОбработатьСоединения()
        {
            ...
            while (!(ОстановитьСервер))
            {
                ...
                while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50))
                {
                    ...
                    var ЕстьРезультат = !(структЗадача.Результат is null);
                    if (!ЕстьРезультат)
                    {
                    ...
                    {
                        if (!(структЗадача.Соединение.Статус == "Запись"))
                        {
                            структЗадача.Соединение.Закрыть();
                            структЗадача.Соединение = null;
                            структЗадача.Этап = "Удалить";
                        }
                    }

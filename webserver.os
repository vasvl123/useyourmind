	...
	Если НЕ Метод = Неопределено Тогда
		...
		Если Запрос.ИмяКонтроллера = "tasks" Тогда
			Если Запрос.ИмяМетода = "stopserver" Тогда
				...
				ПередатьДанные(Параметры.Хост, Параметры.ПортС, Новый Структура("cmd", "stopserver"));
			ИначеЕсли Запрос.ИмяМетода = "restartserver" Тогда
				Задача.Вставить("Результат", ПолучитьОбласть("ОбластьПеренаправить", Новый Структура("Путь, Пауза, Текст", "/", 3000, "wait...")));
				ПередатьДанные(Параметры.Хост, Параметры.ПортС, Новый Структура("cmd", "restartserver"));
			Иначе
	...
	Для каждого элЗадача Из Задачи Цикл
		...
		Если НЕ Задача.структКонтроллер = Неопределено Тогда
			Если НЕ Задача.ПереданоВКонтроллер = Истина Тогда
				Задача.ПереданоВКонтроллер = ПередатьДанные(структКонтроллер.Хост, структКонтроллер.Порт, Задача.ПараметрыЗапроса);
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбработатьСоединения()
	...
	Пока Не ОстановитьСервер Цикл
		...
		Если НЕ Соединение = Неопределено Тогда
			...
			ИначеЕсли ТипЗнч(ДанныеВходящие) = Тип("Строка") Тогда
			...
			ИначеЕсли ТипЗнч(ДанныеВходящие) = Тип("Структура") Тогда
				...
				ИначеЕсли КонтроллерЗапрос.Свойство("taskid") Тогда

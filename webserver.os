...
Процедура ОбработатьЗадачи()
	...
	Если мЗадачи.Количество() Тогда
		...
		Пока НЕ тЗадача > мЗадачи.Количество() - 1 И НЕ ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 100 Цикл
			Задача = мЗадачи.Получить(тЗадача);
...
Процедура ОбработатьСоединения()
	...
	Пока Не ОстановитьСервер Цикл
		...
		Соединение = TCPСерверО.ПолучитьСоединение(Таймаут);
		...
		Если НЕ Соединение = Неопределено Тогда
			...
			СоединенияО.Вставить(Соединение, Новый Массив);

		КонецЕсли;

		Для каждого зСоединение Из СоединенияО Цикл
			Соединение = зСоединение.Ключ;
			...
			Если Соединение.Статус = "Успех" Тогда
				...
				Если КонтроллерЗапрос = Неопределено Тогда
					//СоединенияО.Вставить(Соединение, мДанные);
					Продолжить;
				КонецЕсли;
				...
				Соединение.Закрыть();
				СоединенияО.Удалить(Соединение);

...
Функция ОбработатьЗапросКлиента(Запрос, Знач Соединение)
	...
	Сообщить(СокрЛП(ТекущаяДата()) + " -> taskid=" + Задача.Ид + " " + СокрЛП(Запрос.Заголовок.Получить("Method")) + " " + Запрос.Заголовок.Получить("Path"));
...
Процедура ОбработатьОтветСервера(Задача)
	...
	Если Задача.Соединение.Активно Тогда
		...
		Задача.Соединение.ОтправитьСтроку(ТекстОтветаКлиенту + ПС);
	...
	Для каждого элЗадача Из Задачи Цикл
		...
		Если НЕ Задача.Результат = Неопределено Тогда
			...
			Если НЕ Задача.Соединение = Неопределено Тогда
				Если Задача.Соединение.Активно Тогда
					...
					Сообщить(СокрЛП(ТекущаяДата()) + " <- taskid=" + СокрЛП(Задача.Ид) + " time=" + (ТекущаяДата() - Задача.ВремяНачало));
				КонецЕсли;
			КонецЕсли;
		...
		Иначе
			...
			Если Задача.Процесс = Неопределено Тогда
				СсылкаНаПроцесс = Задача.ИмяКонтроллера + "/" + Задача.ИмяМетода;
				Процесс = Процессы.Получить(СсылкаНаПроцесс);
				Если НЕ Процесс = Неопределено Тогда
					Если Процесс.Процесс.Завершен Тогда
						Процессы.Удалить(СсылкаНаПроцесс);
						Продолжить;
					КонецЕсли;
				...
				КонецЕсли;
				Если Задача.Процесс = Неопределено Тогда
					Сообщить("Запустил процесс " + Задача.ИмяМетода);
					Процесс = Новый Структура("Свободен, Процесс", Ложь, СоздатьПроцесс("c:\os19\bin\oscript.exe " + ОбъединитьПути(ТекущийКаталог(), Задача.ИмяКонтроллера, Задача.ИмяМетода + ".os"), , Истина, Истина));
					Задача.Вставить("Процесс", Процесс);
					Задача.Процесс.Процесс.Запустить();
					Процессы.Вставить(СсылкаНаПроцесс, Задача.Процесс);
				КонецЕсли;
				
				СтрокаПараметров = "Перем Данные Экспорт; Данные = Новый Структура();";
				Для Каждого ПараметрЗапроса Из Задача.ПараметрыЗапроса Цикл
					СтрокаПараметров = СтрокаПараметров + " Данные.Вставить(""" + ПараметрЗапроса.Ключ + """, """ + ПараметрЗапроса.Значение + """);"
				КонецЦикла;

				Задача.Вставить("ВремяНачало", ТекущаяДата());
				Задача.Процесс.Процесс.ПотокВвода.ЗаписатьСтроку(СтрокаПараметров);
				
			КонецЕсли;

			Если НЕ Задача.Процесс.Процесс = Неопределено Тогда
				
				Если НЕ Задача.Процесс.Процесс.ПотокВывода.ЕстьДанные Тогда
					Приостановить(1);
				КонецЕсли;
				
				Сообщение = Задача.Процесс.Процесс.ПотокВывода.Прочитать();
				...
				Если НЕ Сообщение = Неопределено Тогда
					...
					Если НЕ Лев(Сообщение, 2) = "<!" Тогда
						Сообщение = ПолучитьОбласть("ОбластьШапка") + Сообщение + ПолучитьОбласть("ОбластьПодвал");
					КонецЕсли;
					
					Задача.Результат = Сообщение;

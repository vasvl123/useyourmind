...
Перем Загрузка;
Перем ВсеДанные;
Перем ОшибокОжиданияЧтения;
Перем ОшибокОжиданияЗаписи;
...
Функция ОбработатьЗапросКлиента(Запрос, Знач Соединение)
	...
	Если Запрос.ИмяКонтроллера = "tasks" Тогда
		...
		ИначеЕсли Запрос.ИмяМетода = "restartserver" Тогда
			Задача.Вставить("Результат", ПолучитьОбласть("ОбластьПеренаправить", Новый Структура("Путь, Пауза", "/", 1500)));
...
Процедура ОбработатьДанные(Задача, Записать = Ложь)
	Перем Команда, ИмяДанных, БазаДанных, ПозицияДанныхФайла, Режим;
	...
	Если Задача.ИмяКонтроллера = "showdata" Тогда
		...
		Если НЕ БазаДанных = Неопределено Тогда
			...
			Если НЕ ИмяДанных = Неопределено Тогда
				...
				ИначеЕсли НЕ Команда = "savedata" И НЕ Записать Тогда
					Если Данные.ОткрытьПотокДанных() Тогда
						Заголовок = Данные.ПолучитьФайл(ИмяДанных);
						Если НЕ Заголовок = Неопределено Тогда
							Заголовок.Свойство("dataposition", ПозицияДанныхФайла);
							Заголовок.Свойство("mode", Режим);
							ФайлДанных = ПозицияДанныхФайла;
							Запрос.Вставить("mode", Режим);
						КонецЕсли;
					КонецЕсли;
	...
	Для каждого элЗадача Из Задачи Цикл
		...
		Если Задача.структПроцесс = Неопределено Тогда
			...
			Если ПарИдПроцесса = "" Тогда
				...
				Задача.Вставить("Результат", ПолучитьОбласть("ОбластьПеренаправить", Новый Структура("Путь, Пауза", "/showdata/" + ИдПроцесса, 0)));
				Продолжить;
			Иначе
		...
		Если Задача.СоединениеПроцесс = Неопределено Тогда
			Если НЕ Задача.структПроцесс.Соединение = Неопределено Тогда
				Если Задача.структПроцесс.Соединение.Активно Тогда
					...
					Исключение
						ОшибокОжиданияЗаписи = ОшибокОжиданияЗаписи + 1;
						Продолжить;
					КонецПопытки;
				КонецЕсли;
			КонецЕсли;
...
Процедура ОбработатьСоединения() Экспорт
	...
	ПустойЦикл = 0;
	РабочийЦикл = 0;
	ОшибокОжиданияЧтения = 0;
	ОшибокОжиданияЗаписи = 0;
	...
	Пока Истина Цикл
		...
		Попытка
			...
			Если ПустойЦикл + РабочийЦикл > 999 Тогда
				...
				Если НЕ ОшибокОжиданияЧтения + ОшибокОжиданияЗаписи = 0 Тогда
					Загрузка = Загрузка + Символы.ПС + "Ошибки чт/зап " + ОшибокОжиданияЧтения + "/" + ОшибокОжиданияЗаписи;
				КонецЕсли;
				ПустойЦикл = 0;
				РабочийЦикл = 0;
				ОшибокОжиданияЧтения = 0;
			...
			Исключение
				ОшибокОжиданияЧтения = ОшибокОжиданияЧтения + 1;
				Продолжить;
			КонецПопытки;

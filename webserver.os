...
Перем Хост, Порт, ПортД, ПортП, ПараметрХост;
...
Процедура ОбработатьЗадачи()
	...
	Для каждого элЗадача Из Задачи Цикл
		...
		Если Задача.структПроцесс = Неопределено Тогда
			...
			Если Задача.ИмяКонтроллера = "" Тогда
				...
				Если Задача.ПараметрыЗапроса.Свойство("cmd", cmd) Тогда
					...
					Если cmd = "auth" ИЛИ cmd = "reg" Тогда
						...
						Задача.ПараметрыЗапроса = ВыполнитьЗапросКСерверуДанных(Задача.ПараметрыЗапроса);
			...
			Если структПроцесс = Неопределено Тогда
				ЗапуститьПриложение("oscript " + ОбъединитьПути(ТекущийКаталог(), "showdata.os " + ПортП + " " + Хост + " " + ПарИдПроцесса + " " + Задача.УдаленныйУзел + " " + Кодировать(Субъект) + " " + Кодировать(ПараметрХост)), ТекущийКаталог());
		...
		Если Задача.СоединениеПроцесс = Неопределено Тогда
			Если НЕ Задача.структПроцесс.Соединение = Неопределено Тогда
				Если Задача.структПроцесс.Соединение.Активно Тогда
					Попытка
						Задача.структПроцесс.Соединение.ОтправитьДвоичныеДанные(СтруктуруВДвоичныеДанные(Задача.ПараметрыЗапроса));
...
Процедура ЛогСообщить(Сообщение, Тип = 0)
	...
	ВыполнитьЗапросКСерверуДанных(Новый Структура("БазаДанных, Заголовок, Команда", "log", Новый Структура("Источник, Тип, Сообщение", "webserver.os", Тип, Сообщение), "ЗаписатьЗаголовок"));
КонецПроцедуры
...
Процедура ОбработатьСоединения() Экспорт
	...
	TCPСерверП = Новый TCPСервер(ПортП);
	TCPСерверП.Запустить();
	ЛогСообщить("Сервер процессов запущен на порту: " + СокрЛП(ПортП));
	...
	Пока Истина Цикл
					...
					Если ЗамерВремени - структПроцесс.ВремяНачало > 30 * 60 Тогда
						Если НЕ структПроцесс.ИдПроцесса = "0" Тогда // Гостевой процесс не завершаем
							...
							Если НЕ структПроцесс.Соединение = Неопределено Тогда
								структПроцесс.Соединение.ОтправитьДвоичныеДанные(СтруктуруВДвоичныеДанные(Новый Структура("cmd, taskid", "killproc", "")));
		...
		Иначе
			...
			// Ожидаем подключение процесса
			Соединение = TCPСерверП.ОжидатьСоединения(Таймаут);
			...
			Если НЕ Соединение = Неопределено Тогда
				...
				Исключение
					дДанныеВходящие = Неопределено;
					Соединение.Закрыть();
				КонецПопытки;

				Если НЕ дДанныеВходящие = Неопределено Тогда

					РабочийЦикл = РабочийЦикл + 1;

					ПроцессЗапрос = ДвоичныеДанныеВСтруктуру(дДанныеВходящие);

					Если ПроцессЗапрос.Свойство("procid") Тогда
						//ЛогСообщить(ТекстовыеДанныеВходящие);
						Попытка
					...
					ИначеЕсли ПроцессЗапрос.Свойство("taskid") Тогда

						Задача = Задачи.Получить(ПроцессЗапрос.taskid);
						Если НЕ Задача = Неопределено Тогда
							Задача.Результат = ПроцессЗапрос.Результат;
						КонецЕсли;
						Соединение.Закрыть();
					КонецЕсли;
	...
	Если Локальный Тогда
		Для каждого элПроцесс Из Процессы Цикл
			...
			КонецЕсли;
		КонецЦикла;
		ВыполнитьЗапросКСерверуДанных(Новый Структура("Команда", "stopserver"));
	КонецЕсли;

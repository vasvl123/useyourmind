...
Функция РазобратьЗапросКлиента(ТекстЗапроса, Соединение)
	...
	Если Метод = "POST" Тогда
		// обработать позже
		Заголовок.Вставить("POSTData", Новый Структура);
...
Процедура РазобратьДанныеЗапроса(Задача)

	POSTСтруктура = Задача.ПараметрыЗапроса;
	Содержимое = Задача.Заголовок.Получить("Content-Type");
	Если Содержимое = "text/plain;charset=UTF-8" Тогда // параметры строкой
		// Получим данные запроса
		POSTДанные = Задача.Соединение.ПолучитьСтроку();
	...
	ИначеЕсли Лев(Содержимое, 20) = "multipart/form-data;" Тогда // параметры формы
		Разделитель = Задача.Заголовок.Получить("Разделитель");
		мТекстовыеДанные = Задача.Соединение.ПолучитьСтроку("windows-1251");
		Пока Истина Цикл
...
Процедура ОбработатьОтветСервера(Задача)
	...
	Попытка
		...
		Попытка
			...
			Задача.Соединение.ОтправитьДвоичныеДанныеАсинхронно(СоединитьДвоичныеДанные(мДанные), Ложь);
			Задача.Этап = "Вернуть";
...
Процедура ОбработатьЗадачи()
	...
	Пока к > 0 И НЕ ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50 Цикл
		...
		Если Задача.Этап = "Данные" Тогда
			Если Задача.Соединение.Статус = "Данные" Тогда
				РазобратьДанныеЗапроса(Задача);
				Задача.Этап = "Новая";
			ИначеЕсли Задача.Соединение.Статус = "Ошибка" Тогда
...
Процедура ОбработатьСоединения()
	...
	Пока Не ОстановитьСервер Цикл
		...
		Пока к > 0 Цикл
			...
			Если Соединение.Статус = "Заголовки" ИЛИ Соединение.Статус = "Данные" Тогда
				...
				Если НЕ ТекстовыеДанныеВходящие = "" Тогда
					...
					Попытка
						Запрос = РазобратьЗапросКлиента(ТекстовыеДанныеВходящие, Соединение);

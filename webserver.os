...
Процедура ОбработатьДанные(Задача)
	...
	Если Задача.ИмяКонтроллера = "showdata" Тогда
		Если НЕ Задача.ИмяМетода = "" Тогда
			...
			Данные = ВсеДанные.Получить(Задача.ИмяМетода);
			Если Данные = Неопределено Тогда
				Данные = Новый dbaccess(Задача.ИмяМетода);
				ВсеДанные.Вставить(Задача.ИмяМетода, Данные);
			КонецЕсли;

			Запрос.Свойство("cmd", Команда);

			Если Команда = "writedata" Тогда
				ФайлДанных = "succsess";
			Иначе

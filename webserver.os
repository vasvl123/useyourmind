...
Процедура ОбработатьЗадачи()
	...
	Для каждого элПеречень Из ПереченьЗадач Цикл
		...
		Если НЕ Задача.Соединение = Неопределено Тогда
			врнач = ТекущаяУниверсальнаяДатаВМиллисекундах();
			Если врнач - Задача.ВремяНачало > 30 * 1000 Тогда
				Задача.ВремяНачало = врнач;
			...
			Если Соединение.Статус = "Успех" Тогда
				...
				Попытка
					...
					Если НЕ дд.Размер() = 0 Тогда
						мДанные.Добавить(дд);
						КонтроллерЗапрос = ПолучитьОбъектИзДвоичныхДанных(СоединитьДвоичныеДанные(мДанные));
					КонецЕсли;
				Исключение
					Сообщить("webserver: " + ОписаниеОшибки());
				КонецПопытки;

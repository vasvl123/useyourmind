	...
	Если Параметры.Свойство("unm", Имя) Тогда
		...
		Если НЕ "" + Имя = "" Тогда
			...
			Если НЕ Профиль = Неопределено Тогда
				...
				Сообщить("Пароль: "+Профиль.Пароль);
...
Функция Расшифровать(Шифр, КлючШифрования) Экспорт
	...
	Для Счетчик = 0 ПО 31 Цикл
		...
		Если ЗначениеИсходныхДанных < 0 Тогда
			ЗначениеИсходныхДанных = ЗначениеИсходныхДанных + 255;
		КонецЕсли;
...
Функция ВыполнитьРегистрацию(Параметры)
	...
	Перем Пароль;
	Результат = Ложь;
	СтатусСубъекта = "Введите свое имя";
	Если Параметры.Свойство("unm", Имя) Тогда
		Если НЕ Имя = "" Тогда
			...
			Если НЕ Профиль.Пароль = Неопределено Тогда
				СтатусСубъекта = "Такое имя уже существует";
			Иначе
				СтатусСубъекта = "Укажите свой почтовый ящик";
				Если Параметры.Свойство("mail", Почта) Тогда
					Если НЕ Почта = "" Тогда
						...
						Если Профиль.Ключ = Неопределено Тогда
							...
							Профиль.Ключ = Ключ;
							СтатусСубъекта = "Секретный ключ отправлен на почту";
							Сообщить(Ключ);
						Иначе
							СтатусСубъекта = "Введите пароль два раза";
							Если Параметры.Свойство("pwd2", Пароль) Тогда
								СтатусСубъекта = "Пароли не совпадают";
								...
								Сообщить(Пароль);
								Сообщить(ПолучитьBase64СтрокуИзДвоичныхДанных(Хэш.ХешСумма));
								Если ПолучитьBase64СтрокуИзДвоичныхДанных(Хэш.ХешСумма) = Раскодировать(Параметры.pwd) Тогда
									СтатусСубъекта = "Регистрация выполнена";
		...
		Если Задача.структПроцесс = Неопределено Тогда
			...
			Если Задача.ИмяКонтроллера = "" Тогда
				...
				Если Задача.ПараметрыЗапроса.Свойство("cmd", cmd) Тогда
					Если cmd = "auth" Тогда
						Если ПроверкаАвторизации(Задача.ПараметрыЗапроса) Тогда
					...
					ИначеЕсли cmd = "reg" Тогда
						Если ВыполнитьРегистрацию(Задача.ПараметрыЗапроса) Тогда
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;

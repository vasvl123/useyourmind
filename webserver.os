				...
				Если Параметры.Свойство("mail", Почта) Тогда
					Если НЕ Почта = "" Тогда
						...
						Если Профиль.Ключ = "" Тогда
							...
							ТекстСообщение = "Секретный ключ: " + Ключ;
	...
	Для каждого элЗадача Из Задачи Цикл
		...
		Если Задача.структПроцесс = Неопределено Тогда
			...
			Если Задача.ИмяКонтроллера = "showdata" Тогда
				...
				Если ТипЗнч(структПроцесс) =  Тип("Структура") Тогда
					Если НЕ структПроцесс.УдаленныйУзел = Задача.УдаленныйУзел Тогда
						...
						Задача.ПараметрыЗапроса.Вставить("unm", КодироватьСтроку(структПроцесс.Субъект, СпособКодированияСтроки.КодировкаURL));
						ПарИдПроцесса = "0";
						Задача.ИмяКонтроллера = "";
						Задача.ИмяМетода = "login";
						структПроцесс = Неопределено;
					КонецЕсли;
				ИначеЕсли структПроцесс = Неопределено Тогда
					ПарИдПроцесса = "0";
					Задача.ИмяКонтроллера = "";
					Задача.ИмяМетода = "";
				КонецЕсли;
			...
			Если структПроцесс = Неопределено Тогда
				...
				структПроцесс = Новый Структура("ИдПроцесса, Процесс, УдаленныйУзел, Субъект, Соединение", ПарИдПроцесса, Процесс, Задача.УдаленныйУзел, Субъект);

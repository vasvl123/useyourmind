	...
	Если Параметры.Свойство("unm", Имя) Тогда
		...
		Если НЕ "" + Имя = "" Тогда
			Если Пароли = Неопределено Тогда
				Пароли = Новый Соответствие;
				Пароли.Вставить("admin", "8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918");
			КонецЕсли;
			Пароль = Пароли.Получить(Имя);
			...
			Если НЕ Пароль = Неопределено Тогда
				...
				Если НРег(СтрЗаменить(Лев(Хэш.ХешСумма, 95), " ", "")) = Параметры.pwd Тогда
...
Процедура ОбработатьЗадачи()
	...
	Для каждого элЗадача Из Задачи Цикл
		...
		Если Задача.структПроцесс = Неопределено Тогда
			...
			Если структПроцесс = Неопределено Тогда
				Процесс = СоздатьПроцесс("oscript " + ОбъединитьПути(ТекущийКаталог(), "showdata.os " + ПарИдПроцесса + " " + Порт));

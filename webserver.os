	...
	Для каждого элПеречень Из ПереченьЗадач Цикл
		...
		Если Задача.Этап = "Новая" Тогда
			...
			Иначе // запустить новый процесс
				...
				Если НЕ ПередатьДанные(Параметры.Хост, Параметры.ПортС, Новый Структура("procid, procnm, data, УдаленныйУзел, cmd", ПарИдКонтроллера, Задача.ИмяКонтроллера, Задача.ИмяМетода, Задача.УдаленныйУзел, "startproc")) = Неопределено Тогда // запустить новый процесс контроллера
	...
	Пока Не ОстановитьСервер Цикл
		...
		Если НЕ Соединение = Неопределено Тогда
			...
			Попытка
				...
				ТекстовыеДанныеВходящие = ПолучитьСтрокуИзДвоичныхДанных(Соединение.ПрочитатьДвоичныеДанные(), "windows-1251");
			Исключение
			КонецПопытки;

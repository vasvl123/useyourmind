	...
	Для каждого элЗадача Из Задачи Цикл
		...
		Если Задача.структПроцесс = Неопределено Тогда
			...
			Если структПроцесс = Неопределено Тогда
				...
				Задача.Вставить("Результат", ПолучитьОбласть("ОбластьПеренаправить", Новый Структура("Путь, Пауза", Хост + ":" + Порт + "/showdata/" + ПарИдПроцесса, 0)));
			КонецЕсли;

...
Функция ОбработатьЗапросКлиента(Запрос, Знач Соединение)
	...
	Если НЕ Метод = Неопределено Тогда
		...
		ИначеЕсли Запрос.ИмяКонтроллера = "public" Тогда
			Задача.ИмяКонтроллера = "webdata";
			Задача.ПараметрыЗапроса.Вставить("sdb", "public");
			Задача.ПараметрыЗапроса.Вставить("data", Задача.ИмяМетода); // сразу открыть запись

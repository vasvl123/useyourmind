	...
	Если Не Путь = Неопределено Тогда
		...
		Иначе
			Если Найти(ИмяМетода, "?") Тогда
				...
				GETДанные = РаскодироватьСтроку(GETДанные, СпособКодированияСтроки.КодировкаURL);
...
Процедура ОбработатьЗадачи()
	...
	Для каждого элЗадача Из Задачи Цикл
			...
			Если НЕ Задача.структПроцесс.Соединение = Неопределено Тогда
				Если Задача.структПроцесс.Соединение.Активно Тогда
					Попытка
						...
						Задача.СоединениеПроцесс = Задача.структПроцесс.Соединение;
		...
		Если НЕ Задача.СоединениеПроцесс = Неопределено Тогда
			Если Задача.СоединениеПроцесс.Активно Тогда
				Попытка
					СтрокаДанные = Задача.СоединениеПроцесс.ПрочитатьСтроку();
					Задача.СоединениеПроцесс.Закрыть();

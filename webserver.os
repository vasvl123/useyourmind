...
Процедура ОбработатьОтветСервера(Задача)
	...
	Если Задача.Соединение = Неопределено Тогда
		...
		Если Лев(ТекстовыеДанныеВходящие, 6) = "procid" Тогда
			...
			Попытка
				...
				Если структПроцесс = Неопределено Тогда
					...
					структПроцесс = Новый Структура("ИдПроцесса, Процесс, УдаленныйУзел, Субъект, ВремяНачало, Соединение", ПроцессЗапрос.procid, Неопределено, "", ПроцессЗапрос.unm, ТекущаяДата());

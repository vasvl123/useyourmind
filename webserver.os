...
Процедура ОбработатьЗадачи()
	...
	Для каждого элЗадача Из Задачи Цикл
		...
		Если НЕ Задача.ЗадачаЗапрос = Неопределено Тогда
			Если НЕ Задача.структПроцесс.структСоединение = Неопределено Тогда
				Если Задача.структПроцесс.структСоединение.ЗадачаЗапрос = Неопределено Тогда
					Задача.структПроцесс.структСоединение.Вставить("ЗадачаЗапрос", Задача.ЗадачаЗапрос);
					Задача.ЗадачаЗапрос = Неопределено;
					Прервать;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
			...
			Если Задача.структПроцесс = Неопределено Тогда
				...
				Если Задача.ПараметрыЗапроса.Свойство("procid", ПарИдПроцесса) Тогда
					...
					Иначе
						...
						Задача.Результат = "Процесс " + ПарИдПроцесса + " не найден!";
						Продолжить;
					КонецЕсли;
				КонецЕсли;
				...
				Если структПроцесс = Неопределено Тогда
					...
					Процесс = СоздатьПроцесс("oscript " + ОбъединитьПути(ТекущийКаталог(), Задача.ИмяКонтроллера, Задача.ИмяМетода + ".os " + ПарИдПроцесса));
					структПроцесс = Новый Структура("ИдПроцесса, Процесс, структСоединение", ПарИдПроцесса, Процесс);
				...
				Задача.структПроцесс.Вставить("ВремяНачало", ТекущаяДата());
				Задача.Вставить("ВремяНачало", ТекущаяДата());
				Задача.Вставить("ЗадачаЗапрос", Задача.ПараметрыЗапроса);
	...
	Для каждого элПроцесс Из Процессы Цикл
		...
		//Если НЕ структПроцесс.Процесс = Неопределено Тогда

			Если НЕ структПроцесс.структСоединение = Неопределено Тогда
				структСоединение = структПроцесс.структСоединение;
				Если НЕ структСоединение.Соединение = Неопределено Тогда

					Соединение = структСоединение.Соединение;
					Если Соединение.Активно Тогда
						Если НЕ структСоединение.ЗадачаЗапрос = Неопределено ИЛИ
							структСоединение.ПроцессЗапрос.Свойство("taskid") Тогда

							Попытка
								Соединение.ОтправитьСтроку(СтруктуруВСтроку(структСоединение.ЗадачаЗапрос));
								//Сообщить(СтруктуруВСтроку(структСоединение.ЗадачаЗапрос));
								Если структСоединение.ПроцессЗапрос.Свойство("taskid") Тогда
									СтрокаДанные = Соединение.ПрочитатьСтроку();
									Задача = Задачи.Получить(структСоединение.ПроцессЗапрос.taskid);
									Если НЕ Задача = Неопределено Тогда
										Задача.Результат = СтрокаДанные;
										//Сообщить("Получил результат задачи " + Задача.ИдЗадачи);
									КонецЕсли;
								КонецЕсли;
							Исключение
								//Сообщить("НЕ получил результат задачи: " + структСоединение.ПроцессЗапрос.taskid);
							КонецПопытки;
						Иначе
							Продолжить;
						КонецЕсли;
					КонецЕсли;
					структПроцесс.структСоединение = Неопределено;
					Соединение.Закрыть();
	...
	Пока Истина Цикл
		...
		Попытка
			...
			Если Лев(ТекстовыеДанныеВходящие, 6) = "procid" Тогда
				...
				структПроцесс.Вставить("структСоединение", Новый Структура("Соединение, ПроцессЗапрос, ЗадачаЗапрос", Соединение, ПроцессЗапрос));
				Соединение = Неопределено;
				Продолжить;
			КонецЕсли;
			...
			Исключение
				Сообщить("Ошибка разбора запроса:");
				Сообщить(ТекстовыеДанныеВходящие);
				Попытка
					Соединение.ОтправитьСтроку("чё хотел то?");
				Исключение
				КонецПопытки;
				Соединение.Закрыть();
				Соединение = Неопределено;

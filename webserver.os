...
Функция ОбработатьЗапросКлиента(Запрос, Знач Соединение)
	...
	ИначеЕсли Запрос.ИмяКонтроллера = "" Тогда
		...
		Если НЕ Запрос.ИмяМетода = "" Тогда
			Файл = Новый Файл(ОбъединитьПути(ТекущийКаталог(), "data" , ".files", Запрос.ИмяМетода));
...
Процедура ОбработатьСоединения() Экспорт
	...
	TCPСервер.Запустить();
	ЛогСообщить(СокрЛП(ТекущаяДата()) + " - Сервер запущен на порту: " + СокрЛП(Порт));
		...
		Если ПустойЦикл + РабочийЦикл > 999 Тогда
			...
			Если НЕ Порт = 8888 Тогда
				Для каждого элПроцесс Из Процессы Цикл
					...
					Если ЗамерВремени - структПроцесс.ВремяНачало > 20 Тогда
						Если НЕ структПроцесс.Процесс.Завершен Тогда
							ЛогСообщить("Процесс " + структПроцесс.ИдПроцесса + " простаивает, завершил.");

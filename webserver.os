...
Функция ОбработатьДанные(Запрос)
	...
	Если НЕ "" + БазаДанных = "" Тогда // имя контейнера указано
		...
		Если НЕ "" + ФайлДанных = "" Тогда // добавить файл в контейнер
		...
		ИначеЕсли НЕ "" + ПозицияДанных = "" Тогда // прочитать файл по позиции в контейнере
			Если Данные.ПолучитьФайл(Число(ПозицияДанных)) Тогда
				Ответ = ПозицияДанных;
			КонецЕсли;
		...
		КонецЕсли;
	КонецЕсли;
	Возврат ПолучитьДвоичныеДанныеИзСтроки(Ответ);
	...
	Если Параметры.Свойство("unm", Имя) Тогда
		Если НЕ Имя = "" Тогда
			...
			Если Профиль = Неопределено Тогда
				...
				Профиль = Новый Структура("Ключ, Имя, Пароль, Почта, Дата, УдаленныйУзел", "", Имя, "", "", ТекущаяДата(), Задача.УдаленныйУзел);
			КонецЕсли;
			Если НЕ Профиль.Пароль = "" Тогда
	...
	Для каждого элЗадача Из Задачи Цикл
		...
		Если Задача.структПроцесс = Неопределено Тогда
				...
				Если НЕ структПроцесс = Неопределено Тогда
					Если структПроцесс.Свойство("ИдПроцесса") Тогда // Прошел авторизацию
						Если НЕ структПроцесс.УдаленныйУзел = Задача.УдаленныйУзел Тогда
							Сообщить("Необходимо переавторизоваться");
				...
				Если Задача.ПараметрыЗапроса.Свойство("cmd", cmd) Тогда
					...
					Если ПрошелАвторизацию Тогда
						Если Задача.ПараметрыЗапроса.Свойство("procid", ПарИдПроцесса) Тогда
							...
							Иначе
								Процессы.Вставить(ПарИдПроцесса, Новый Структура("Субъект, ВремяНачало", Задача.ПараметрыЗапроса.Субъект, ТекущаяДата()));
							КонецЕсли;
						КонецЕсли;
						ПарИдПроцесса = "0";
					КонецЕсли;
			...
			Если структПроцесс = Неопределено Тогда
				ЗапуститьПриложение("oscript " + ОбъединитьПути(ТекущийКаталог(), "showdata.os " + Порт + " " + ПарИдПроцесса + " " + КодироватьСтроку(Субъект, СпособКодированияСтроки.КодировкаURL)), ТекущийКаталог());
				структПроцесс = Новый Структура("ИдПроцесса, Процесс, УдаленныйУзел, Субъект, ВремяНачало, Соединение", ПарИдПроцесса, Неопределено, Задача.УдаленныйУзел, Субъект, ТекущаяДата());
...
Процедура ОбработатьСоединения() Экспорт
	...
	Пока Истина Цикл
		...
		Если Лев(ТекстовыеДанныеВходящие, 6) = "procid" Тогда
			...
			Попытка
				...
				Если структПроцесс = Неопределено Тогда
					...
					структПроцесс = Новый Структура("ИдПроцесса, Процесс, УдаленныйУзел, Субъект, ВремяНачало, Соединение", ПроцессЗапрос.procid, Неопределено, УдаленныйУзелАдрес(Соединение.УдаленныйУзел), ПроцессЗапрос.unm, ТекущаяДата());
		...
		ИначеЕсли Лев(ТекстовыеДанныеВходящие, 4) = "osdb" Тогда
			Попытка
				Соединение.ОтправитьДвоичныеДанные(ОбработатьДанные(СтрокуВСтруктуру(ТекстовыеДанныеВходящие)));
			Исключение
				ЛогСообщить(ОписаниеОшибки());
			КонецПопытки;
			Соединение.Закрыть();

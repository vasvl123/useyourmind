...
Перем ИмяДанных;
	...
	Пока ПозицияДанных > 8 Цикл
		...
		СтрЗаголовок = Заголовок.ПолучитьТекст();
		Заголовок = СтрокуВСтруктуру(СтрЗаголовок);

		Индекс.ДобавитьСтроку(СтрЗаголовок);
		ПозицияДанных = Число(Заголовок.dataposition);

	КонецЦикла;

	ИмяФайла = ОбъединитьПути(КаталогФайловДанных, "index");
	Индекс.Записать(ИмяФайла);
...
Функция ПолучитьФайл(ИмяДанныхНайти) Экспорт
	...
	Пока ПозицияДанных > 8 Цикл
		...
		ПозицияДанных = Число(Заголовок.dataposition);
		Если Заголовок.data = ИмяДанныхНайти Тогда
			ПрочитатьДанные(ПозицияДанных);
			Возврат Заголовок;
		КонецЕсли;
...
Функция ДобавитьДанные(НовыеДанные) Экспорт
	...
	ИмяФайла = ОбъединитьПути(КаталогФайловДанных, "index");
	ФайлИндекс = Новый ТекстовыйДокумент;
	Попытка
		ФайлИндекс.Прочитать(ИмяФайла);
	Исключение
		ПолучитьСписокФайлов();
		ФайлИндекс.Прочитать(ИмяФайла);
	КонецПопытки;

	ФайлИндекс.ВставитьСтроку(1, Заголовок);
	ФайлИндекс.Записать(ИмяФайла);

	// ПозицияЗаголовка = ПотокДанных.ТекущаяПозиция();
	// ЗД = Новый ЗаписьДанных(ПотокДанных);
	// ЗД.ЗаписатьЦелое32(СтрДлина(Заголовок));

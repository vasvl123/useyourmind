...
Функция ПолучитьСвязь(Код, Старший = Неопределено) Экспорт
...
Функция НоваяСвязь(стрСвязь, Запомнить = Истина) Экспорт
	...
	Иначе
		...
		Если Связь.Свойство("Значение") Тогда
			...
			ОбъемДанных = 12 + дЗначение.Размер();
		КонецЕсли;
		...
		Буфер.ЗаписатьЦелое32(0, Связь.Что);
		ПотокДанных.Записать(Буфер, 0, 4);

		Буфер = Новый БуферДвоичныхДанных(4);
		Буфер.ЗаписатьЦелое32(0, Связь.Как);
		ПотокДанных.Записать(Буфер, 0, 4);

		Если ОбъемДанных = 22 Тогда
			Буфер = Новый БуферДвоичныхДанных(4);
			Буфер.ЗаписатьЦелое32(0, Связь.Чем);
			ПотокДанных.Записать(Буфер, 0, 4);
		Иначе
	...
	Если Запомнить = Истина Тогда
		Связи.Вставить(Связь.Код, Связь);
	КонецЕсли;

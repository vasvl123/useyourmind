...
Функция ПолучитьУзел(Код, Старший = Неопределено) Экспорт
	...
	ОткрытьПотокДанных(Ложь, Код);

	Буфер = Новый БуферДвоичныхДанных(2);
	ПотокДанных.Прочитать(Буфер, 0, 2);
	ОбъемДанных = Буфер.ПрочитатьЦелое16(0);

	Буфер = Новый БуферДвоичныхДанных(ОбъемДанных);
	ПотокДанных.Прочитать(Буфер, 0, ОбъемДанных);

	Узел.Вставить("Дата", Буфер.ПрочитатьЦелое64(0));
	Узел.Вставить("Связь", Буфер.ПрочитатьЦелое32(8));
	ОбъемДанных = Буфер.ПрочитатьЦелое16(0);
	Буфер.ПрочитатьЦелое16(0)
...
Функция НоваяСвязь(стрСвязь) Экспорт

	Связь = Новый Структура(стрСвязь);

	Если Связь.Свойство("Врем") Тогда

		сКоличество = сКоличество + 1;
		Связь.Вставить("Код", "s" + сКоличество);

	Иначе

		ОткрытьПотокДанных(Истина);

		Связь.Начало = ТекущаяУниверсальнаяДатаВМиллисекундах();

		Буфер = Новый БуферДвоичныхДанных(8);
		Буфер.ЗаписатьЦелое64(0, Связь.Начало);
		ПотокДанных.Записать(Буфер, 0, 8);

		ОбъемДанных = 22;

		Если Связь.Свойство("Значение") Тогда
			дЗначение = ПолучитьДвоичныеДанныеИзСтроки(Связь.Значение);
			ОбъемДанных = 18 + дЗначение.Размер();
		КонецЕсли;

		Буфер = Новый БуферДвоичныхДанных(2);
		Буфер.ЗаписатьЦелое16(0, ОбъемДанных);
		ПотокДанных.Записать(Буфер, 0, 2);

		Связь.Вставить("Код", ПотокДанных.ТекущаяПозиция());

		Буфер = Новый БуферДвоичныхДанных(4);
		Буфер.ЗаписатьЦелое32(0, Связь.Что.Код);
		ПотокДанных.Записать(Буфер, 0, 4);

		Буфер = Новый БуферДвоичныхДанных(4);
		Буфер.ЗаписатьЦелое32(0, Связь.Агент.Код);
		ПотокДанных.Записать(Буфер, 0, 4);

		Связь.Конец = 0;

		Буфер = Новый БуферДвоичныхДанных(8);
		Буфер.ЗаписатьЦелое64(0, Связь.Конец);
		ПотокДанных.Записать(Буфер, 0, 8);

		Буфер = Новый БуферДвоичныхДанных(4);
		Буфер.ЗаписатьЦелое32(0, Связь.Как.Код);
		ПотокДанных.Записать(Буфер, 0, 4);

		Если ОбъемДанных = 22 Тогда
			Буфер = Новый БуферДвоичныхДанных(4);
			Буфер.ЗаписатьЦелое32(0, Связь.Чем.Код);
			ПотокДанных.Записать(Буфер, 0, 4);
		Иначе
			Буфер = ПолучитьБуферДвоичныхДанныхИзДвоичныхДанных(дЗначение);
	...
	Связи.Вставить(Связь.Код, Связи);
...
Функция ПриСозданииОбъекта(обПроцесс, знБазаДанных) Экспорт
	БазаДанных = знБазаДанных;
	ИмяФайлаДанных = ОбъединитьПути(ТекущийКаталог(), "data", знБазаДанных  + ".osdb");
	Процесс = обПроцесс;

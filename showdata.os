...
Функция НоваяВкладка(Параметры)
	...
	Данные = ВсеДанные.Получить(БазаДанных + "/" + ИмяДанных + "/" + ПозицияДанных);
	Если Данные = Неопределено Тогда
		Если НЕ "" + БазаДанных = "" Тогда
			...
			КонецПопытки;
		КонецЕсли;
		Данные = Новый pagedata(ЭтотОбъект, ОбъединитьПути(ТекущийКаталог(), "data", БазаДанных + ".files", ИмяФайла));
		ВсеДанные.Вставить(БазаДанных + "/" + ИмяДанных + "/" + ПозицияДанных, Данные);
	КонецЕсли;
	Данные.Обновить = Истина;
	Возврат НоваяСтруктураВкладка(Данные, "data", ИмяДанных, "view", "1", БазаДанных, ИмяДанных, ПозицияДанных);
...
Функция ПоказатьМенюИнструменты(Вкладка, Узел, ЭтоАтрибут = Ложь)
	...
	Иначе
		...
		// ПараметрыШаблона.Вставить("ПараметрКоманда", Команды["ЗначениеОбъекта"]);
...
Функция ОтобразитьDOM(Вкладка, КодУзла, Родитель = Неопределено, Знач Обновить = Ложь, Знач ТипУзла = "", Знач НачальныйУзел = Ложь)

	Данные = Вкладка.Данные;

	Узел = Данные.ПолучитьУзел(КодУзла, Родитель);
	...
	Если структЗадача.Этап = "СформироватьОтвет" Тогда
		...
		Для каждого ЭлДанные Из ВсеДанные Цикл
			...
			Попытка
				...
				Шагов = текДанные.Шаг - Шагов;
				структЗадача.Вставить("Шагов", структЗадача.Шагов + Шагов);
			Исключение
		...
		Если НЕ структЗадача.Результат = "" Тогда
			структЗадача.Результат = структЗадача.Результат + Шаблон.ПолучитьОбласть("ОбластьСтатус", Новый Структура("ПараметрСтатусСообщение", "" + Цел(100*(ТекущаяДата() - структЗадача.ВремяНачало))/100 + " с. " + структЗадача.Шагов + " ш."));
			структЗадача.Этап = "ЕстьРезультат";
		Иначе
...
Функция УзелСостояниеЗначение(Вкладка, Узел, СостояниеИмя, СостояниеЗначение, Событие = Ложь) Экспорт
	...
	Если Событие Тогда
		Если СостояниеИмя = "ОбновитьУзел" И СостояниеЗначение = Истина Тогда
			Вкладка.Данные.ОбновитьОбъект(Узел);
		КонецЕсли;
	КонецЕсли;
	Возврат Неопределено;
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие.Имя = Команды["РедактироватьУзел"] Тогда
			...
			ТекущееОкно = НоваяСтруктураВкладка(Данные, "win", Вкладка.ИмяДанных + " ." + Узел.Имя, "struct", Узел.Код, Вкладка.БазаДанных, Вкладка.ИмяДанных, Вкладка.ПозицияДанных);
		...
		ИначеЕсли Действие.Имя = Команды["ВставитьАтрибут"] Тогда
			Если НЕ Буфер = Неопределено Тогда
				НовыйУзел = Данные.КопироватьВетку(Буфер.Получить(БуферУзел), Узел, Узел.Код, , Истина);
		...
		ИначеЕсли Действие.Имя = Команды["ВставитьДочерний"] Тогда
			Если НЕ Буфер = Неопределено Тогда
				НовыйУзел = Данные.КопироватьВетку(Буфер.Получить(БуферУзел), Узел, Узел.Код);
		...
		ИначеЕсли Действие.Имя = Команды["ВставитьСоседний"] Тогда
			Если НЕ Буфер = Неопределено Тогда
				НовыйУзел = Данные.КопироватьВетку(Буфер.Получить(БуферУзел), Узел, Узел.Родитель);
		...
		ИначеЕсли Действие.Имя = Команды["ЗначениеУзла"] Тогда
			...
			Иначе
				...
				УзелКод = Узел.Код;
			КонецЕсли;
			ТекущееОкно = НоваяСтруктураВкладка(ДанныеВкладки, "win", Вкладка.ИмяДанных + " Значение " + " ." + Узел.Имя, ТипУзла, УзелКод, "", "", "");
			Возврат Истина;

		// ИначеЕсли Действие.Имя = Команды["ЗначениеОбъекта"] Тогда
		//
...
Функция ЗаписатьСобытие(стрЗаголовок, стрСообщение, ТипСобытия = 0, ПараметрКоманда = "") Экспорт
	Если НЕ Журнал = Неопределено Тогда
		НоваяСтрока = Журнал.Объект.НовыйДочерний(Журнал.СписокСтрок, Новый Структура("Имя, Значение", ТекущаяДата(), ?(ТипСобытия = 0, "Общее", ?(ТипСобытия = 1, "Успех", ?(ТипСобытия = 2, "Внимание", "Ошибка"))) + " [" + стрЗаголовок + "] " + стрСообщение), Истина);
		Журнал.Объект.ОбновитьОбъект(Журнал.СписокСтрок);
	КонецЕсли;
...
Если АргументыКоманднойСтроки.Количество() Тогда
	...
	Попытка
		Журнал = Новый Структура("Объект, ДанныеЖурнала, СписокСтрок");
		Журнал.Объект = Новый pagedata(ЭтотОбъект, ОбъединитьПути(ТекущийКаталог(), "data", ".files", "log"));
		ВсеДанные.Вставить("/log/", Журнал.Объект);
		// Узел куда вставлять события
		Журнал.СписокСтрок = Журнал.Объект.НайтиОбъект(Журнал.Объект.Корень, "Данные");
		Журнал.СписокСтрок = Журнал.Объект.Дочерний(Журнал.СписокСтрок); // Ссылка
		Журнал.СписокСтрок = Журнал.Объект.Дочерний(Журнал.СписокСтрок); // Список
	...
	// Команды.Вставить("ЗначениеОбъекта", "showenv");

...
Функция ВыполнитьЗапросКСерверуДанных(стрЗапрос, Данные = Неопределено) Экспорт
	...
	Попытка
		...
		Запрос = СтрокуВСтруктуру(стрЗапрос);
...
Функция ПолучитьДанные(БазаДанных, ИмяДанных, ПозицияДанных, ВсеДанныеВставить = Истина)
	...
	Если Данные = Неопределено Тогда
		Если НЕ ПозицияДанных = "" ИЛИ БазаДанных = "" Тогда // запросить данные с сервера
			Текст = ВыполнитьЗапросКСерверуДанных(СтруктуруВСтроку(Новый Структура("БазаДанных, ИмяДанных, ПозицияДанных", БазаДанных, ИмяДанных, ПозицияДанных)));
		Иначе
	...
	Если НЕ Действие = Неопределено Тогда
		...
		Если Действие = "ЗапросДанных" Тогда
			...
			Пока Истина Цикл
				...
				Результат = СтрокуВСтруктуру(ВыполнитьЗапросКСерверуДанных(СтруктуруВСтроку(ЗапросДанных)));
		...
		ИначеЕсли Действие = Команда["СохранитьДанные"] Тогда
			...
			Если НЕ Вкладка = Неопределено Тогда
				Результат = ВыполнитьЗапросКСерверуДанных(СтруктуруВСтроку(Новый Структура("БазаДанных, Заголовок, Команда", Вкладка.Данные.БазаДанных, Новый Структура("ИмяДанных, ТипДанных", Вкладка.Данные.ИмяДанных, 3), "ЗаписатьДанные")), Вкладка.Данные.СохранитьДанные());
				Если НЕ Результат = Неопределено Тогда
					Вкладка.Данные.ПозицияДанных = Результат;
					ЗаписатьСобытие(Вкладка.Данные.БазаДанных + "/" + Вкладка.Данные.ИмяДанных + "/" + Результат, "Данные сохранены", 1);
				КонецЕсли;
			КонецЕсли;
			// системное событие
			Если НЕ Настройки = Неопределено Тогда
		...
		ИначеЕсли Действие = Команда["ЗагрузитьHTML"] Тогда
			...
			Иначе
				...
				НовыйУзел = Новый Структура("Имя, Значение", "", "");
				Данные.ЗагрузитьHTML(Данные.НовыйДочерний(Узел, НовыйУзел), СтруктураHTML, 0);
...
Функция ЗаписатьСобытие(стрЗаголовок, стрСообщение, ТипСобытия = 0, ПараметрКоманда = "") Экспорт
	...
	ВыполнитьЗапросКСерверуДанных(СтруктуруВСтроку(Новый Структура("БазаДанных, Заголовок, Команда", "log", Новый Структура("Источник, Субъект, Тип, Сообщение", стрЗаголовок, Субъект, ТипСобытия, стрСообщение), "ЗаписатьЗаголовок")));
...
Если АргументыКоманднойСтроки.Количество() Тогда
	...
	СтруктЗапрос = СтруктуруВСтроку(Новый Структура("procid, unm, ip", procid, Субъект, УдаленныйУзел));
	...
	Пока Истина Цикл
		...
		Если Соединение = Неопределено Тогда
			ПередатьСтроку(Соединение, СтруктЗапрос);
		Иначе

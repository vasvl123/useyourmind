	...
	Если СписокБаз.Количество() Тогда
		Для каждого БазаДанных Из СписокБаз Цикл
			...
			Если НЕ Индекс.Существует() Тогда
				...
				КонецПопытки;
			КонецЕсли;
			Индекс = Новый pagedata(ОбъединитьПути(ТекущийКаталог(), "data", БазаДанных.ИмяБезРасширения + ".files", ИндексИмя));
			Для н = 1 По Индекс.Данные.КоличествоСтрок() Цикл
				нУзел = Индекс.ПолучитьУзел("" + н);
				Узел = Данные.НовыйСоседний(Узел, Новый Структура("Имя, Значение", "p", БазаДанных.ИмяБезРасширения + " / " + нУзел.data));
			КонецЦикла;
			ОсвободитьОбъект(Индекс);
		КонецЦикла;
	КонецЕсли;
КонецФункции

...
Функция ПолучитьДанные(БазаДанных, ИмяДанных, ПозицияДанных)
	...
	Если Данные = Неопределено Тогда
		Если НЕ "" + БазаДанных = "" И НЕ "" + ПозицияДанных = "" Тогда
			...
			Попытка
				ПозицияДанных = ПолучитьСтрокуИзДвоичныхДанных(Соединение.ПрочитатьДвоичныеДанные());
			Исключение
...
Функция ЗапросВыполнитьЗадачу(структЗадача)
	...
	Если структЗадача.Этап = "ВыполнитьЗадачу" Тогда
		...
		Иначе
			Если ОбработатьКоманду(структЗадача) Тогда
				Если ВыполнитьДействия(структЗадача) = Истина Тогда
					СформироватьОтвет();
				Иначе
					структЗадача.Этап = "ЕстьРезультат"; // не надо отправлять
				КонецЕсли;
			Иначе
		...
		Если НЕ структЗадача.Этап = "ЕстьРезультат" Тогда
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
			...
			Если НЕ Вкладка = Неопределено Тогда
				...
				Иначе
					...
					Если ПередатьСтроку(Соединение, "osdb" + Символы.Таб + Вкладка.БазаДанных + Символы.Таб + "data" + Символы.Таб + Вкладка.ИмяДанных + Символы.Таб + "datafile" + Символы.Таб + datafile) Тогда
						Попытка
							ФайлИмя = ПолучитьСтрокуИзДвоичныхДанных(Соединение.ПрочитатьДвоичныеДанные());
		...
		ИначеЕсли Действие.Имя = Команда["РедактироватьУзел"] Тогда
			...
			ТекущееОкно = НоваяСтруктураВкладка(Данные, "win", Вкладка.ИмяДанных + "  " + Узел.Имя + ":" + СтрЭкранироватьРазметку(Данные.УзелСвойство(Узел, "Значение"), 20), "struct", Узел.Код, Вкладка.БазаДанных, Вкладка.ИмяДанных, Вкладка.ПозицияДанных);
		...
		ИначеЕсли Действие.Имя = Команда["ЗагрузитьHTML"] Тогда
			...
			Если СтруктураHTML = Неопределено Тогда
				ТекущееОкно = НоваяСтруктураВкладка(Данные, "win", Вкладка.ИмяДанных + " " + Узел.Имя + ":" + СтрЭкранироватьРазметку(Данные.УзелСвойство(Узел, "Значение"), 20) + " import", "struct", Узел.Код, Вкладка.БазаДанных, Вкладка.ИмяДанных, Вкладка.ПозицияДанных);
				ОбновитьСостояние(ТекущееОкно, Узел, "УзелОткрыт", Истина);
		...
		ИначеЕсли Действие.Имя = Команда["СтруктураДанных"] Тогда
			...
			ТекущееОкно = НоваяСтруктураВкладка(Данные, "win", Вкладка.ИмяДанных + " " + Узел.Имя + ":" + СтрЭкранироватьРазметку(Данные.УзелСвойство(Узел, "Значение"), 20), "struct", Вкладка.ИдУзла, Вкладка.БазаДанных, Вкладка.ИмяДанных, Вкладка.ПозицияДанных);
			ОбновитьСостояние(ТекущееОкно, Узел, "УзелОткрыт", Истина);
			Возврат Истина;

...
Функция НоваяВкладка(Параметры) Экспорт
	...
	Если Данные = Неопределено Тогда
		Если НЕ "" + БазаДанных = "" Тогда
			...
			КонецПопытки;
		КонецЕсли;
		Данные = Новый pagedata(ЭтотОбъект, БазаДанных, ИмяФайла);
		ВсеДанные.Вставить(БазаДанных + "/" + ИмяДанных + "/" + ПозицияДанных, Данные);
	КонецЕсли;
	Данные.Обновить = Истина;
	ТекущиеДанные = НоваяСтруктураВкладка(Данные, "data", ИмяДанных, "view", "1", БазаДанных, ИмяДанных, ПозицияДанных);
...
Функция НастройкаЗначение(Знач Узел, Объект) Экспорт
	Если НЕ Настройки = Неопределено Тогда
		НастройкаУзел = Настройки.НайтиУзел(Настройки.Корень, Узел.Значение);
		Если НЕ НастройкаУзел = Неопределено Тогда
			Связи = Настройки.УзелСостояние(НастройкаУзел, "Связи");
			Если Связи = Неопределено Тогда
				Связи = Новый Соответствие;
				Настройки.УзелСостояниеЗначение(НастройкаУзел, "Связи", Связи);
			КонецЕсли;
			Связи.Вставить(Узел, Объект);
		КонецЕсли;
		Возврат Настройки.Интерпретировать(Настройки.Дочерний(НастройкаУзел));
	КонецЕсли;
	Возврат Неопределено;
КонецФункции
		...
		ИначеЕсли Действие.Имя = Команда["СохранитьДанные"] Тогда
			...
			Если НЕ Настройки = Неопределено Тогда
				...
				Если НЕ СистемныйУзел = Неопределено Тогда
					...
					Атр = Настройки.НовыйАтрибут(СобытиеУзел, Новый Структура("Имя, Значение", "ИмяФайла", Вкладка.Данные.ИмяФайла), Истина);
					...
					Связи = Настройки.УзелСостояние(СистемныйУзел, "Связи");
					Если НЕ Связи = Неопределено Тогда
						Для каждого элСвязь Из Связи Цикл
							элСвязь.Значение.УзлыОбновить.Вставить(СистемныйУзел, Связи);
						КонецЦикла;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;

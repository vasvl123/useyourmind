...
Функция ПоказатьМенюИнструменты(Вкладка, Узел, ЭтоАтрибут = Ложь)
	...
	Иначе
		...
		ПараметрыШаблона.Вставить("ПараметрПодсказка", "Объекты окружения");
		ПараметрМенюИнструменты = ПараметрМенюИнструменты + Шаблон.ПолучитьОбласть("ОбластьМенюИнструменты", ПараметрыШаблона);
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие.Имя = Команды["Окружение"] Тогда
			...
			Элементы = Данные.НовыйУзел(Новый Структура("Имя", "Объекты окружения"), Истина);
			ТипУзла = "struct";
			УзелКод = Элементы.Код;
			ТекущееОкно = НоваяСтруктураВкладка(Данные, "win", Вкладка.ИмяДанных + " Окружение " + Узел.Имя + " : " + Данные.УзелСвойство(Узел, "Значение"), ТипУзла, УзелКод, "", "", "");
			УзелСостояниеЗначение(ТекущееОкно, Элементы, "УзелОткрыт", Истина);

			Окружение = Данные.УзелСостояние(Узел, "Окружение");
			Если НЕ Окружение = Неопределено Тогда
				Для каждого элСоответсвие Из Окружение Цикл
					Элемент = Данные.НовыйДочерний(Элементы, Новый Структура("Имя, Значение", элСоответсвие.Ключ, элСоответсвие.Значение), Истина);
					Если ТипЗнч(элСоответсвие.Значение) = Тип("Структура") Тогда
						Элемент.Вставить("Дочерний", элСоответсвие.Значение.Код);
						// при отображении окружения не нужно отображать соседние узлы
						УзелСостояниеЗначение(ТекущееОкно, элСоответсвие.Значение, "НачальныйУзел", Истина);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Если АргументыКоманднойСтроки.Количество() Тогда
	...
	Попытка
		...
		Журнал.СписокСтрок = Журнал.Объект.ЭлементОкружения(Журнал.Объект.Корень, "Данные");

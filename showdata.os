...
Перем Задачи;
Перем ЗадачаОбновить; // если событие на сервере
Перем ЗадачаОбновитьВыполнить;
...
Функция ПолучитьДанные(БазаДанных, ИмяДанных, ПозицияДанных)
	...
	ИначеЕсли Данные = Неопределено Тогда
		Если НЕ ПозицияДанных = "" ИЛИ БазаДанных = "" Тогда // запросить данные с сервера
			Запрос = Новый Структура("БазаДанных, ИмяДанных, ПозицияДанных", БазаДанных, ИмяДанных, ПозицияДанных);
			Запрос.Вставить("ОбратныйЗапрос", Новый Структура("БазаДанных, ИмяДанных, ПозицияДанных, cmd", БазаДанных, ИмяДанных, ПозицияДанных, "ДобавитьДанные"));
			ПередатьДанные(Параметры.Хост, Параметры.ПортД, Запрос);
...
Функция ЗапросВыполнитьЗадачу(структЗадача)
	...
	Сообщить("Задача=" + структЗадача.ИдЗадачи + " " + УзелСвойство(структЗадача.Запрос, "cmd") + " время=" + (ТекущаяДата() - структЗадача.ВремяНачало));
	...
	Если структЗадача.Этап = "ВыполнитьЗадачу" Тогда
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
		Если структЗадача.Тип = "Служебный" Тогда
			Если СформироватьОтвет = Истина Тогда
				ЗадачаОбновитьВыполнить = Истина;
				структЗадача.Этап = "УдалитьЗадачу";
			КонецЕсли;
		ИначеЕсли СформироватьОтвет = Истина ИЛИ ОбновитьВкладки = Истина Тогда
	...
	Если структЗадача.Этап = "СформироватьОтвет" Тогда
		Если НЕ Субъект = "Гость" Тогда
			СформироватьОтвет();
			Если НЕ структЗадача.Действие = Команда["ОбновитьДанные"] ИЛИ НЕ ОбщийРезультат = "" Тогда
				структЗадача.Этап = "ЕстьРезультат";
			КонецЕсли;
		Иначе // многостраничный гостевой режим
			...
			Если НЕ Данные = Неопределено Тогда
				Если структЗадача.Запрос.Свойство("РезультатДанные") Тогда
					Данные.ОбновитьДанные(структЗадача.Запрос.РезультатДанные.Результат);
				КонецЕсли;
				Команд = Данные.К;
				...
				Если ПарИдПроцесса = "" Тогда
					ПарИдПроцесса = ПолучитьИД();
				КонецЕсли;
				ОбщийРезультат = ПоказатьСтраницу(ОбщийРезультат + Данные.Представление, ПарИдПроцесса);
				Если структЗадача.Запрос.Свойство("РезультатДанные") Тогда
					Данные.ПриСозданииОбъекта(ЭтотОбъект, Данные.ПолучитьТекст(), , ИмяДанных); // обнулить состояние
				КонецЕсли;
				структЗадача.Этап = "ЕстьРезультат";
			КонецЕсли;
		КонецЕсли;
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		Если Действие = "ДобавитьДанные" Тогда

			Если Запрос.РезультатДанные.Свойство("Результат") Тогда
		...
		ИначеЕсли Действие = "ЗапросДанных" Тогда
			...
			Если НЕ Запрос.Свойство("РезультатДанные") Тогда
				стрЗапрос = Новый Структура("ЗапросДанных, cmd", Запрос.ЗапросДанных, "ЗапросДанных");
				...
				стрЗапрос.Вставить("ОбратныйЗапрос", Новый Структура("taskid, cmd", структЗадача.ИдЗадачи, "ЗапросДанных"));
			...
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
				...
				ИначеЕсли ЗапросДанные.ЗаголовокНайден Тогда
					...
					ЗадачаОбновитьВыполнить = Истина;
				КонецЕсли;
				Если РезультатДанные.Ответ = "ЗапросЗавершен" Тогда
					Возврат Ложь;
				КонецЕсли;
			КонецЕсли;
		...
		ИначеЕсли Действие = Команда["ОбновитьСтраницу"] Тогда
			Если НЕ Вкладки.Количество() Тогда
				Если НоваяВкладка(Новый Структура("data", ИмяДанных)) = Неопределено Тогда
		...
		ИначеЕсли структЗадача.Действие = Команда["Авторизация"]
			ИЛИ структЗадача.Действие = Команда["Регистрация"] Тогда
			Если НЕ Запрос.Свойство("РезультатДанные") Тогда // отправить запрос к дата-серверу
				ОбратныйЗапрос = Новый Структура("data, taskid, cmd", Запрос.data, структЗадача.ИдЗадачи, структЗадача.Действие);
				ПередатьДанные(Параметры.Хост, Параметры.ПортД, Новый Структура("ЗапросДанные, ОбратныйЗапрос, cmd", Запрос, ОбратныйЗапрос, Запрос.cmd));
				Запрос.Вставить("РезультатДанные", Неопределено);
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
				ПрошелАвторизацию = Запрос.РезультатДанные.Результат.ПрошелАвторизацию;
				Если ПрошелАвторизацию Тогда
					Если ПередатьДанные(Параметры.Хост, Параметры.ПортС, Новый Структура("procid, Субъект, cmd", Запрос.РезультатДанные.Результат.procid, Запрос.РезультатДанные.Результат.unm, "startproc")) Тогда // запустить новый процесс контроллера
						ОбщийРезультат = ОбщийРезультат + Шаблон.ПолучитьОбласть("ОбластьПеренаправить", Новый Структура("ПараметрПуть, ПараметрПауза, ПараметрТекст", "/showdata/" + Запрос.РезультатДанные.Результат.procid, 10, "success"));
					КонецЕсли;
				КонецЕсли;
				Возврат Истина;
			КонецЕсли;
		...
		Если Действие = Команда["НоваяВкладка"] Тогда
			Если НоваяВкладка(Запрос) = Неопределено Тогда
				Возврат "ПродолжитьВыполнение";
			КонецЕсли;
			Возврат Истина;
		КонецЕсли;
...
Функция НоваяЗадача(Запрос, Тип = "Запрос") Экспорт
	Перем ИдЗадачи;
	...
	Если Задача = Неопределено Тогда
		структЗадача = Новый Структура("ИдЗадачи, Тип, Этап, Запрос, Действие, Результат, ВремяНачало", ИдЗадачи, Тип, "ВыполнитьЗадачу", Запрос, Неопределено, "", ТекущаяДата());
		Сообщить("Новая задача=" + структЗадача.ИдЗадачи + " " + УзелСвойство(структЗадача.Запрос, "cmd") + " время=" + (ТекущаяДата() - структЗадача.ВремяНачало));
		Если УзелСвойство(Запрос, "cmd") = Команда["ОбновитьДанные"] Тогда
			Если НЕ ЗадачаОбновить = Неопределено Тогда
				...
				Задачи.Вставить(ЗадачаОбновить.ИдЗадачи, ЗадачаОбновить);
			КонецЕсли;
	...
	Пока Истина Цикл
		...
		Если Задачи.Количество() Тогда
			...
			Для каждого структЗадача Из ПереченьЗадач Цикл
				Если НЕ структЗадача.Этап = "Приостановить" Тогда
					Попытка

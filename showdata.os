...
Функция ПоказатьВкладки()
	...
	Для каждого элВкладка Из ВкладкиСписок Цикл
		...
		Если НЕ Вкладка.Режим = "prop" Тогда
			...
			Если НЕ Вкладка.Режим = "text" Тогда
	...
	ПараметрыШаблона.Вставить("ПараметрЗаголовокСтраницы", "" + УзелСвойство(ТекущиеДанные, "Заголовок") + " - Разумное проектирование");
...
Функция НоваяСтруктураВкладка(Данные, ТипВкладки, Заголовок, Режим, УзелКод)
	...
	ОбщийРезультат = ОбщийРезультат + "<div id='insert'><script>newtab('" + НоваяВкладка.ИдВкладки + "','" + НоваяВкладка.ТипВкладки + "','" + НоваяВкладка.ИдУзла + "');</script></div>";
...
Функция ПоказатьСтруктуруУзла(Вкладка, Узел, ЭтоАтрибут = Ложь, Атрибуты = "", Дочерний = "") Экспорт
	...
	ПараметрыШаблона.Вставить("ПараметрНадписьНаКнопке", ?(УзелСвойство(Узел, "Дочерний") = Неопределено, "-", ?(УзелОткрыт, "⚪" , "⚫")));
...
Функция ОтобразитьDOM(Вкладка, Узел, Родитель = Неопределено, Знач Обновить = Ложь, Знач ТипУзла = "", Знач НачальныйУзел = Ложь)
	...
	Если ТипУзла = "" Тогда
		Если Данные.УзелСостояние(Узел, "ЭтоАтрибут") = Истина Тогда
			ТипУзла = "Атрибут";
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ НачальныйУзел Тогда
		...
		Если НЕ УзелСоседний = Неопределено Тогда
			Соседний = ОтобразитьDOM(Вкладка, УзелСоседний, Родитель, Обновить, ТипУзла);
...
Функция Диалог(Вкладка, Узел)
	...
	ЭтоАтрибут = (Вкладка.Данные.УзелСостояние(Узел, "ЭтоАтрибут") = Истина);
...
Функция СформироватьОтвет(парДанные = Неопределено)
	...
	Для каждого элДанные Из ВсеДанные Цикл
		...
		//Команд = Команд - Вкладка.Данные.К;
		Если НЕ Данные = "НетДанных" Тогда
	...
	Для каждого элВкладка Из Вкладки Цикл
		...
		Если Вкладка.Режим = "struct" ИЛИ Вкладка.Режим = "prop" Тогда
			Если Вкладка.ОбновитьУзел = Истина Тогда
				...
				КонецЕсли;
				Если Вкладка.ТипВкладки = "prop" Тогда
					ПараметрыШаблона = Новый Структура;
					ПараметрыШаблона.Вставить("ПараметрОбъект", Вкладка.Заголовок);
					ПараметрыШаблона.Вставить("ПараметрИдУзла", Узел.Код);
					ПараметрыШаблона.Вставить("ПараметрВкладка", Вкладка.ИдВкладки);
					ПараметрыШаблона.Вставить("ПараметрСвойство", Ответ);
					Ответ = Шаблон.ПолучитьОбласть("ОбластьСвойство", ПараметрыШаблона);
				КонецЕсли;
			Иначе
				Ответ = "";
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = "ЗапросДанных" Тогда
			...
			Если Запрос.Свойство("ЗапросДанных") Тогда
				Если Запрос.ЗапросДанных.Команда = "ЗагруженныеДанные" Тогда
					...
					Для каждого элОбъектДанных Из ВсеДанные Цикл
						...
						стрУзел = Новый Структура("Имя, Значение", "з" + Позиция, Заголовок);
						Если кУзел = Неопределено Тогда
							кУзел = Запрос.Данные.НовыйДочерний(Запрос.Узел, стрУзел, Истина);
						Иначе
							кУзел = Запрос.Данные.НовыйСоседний(кУзел, стрУзел, Истина);
						КонецЕсли;
						Позиция = Позиция + 1;
						Запрос.Узел.Вставить(кУзел.Имя, кУзел.Значение); // добавить запись как свойство
					КонецЦикла;
			...
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
				...
				ИначеЕсли ЗапросДанные.ЗаголовокНайден Тогда
					...
					кУзел = УзелСвойство(Запрос, "кУзел");
					стрУзел = Новый Структура("Имя, Значение", "з" + ЗапросДанные.Позиция, Заголовок);
					Если кУзел = Неопределено Тогда
						кУзел = Данные.НовыйДочерний(Запрос.Узел, стрУзел, Истина);
					Иначе
		...
		ИначеЕсли Действие = Команда["ОбновитьСтраницу"] Тогда
			...
			Для каждого элВкладка Из Вкладки Цикл
				...
				Скрипт = Скрипт + "newtab('" + Вкладка.ИдВкладки + "','" + Вкладка.ТипВкладки + "','" + Вкладка.ИдУзла + "');";
			КонецЦикла;
		...
		ИначеЕсли Действие = Команда["НоваяБаза"] Тогда
			стрСообщение = "";
			БазаДанных = УзелСвойство(Запрос, "osdb");
			Если НЕ "" + БазаДанных = "" Тогда
				Если НЕ Запрос.Свойство("РезультатДанные") Тогда // отправить запрос к дата-серверу
					...
					ПередатьДанные(Параметры.Хост, Параметры.ПортД, Новый Структура("БазаДанных, ОбратныйЗапрос, Команда", БазаДанных, ОбратныйЗапрос, "ПолучитьДанные"));
					Запрос.Вставить("РезультатДанные", Неопределено);
				ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
					Если Запрос.РезультатДанные.Ответ = "Успешно" Тогда
						стрСообщение = "Создана новая база данных " + Запрос.БазаДанных;
						ЗаписатьСобытие(БазаДанных, стрСообщение, 1);
					Иначе
						// Сообщить об ошибке
					КонецЕсли;
					Возврат Истина;
		...
		Если НЕ tab = "" И НЕ tab = "undefined" Тогда
			...
			Если Действие = Команда["ЗакрытьВкладку"] Тогда
				...
				Если ВкладкиСписок.Количество() = Вкладки.Количество() Тогда // нет свойств
					ОбщийРезультат = ОбщийРезультат + "<div id='insert'><script>$('body').removeClass('aside-menu-show');</script></div>";
				КонецЕсли;
		...
		Если Действие = Команда["Авторизация"] ИЛИ Действие = Команда["Регистрация"] Тогда
			...
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
				...
				Если НЕ ДочернийУзел = Неопределено Тогда
					Данные.УдалитьУзел(ДочернийУзел, , Ложь);
				КонецЕсли;
				кУзел = Неопределено;
				Для каждого Параметр Из ЗапросДанные Цикл
					стрУзел = Новый Структура("Имя, Значение", Параметр.Ключ, Параметр.Значение);
					Если кУзел = Неопределено Тогда
						кУзел = Данные.НовыйДочерний(Узел, стрУзел, Истина);
					Иначе
						кУзел = Данные.НовыйСоседний(кУзел, стрУзел, Истина);
					КонецЕсли;
				КонецЦикла;
		...
		ИначеЕсли Действие = Команда["ПриИзменении"] Тогда
			...
			Если НЕ ЗначениеПоля = Неопределено Тогда
				...
				Если НЕ АтрибутЗначение = Неопределено Тогда
					зСвойство = Данные.Дочерний(АтрибутЗначение);
					Если НЕ зСвойство = Неопределено Тогда
						Свойство = Данные.ПолучитьСвойствоПоСсылке(зСвойство);
						Данные.НовоеЗначениеУзла(Свойство, Новый Структура("Имя, Значение", "Строка", ЗначениеПоля), Истина);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			Возврат Истина;
		...
		ИначеЕсли Действие = Команда["ПриНажатии"] Тогда
			...
			Если НЕ АтрибутПриНажатии = Неопределено Тогда
				уСвойство = Данные.Дочерний(АтрибутПриНажатии);
				Если НЕ уСвойство = Неопределено Тогда
					Свойство = Данные.ПолучитьСвойствоПоСсылке(уСвойство);
					уЗначение = Данные.Соседний(уСвойство);
					Если НЕ уЗначение = Неопределено Тогда
						сЗначение = Данные.ПолучитьСвойствоПоСсылке(уЗначение);
						Значение = Данные.ЗначениеСвойства(сЗначение);
						Данные.НовоеЗначениеУзла(Свойство, Значение, Истина);
					Иначе
						Данные.НовоеЗначениеУзла(Свойство, Новый Структура("Имя, Значение", "Нажатие", Узел.Код), Истина);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			Возврат Истина;
		...
		ИначеЕсли Действие = Команда["ПриОтправке"] Тогда
			...
			Если НЕ АтрибутПриОтправке = Неопределено Тогда
				зСвойство = Данные.Дочерний(АтрибутПриОтправке);
				Если НЕ зСвойство = Неопределено Тогда
					Свойство = Данные.ПолучитьСвойствоПоСсылке(зСвойство);
					Данные.НовоеЗначениеУзла(Свойство, Новый Структура("Имя, Значение", "Отправка", Узел.Код), Истина);
				КонецЕсли;
			КонецЕсли;
			Возврат Истина;
		...
		ИначеЕсли Действие = Команда["СвойстваУзла"] Тогда
			...
			Для каждого элВкладка Из Вкладки Цикл
				Вкладка = элВкладка.Значение;
				Если Вкладка.Режим = "prop" И Вкладка.ИдУзла = Узел.Код Тогда // уже есть такая
					вклСвойства = Вкладка;
					ОбщийРезультат = ОбщийРезультат + "<div id='insert'><script>showprop('#" + вклСвойства.ИдВкладки + "_0');</script></div>";
			...
			Если вклСвойства = Неопределено И ПоказатьСвойство Тогда
				вклСвойства = НоваяСтруктураВкладка(Данные, "prop", СтрЭкранироватьРазметку(Данные.ИмяДанных , 20), "prop", Узел.Код);
	...
	Пока Истина Цикл
		...
		Если НЕ Локальный Тогда
			Если НЕ procid = "0" Тогда // основной процесс не завершаем
				Если ТекущаяУниверсальнаяДатаВМиллисекундах() - ВремяНачало > 1000 * 30 * 60 Тогда
					Сообщить("Контроллер procid=" + procid + " простаивает.");
					ОбщийРезультат = "<div id='tabs' /><div class='win' /><div id='data' class='my-4'><h3>сеанс завершен</h3><script>aupd=false</script></div>";
					ЗадачаОбновитьВыполнить = Истина;
					ОстановитьСервер = Истина;
				КонецЕсли;
			КонецЕсли;

			...
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ сервера
				...
				Если НЕ ЗапросДанные = Неопределено Тогда
					Для каждого Вариант Из ЗапросДанные Цикл
						Запись = ИмяЗначение(Вариант.Значение.Слово, Вариант.Значение.Результат);
						Запрос.Данные.НовоеЗначениеУзла(Запрос.Свойства.д.Формы, Запись, Запрос.Данные.Служебный(Запрос.Свойства.д.Формы), Истина); // Добавить запись в источник данных
					КонецЦикла;
					Запрос.РезультатДанные = Неопределено;
					ЗадачаОбновитьВыполнить = Истина;
				КонецЕсли;

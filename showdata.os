...
Функция ПоказатьПанель()
	...
	ПараметрыШаблона.Вставить("ПараметрБазаДанных", БазаДанных);
	ПараметрыШаблона.Вставить("ПараметрИмяДанных", ИмяДанных);
	ПараметрыШаблона.Вставить("ПараметрФайлДанных", "" + procid + ".s");
...
Функция ЗапросВыполнитьЗадачу(структЗадача)
	...
	Если структЗадача.Этап = "ВыполнитьЗадачу" Тогда
		...
		Если ОбработатьКоманду(структЗадача) Тогда
...
Функция ОбновитьСостояние(Вкладка, Узел, Состояние, Значение)
	...
	ИначеЕсли Состояние = "ОбновитьПодчиненный" ИЛИ Состояние = "ОбновитьУзел" Тогда
		ОбновитьСостояние(Вкладка, Данные.Старший(Узел), "ОбновитьПодчиненный", Истина);

	КонецЕсли;

	Если НЕ Результат = Ложь Тогда
		УзелСостояниеЗначение(Вкладка, Узел, Состояние, Значение);
		Если НЕ Состояние = "ОбновитьПодчиненный" Тогда
			...
			КонецЕсли;
		КонецЕсли;
		УзелСостояниеЗначение(Вкладка, Узел, "Структура", "");
		УзелСостояниеЗначение(Вкладка, Узел, "Вид", "");
	КонецЕсли;
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		Если Действие.Имя = "ОбновитьDOM" Тогда
			Возврат Истина;
		ИначеЕсли Действие.Имя = "loadpage" Тогда
		...
		ИначеЕсли Действие.Имя = "savedata" Тогда
			Если НЕ ТекущиеДанные = Неопределено Тогда
				ТекущиеДанные.Данные.СохранитьДанные(ОбъединитьПути(ТекущийКаталог(), "data", ТекущиеДанные.БазаДанных + ".files", Запрос.datafile));
				структЗадача.Результат = Шаблон.ПолучитьОбласть("ОбластьУведомление",
					Новый Структура("ПараметрТекстУведомления, ПараметрЗаголовокУведомления, ПараметрТипУведомления",
						"Данные сохранены", "showdata", 1));
			КонецЕсли;
			Возврат Истина;
		КонецЕсли;
			...
			ИначеЕсли Действие.Имя = "ЗакрытьВкладку" Тогда
				...
				Если ТекущаяВкладка >= tab Тогда
					...
					Если ТекущаяВкладка >= 0 Тогда
						элВкладка = Вкладки.Получить(ТекущаяВкладка);
						Вкладка = элВкладка.Значение;
						Вкладка.Вставить("ОбновитьУзел", Истина);
					Иначе
						ТекущаяВкладка = Неопределено;
						структЗадача.Результат = "<div id='data'/>";
					КонецЕсли;
					ОбновитьПанель = Истина;
				КонецЕсли;
			КонецЕсли;

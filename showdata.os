...
Функция СформироватьОтвет(парВкладка = Неопределено)
	...
	Для каждого ЭлДанные Из ВсеДанные Цикл
		...
		Если текДанные = "НетДанных" Тогда
			Продолжить;
		ИначеЕсли НЕ парВкладка = Неопределено Тогда
			Если НЕ парВкладка.Данные = текДанные Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
	...
	Для каждого элВкладка Из Вкладки Цикл
		Вкладка = элВкладка.Значение;
		Если НЕ парВкладка = Неопределено Тогда
			Если НЕ парВкладка = Вкладка Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Функция ЗапросВыполнитьЗадачу(структЗадача)
	...
	Если структЗадача.Этап = "ВыполнитьЗадачу" Тогда
		...
		Если НЕ структЗадача.Действие = Неопределено Тогда
			...
			КонецЕсли;
		Иначе
			Если НЕ Субъект = "Гость" Тогда // убрать НЕ
				структЗадача.Этап = "СформироватьОтвет";
			Иначе
	...
	Если структЗадача.Этап = "СформироватьОтвет" Тогда
		Если Субъект = "Гость" Тогда // поставить НЕ
			Если структЗадача.Действие = Команда["ОбновитьДанные"] ИЛИ структЗадача.Действие = Команда["ОбновитьСтраницу"] Тогда
				СформироватьОтвет();
				Если ОбщийРезультат = "" Тогда // ничего не поменялось
					структЗадача.Этап = "ВыполнитьЗадачу";
				Иначе
					структЗадача.Этап = "ЕстьРезультат";
				КонецЕсли;
			Иначе
				СформироватьОтвет(Вкладки.Получить(ТекущаяВкладка)); // сначала обновить текущую вкладку
				...
				ИначеЕсли ЗапросДанные.ЗаголовокНайден Тогда
					...
					Если ЗапросДанные.Свойство("Позиция") Тогда
						Если ЗапросДанные.Позиция = "1" Тогда // очистить существующие записи
							дУзел = Запрос.Данные.Дочерний(Запрос.Узел);
							Если НЕ дУзел = Неопределено Тогда
								Запрос.Данные.УдалитьУзел(дУзел, Истина, Ложь);
					...
					Если кУзел = Неопределено Тогда
						кУзел = Запрос.Данные.НовыйДочерний(Запрос.Узел, Новый Структура("Имя", "Строка"), Истина);
					Иначе
						кУзел = Запрос.Данные.НовыйСоседний(кУзел, Новый Структура("Имя", "Строка"), Истина);
					КонецЕсли;
					...
					Для каждого элПоле Из СписокПолей Цикл
						Если аУзел = Неопределено Тогда
							аУзел = Запрос.Данные.НовыйАтрибут(кУзел, Новый Структура("Имя, Значение", элПоле.Ключ, УзелСвойство(Заголовок, элПоле.Ключ)), Истина);
						Иначе
				...
				Если НЕ АтрибутЗначение = Неопределено Тогда
					...
					Если НЕ УзелЗначение = Неопределено Тогда
						...
						Если НЕ Определение = Неопределено Тогда
							Данные.УзелУстановитьЗначение(УзелЗначение, Определение, ЗначениеПоля);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		ИначеЕсли Действие = Команда["ПриНажатии"] Тогда
			...
			Если НЕ АтрибутПриНажатии = Неопределено Тогда
				...
				Если НЕ УзелСценарий = Неопределено Тогда
					//ОбщийРезультат = ОбщийРезультат +
					Вкладка.Данные.Интерпретировать(УзелСценарий);
					//ОбновитьУзел(УзелСценарий);
				КонецЕсли;
			КонецЕсли;
			Возврат Истина;
		...
		Если НЕ Локальный Тогда
			Если НЕ procid = "0" Тогда // основной процесс не завершаем
				Если ТекущаяДата() - ВремяНачало > 30 * 60 Тогда
					...
					ОбщийРезультат = "<div id='data' class='my-4'><h3>session timeout</h3></div>";
					ЗадачаОбновитьВыполнить = Истина;
					ОстановитьСервер = Истина;
				КонецЕсли;
			КонецЕсли;
		...
		Если Задачи.Количество() Тогда
			...
			Для каждого структЗадача Из ПереченьЗадач Цикл
				...
				Если структЗадача.Действие = Команда["ОбновитьДанные"] Тогда
					Если НЕ ЗадачаОбновитьВыполнить Тогда
						Продолжить;
					КонецЕсли;
					ЗадачаОбновитьВыполнить = Ложь;
				КонецЕсли;

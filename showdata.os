...
Функция СкроллЗначение(Знач Скролл = Неопределено, Знач Режим = Неопределено, Знач Значение = Неопределено)
...
Функция ПоказатьПанель()
	...
	Для каждого элВкладка Из Вкладки Цикл
		...
		Вкладка.Вставить("tab", НомерВкладки);
...
Функция ПоказатьМенюИнструменты(Вкладка, Узел, ЭтоАтрибут = Ложь)
	...
	Если НЕ этоАтрибут = Истина Тогда
		...
		Если НЕ УзелСостояние(Вкладка, Узел, "Режим") = "node" Тогда
			ПараметрыШаблона.Вставить("ПараметрКоманда", "change");
	...
	Если структЗадача.Этап = "СформироватьОтвет" Тогда
		...
		Если НЕ ТекущееОкно = Неопределено Тогда
			...
			Если ТекущееОкно.ОбновитьУзел = Истина Тогда
				...
				ПараметрыШаблона.Вставить("ПараметрВкладка", ТекущееОкно.tab);
...
Функция ОбновитьСостояние(Вкладка, Узел, Состояние, Значение, Событие = Истина, НачальныйУзел = Ложь)
	...
	Если Состояние = "Изменить" Тогда
		ОбновитьСостояние(Вкладка, Данные.Атрибут(Узел), "Изменить", Значение, Событие);
		ОбновитьСостояние(Вкладка, Данные.Дочерний(Узел), "Изменить", Значение, Событие);
		Если НачальныйУзел Тогда
			ОбновитьСостояние(Вкладка, Узел, "УзелРедактируется", Значение, Событие);
		Иначе
			ОбновитьСостояние(Вкладка, Данные.Соседний(Узел), "Изменить", Значение, Событие);
		КонецЕсли;
	...
	Если НЕ Действие = Неопределено Тогда
		...
		Если НЕ ТекущиеДанные = Неопределено Тогда
			...
			Если НЕ scrolled = Неопределено Тогда
				СкроллЗначение(ТекущиеДанные.Скролл, ТекущиеДанные.Режим, scrolled);
			КонецЕсли;
		КонецЕсли;
		...
		Если НЕ tab = Неопределено Тогда
			tab = Число(tab);
			Если tab > Вкладки.Количество()-1 Тогда
				Возврат Ложь;
			КонецЕсли;

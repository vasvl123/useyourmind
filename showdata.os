...
Функция ПоказатьВкладки()
	...
	Для каждого элВкладка Из ВкладкиСписок Цикл
		...
		Если НЕ Вкладка.ТипВкладки = "prop" Тогда
			...
			Если НЕ Вкладка.ТипВкладки = "text" Тогда
	...
	ПараметрыШаблона.Вставить("ПараметрЗаголовокСтраницы", "" + УзелСвойство(ТекущиеДанные, "Заголовок"));
...
Функция ПолучитьДанные(структЗадача)
	...
	ИначеЕсли Данные = Неопределено Тогда
		ЗадачаПараметры = Новый Структура("БазаДанных, ИмяДанных, ПозицияДанных, ИмяШаблона, cmd", БазаДанных, ИмяДанных, ПозицияДанных, ИмяШаблона, "ПолучитьДанные");
...
Функция НачальнаяСтраница(Содержимое, Вкладки)
	ОбластьПанельМенюОкно = ?(Субъект = "", "ОбластьПанельОкно", "ОбластьПанельМенюОкно");
	Текст = Шаблон.ПолучитьОбласть("ОбластьШапка", Новый Структура("ПараметрХост", ПараметрХост))
		+ Шаблон.ПолучитьОбласть(ОбластьПанельМенюОкно, Новый Структура("ПараметрХост, ПараметрСодержимое, ПараметрВкладки", ПараметрХост, Содержимое, Вкладки))
...
Функция СформироватьОтвет(парДанные = Неопределено)
	...
	Для каждого элДанные Из ВсеДанные Цикл
		...
		Если НЕ ПарДанные = Неопределено Тогда
			Если НЕ Данные = ПарДанные Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
	...
	Для каждого элВкладка Из Вкладки Цикл
		...
		КонецЕсли;
		Если НЕ ПарДанные = Неопределено Тогда
			Если НЕ Вкладка.Данные = ПарДанные Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Функция ЗапросВыполнитьЗадачу(Знач структЗадача)
	...
	Если структЗадача.Этап = "ЕстьРезультат" Тогда
		...
		// 	ОбщийРезультат = ОбщийРезультат + Шаблон.ПолучитьОбласть("ОбластьОбновить");
		// 	ЗадачаОбновитьВремя = ПолучитьИД();
		// КонецЕсли;
	...
	// Если НЕ Действие = Неопределено Тогда
		...
		Если Действие = "ПолучитьДанные" Тогда // загрузить данные с дата-сервера
			...
			Если НЕ Запрос.Свойство("РезультатДанные") Тогда
				...
				Иначе
					стрЗапрос = Новый Структура("БазаДанных, ИмяДанных, ПозицияДанных, Команда", БазаДанных, ИмяДанных, ПозицияДанных, "ПолучитьДанные");
		...
		ИначеЕсли Действие = Команда["ОбновитьСтраницу"] Тогда
			...
			тВкладка.ОбновитьУзел = Ложь;
			Ответ = "" + СтрЗаменить(Представление, "id='_", "id='" + тВкладка.ИдВкладки + "_");
			Содержимое = "<div id='data'><div " + ?(Субъект = "", "class='my-4'", "") + " id='" + тВкладка.ИдВкладки + "_0'>" + Ответ + "</div></div><div id='win'></div>";
			ОбщийРезультат = НачальнаяСтраница(Содержимое, ПоказатьВкладки());
			ОбновитьВкладки = Ложь;
			Для каждого элВкладка Из Вкладки Цикл
				Вкладка = элВкладка.Значение;
				Если НЕ Вкладка = тВкладка Тогда // текущую уже вставили
					Вкладка.ОбновитьУзел = Истина;
					ОбщийРезультат = ОбщийРезультат + "<script>newtab('" + Вкладка.ИдВкладки + "','" + Вкладка.ТипВкладки + "','" + Вкладка.ИдУзла + "');</script>";
				КонецЕсли;
			КонецЦикла;
			Возврат Истина;
		...
		Если Действие = Команда["Авторизация"] ИЛИ Действие = Команда["Регистрация"] Тогда
			...
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
				...
				Если НЕ ДочернийУзел = Неопределено Тогда
					Данные.УдалитьУзел(ДочернийУзел, , Истина);
				КонецЕсли;
				кУзел = Узел;
				...
				Если ЗапросДанные.ПрошелАвторизацию Тогда
					...
					//ИзменитьАдрес();
					ОбщийРезультат = ОбщийРезультат + Шаблон.ПолучитьОбласть("ОбластьПеренаправить", Новый Структура("ПараметрПуть, ПараметрПауза, ПараметрТекст", "/procid/" + procid, 0, ""));
					Возврат Истина;
				КонецЕсли;
...
Функция ПередатьДанные(Хост, Порт, стрДанные) Экспорт
	...
	Попытка
		...
		Соединение.ТаймаутОтправки = 3000;

...
Функция НоваяВкладка(Параметры)
	...
	Иначе
		...
		КонецЕсли;
	КонецЕсли;
	ВсеДанные.Вставить(Данные, ИмяДанных);
...
Функция ПоказатьГлавноеМеню(Вкладка)
	...
	ПараметрыШаблона.Вставить("ПараметрКоманда", Команды["СохранитьДанные"]);
	...
	ПараметрыШаблона.Вставить("ПараметрКоманда", Команды["РежимРедактора"]);
	ПараметрыШаблона.Вставить("ПараметрПодсказка", "Дизайнер");
	ПараметрМенюИнструменты = ПараметрМенюИнструменты + Шаблон.ПолучитьОбласть("ОбластьМенюИнструменты", ПараметрыШаблона);

	ПараметрыШаблона.Вставить("ПараметрКоманда", Команды["ВкладкаСтруктура"]);
	ПараметрыШаблона.Вставить("ПараметрПодсказка", "Структура");
	ПараметрМенюИнструменты = ПараметрМенюИнструменты + Шаблон.ПолучитьОбласть("ОбластьМенюИнструменты", ПараметрыШаблона);

	ПараметрыШаблона = Новый Структура;
	ПараметрыШаблона.Вставить("ПараметрВидМеню", Команды["ГлавноеМеню"]);
...
Функция ПоказатьМенюИнструменты(Вкладка, Узел, ЭтоАтрибут = Ложь)
	...
	Если Вкладка.Режим = "design" Тогда
		...
		Если НЕ УзелСостояние(Вкладка, Узел, "Изменить") = Истина Тогда
			ПараметрыШаблона.Вставить("ПараметрКоманда", Команды["Структура"]);
			ПараметрыШаблона.Вставить("ПараметрПодсказка", "Структура");
		Иначе
	...
	Иначе
		...
		ПараметрыШаблона.Вставить("ПараметрКоманда", Команды["РедактироватьИмя"]);
	...
	Если НЕ этоАтрибут = Истина Тогда
		...
		ПараметрыШаблона.Вставить("ПараметрКоманда", Команды["УдалитьДочерние"]);
...
Функция ЗапросВыполнитьЗадачу(структЗадача)
	...
	Если структЗадача.Этап = "СформироватьОтвет" Тогда
		...
		Если ОбновитьПанель = Истина Тогда
		...
		Для каждого ЭлДанные Из ВсеДанные Цикл
			Попытка
				ЭлДанные.Ключ.ОбновитьПредставление();
			Исключение
				Сообщить(ОписаниеОшибки());
			КонецПопытки;
			ЭлДанные.Ключ.Обновить = Ложь;
		КонецЦикла;
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		Если Действие.Имя = Команды["ГлавноеМеню"] Тогда
		...
		Если Действие.Имя = Команды["РежимРедактора"] Тогда
		...
		Если Действие.Имя = Команды["ВкладкаСтруктура"] Тогда
			Если ТекущиеДанные = Неопределено Тогда
				Возврат Ложь;
			КонецЕсли;
		...
		Если Действие.Имя = Команды["Выполнить"] Тогда
			ОбщийРезультат = ОбщийРезультат + Вкладка.Данные.Интерпретировать(Вкладка.Данные.Окружение, Узел);
			Возврат Истина;
		...
		ИначеЕсли Действие.Имя = Команды["УдалитьДочерние"] Тогда
			Если НЕ УзелСвойство(Узел, "Дочерний") = Неопределено Тогда
		...
		ИначеЕсли Действие.Имя = Команды["Структура"] Тогда
		...
		ИначеЕсли Действие.Имя = Команды["ОкноРедактор"] Тогда
			ОбновитьСостояние(Вкладка, Узел, "УзелОткрыт", Истина);
...
Если АргументыКоманднойСтроки.Количество() Тогда
	...
	Команды.Вставить("Структура", "struct");
	...
	Команды.Вставить("ОкноРедактор", "editwin");
	Команды.Вставить("СохранитьДанные", "savedata");
	Команды.Вставить("ГлавноеМеню", "mainmenu");
	Команды.Вставить("РежимРедактора", "designmode");
	Команды.Вставить("ВкладкаСтруктура", "edittab");
	...
	Команды.Вставить("УдалитьДочерние", "childremove");

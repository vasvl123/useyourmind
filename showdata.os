...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие.Имя = Команды["СтруктураДанных"] Тогда
			...
			ТекущееОкно = НоваяСтруктураВкладка(Данные, "win", Вкладка.ИмяДанных + " Значение " + Узел.Имя + " : " + Данные.УзелСвойство(Узел, "Значение"), "struct", Вкладка.ИдУзла, Вкладка.БазаДанных, Вкладка.ИмяДанных);
...
Функция ЗаписатьСобытие(стрЗаголовок, стрСообщение, ТипСобытия = 0, ПараметрКоманда = "") Экспорт
	Если НЕ Журнал = Неопределено Тогда
		...
		Журнал.Объект.Интерпретировать(Журнал.ДанныеЖурнала);
	КонецЕсли;
...
Если АргументыКоманднойСтроки.Количество() Тогда
	...
	Попытка
		...
		Журнал.ДанныеЖурнала = Журнал.Объект.Родитель(Журнал.Объект.Родитель(Журнал.СписокСтрок));

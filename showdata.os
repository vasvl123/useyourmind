...
Функция ВыполнитьДействия(структЗадача)
	...
	// Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = Команда["ОстановитьСервер"] Тогда
			ОбщийРезультат = "<div id='data' class='data'><h3>завершение ...</h3></div>";
		...
		ИначеЕсли Действие = Команда["ПерезапуститьСервер"] Тогда
			ОбщийРезультат = "<div id='data' class='data'><h3>перезапуск ...</h3></div>";
		...
		ИначеЕсли Действие = Команда["ОбновитьСтраницу"] Тогда
			...
			Если НЕ ТекущиеДанные = Неопределено Тогда
				Если ТекущиеДанные.Данные.ОбъектыОбновить.Количество() Тогда // подождать обновления всех узлов
					Возврат Неопределено;
				КонецЕсли;
				Если УзелСвойство(ТекущиеДанные, "Меню") = "Нет" Тогда
					Меню = ПолучитьОбласть("ОбластьПанельНетМеню");
				КонецЕсли;
				Заголовок = УзелСвойство(ТекущиеДанные, "ЗаголовокСтраницы");
			КонецЕсли;
			Если Меню = "" Тогда
				Меню = ПолучитьОбласть(?(Субъект = "", "ОбластьПанельМеню1", "ОбластьПанельМеню2"));
			КонецЕсли;
		...
		Если Действие = Команда["Авторизация"] ИЛИ Действие = Команда["Регистрация"] Тогда
			...
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
				...
				Если ЗапросДанные.ПрошелАвторизацию Тогда
					...
					ОбщийРезультат = ОбщийРезультат + ПолучитьОбласть("ОбластьПанельМеню2");
				КонецЕсли;
		...
		Если НЕ Локальный Тогда
			Если НЕ procid = "0" Тогда // новый процесс не завершаем
				...
				Если бВремя > 1000 * 30 * 60 Тогда // бездействие 30 мин
					...
					ОбщийРезультат = "<div id='tabs' /><div class='win' /><div id='data' class='data'><h3>сеанс завершен</h3></div><script>aupd=false</script>";
					ОстановитьСервер = Истина;

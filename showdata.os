...
Функция ПерваяВкладка(Данные)
	...
	дУзел = Данные.НовыйДочерний(Узел, Новый Структура("Имя", "a"));
	...
	Данные.НовыйДочерний(дУзел, Новый Структура("Имя, Значение", "p", "Объектно-ориентированная система управления базами данных в среде OneScript"));
	...
	Если СписокБаз.Количество() Тогда
		Для каждого БазаДанных Из СписокБаз Цикл
			...
			Для н = 1 По Индекс.КоличествоСтрок() Цикл
				...
				Узел = Данные.НовыйДочерний(Узел, Новый Структура("Имя, Значение", "a", нУзел.data));
				аУзел = Данные.НовыйАтрибут(Узел, Новый Структура("Имя, Значение", "href", ""));
				аУзел = Данные.НовыйСоседний(аУзел, Новый Структура("Имя, Значение", "onclick", "sendcmd('osdb=" + БазаДанных.ИмяБезРасширения + "&data=" + нУзел.data + "&dataposition=" + нУзел.dataposition + "&cmd=newtab'); return false;"));
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
	...
	Данные.НовыйАтрибут(дУзел, Новый Структура("Имя, Значение", "class", "btn btn btn-primary btn-sm"));
...
Функция НоваяВкладка(Параметры)
	...
	Иначе
		...
		// Данные = Новый pagedata(ОбъединитьПути(ТекущийКаталог(), "add", "datalisp.txt"));
	КонецЕсли;
...
Функция ПоказатьУзел(Узел, Вкладка, ЭтоАтрибут = Ложь, Атрибуты = "", Дочерний = "") Экспорт
	...
	Иначе
		...
		Если Узел.Имя = "#text" ИЛИ Узел.Имя = "Строка" Тогда
			Узел.Имя = "text";
		КонецЕсли;
...
Функция ПоказатьМенюИнструменты(Вкладка, Узел, ЭтоАтрибут = Ложь)
	...
	Если УзелСостояние(Вкладка, Узел, "УзелРедактируется") = Истина Тогда
...
Функция Интерпретировать(Узел, Вкладка) Экспорт

	Представление = "<div id=" + Вкладка.ИдВкладки + "_" + Узел.Код + ">";
	Попытка
		Значение = Вкладка.Данные.Интерпретировать(Вкладка.Данные.Окружение, Узел);
		Если ТипЗнч(Значение) = Тип("Структура") Тогда
			Значение = ОтобразитьDOM(Вкладка, Значение.Код, , "Дочерний");
		КонецЕсли;
	...
	Если ОбновитьУзел Тогда
		...
		Если Представление = "" Тогда
			...
			Если НЕ (Вкладка.Режим = "struct" ИЛИ УзелИзменить) ИЛИ УзелСостояние(Вкладка, Узел, "УзелОткрыт") = Истина  Тогда
				...
				Если НЕ УзелДочерний = Неопределено И НЕ (Узел.Имя = "Список" И НЕ Вкладка.Режим = "struct") Тогда
			...
			Иначе
				Если Узел.Имя = "Список" ИЛИ Узел.Имя = "Перем"  ИЛИ Узел.Имя = "Свойство" Тогда
					Представление = Интерпретировать(Узел, Вкладка);
				Иначе
			...
			Иначе
				Если domupdate = "0" Тогда
					Если НЕ Вкладки.Количество() Тогда
						ТекущиеДанные = НоваяВкладка(Новый Структура("data", "OneScriptDB"));
					КонецЕсли;
					Скрипт = "";

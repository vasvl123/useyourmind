...
Функция ПолучитьДанныеССервера(БазаДанных, ИмяДанных, ПозицияДанных) Экспорт
	Перем Данные;
	Соединение = Неопределено;
	ПередатьСтроку(Соединение, "osdb" + Символы.Таб + БазаДанных + Символы.Таб + "data" + Символы.Таб + ИмяДанных + Символы.Таб + "dataposition" + Символы.Таб + ПозицияДанных);
	Попытка
...
Функция ПередатьДанныеНаСервер(Данные, БазаДанных, ИмяДанных) Экспорт
	стрСообщение = "";
	кСообщение = 1;
	Соединение = Неопределено;
	Ответ = "";
	Если ПередатьСтроку(Соединение, "osdb" + Символы.Таб + БазаДанных + Символы.Таб + "data" + Символы.Таб + ИмяДанных + Символы.Таб + "datatype" + Символы.Таб + "3") Тогда
		Попытка
			Если Соединение.ПрочитатьСтроку() = "?" Тогда
		...
		КонецПопытки;
	КонецЕсли;
	ЗаписатьСобытие(БазаДанных + "/" + ИмяДанных + "/" + Ответ, стрСообщение, кСообщение);
	Если НЕ Соединение = Неопределено Тогда
		Соединение.Закрыть();
		Соединение = Неопределено;
	КонецЕсли;
	Возврат Ответ;
КонецФункции // ПередатьДанныеНаСервер()
...
Функция СформироватьОтвет()
	...
	Если НЕ ОбщийРезультат = "" Тогда
		ОбщийРезультат = ОбщийРезультат + Шаблон.ПолучитьОбласть("ОбластьСтатус", Новый Структура("ПараметрСтатусСообщение", "" + Цел(100*(ТекущаяДата() - ВремяНачало))/100 + " с. " + Команд + " к."));
	КонецЕсли;
	...
	Если структЗадача.Этап = "ВыполнитьЗадачу" Тогда
		...
		Если Субъект = "Гость" Тогда
			...
			Если структЗадача.Запрос.ПараметрыЗапросаКоличество = "0" Тогда // статичная страница
				...
				Данные = ВсеДанные.Получить(ПутьДанные);
				Если Данные = Неопределено Тогда
					Данные = Новый pagedata(ЭтотОбъект, "", ИмяДанных, "");
					ВсеДанные.Вставить(ПутьДанные, Данные);
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие.Имя = Команда["ОбновитьСтраницу"] Тогда
		...
		ИначеЕсли Действие.Имя = Команда["СохранитьДанные"] Тогда
			...
			Если НЕ Вкладка = Неопределено Тогда
				Вкладка.Данные.СохранитьДанные();
			КонецЕсли;
			// системное событие
			Если НЕ Настройки = Неопределено Тогда
		...
		ИначеЕсли Действие.Имя = Команда["ЗагрузитьHTML"] Тогда
			...
			Если СтруктураHTML = Неопределено Тогда
				ТекущееОкно = НоваяСтруктураВкладка(Данные, "win", Вкладка.ИмяДанных + " " + Узел.Имя + ":" + СтрЭкранироватьРазметку(Данные.УзелСвойство(Узел, "Значение"), 20) + " import", "struct", Узел.Код);
				ОбновитьСостояние(ТекущееОкно, Узел, "УзелОткрыт", Истина);
		...
		ИначеЕсли Действие.Имя = Команда["СтруктураДанных"] Тогда
			...
			ТекущееОкно = НоваяСтруктураВкладка(Данные, "win", Вкладка.ИмяДанных + " " + Узел.Имя + ":" + СтрЭкранироватьРазметку(Данные.УзелСвойство(Узел, "Значение"), 20), "struct", Вкладка.ИдУзла);
			ОбновитьСостояние(ТекущееОкно, Узел, "УзелОткрыт", Истина);
			Возврат Истина;
		...
		ИначеЕсли Действие.Имя = Команда["ЗначениеУзла"] Тогда
			...
			Иначе
				...
				УзелКод = Узел.Код;
			КонецЕсли;
			ТекущееОкно = НоваяСтруктураВкладка(ДанныеВкладки, "win", Вкладка.ИмяДанных + " Значение " + " " + Узел.Имя + ":" + СтрЭкранироватьРазметку(Данные.УзелСвойство(Узел, "Значение"), 20), ТипУзла, УзелКод);
			Возврат Истина;

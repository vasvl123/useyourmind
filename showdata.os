...
Функция ВыполнитьЗапросКСерверуДанных(стрЗапрос) Экспорт
	Перем Ответ;
	Попытка
		Соединение = Новый TCPСоединение(Параметры.Хост, Параметры.ПортД);
		Соединение.ОтправитьДвоичныеДанные(СтруктуруВДвоичныеДанные(стрЗапрос));
		Ответ = ДвоичныеДанныеВСтруктуру(Соединение.ПрочитатьДвоичныеДанные());
	Исключение
...
Функция НоваяЗадача(Запрос) Экспорт
	структЗадача = Новый Структура("Тип, Этап, Запрос, Действие, Результат", "Запрос", "ВыполнитьЗадачу", Запрос, Неопределено, "");
	Если УзелСвойство(Запрос, "cmd") = Команда["ОбновитьДанные"] Тогда
		Если НЕ ЗадачаОбновить = Неопределено Тогда
			ЗадачаОбновить.Этап = "УдалитьЗадачу";
			Задачи.Добавить(ЗадачаОбновить);
		КонецЕсли;
		ЗадачаОбновить = структЗадача;
	Иначе
		Задачи.Добавить(структЗадача);
	КонецЕсли;
КонецФункции // НоваяЗадачаЗапросаДанных()


Функция ПолучитьДанные(БазаДанных, ИмяДанных, ПозицияДанных, ВсеДанныеВставить = Истина)
	ПутьДанные = БазаДанных + "/" + ИмяДанных + "/" + ПозицияДанных;
	Данные = ВсеДанные.Получить(ПутьДанные);
	Если Данные = Неопределено Тогда
		Если НЕ ПозицияДанных = "" ИЛИ БазаДанных = "" Тогда // запросить данные с сервера
			Ответ = ВыполнитьЗапросКСерверуДанных(Новый Структура("БазаДанных, ИмяДанных, ПозицияДанных", БазаДанных, ИмяДанных, ПозицияДанных));
			Если Ответ.Свойство("дДанные") Тогда
				Текст = ПолучитьСтрокуИзДвоичныхДанных(Ответ.дДанные);
			Иначе
		...
		Иначе
			Текст = ""; // создаем новый файл
		КонецЕсли;
		Данные = Новый pagedata(ЭтотОбъект, Текст, БазаДанных, ИмяДанных, ПозицияДанных);
		Если ВсеДанныеВставить Тогда
...
Функция НоваяВкладка(Параметры) Экспорт
	...
	Если Режим = "" Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ТекущиеДанные = НоваяСтруктураВкладка(Данные, ТипВкладки, ИмяДанных, Режим, Данные.Корень.Код);
...
Функция ПоказатьСтраницу(ТекстСтраницы, ПарИдПроцесса)
	Текст = Шаблон.ПолучитьОбласть("ОбластьШапка", Новый Структура("ПараметрХост", ПараметрХост))
		+ Шаблон.ПолучитьОбласть("ОбластьПоказатьСтраницу", Новый Структура("ПараметрИдПроцесса, ПараметрХост, ПараметрТекстСтраницы", ПарИдПроцесса, ПараметрХост, ТекстСтраницы));
	Возврат Текст;
КонецФункции
...
Функция ЗапросВыполнитьЗадачу(структЗадача)
	...
	Сообщить("ВыполнитьЗадачу=" + структЗадача.Запрос.taskid);

	Если структЗадача.Этап = "ВыполнитьЗадачу" Тогда

		структЗадача.Вставить("ВремяНачало", ТекущаяДата());

		Если Субъект = "Гость" Тогда // многостраничный режим
			НетПараметров = (структЗадача.Запрос.Количество() = 0);
			ПарИдПроцесса = "" + УзелСвойство(структЗадача.Запрос, "procid");
			ПрошелАвторизацию = Ложь;
			Если НЕ НетПараметров Тогда // страница с параметрами, данные не сохраняются
				Если структЗадача.Запрос.Свойство("cmd", структЗадача.Действие) Тогда
					Если структЗадача.Действие = Команда["Авторизация"] ИЛИ
						структЗадача.Действие = Команда["Регистрация"] Тогда
							структЗадача.Запрос = ВыполнитьЗапросКСерверуДанных(структЗадача.Запрос);
							ПрошелАвторизацию = структЗадача.Запрос.ПрошелАвторизацию;
					ИначеЕсли структЗадача.Действие = Команда["ОстановитьСервер"] Тогда
			...
			Если ПрошелАвторизацию Тогда
				Если ПередатьДанные(Параметры.Хост, Параметры.ПортС, структЗадача.Запрос) Тогда
					ОбщийРезультат = Шаблон.ПолучитьОбласть("ОбластьПеренаправить", Новый Структура("ПараметрПуть, ПараметрПауза, ПараметрТекст", "/showdata/" + ПарИдПроцесса, 10, "success"));
				КонецЕсли;
			Иначе
				...
				Если ИмяДанных = "" Тогда
					ИмяДанных = "start";
				КонецЕсли;
				Данные = ПолучитьДанные("", ИмяДанных, "", НетПараметров);
				Данные.ОбновитьДанные(структЗадача.Запрос);
				Команд = Данные.К;
				Данные.ОбновитьПредставление();
				Команд = Данные.К - Команд;
				Сообщить("Команд " + Команд);
				Если ПарИдПроцесса = "" Тогда
					ПарИдПроцесса = ТекущаяУниверсальнаяДатаВМиллисекундах();
				КонецЕсли;
				ОбщийРезультат = ПоказатьСтраницу(Данные.Представление, ПарИдПроцесса);
				Если НЕ НетПараметров Тогда
					ОсвободитьОбъект(Данные);
				КонецЕсли;
			КонецЕсли;
		Иначе // режим одностраничного приложения
			Если структЗадача.Запрос.Свойство("cmd", структЗадача.Действие) Тогда
				Если ВыполнитьДействия(структЗадача) = Истина Тогда
					СформироватьОтвет();
				КонецЕсли;
			Иначе
				ОбщийРезультат = НачальнаяСтраница();
			КонецЕсли;
		КонецЕсли;

		Если структЗадача.Этап = "ВыполнитьЗадачу" Тогда
			Если НЕ ОбщийРезультат = "" ИЛИ НЕ структЗадача.Действие = Команда["ОбновитьДанные"] Тогда
				структЗадача.Результат = ОбщийРезультат;
				ОбщийРезультат = "";
				структЗадача.Этап = "ЕстьРезультат";
			КонецЕсли;
		ИначеЕсли структЗадача.Этап = "ПродолжитьВыполнение" Тогда
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		Если Действие = "ЗапросДанных" Тогда

			начВремя = ТекущаяДата();
			Обновление = Ложь;
			Пока Истина Цикл
				ЗапросДанных = Запрос.ЗапросДанных;
				Результат = ВыполнитьЗапросКСерверуДанных(ЗапросДанных);
				Если Результат.Результат = "ОшибкаПотокаДанных" Тогда
					// сообщить об ошибке
					Возврат Ложь;
				Иначе
					Если Результат.Результат = "ЗаголовокНайден" Тогда
						...
						Если ЗапросДанных.Свойство("ЧислоЗаписей", ЧислоЗаписей) Тогда
							ЧислоЗаписей = Число(ЧислоЗаписей) - 1;
						Иначе
							ЧислоЗаписей = - 1;
						КонецЕсли;
						ЗапросДанных.Вставить("ЧислоЗаписей", ЧислоЗаписей);
						Если ЗапросДанных.Свойство("СписокПолей", СписокПолей) Тогда
						...
						Обновление = Истина;
						нУзел = Запрос.Данные.НовыйДочерний(Запрос.Узел, Новый Структура("Имя", "Строка"), Истина);
						Для каждого элПоле Из Заголовок Цикл
							Поле = элПоле.Ключ;
							Если НЕ СписокПолей = Неопределено Тогда
								Если НЕ СписокПолей.Свойство(Поле) Тогда
									Продолжить;
								КонецЕсли;
							КонецЕсли;
							Запрос.Данные.НовыйАтрибут(нУзел, Новый Структура("Имя, Значение", Поле, элПоле.Значение), Истина);
						КонецЦикла;
						Если ЧислоЗаписей = 0 Тогда // получены все записи
							Прервать;
						КонецЕсли;
					КонецЕсли;
					ЗапросДанных.Вставить("ПозицияДанных", Результат.ПозицияДанных);
					Если НЕ Число(ЗапросДанных.ПозицияДанных) > 12 Тогда
						Прервать;
					КонецЕсли;
				КонецЕсли;
				Если ТекущаяДата() - начВремя > 0.25 Тогда // долго ищет
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если Число(ЗапросДанных.ПозицияДанных) > 12 И НЕ ЧислоЗаписей = 0 Тогда
				структЗадача.Этап = "ПродолжитьВыполнение"; // выполнение будет продолжено в фоне
			КонецЕсли;
		...
		ИначеЕсли Действие = Команда["СохранитьДанные"] Тогда
			...
			Если НЕ Вкладка = Неопределено Тогда
				Ответ = ВыполнитьЗапросКСерверуДанных(Новый Структура("БазаДанных, Заголовок, Команда, дДанные", Вкладка.Данные.БазаДанных, Новый Структура("ИмяДанных, ТипДанных", Вкладка.Данные.ИмяДанных, 3), "ЗаписатьДанные", ПолучитьДвоичныеДанныеИзСтроки(Вкладка.Данные.СохранитьДанные())));
				Если Ответ.Результат = "Успешно" Тогда
					Если Ответ.Свойство("ПозицияДанных") Тогда
						Вкладка.Данные.ПозицияДанных = Ответ.ПозицияДанных;
					КонецЕсли;
		...
		Если Действие = Команда["ПриИзменении"] Тогда
		...
		ИначеЕсли Действие = Команда["ЗагрузитьHTML"] Тогда
			...
			Иначе
				...
				Данные.ЗагрузитьHTML(Узел, СтруктураHTML, 0);
...
Функция ЗаписатьСобытие(стрЗаголовок, стрСообщение, ТипСобытия = 0, ПараметрКоманда = "") Экспорт
	...
	ВыполнитьЗапросКСерверуДанных(Новый Структура("БазаДанных, Заголовок, Команда", "log", Новый Структура("Источник, Субъект, Тип, Сообщение", стрЗаголовок, Субъект, ТипСобытия, стрСообщение), "ЗаписатьЗаголовок"));
...
Функция УстановитьПараметры(знПараметры)
	...
	Если НЕ Субъект = "Гость" Тогда
		Настройки = ПолучитьДанные("", "cfg", "");
	КонецЕсли;
...
Процедура ОбработатьСоединения()
	...
	Пока Истина Цикл
		...
		Если НЕ Локальный Тогда
			Если НЕ procid = "0" Тогда // основной процесс не завершаем
				Если ТекущаяДата() - ВремяНачало > 30 * 60 Тогда
					...
					Если НЕ ЗадачаОбновить = Неопределено Тогда
						ОбщийРезультат = "<div id='data' class='my-4'><h3>Время сеанса истекло</h3></div>";
						ЗапросВыполнитьЗадачу(ЗадачаОбновить);
					КонецЕсли;
					ОстановитьСервер = Истина;
				КонецЕсли;
		...
		Пока ПустойЦикл < 50 И Соединение = Неопределено Цикл
	...
	ПередатьДанные(Параметры.Хост, Параметры.ПортВ, Новый Структура("procid, cmd", procid, "stopserver"));
	ПередатьДанные(Параметры.Хост, Параметры.ПортД, Новый Структура("procid, cmd", procid, "stopserver"));
	ПередатьДанные(Параметры.Хост, Параметры.ПортС, Новый Структура("procid, cmd", procid, "stopserver"));

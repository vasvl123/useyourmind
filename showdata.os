		...
		ИначеЕсли Действие = "ЗапросДанных" Тогда
			...
			Если Запрос.Свойство("ЗапросДанных") Тогда
				Если Запрос.ЗапросДанных.Команда = "ЗагруженныеДанные" Тогда
					Позиция = 1;
					ЗапросДанные = Неопределено;
					пЗапросДанные = Неопределено;
					Для каждого элОбъектДанных Из ВсеДанные Цикл
						Заголовок = Новый Структура;
						нЗапросДанные = Новый Структура("Позиция, Заголовок", Позиция, Заголовок);
						Если пЗапросДанные = Неопределено Тогда
							ЗапросДанные = нЗапросДанные;
						Иначе
							пЗапросДанные.Вставить("Соседний", нЗапросДанные);
						КонецЕсли;
						пЗапросДанные = нЗапросДанные;
						...
						Позиция = Позиция + 1;
					КонецЦикла;
					Запрос.Данные.НовоеЗначениеУзла(Запрос.Узел, ЗапросДанные, Истина);

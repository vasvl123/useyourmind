...
Функция ЗапросВыполнитьЗадачу(структЗадача)
	...
	Если структЗадача.Этап = "ВыполнитьЗадачу" Тогда
		...
		Если НЕ структЗадача.Действие = Неопределено Тогда
			...
			КонецЕсли;
		Иначе
			Если Субъект = "Гость" Тогда // убрать НЕ
				структЗадача.Этап = "СформироватьОтвет";
			Иначе
	...
	Если структЗадача.Этап = "СформироватьОтвет" Тогда
		Если НЕ Субъект = "Гость" Тогда // поставить НЕ
			Если структЗадача.Действие = Команда["ОбновитьДанные"] Тогда
				СформироватьОтвет();
			Иначе
				СформироватьОтвет(Вкладки.Получить(ТекущаяВкладка).Данные); // сначала обновить данные текущей вкладки
			...
			Для каждого структЗадача Из ПереченьЗадач Цикл
				...
				Если структЗадача.Действие = Команда["ОбновитьДанные"] Тогда
					Если ЗадачаОбновить = Неопределено Тогда
						...
						КонецЕсли;
					Иначе
						структЗадача.Этап = "ЕстьРезультат"; // убрать дублирование
					КонецЕсли;
					ЗадачаОбновитьВыполнить = Ложь;
				КонецЕсли;
				Попытка

...
Функция ВыполнитьЗапросКСерверуДанных(стрЗапрос, Данные = Неопределено) Экспорт
	...
	Попытка
		...
		Если НЕ Данные = Неопределено Тогда
			...
			Ответ = ПолучитьСтрокуИзДвоичныхДанных(Соединение.ПрочитатьДвоичныеДанные());
			ЗаписатьСобытие(Запрос.osdb + "/" + Запрос.data + "/" + Ответ, "Данные сохранены", 1);
		КонецЕсли;
	Исключение
		ЗаписатьСобытие(Запрос.osdb + "/" + Запрос.data + "/" + Ответ, ОписаниеОшибки(), 3);
	КонецПопытки;
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		Если Действие.Имя = Команда["ЗапросДанных"] Тогда

			Заголовок = ВыполнитьЗапросКСерверуДанных(СтруктуруВСтроку(Запрос));

		ИначеЕсли Действие.Имя = Команда["ПриИзменении"] Тогда
...
Функция ЗаписатьСобытие(стрЗаголовок, стрСообщение, ТипСобытия = 0, ПараметрКоманда = "") Экспорт
	...
	Если НЕ Журнал = Неопределено Тогда
		...
		Журнал.Объект.ОбновитьУзел(Журнал.Данные); // обновление
	КонецЕсли;
...
Если АргументыКоманднойСтроки.Количество() Тогда
	...
	Команда.Вставить("ЗапросДанных", "dataquery");

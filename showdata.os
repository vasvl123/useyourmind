...
Функция ПоказатьСтраницу(ТекстСтраницы)
	Текст = Шаблон.ПолучитьОбласть("ОбластьШапка", Новый Структура("ПараметрХост", ПараметрХост))
		+ Шаблон.ПолучитьОбласть("ОбластьПоказатьСтраницу", Новый Структура("ПараметрИдПроцесса, ПараметрХост, ПараметрТекстСтраницы", ТекущаяУниверсальнаяДатаВМиллисекундах(), ПараметрХост, ТекстСтраницы));
	Возврат Текст;
КонецФункции
...
Функция ЗапросВыполнитьЗадачу(структЗадача)
	...
	Если структЗадача.Этап = "ВыполнитьЗадачу" Тогда
		...
		Если Субъект = "Гость" Тогда
			...
			ОбщийРезультат = ПоказатьСтраницу(Данные.Представление);
		Иначе
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		Если Действие.Имя = Команда["Авторизация"] Тогда

			Если НЕ Субъект = "Гость" Тогда
				Субъект = УзелСвойство(Запрос, "Субъект");
				//ИмяДанных = "" + УзелСвойство(структЗадача.Запрос, "data");
				//Если ИмяДанных = "" Тогда
					ИмяДанных = "expldb";
				//КонецЕсли;
				ОбщийРезультат = "<!DOCTYPE html><html><script type='text/javascript'>setTimeout(function(){window.location.href = '/showdata/" + procid + "';}, 10);</script>success...</html>";
			КонецЕсли;
			Возврат Ложь;

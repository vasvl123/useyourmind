...
Функция ПоказатьВкладки()
	...
	ПараметрыШаблона.Вставить("ПараметрЗаголовокСтраницы", ТекущиеДанные.Заголовок + " - OneScriptDB");
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие.Имя = Команды["ЗначениеУзла"] Тогда
			...
			Если Окружение = Неопределено Тогда
				Окружение = Данные.Окружение;
			КонецЕсли;
			УзелЗначение = Данные.Интерпретировать(Окружение, Узел);

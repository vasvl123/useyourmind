			...
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
				...
				ИначеЕсли ЗапросДанные.ЗаголовокНайден Тогда
					...
					Если кУзел = Неопределено Тогда
						кУзел = Данные.НовыйДочерний(Запрос.Узел, Новый Структура("Имя, Значение", "r" + ЗапросДанные.Позиция, Заголовок), Истина);
					Иначе
						кУзел = Данные.НовыйСоседний(кУзел, Новый Структура("Имя, Значение", "r" + ЗапросДанные.Позиция, Заголовок), Истина);
					КонецЕсли;

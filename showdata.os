...
Функция НачальнаяСтраница()
	...
	Текст = Текст + Шаблон.ПолучитьОбласть("ОбластьМенюПанель", Новый Структура("ПараметрХост", ПараметрХост));
	Текст = Текст + Шаблон.ПолучитьОбласть("ОбластьПодвал", Новый Структура("ПараметрИдПроцесса, ПараметрХост", procid, ПараметрХост));
	Возврат Текст;
КонецФункции
	...
	Если структЗадача.Этап = "ВыполнитьЗадачу" Тогда
		...
		Иначе
			...
			Если Гость Тогда
				...
				Тело = Данные.НайтиУзел(Данные.Корень, "body");
				Если НЕ Тело = Неопределено Тогда
					Данные.Фронт = Тело;
				КонецЕсли;
				Команд = Данные.К;
				Данные.ОбновитьПредставление(Истина);

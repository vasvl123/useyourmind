...
Функция НоваяСтруктураВкладка(Данные, ТипВкладки, Заголовок, Режим, УзелКод)
	...
	ОбщийРезультат = ОбщийРезультат + "<div id='script'><script>newtab('" + НоваяВкладка.ИдВкладки + "','" + НоваяВкладка.ТипВкладки + "','" + НоваяВкладка.ИдУзла + "');</script></div>";
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = Команда["ОбновитьСтраницу"] Тогда
			...
			КонецЦикла;
			ОбщийРезультат = "<div id='script'><script>" + Скрипт + "</script></div>";
			ОбновитьВкладки = Истина;
			Возврат Истина;
		...
		Если НЕ tab = "" И НЕ tab = "undefined" Тогда
			...
			Если Действие = Команда["ЗакрытьВкладку"] Тогда
				...
				Если ВкладкиСписок.Количество() = Вкладки.Количество() Тогда // нет свойств
					ОбщийРезультат = ОбщийРезультат + "<div id='script'><script>$('body').removeClass('aside-menu-show');</script></div>";
				КонецЕсли;
		...
		ИначеЕсли Действие = Команда["СвойстваУзла"] Тогда
			...
			Для каждого элВкладка Из Вкладки Цикл
				...
				Если Вкладка.ТипВкладки = "prop" И Вкладка.ИдУзла = Узел.Код Тогда // уже есть такая
					вклСвойства = Вкладка;
					ОбщийРезультат = ОбщийРезультат + "<div id='script'><script>showprop('#" + вклСвойства.ИдВкладки + "_0');</script></div>";
		...
		Если НЕ Локальный Тогда
			Если НЕ procid = "0" Тогда // основной процесс не завершаем
				Если ТекущаяУниверсальнаяДатаВМиллисекундах() - ВремяНачало > 1000 * 30 * 60 Тогда
					...
					ОбщийРезультат = "<div id='tabs' /><div class='win' /><div id='data' class='my-4'><h3>сеанс завершен</h3></div><div id='script'><script>aupd=false</script></div>";
					ЗадачаОбновитьВыполнить = Истина;
					ОстановитьСервер = Истина;
				КонецЕсли;
			КонецЕсли;

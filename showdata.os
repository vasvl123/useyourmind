...
Функция ПередатьДанные(Хост, Порт, стрДанные) Экспорт
	...
	Попытка
		...
		Соединение.ОтправитьДвоичныеДанныеАсинхронно(ПолучитьДвоичныеДанныеИзОбъекта(стрДанные));
		//Соединение.Закрыть();
			...
			Если Соединение.Статус = "Успех" Тогда
				...
				Попытка
					...
					Если НЕ дд.Размер() = 0 Тогда
						мДанные.Добавить(дд);
						Запрос = ПолучитьОбъектИзДвоичныхДанных(СоединитьДвоичныеДанные(мДанные));
					КонецЕсли;
				Исключение

...
Функция ПолучитьСписокБаз()
	Стр = "Код	1	Имя	СписокБаз";
	СписокБаз = НайтиФайлы(ОбъединитьПути(ТекущийКаталог(), "data"), "*.osdb", Истина);
	Если СписокБаз.Количество() Тогда
		Код = 2;
		Стр = Стр + "	Дочерний	" + Код + Символы.ПС;
		Для каждого БазаДанных Из СписокБаз Цикл
			Стр = Стр + "Код	" + Код + "	Имя	БазаДанных	Атрибут	" + (Код + 1) + Символы.ПС;
			Если Код > 2 Тогда
				Стр = Стр + "	Соседний	" + (Код + 2) + Символы.ПС;
			КонецЕсли;
			Стр = Стр + "Код	" + (Код + 1) + "	Имя	Имя	Значение	" + БазаДанных.ИмяБезРасширения;
			Код = Код + 2;
		КонецЦикла;
	КонецЕсли;
	Возврат Стр;
...
Функция НоваяВкладка(Параметры)
	...
	Иначе
		//Данные = Новый pagedata(ПолучитьСписокБаз());
...
Функция ЗначениеУзлаЛисп(Узел, Вкладка, ЭтоАтрибут = Ложь, Атрибуты = "", Дочерний = "") Экспорт

	Представление = "";
	Если НЕ ЭтоАтрибут Тогда
		Представление = "<div " + Атрибуты + " id=" + Вкладка.ИдВкладки + "_" + Узел.Код + ">";
		Попытка
			Если Узел.Имя = "Объект" Тогда
				Представление = Представление + Вкладка.Данные.Интерпретировать(Вкладка.Данные.Окружение, Узел);
			КонецЕсли;
		Исключение
			Представление = Представление + ОписаниеОшибки();
		КонецПопытки;
		Представление = Представление + Дочерний + "</div>";
	Иначе
		Представление = Узел.Имя + "=" + Узел.Значение;
	КонецЕсли;
...
Функция ОтобразитьDOM(Вкладка, КодУзла, Знач ОбновитьУзел = Ложь, Знач ТипУзла = "", Знач НачальныйУзел = Ложь)
	...
	Если ОбновитьУзел Тогда
		...
		Если Представление = "" Тогда
			...
			Иначе
				Если НЕ ИменаУзловЛисп.Найти(Узел.Имя) = Неопределено Тогда
					Представление = ЗначениеУзлаЛисп(Узел, Вкладка, (ТипУзла = "Атрибут"), Атрибуты, Дочерний);
				Иначе
...
Если АргументыКоманднойСтроки.Количество() Тогда
	...
	ИменаУзловЛисп = Новый Массив;
	ИменаУзловЛисп.Добавить("Число");
	ИменаУзловЛисп.Добавить("Строка");
	ИменаУзловЛисп.Добавить("Объект");
	ИменаУзловЛисп.Добавить("Список");

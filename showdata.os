		...
		ИначеЕсли Действие = Команда["СвойстваУзла"] Тогда
			...
			Для каждого элВкладка Из Вкладки Цикл
				...
				Если Вкладка.Режим = "prop" И Вкладка.ИдУзла = Узел.Код Тогда // уже есть такая
					вклСвойства = Вкладка;
					ОбщийРезультат = ОбщийРезультат + "<div id='insert'><script>showprop('" + вклСвойства.ИдВкладки + "_" + вклСвойства.ИдУзла + "');</script></div>";
					Прервать;
				КонецЕсли;
			КонецЦикла;
			...
			Если Запрос.Свойство("prop") Тогда // установить значения свойств объекта
				...
				Для каждого эл Из мсв Цикл
					...
					Если Свойства.Свойство(элм[0], усв) Тогда
						...
						Если НЕ вклСвойства = Неопределено Тогда
			...
			Если вклСвойства = Неопределено Тогда
				вклСвойства = НоваяСтруктураВкладка(Данные, "prop", Вкладка.Данные.ИмяДанных + "  " + Узел.Имя + ":" + СтрЭкранироватьРазметку(Данные.УзелСвойство(Узел, "Значение"), 20), "prop", Узел.Код);

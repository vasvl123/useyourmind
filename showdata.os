...
Функция ВыполнитьДействия(структЗадача)
	...
	// Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = "ФормаСлова" Тогда // запрос к серверу морфологии
			...
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ сервера
				...
				Если НЕ ЗапросДанные = Неопределено Тогда
					Для каждого Вариант Из ЗапросДанные Цикл
						Запись = ИмяЗначение(Вариант.Ключ, Вариант.Значение);
						Запрос.Данные.НовоеЗначениеУзла(Запрос.Свойства.д.Варианты, Запись, Запрос.Данные.Служебный(Запрос.Свойства.д.Варианты), Истина); // Добавить запись в источник данных
					КонецЦикла;
					Запрос.РезультатДанные = Неопределено;
					ЗадачаОбновитьВыполнить = Истина;
				КонецЕсли;

...
Функция ВыполнитьДействия(структЗадача)
	...
	// Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = "Морфология" Тогда // запрос к серверу морфологии

			Если НЕ Запрос.Свойство("РезультатДанные") Тогда
				стрЗапрос = Новый Структура("Слова, ОбратныйЗапрос, cmd", Запрос.Слова, Новый Структура("ИдЗадачи, Хост, Порт", структЗадача.ИдЗадачи, Параметры.Хост, Порт), Запрос.мДействие);
			...
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ сервера
				РезультатДанные = Запрос.РезультатДанные;
				Запрос.Библиотека.ОбработатьОтвет(Запрос.мДействие, Запрос.Данные, Запрос.Свойства, РезультатДанные.Результат);
				Запрос.РезультатДанные = Неопределено;
				Возврат Истина;
			КонецЕсли;
			Возврат Ложь;
		...
		ИначеЕсли Действие = Команда["ЗавершитьПроцесс"] Тогда
			...
			Если НЕ ОстановитьСервер Тогда
				ОбщийРезультат = ОбщийРезультат + "<div id='container' class='container-fluid data'><h3>сессия завершена</h3></div>";
			...
			Если структЗадача.Этап = "ЕстьРезультат" Тогда
				Если структЗадача.Запрос.Свойство("taskid") Тогда
					Если структЗадача.Результат = "" Тогда
						...
						РезультатОбновить = "";
					КонецЕсли;
					Если НЕ ПередатьДанные(Параметры.Хост, Параметры.ПортВ, Новый Структура("procid, taskid, ContentType, Результат", procid, структЗадача.Запрос.taskid, "text/html", ПолучитьДвоичныеДанныеИзСтроки(структЗадача.Результат))) = Неопределено Тогда
						структЗадача.Этап = "УдалитьЗадачу";
					КонецЕсли;
				Иначе
					структЗадача.Этап = "УдалитьЗадачу";

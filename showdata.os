...
Функция ПоказатьСтруктуруУзла(Вкладка, Узел, ЭтоАтрибут = Ложь, Атрибуты = "", Дочерний = "") Экспорт
	...
	ПараметрыШаблона = Новый Структура;
	ПараметрыШаблона.Вставить("ПараметрИдУзла", "" + Вкладка.ИдВкладки + "_" + Узел.Код);
	ПараметрыШаблона.Вставить("ПараметрКоманда", ?(НЕ этоАтрибут = Истина ИЛИ Узел.Код = Вкладка.ИдУзла, ?(УзелОткрыт, Команда["ЗакрытьУзел"], Команда["ОткрытьУзел"]), Команда["СвойстваУзла"]));
...
Функция ОкноОбъекта(Узел)

	НоваяСтруктураВкладка("Свойства объекта", "win", Узел.Значение, "text", Неопределено);

КонецФункции // ОкноОбъекта()
...
Функция СформироватьОтвет(парДанные = Неопределено)
	...
	Для каждого элВкладка Из Вкладки Цикл
		...
		Если Вкладка.Режим = "struct" ИЛИ Вкладка.Режим = "prop" Тогда
			...
			КонецЕсли;
		ИначеЕсли Вкладка.Режим = "text" Тогда
			Ответ = Вкладка.Данные;
		Иначе
	...
	Если НЕ Действие = Неопределено Тогда
		...
		Если НЕ tab = "" И НЕ tab = "undefined" Тогда
			...
			Если Действие = Команда["ЗакрытьВкладку"] Тогда
				...
				Если НЕ ТекущееОкно = Неопределено И ТекущееОкно.ИдВкладки = ТекущаяВкладка Тогда
		...
		ИначеЕсли Действие = Команда["СвойстваУзла"] Тогда
			...
			Если НЕ Вкладка.Режим = "design" Тогда // показать меню объекта

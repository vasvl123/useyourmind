...
Перем Порт;
Перем Локальный;
Перем МоментЗапуска, ВремяНачало;
...
Функция ПоказатьВкладки()
	Текст = "";
	Если НЕ procid = "1" И ВкладкиСписок.Количество() Тогда
...
Функция ВыполнитьДействия(структЗадача)
	...
	// Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = "Морфология" ИЛИ Действие = "ВнешниеДанные" Тогда // запрос к внешним данным

			Если НЕ Запрос.Свойство("РезультатДанные") Тогда
				стрЗапрос = Новый Структура("ОбратныйЗапрос, cmd", Новый Структура("ИдЗадачи, Хост, Порт", структЗадача.ИдЗадачи, Параметры.Хост, Порт), Запрос.вДействие);
				Для каждого пар Из Запрос.Параметры Цикл
					стрЗапрос.Вставить(пар.Ключ, пар.Значение);
				КонецЦикла;
				Если НЕ ПередатьДанные(Параметры.Хост, ?(Действие = "Морфология", Параметры.ПортМ, Параметры.ПортД), стрЗапрос) = Неопределено Тогда
					Запрос.Вставить("РезультатДанные", Неопределено);
				КонецЕсли;
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ сервера
				РезультатДанные = Запрос.РезультатДанные;
				Запрос.Библиотека.ОбработатьОтвет(Запрос.вДействие, Запрос.Данные, Запрос.Свойства, РезультатДанные.Результат);
				Запрос.РезультатДанные = Неопределено;
				Возврат Истина;
			КонецЕсли;
			Возврат Ложь;
		...
		ИначеЕсли Действие = Команда["ЗавершитьПроцесс"] Тогда
			...
			Если НЕ ОстановитьСервер Тогда
	...
	Пока Истина Цикл
		...
		Если НЕ Локальный Тогда
			Если НЕ procid = "0" И НЕ procid = "1" Тогда // новый и общий процессы не завершаем
				...
				Если бВремя > 1000 * 30 * 60 ИЛИ (ЗадачаОбновить = Неопределено И Субъект = "" И бВремя > 1000 * 3 * 60) Тогда // бездействие
			...
			Если структЗадача.Этап = "ЕстьРезультат" Тогда
				Если структЗадача.Запрос.Свойство("taskid") Тогда
					Если структЗадача.Результат = "" Тогда
						...
						РезультатОбновить = "";
					КонецЕсли;
					Если НЕ ПередатьДанные(Параметры.Хост, Параметры.ПортВ, Новый Структура("procid, taskid, ContentType, Результат", procid, структЗадача.Запрос.taskid, структЗадача.Содержимое, структЗадача.Результат)) = Неопределено Тогда
						структЗадача.Этап = "УдалитьЗадачу";
					КонецЕсли;
				Иначе
					структЗадача.Этап = "УдалитьЗадачу";

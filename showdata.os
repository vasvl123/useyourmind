...
Функция ПолучитьДанные(БазаДанных, ИмяДанных, ПозицияДанных)
	...
	ИначеЕсли Данные = Неопределено Тогда
		Если НЕ ПозицияДанных = "" ИЛИ БазаДанных = "" Тогда // запросить данные с сервера
			ЗадачаПараметры = Новый Структура("БазаДанных, ИмяДанных, ПозицияДанных, cmd", БазаДанных, ИмяДанных, ПозицияДанных, "ДобавитьДанные");
			НоваяЗадача(ЗадачаПараметры, "Служебный");
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		Если Действие = "ДобавитьДанные" Тогда // загрузить данные с дата-сервера
			...
			Если НЕ Запрос.Свойство("РезультатДанные") Тогда
				...
				стрЗапрос.Вставить("ОбратныйЗапрос", Новый Структура("taskid", структЗадача.ИдЗадачи));
		...
		ИначеЕсли Действие = "ЗапросДанных" Тогда // выполнить запрос к дата-серверу
				...
				ИначеЕсли ЗапросДанные.ЗаголовокНайден Тогда
					...
					Для каждого элПоле Из СписокПолей Цикл
						Если аУзел = Неопределено Тогда
							аУзел = Запрос.Данные.НовыйАтрибут(нУзел, Новый Структура("Имя, Значение", элПоле.Ключ, УзелСвойство(Заголовок, элПоле.Ключ)), Истина);
						Иначе
							аУзел = Запрос.Данные.НовыйСоседний(аУзел, Новый Структура("Имя, Значение", элПоле.Ключ, УзелСвойство(Заголовок, элПоле.Ключ)), Истина);
						КонецЕсли;
					КонецЦикла;
		...
		ИначеЕсли Действие = "НоваяБаза" Тогда
			...
			Если НЕ "" + Запрос.БазаДанных = "" Тогда
				Если НЕ Запрос.Свойство("РезультатДанные") Тогда // отправить запрос к дата-серверу
					ОбратныйЗапрос = Новый Структура("taskid", структЗадача.ИдЗадачи);
		...
		ИначеЕсли структЗадача.Действие = Команда["Авторизация"]
			...
			Если НЕ Запрос.Свойство("РезультатДанные") Тогда // отправить запрос к дата-серверу
				ОбратныйЗапрос = Новый Структура("taskid", структЗадача.ИдЗадачи);
				ПередатьДанные(Параметры.Хост, Параметры.ПортД, Новый Структура("ЗапросДанные, ОбратныйЗапрос, Команда", Запрос, ОбратныйЗапрос, Запрос.cmd));
				Запрос.Вставить("РезультатДанные", Неопределено);
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
...
Функция НоваяЗадача(Запрос, Тип = "Запрос") Экспорт
	...
	Если Задача = Неопределено Тогда
		...
		Сообщить("Новая задача " + Тип + "=" + структЗадача.ИдЗадачи + " " + УзелСвойство(структЗадача.Запрос, "cmd") + " время=" + (ТекущаяДата() - структЗадача.ВремяНачало));

...
Функция НачальнаяСтраница()
	Текст = Шаблон.ПолучитьОбласть("ОбластьШапка", Новый Структура("ПараметрИдПроцесса, ПараметрХост", procid, ПараметрХост));
	Текст = Текст + Шаблон.ПолучитьОбласть("ОбластьПодвал", Новый Структура("ПараметрХост", ПараметрХост));
	Возврат Текст;
КонецФункции
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие.Имя = Команда["ЗагрузитьHTML"] Тогда
			...
			СтруктураHTML = УзелСвойство(Запрос, "value");
			Если СтруктураHTML = Неопределено Тогда
				ТекущееОкно = НоваяСтруктураВкладка(Данные, "win", Вкладка.ИмяДанных + " ." + Узел.Имя + " import", "struct", Узел.Код, Вкладка.БазаДанных, Вкладка.ИмяДанных, Вкладка.ПозицияДанных);
				ОбновитьСостояние(ТекущееОкно, Узел, "УзелОткрыт", Истина);
				ОбновитьСостояние(ТекущееОкно, Узел, "ЗагрузитьHTML", Истина);
			Иначе
				// Вызвать обработку импорта
				// Данные.ЗагрузитьHTML(Узел, СтруктураHTML)
			КонецЕсли;
			Возврат Истина;

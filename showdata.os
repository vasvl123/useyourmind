...
Если АргументыКоманднойСтроки.Количество() Тогда
	...
	procid = АргументыКоманднойСтроки[1];
	Субъект = РаскодироватьСтроку(АргументыКоманднойСтроки[2], СпособКодированияСтроки.КодировкаURL);
	...
	СтруктЗапрос = СтруктуруВСтроку(Новый Структура("procid, unm", procid, Субъект));
	КоличествоПопыток = 10;

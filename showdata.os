...
Функция ВыполнитьЗапросКСерверуДанных(стрЗапрос) Экспорт
	...
	Исключение
		ЗаписатьСобытие("ВыполнитьЗапросКСерверуДанных", ОписаниеОшибки(), 3);
	КонецПопытки;
...
Функция НоваяЗадача(Запрос) Экспорт
	...
	Задачи.Вставить(0, структЗадача);
...
Функция ЗапросВыполнитьЗадачу(структЗадача)
	...
	Если структЗадача.Этап = "ВыполнитьЗадачу" Тогда
		...
		Если Субъект = "Гость" Тогда
			...
			КонецЕсли;
			Если структЗадача.Запрос.ПараметрыЗапросаКоличество = "0" Тогда // статичная страница
			...
			Иначе // данные не сохраняются
				Данные = ПолучитьДанные("", ИмяДанных, "", Ложь);
		...
		Если структЗадача.Этап = "ПродолжитьВыполнение" Тогда
	...
	Если структЗадача.Этап = "ЕстьРезультат" Тогда
		Соединение = Неопределено;
		Если ПередатьДанные(Соединение, Новый Структура("taskid, Результат", структЗадача.Запрос.taskid, структЗадача.Результат)) Тогда
...
Функция ПередатьДанные(Соединение, стрДанные) Экспорт
	Попытка
		Соединение = Новый TCPСоединение(Хост, Порт);
		Соединение.ТаймаутОтправки = 50;
		дДанные = СтруктуруВДвоичныеДанные(стрДанные);
		Соединение.ОтправитьДвоичныеДанные(дДанные);
		КоличествоПопыток = 0;
		Возврат Истина;
	Исключение
...
Если АргументыКоманднойСтроки.Количество() Тогда
	...
	Хост = АргументыКоманднойСтроки[1];
	procid = АргументыКоманднойСтроки[2];
	УдаленныйУзел = АргументыКоманднойСтроки[3];
	Субъект = РаскодироватьСтроку(АргументыКоманднойСтроки[4], СпособКодированияСтроки.КодировкаURL);
	ПараметрХост = РаскодироватьСтроку(АргументыКоманднойСтроки[5], СпособКодированияСтроки.КодировкаURL);
	...
	СтруктЗапрос = Новый Структура("procid, unm, ip", procid, Субъект, УдаленныйУзел);
	...
	Пока Истина Цикл
		...
		Если Соединение = Неопределено Тогда
			ПередатьДанные(Соединение, СтруктЗапрос);
		Иначе
		...
		Попытка
			...
			КонецЕсли;
			Запрос = Соединение.ПрочитатьДвоичныеДанные();
		Исключение
		...
		Попытка
			Запрос = ДвоичныеДанныеВСтруктуру(Запрос);
			cmd = "" + УзелСвойство(Запрос, "cmd");
			Если cmd = Команда["ЗавершитьПроцесс"] Тогда

		...
		Если Вкладка.Режим = "struct" ИЛИ Вкладка.Режим = "prop" Тогда
			...
			Иначе
				...
				Для каждого элУзел Из Вкладка.УзлыОбновить Цикл
					Ответ = Ответ + ОтобразитьDOM(Вкладка, элУзел.Ключ, , Вкладка.ОбновитьУзел, , Истина);
				КонецЦикла;
...
Функция ОбновитьСостояние(Вкладка, Узел, Состояние, Значение, Событие = Ложь, НачальныйУзел = Ложь) Экспорт
	...
	ИначеЕсли Состояние = "Представление" И Значение = Неопределено Тогда
		...
		Результат = ОбновитьСостояние(Вкладка, Родитель, "Представление", Значение, Событие);
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = Команда["СвойстваУзла"] Тогда
			...
			Если вклСвойства = Неопределено И ПоказатьСвойство Тогда
				вклСвойства = НоваяСтруктураВкладка(Данные, "prop", Вкладка.Данные.ИмяДанных + "  " + Узел.Родитель.Имя + ":" + СтрЭкранироватьРазметку(Узел.Родитель.Значение, 20), "prop", Узел.Код);

	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = Команда["СвойстваУзла"] Тогда
			...
			Если Запрос.Свойство("prop") Тогда // установить значения свойств объекта
				...
				Для каждого элм Из мсв Цикл
					р = СтрНайти(элм, "=");
					Свойства.Вставить(Лев(элм, р - 1), Сред(элм, р + 1));
				КонецЦикла;
				дсв = Данные.Дочерний(Узел);
				Пока НЕ дсв = Неопределено Цикл
					зн = "";
					Если Свойства.Свойство(дсв.Имя, зн) Тогда
						дсв.Вставить("Значение", зн);
					КонецЕсли;
					дсв = Данные.Соседний(дсв);
				КонецЦикла;
			КонецЕсли;

			ТекущееОкно = НоваяСтруктураВкладка(Данные, "prop", Вкладка.Данные.ИмяДанных + "  " + Узел.Имя + ":" + СтрЭкранироватьРазметку(Данные.УзелСвойство(Узел, "Значение"), 20), "prop", Узел.Код);

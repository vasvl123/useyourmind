...
Функция СформироватьОтвет(парДанные = Неопределено)
	...
	Для каждого элВкладка Из Вкладки Цикл
		...
		Вкладка.Данные.ОбновитьПредставление(Узел, Вкладка.ОбновитьУзел);
		Команд = Команд + Вкладка.Данные.К;
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = "ЗапросДанных" Тогда
			...
			Если Запрос.Свойство("ЗапросДанных") Тогда
				Если Запрос.ЗапросДанных.Команда = "ЗагруженныеДанные" Тогда
					...
					Запрос.Данные.ОбновитьСвязи(Запрос.Узел);
					Возврат Истина;
				КонецЕсли;
			КонецЕсли;
		...
		ИначеЕсли Действие = Команда["ПриНажатии"] Тогда
			...
			Если АтрибутПриНажатии = Неопределено Тогда
				стрСобытие = Новый Структура("Имя, Значение", ПолучитьИД(), "Нажатие");
				Данные.ДобавитьСобытие(Узел, стрСобытие); // событие объекта
			Иначе // сценарий // потом убрать
		...
		ИначеЕсли Действие = Команда["ПриОтправке"] Тогда
			...
			Если АтрибутПриОтправке = Неопределено Тогда
				стрСобытие = Новый Структура("Имя, Значение", ПолучитьИД(), "Отправка");
				Данные.ДобавитьСобытие(Узел, стрСобытие); // событие объекта
			Иначе
		...
		ИначеЕсли Действие = Команда["СвойстваУзла"] Тогда
			...
			Если НЕ "" + запрСвойства = "" Тогда // установить значения свойств формы объекта
				...
				Свойства = Данные.УзелСостояние(фУзел, "Свойства");
				усв = Неопределено;
				Для каждого эл Из мсв Цикл
					элм = СтрРазделить(эл, "=");
					Если Свойства.Свойство(элм[0], усв) Тогда

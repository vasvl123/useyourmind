	...
	// Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = "ФормыСлов" Тогда // запрос к серверу морфологии
			...
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ сервера
				РезультатДанные = Запрос.РезультатДанные;
				ЗапросДанные = РезультатДанные.Результат;
				Если НЕ ЗапросДанные = Неопределено Тогда
					Для каждого Вариант Из ЗапросДанные Цикл
						ф = Запрос.Данные.НовыйДочерний(Запрос.Формы, ИмяЗначение(Вариант.Значение.Слово), , Истина);
						Запрос.Формы.с.Вставить(Вариант.Значение.Слово, ф);
						...
						Для каждого Рез Из мРез Цикл
							Если НЕ Рез = "" Тогда
								...
								у = Запрос.Данные.НовоеЗначениеУзла(ф, Запись, Запрос.Данные.Служебный(Запрос.Формы), Истина); // Добавить форму
								Если мФорм.Количество() > 2 Тогда
									Запрос.Данные.НовыйАтрибут(у, ИмяЗначение(мФорм[2], мФорм[3]));
								КонецЕсли;
							КонецЕсли;
						КонецЦикла;
					КонецЦикла;
					...
					ЗадачаОбновитьВыполнить = Истина;
				КонецЕсли;
				// удалить метку задачи
				зУзел = Запрос.Свойства.д.Задачи.Дочерний;
				Пока НЕ зУзел = Неопределено Цикл
					Если зУзел.Имя = структЗадача.ИдЗадачи Тогда
						Запрос.Данные.УдалитьУзел(зУзел);
						Прервать;
					КонецЕсли;
					зУзел = зУзел.Соседний;
				КонецЦикла;
				Возврат Ложь;
			КонецЕсли;
			Возврат "Приостановить";

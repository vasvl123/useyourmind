...
Функция НоваяВкладка(Параметры)
	...
	ИдВкладки = ИдВкладки + 1;
	структВкладка = Новый Структура("ИдВкладки, ТипВкладки, Данные, Заголовок, Режим, ИдУзла, БазаДанных, ИмяДанных, ОбновитьУзел, Скролл",
		"" + ИдВкладки,
		"data",
		Данные,
		ИмяДанных,
...
Функция ПоказатьУзел(Узел, ИдВкладки, ЭтоАтрибут = Ложь, Атрибуты = "", Дочерний = "") Экспорт
	...
	Иначе
		...
		ИначеЕсли Узел.Имя = "#comment" Тогда
			...
			КонецЕсли;
		Иначе
			Атрибуты = Атрибуты + " id=""" + ИдВкладки + "_" + Узел.Код + """";
...
Функция ПоказатьСтруктуруУзла(Узел, ИдВкладки, ЭтоАтрибут = Ложь, Атрибуты = "", Дочерний = "") Экспорт
	...
	Если НЕ УзелСвойство(Узел, "Дочерний") = Неопределено Тогда
		ПараметрыШаблона = Новый Структура;
		ПараметрыШаблона.Вставить("ПараметрИдУзла", ИдВкладки + "_" + Узел.Код);
		ПараметрыШаблона.Вставить("ПараметрИдПроцесса", procid);
	...
	Если УзелРедактируется = Истина Тогда
		...
		ПараметрыШаблона = Новый Структура;
		ПараметрыШаблона.Вставить("ПараметрИдУзла", "b" + ИдВкладки + "_" + Узел.Код);
		ПараметрыШаблона.Вставить("ПараметрМенюИнструменты", ПараметрМенюИнструменты);
...
Функция ОтобразитьDOM(Вкладка, КодУзла, Знач ОбновитьУзел = Ложь, Знач ТипУзла = "", Знач НачальныйУзел = Ложь)
	...
	Если Вкладка.Режим = "design" Тогда
		УзелИзменить = (УзелСостояние(Узел, "Изменить") = Истина);
	КонецЕсли;
	...
	Если ОбновитьУзел Тогда
		...
		Если Представление = "" Тогда
			...
			Дочерний = "";
			Если НЕ (Вкладка.Режим = "node" ИЛИ УзелИзменить) ИЛИ УзелСостояние(Узел, "УзелОткрыт") = Истина  Тогда
			...
			Если Вкладка.Режим = "node" ИЛИ УзелИзменить Тогда
				Представление = ПоказатьСтруктуруУзла(Узел, Вкладка.ИдВкладки, (ТипУзла = "Атрибут"), Атрибуты, Дочерний);
			Иначе
				Представление = ПоказатьУзел(Узел, Вкладка.ИдВкладки, (ТипУзла = "Атрибут"), Атрибуты, Дочерний);
			КонецЕсли;
		КонецЕсли;
...
Функция ЗапросВыполнитьЗадачу(структЗадача)
	...
	Если структЗадача.Этап = "СформироватьОтвет" Тогда
		...
		ИндексВкладки = 0;
		Для каждого элВкладка Из Вкладки Цикл
			Если ИндексВкладки = ТекущееОкно ИЛИ ИндексВкладки = ТекущаяВкладка Тогда
				Вкладка = элВкладка.Значение;
				Ответ = "";
				Режим = Вкладка.Режим;
				Ответ = Ответ + ОтобразитьDOM(Вкладка, Вкладка.ИдУзла, Вкладка.ОбновитьУзел, , Истина);
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие.Имя = "change" Тогда
			Возврат ОбновитьСостояние(Данные, Узел, "Изменить", Истина);

		ИначеЕсли Действие.Имя = "УзелПросмотр" Тогда
			Возврат ОбновитьСостояние(Данные, Узел, "Изменить", Ложь);

		ИначеЕсли Действие.Имя = "edit" Тогда
			ОбновитьСостояние(Данные, Узел, "УзелОткрыт", Истина);
			ТекущееОкно = ТекущаяВкладка + 1;
			ИдВкладки = ИдВкладки + 1;
			структВкладка = Новый Структура("ИдВкладки, ТипВкладки, Данные, Заголовок, Режим, ИдУзла, БазаДанных, ИмяДанных, ОбновитьУзел, Скролл",
				"" + ИдВкладки,
				"win",
				...
				Истина,
				СкроллЗначение());
			Вкладки.Вставить(ТекущееОкно, структВкладка);
			Вкладка.Вставить("ОбновитьУзел", Истина);
			ОбновитьПанель = Истина;
			Возврат Истина;

	...
	Для каждого элВкладка Из Вкладки Цикл
		...
		Если Вкладка.Режим = "struct" ИЛИ Вкладка.Режим = "prop" Тогда
			Если Вкладка.ОбновитьУзел = Истина Тогда
				...
				Ответ = "" + ОтобразитьDOM(Вкладка, Узел, Узел.Родитель, Вкладка.ОбновитьУзел, , Истина);
				Если Вкладка.ТипВкладки = "prop" Тогда
					ПараметрыШаблона = Новый Структура;
					ПараметрыШаблона.Вставить("ПараметрОбъект", Вкладка.Заголовок);
					ПараметрыШаблона.Вставить("ПараметрИдУзла", Узел.Родитель.Код);
					ПараметрыШаблона.Вставить("ПараметрВкладка", Вкладка.ИдВкладки);
			...
			Иначе
				...
				Для каждого элУзел Из Вкладка.УзлыОбновить Цикл
					Узел = элУзел.Ключ;
					Ответ = Ответ + ОтобразитьDOM(Вкладка, Узел, Узел.Родитель, Вкладка.ОбновитьУзел, , Истина);
				КонецЦикла;
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = Команда["СвойстваУзла"] Тогда
			...
			Если вклСвойства = Неопределено И ПоказатьСвойство Тогда
				вклСвойства = НоваяСтруктураВкладка(Данные, "prop", Вкладка.Данные.ИмяДанных + "  " + Узел.Родитель.Имя + ":" + СтрЭкранироватьРазметку(Данные.УзелСвойство(Узел.Родитель, "Значение"), 20), "prop", Узел.Код);
				ОбновитьСостояние(вклСвойства, Узел, "УзелОткрыт", Истина);
			КонецЕсли;

			Если Запрос.Свойство("prop") Тогда // установить значения свойств формы объекта
				фУзел = Данные.ПолучитьУзел(Запрос.propid);
				мсв = СтрРазделить(Запрос.prop, "&");
				...
				Для каждого эл Из мсв Цикл
					...
					КонецЕсли;
				КонецЦикла;
				Данные.УзелСостояниеЗначение(Узел, "Вид", Неопределено); // оптимизация
			КонецЕсли;

...
Функция НоваяВкладка(Параметры)
	...
	структВкладка = Новый Структура("ТипВкладки, ИдВкладки, Данные, Заголовок, Режим, ИдУзла, БазаДанных, ИмяДанных, ОбновитьУзел, Скролл",
		"Страница",
		ТекущаяВкладка,
		Данные,
		ИмяДанных,
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие.Имя = "edit" Тогда
			...
			структВкладка = Новый Структура("ТипВкладки, ИдВкладки, Данные, Заголовок, Режим, ИдУзла, БазаДанных, ИмяДанных, ОбновитьУзел, Скролл",
				...
				Истина,
				СкроллЗначение());
			Вкладки.Вставить(НоваяВкладка, структВкладка);
			структЗадача.Результат = ПоказатьПанель();
			...
			// ПараметрыШаблона.Вставить("ПараметрЗаголовокУзла", ПараметрЗаголовокУзла);
			// ПараметрыШаблона.Вставить("ПараметрДочернийУзел", ПараметрДочернийУзел);
			структЗадача.Результат = структЗадача.Результат + Шаблон.ПолучитьОбласть("ОбластьРедактировать", ПараметрыШаблона);
			структЗадача.Этап = "ЕстьРезультат";
			ОбновитьСостояние(Данные, Узел, "УзелОткрыт", Истина);
			Возврат Ложь;

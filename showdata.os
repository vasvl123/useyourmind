...
Функция СформироватьОтвет()
	...
	Если ОбновитьВкладки = Истина Тогда
		Ответ = Ответ + ПоказатьВкладки();
		Режим = УзелСвойство(Вкладки.Получить(ТекущаяВкладка), "Режим");
		Если Режим = "design" Тогда
			Ответ = Ответ + Шаблон.ПолучитьОбласть("ОбластьРежимРедактор");
		ИначеЕсли Режим = "view" Тогда
			Ответ = Ответ + Шаблон.ПолучитьОбласть("ОбластьРежимПросмотр");
		Иначе
			Ответ = Ответ + Шаблон.ПолучитьОбласть("ОбластьРежимСтруктура");
		КонецЕсли;
	КонецЕсли;
	...
	Для каждого элВкладка Из Вкладки Цикл
		...
		ИначеЕсли Вкладка.Режим = "image" Тогда
			...
			Представление = Шаблон.ПолучитьОбласть("ОбластьКартинка", ПараметрыШаблона);
		Иначе
			Если Вкладка.ОбновитьУзел = Истина Тогда
				Узел = Вкладка.Данные.ПолучитьУзел(Вкладка.ИдУзла);
		...
		Если Вкладка.ТипВкладки = "win" Тогда // добавить окно
			Если Вкладка.ОбновитьУзел = Истина Тогда
				...
				Представление = Шаблон.ПолучитьОбласть("ОбластьПанель", ПараметрыШаблона);
			КонецЕсли;
		КонецЕсли;
		...
		Если ОбновитьВкладки = Истина И Вкладка.ИдВкладки = ТекущаяВкладка Тогда // показать вкладку
			...
			ПараметрыШаблона.Вставить("ПараметрРежим", Вкладка.Режим);
			ПараметрыШаблона.Вставить("ПараметрТекущаяВкладка", ТекущаяВкладка);
			Ответ = Ответ + Шаблон.ПолучитьОбласть("ОбластьСкрипт", ПараметрыШаблона);
		КонецЕсли;
			...
			Если структЗадача.Результат = "" Тогда
				...
				Если СформироватьОтвет Тогда
					...
					Для каждого Данные Из мДанные Цикл
						...
						КонецЕсли;
					КонецЦикла;
					структЗадача.Результат = СформироватьОтвет();
				Иначе
					СформироватьОтвет = Неопределено;
				КонецЕсли;
			КонецЕсли;
...
Процедура ОбработатьСоединения()
	...
	Пока Истина Цикл
		...
		// обработать задачи
		Если мЗадачи.Количество() Тогда
			Если тЗадача = Неопределено Тогда
				тЗадача = 0;
			ИначеЕсли тЗадача > мЗадачи.Количество() - 1 Тогда
				тЗадача = 0;
			КонецЕсли;
			Пока НЕ тЗадача > мЗадачи.Количество() - 1 И НЕ ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 100 Цикл
				структЗадача = мЗадачи.Получить(тЗадача);

...
Функция ЗаписатьСобытие(стрЗаголовок, стрСообщение, ТипСобытия = 0, ПараметрКоманда = "") Экспорт
	Если НЕ Журнал = Неопределено Тогда
		НоваяСтрока = Журнал.НовыйДочерний(ДанныеЖурнала, Новый Структура("Имя, Значение", ТекущаяДата(), ?(ТипСобытия = 0, "Общее", ?(ТипСобытия = 1, "Успех", ?(ТипСобытия = 2, "Внимание", "Ошибка"))) + " [" + стрЗаголовок + "] " + стрСообщение), Истина);
		Журнал.Интерпретировать(Журнал.Корень);
	КонецЕсли;
...
Если АргументыКоманднойСтроки.Количество() Тогда
	...
	Попытка
		...
		Журнал.ОбновитьПредставление(Истина);

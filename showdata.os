...
Функция НачальнаяСтраница(Содержимое)
	БокПанель = Шаблон.ПолучитьОбласть(?(Субъект = "", "ОбластьПанельМеню1", "ОбластьПанельМеню2"));
...
Функция УстановитьПараметры(знПараметры)
	...
	Если зИмяДанных = "" Тогда
		зИмяДанных = "start";
	КонецЕсли;
	зИстДанных = "" + УзелСвойство(Параметры, "sdata");
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если Действие = Неопределено Тогда
		структЗадача.Запрос.Вставить("sdata", зИстДанных);
		...
		ИначеЕсли Действие = Команда["СохранитьДанные"] Тогда
			Если НЕ Запрос.Свойство("РезультатДанные") Тогда
				...
				Если НЕ Вкладка = Неопределено Тогда
					...
					ПередатьДанные(Параметры.Хост, Параметры.ПортД, Новый Структура("ИстДанных, БазаДанных, Заголовок, Команда, дДанные, ОбратныйЗапрос", зИстДанных, Вкладка.Данные.БазаДанных, Новый Структура("ИмяДанных, ТипДанных", Вкладка.Данные.ИмяДанных, 3), "ЗаписатьДанные", ПолучитьДвоичныеДанныеИзСтроки(Вкладка.Данные.СохранитьДанные()), ОбратныйЗапрос));
					Запрос.Вставить("РезультатДанные", Неопределено);
				КонецЕсли;
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
		...
		ИначеЕсли Действие = Команда["НоваяБаза"] Тогда
			...
			Если НЕ "" + БазаДанных = "" Тогда
				Если НЕ Запрос.Свойство("РезультатДанные") Тогда // отправить запрос к дата-серверу
					ОбратныйЗапрос = Новый Структура("ИдЗадачи", структЗадача.ИдЗадачи);
					ПередатьДанные(Параметры.Хост, Параметры.ПортД, Новый Структура("ИстДанных, БазаДанных, ОбратныйЗапрос, Команда", зИстДанных, БазаДанных, ОбратныйЗапрос, "ПолучитьДанные"));
					Запрос.Вставить("РезультатДанные", Неопределено);
				ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
					Если Запрос.РезультатДанные.Ответ = "Успешно" Тогда
		...
		Если Действие = Команда["Авторизация"] ИЛИ Действие = Команда["Регистрация"] Тогда
			...
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
				...
				Если ЗапросДанные.ПрошелАвторизацию Тогда
					...
					НоваяЗадача(Новый Структура("sdata, data, cmd", "admin", "expldb", "newtab"), "Служебный");

...
Функция ПоказатьПанель()
	...
	Для ИдВкладки = 0 По Вкладки.Количество()-1 Цикл
		...
		ПараметрыШаблона.Вставить("ПараметрЗаголовок", Вкладка.ИмяДанных);
...
Функция НоваяВкладка(Параметры)
	...
	структВкладка = Новый Структура("ИдВкладки, Данные, Режим, ИдУзла, БазаДанных, ИмяДанных, ОбновитьУзел, Скролл",
		ТекущаяВкладка,
		Данные,
		Параметры.Режим,
		"1",
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		Если Действие.Имя = "savedata" Тогда
			...
			структЗадача.Результат = "сохранил";
			структЗадача.Этап = "ЕстьРезультат";
			Возврат Ложь;
		КонецЕсли;

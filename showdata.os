...
Функция ВыполнитьДействия(структЗадача)
	...
	// Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = "Морфология" Тогда // запрос к серверу морфологии
			...
			Если НЕ Запрос.Свойство("РезультатДанные") Тогда
				стрЗапрос = Новый Структура("ОбратныйЗапрос, cmd", Новый Структура("ИдЗадачи, Хост, Порт", структЗадача.ИдЗадачи, Параметры.Хост, Порт), Запрос.мДействие);
...
Процедура ОбработатьСоединения()
	...
	Пока Истина Цикл
		...
		Пока к > 0 И НЕ ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50 Цикл
			к = к - 1;
			структЗадача = мЗадачи.Получить(к);
			мЗадачи.Удалить(к);

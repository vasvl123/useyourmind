	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = Команда["ПриНажатии"] Тогда
			...
			Если Узел.Имя = "О" ИЛИ Узел.Имя = "Объект" Тогда
				элСвойство = Данные.ДобавитьСвойство(Узел, Новый Структура("Имя, Значение", "Нажатие", ПолучитьИД()));
				Данные.ДобавитьСобытие(Узел, Новый Структура("Узел, Событие", элСвойство, "Нажатие"));	
			Иначе // функция, потом убрать

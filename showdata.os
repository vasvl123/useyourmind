...
Функция ПоказатьСтруктуруУзла(Вкладка, Узел, ЭтоАтрибут = Ложь, Атрибуты = "", Дочерний = "") Экспорт
	...
	Если НЕ этоАтрибут = Истина ИЛИ Узел.Код = Вкладка.ИдУзла Тогда
		ПараметрыШаблона = Новый Структура;
		ПараметрыШаблона.Вставить("ПараметрИдУзла", "" + Вкладка.ИдВкладки + "_" + Узел.Код);
	...
	Иначе
		ПараметрыШаблона = Новый Структура;
		ПараметрыШаблона.Вставить("ПараметрИдУзла", "" + Вкладка.ИдВкладки + "_" + Узел.Код);
		ПараметрыШаблона.Вставить("ПараметрКоманда", Команда["СвойстваУзла"]);
		...
		Если Вкладка.Режим = "struct" ИЛИ Вкладка.Режим = "prop" Тогда
			Если Вкладка.ОбновитьУзел = Истина Тогда
				...
				Если Вкладка.ТипВкладки = "prop" Тогда
					...
					Ответ = Шаблон.ПолучитьОбласть("ОбластьСвойство", ПараметрыШаблона) + "123";
				КонецЕсли;
			Иначе
				Ответ = "";
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие = Команда["СвойстваУзла"] Тогда
			...
			Если Запрос.Свойство("prop") Тогда // установить значения свойств объекта
				мсв = СтрРазделить(Запрос.prop, "&");
				Свойства = Данные.УзелСостояние(Узел, "Свойства");
				усв = Неопределено;

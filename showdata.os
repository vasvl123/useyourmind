		...
		Иначе // многостраничный режим
			...
			Если НЕ Данные = Неопределено Тогда
				Если структЗадача.Запрос.Свойство("РезультатДанные") Тогда
					Данные.ОбновитьДанные(структЗадача.Запрос.РезультатДанные);
				КонецЕсли;
				Команд = Данные.К;
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли структЗадача.Действие = Команда["Авторизация"]
			...
			Если НЕ Запрос.Свойство("РезультатДанные") Тогда // отправить запрос к дата-серверу
				ПередатьДанные(Параметры.Хост, Параметры.ПортД, Новый Структура("ОбратныйЗапрос, cmd", Запрос, Запрос.cmd));
				Запрос.Вставить("РезультатДанные", Неопределено);
			ИначеЕсли НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
				ПрошелАвторизацию = Запрос.РезультатДанные.Результат.ПрошелАвторизацию;
				Если ПрошелАвторизацию Тогда
					Если ПередатьДанные(Параметры.Хост, Параметры.ПортС, Запрос.РезультатДанные.Результат) Тогда // запустить новый процесс контроллера
						ОбщийРезультат = ОбщийРезультат + Шаблон.ПолучитьОбласть("ОбластьПеренаправить", Новый Структура("ПараметрПуть, ПараметрПауза, ПараметрТекст", "/showdata/" + Запрос.procid, 10, "success"));
					КонецЕсли;
				КонецЕсли;
				Возврат Истина;
			КонецЕсли;

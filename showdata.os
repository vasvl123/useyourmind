...
Функция НоваяВкладка(Параметры)
	БазаДанных = "" + УзелСвойство(Параметры, "БазаДанных");
	ИмяДанных = "" + УзелСвойство(Параметры, "ИмяДанных");
	Данные = Новый pagedata(ОбъединитьПути(ТекущийКаталог(), "data", БазаДанных + ".files", Параметры.ФайлДанных));
	ТекущаяВкладка = Вкладки.Количество();
	структВкладка = Новый Структура("ТипВкладки, Данные, Заголовок, Режим, ИдУзла, БазаДанных, ИмяДанных, ОбновитьУзел, Скролл",
		"data",
		Данные,
		ИмяДанных,
		Параметры.Режим,
		...
		Для каждого элВкладка Из Вкладки Цикл
			Если ИндексВкладки = ТекущееОкно ИЛИ ИндексВкладки = ТекущаяВкладка Тогда
				...
				Если Вкладка.ОбновитьУзел = Истина Тогда
					Ответ = "<div id='" + Вкладка.ТипВкладки + "'>" + Ответ + Шаблон.ПолучитьОбласть("ОбластьСкролл", Новый Структура("ПараметрСкролл", СкроллЗначение(Вкладка.Скролл, Вкладка.Режим))) + "</div>";
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		Если Действие.Имя = "savedata" Тогда
			Данные.СохранитьДанные(ОбъединитьПути(ТекущийКаталог(), "data", Вкладка.БазаДанных + ".files", Запрос.datafile));
		...
		ИначеЕсли Действие.Имя = "edit" Тогда
			...
			// ПараметрыШаблона.Вставить("ПараметрЗаголовокУзла", ПараметрЗаголовокУзла);
			// ПараметрыШаблона.Вставить("ПараметрДочернийУзел", ПараметрДочернийУзел);
			структЗадача.Результат = Шаблон.ПолучитьОбласть("ОбластьРедактировать", ПараметрыШаблона);
			ОбновитьСостояние(Данные, Узел, "УзелОткрыт", Истина);
			Возврат Истина;

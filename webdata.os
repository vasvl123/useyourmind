...
Функция ПередатьДанные(Хост, Порт, стрДанные) Экспорт
	...
	Исключение
		...
		Если Соединение = Неопределено Тогда
			Сообщить("webdata: Хост недоступен: " + Хост + ":" + Порт);
		Иначе
			Соединение.Закрыть();
			...
			Если НЕ Запрос.РезультатДанные = Неопределено Тогда // получен ответ дата-сервера
				...
				Иначе
					Если Запрос.РезультатДанные.Ответ = "Успешно" Тогда
						Если УзелСвойство(Запрос.РезультатДанные.Результат, "ТипДанных") = "2" Тогда // это файл
							ПутьДанные = Запрос.ИмяДанных;
							ВсеДанные.Удалить(ПутьДанные);
							структЗадача.ЗадачаВладелец.Результат = Запрос.РезультатДанные.Результат.Данные;
							структЗадача.ЗадачаВладелец.Этап = "ЕстьРезультат";
						Иначе // это структура данных
...
Функция ЗаписатьСобытие(стрЗаголовок, стрСообщение, ТипСобытия = 0, ПараметрКоманда = "") Экспорт
	...
	ПередатьДанные(Параметры.Хост, Параметры.ПортД, Новый Структура("БазаДанных, Заголовок, Команда", "log", Новый Структура("Источник, Субъект, Тип, Сообщение", стрЗаголовок, "Гость", ТипСобытия, стрСообщение), "ЗаписатьЗаголовок"));

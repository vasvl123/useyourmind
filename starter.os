...
стрКонтроллер = Новый Структура("procid, Хост, Порт, ПортС, ПортВ, ПортД, Субъект, Локальный, ПараметрХост, cmd", "0", Хост, ПортК, Порт, ПортВ, ПортД, "Гость", Локальный, ПараметрХост, "init");
			...
			Если ДанныеВходящие.Свойство("cmd", cmd) Тогда
				...
				Если cmd = "auth" ИЛИ cmd = "reg" Тогда
					...
					Если ДанныеВходящие.Свойство("ПрошелАвторизацию", ПрошелАвторизацию) Тогда
						Если ПрошелАвторизацию Тогда // запустить новый контроллер
							НовыйПортК = ПортК - 1;
							Пока НЕ ЗапуститьПроцесс("showdata.os", НовыйПортК) Цикл
								НовыйПортК = НовыйПортК - 1;
							КонецЦикла;
							стрКонтроллер = Новый Структура("procid, Хост, Порт, ПортС, ПортВ, ПортД, Субъект, Локальный, ПараметрХост, cmd", ДанныеВходящие.procid, Хост, НовыйПортК, Порт, ПортВ, ПортД, ДанныеВходящие.Субъект, Локальный, ПараметрХост, "init");
				...
				ИначеЕсли cmd = "stopserver" ИЛИ cmd = "restartserver" Тогда
					Если ДанныеВходящие.Свойство("procid") Тогда // процесс завершен
						...
						КонецЕсли;
					ИначеЕсли Локальный Тогда
						// Завершить процессы
						Для каждого элКонтроллер Из Контроллеры Цикл
							ПередатьДанные(элКонтроллер.Значение.Хост, элКонтроллер.Значение.Порт, Новый Структура("cmd", "stopserver"));
						КонецЦикла;

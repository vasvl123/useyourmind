...
стрКонтроллер = Новый Структура("procid, Хост, Порт, ПортС, ПортВ, ПортД, Субъект, cmd", "0", Хост, ПортК, Порт, ПортВ, ПортД, "Гость", "init");
			...
			Если ДанныеВходящие.Свойство("cmd", cmd) Тогда
				...
				Если cmd = "auth" ИЛИ cmd = "reg" Тогда
					...
					Если ДанныеВходящие.Свойство("ПрошелАвторизацию", ПрошелАвторизацию) Тогда
						Если ПрошелАвторизацию Тогда // запустить новый контроллер
							...
							КонецЦикла;
							стрКонтроллер = Новый Структура("procid, Хост, Порт, ПортС, ПортВ, ПортД, Субъект, cmd", ДанныеВходящие.procid, Хост, НовыйПортК, Порт, ПортВ, ПортД, ДанныеВходящие.Субъект, "init");
				...
				ИначеЕсли cmd = "stopserver" ИЛИ cmd = "restartserver" Тогда
					...
					ИначеЕсли Локальный Тогда
						...
						Если cmd = "restartserver" И Локальный Тогда

...
Пока Истина Цикл
	с0 = ПередатьДанные(Хост, ПортВ, Новый Структура("Хост, Порт, ПортС, ПортД, cmd", Хост, ПортВ, Порт, ПортД, "init"));
	Прервать;
	Если НЕ с0 = Неопределено Тогда
		Если с0.Статус = "Успех" Тогда
			Прервать;
		КонецЕсли;
	КонецЕсли;
	Приостановить(200);
			...
			Если ДанныеВходящие.Свойство("cmd", cmd) Тогда
				...
				Если cmd = "startproc" Тогда
					Если Контроллеры.Получить(ДанныеВходящие.procid) = Неопределено Тогда
						...
						стрКонтроллер = Новый Структура("procid, Хост, Порт, ПортС, ПортВ, ПортД, ПортМ, Субъект, Локальный, ПараметрХост, sdata, data, УдаленныйУзел, cmd", ДанныеВходящие.procid, Хост, ПортК, Порт, ПортВ, ПортД, ПортМ, "", Локальный, ПараметрХост, ДанныеВходящие.sdata, ДанныеВходящие.data, ДанныеВходящие.УдаленныйУзел, "init");

			...
			Если ДанныеВходящие.Свойство("cmd", cmd) Тогда
				...
				Если cmd = "startproc" Тогда
					Если Контроллеры.Получить(ДанныеВходящие.procid) = Неопределено Тогда
						...
						КонецЦикла;
						стрКонтроллер = Новый Структура("procid, Хост, Порт, ПортС, ПортВ, ПортД, Субъект, Локальный, ПараметрХост, data, cmd", ДанныеВходящие.procid, Хост, НовыйПортК, Порт, ПортВ, ПортД, "Гость", Локальный, ПараметрХост, ДанныеВходящие.data, "init");

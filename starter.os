...
Порт1Ск = 8885; // Порт конфигураций 1С
Порт1С = 8884; // Порт сервера 1С
		...
		Если cmd = "startproc" Тогда
			Если НЕ showdata = Неопределено Тогда
				Если Контроллеры.Получить(ДанныеВходящие.procid) = Неопределено Тогда
					...
					стрКонтроллер = Новый Структура("procid, Хост, Порт, ПортС, ПортВ, ПортД, ПортМ, Порт1С, УдаленныйУзел, Локальный, ПараметрХост, cmd", ДанныеВходящие.procid, Хост, ПортК, Порт, ПортВ, ПортД, ПортМ, Порт1С, "", Локальный, ПараметрХост, "init");

				...
				Если cmd = "startproc" Тогда
					Если Контроллеры.Получить(ДанныеВходящие.procid) = Неопределено Тогда
						...
						Иначе
							...
							КонецЦикла;
						КонецЕсли;
						стрКонтроллер = Новый Структура("procid, Хост, Порт, ПортС, ПортВ, ПортД, Субъект, Локальный, ПараметрХост, data, УдаленныйУзел, cmd", ДанныеВходящие.procid, Хост, НовыйПортК, Порт, ПортВ, ПортД, "", Локальный, ПараметрХост, ДанныеВходящие.data, ДанныеВходящие.УдаленныйУзел, "init");

	...
	Если структЗадача.Этап = "СформироватьОтвет" Тогда
		Если НЕ ТекущаяВкладка = Неопределено Тогда
			...
			Если Вкладка.ОбновитьУзел = Истина Тогда
				Ответ = "<div id='data'>" + Ответ + Шаблон.ПолучитьОбласть("ОбластьСкролл", Новый Структура("ПараметрСкролл", Вкладка.Скролл)) + "</div>";
...
Функция ВыполнитьДействия(структЗадача)
	...
	Если НЕ Действие = Неопределено Тогда
		...
		ИначеЕсли Действие.Имя = "УзелПросмотр" Тогда
			...
			структВкладка = Новый Структура("ИдВкладки, Данные, Режим, ИдУзла, Заголовок, ОбновитьУзел, Скролл", ТекущаяВкладка, Данные, "view", Узел.Код, Узел.Имя, Истина, 0);
		...
		ИначеЕсли Действие.Имя = "edit" Тогда
 			ТекущаяВкладка = ТекущаяВкладка + 1;
			структВкладка = Новый Структура("ИдВкладки, Данные, Режим, ИдУзла, Заголовок, ОбновитьУзел, Скролл", ТекущаяВкладка, Данные, "node", Узел.Код, Узел.Имя, Истина, 0);
			Вкладки.Вставить(ТекущаяВкладка, структВкладка);
			структЗадача.Результат = ПоказатьПанель();
			структЗадача.Этап = "ЕстьРезультат";
			ОбновитьСостояние(Данные, Узел, "ОбновитьУзел", Истина);

...
Функция ПоказатьСтруктуруУзла(Данные, Запрос, КодУзла = Неопределено, этоАтрибут = Ложь, ПервыйВызов = Истина) Экспорт
	...
	Если Узел.Свойство("Дочерний") Тогда
		КнопкиУзла = Шаблон.ПолучитьОбласть("ОбластьКнопкаУзел",
	...
	Если УзелИзменить = Истина 	Тогда
		КнопкиУзла = КнопкиУзла +
			Шаблон.ПолучитьОбласть("ОбластьКнопкаУзел", Новый Структура("ПараметрИдУзла, ПараметрИдПроцесса, ПараметрРежим, ПараметрКоманда, ПараметрНадписьНаКнопке",
				Узел.Код,
				Запрос.procid,
				Запрос.mode,
	...
	Иначе
		// КнопкиУзла = КнопкиУзла +
		// 	Шаблон.ПолучитьОбласть("ОбластьКнопкаУзел",
		// 		Новый Структура("ПараметрИдУзла, ПараметрИдПроцесса, ПараметрРежим, ПараметрКоманда, ПараметрНадписьНаКнопке",
		// 			Узел.Код,
		// 			Запрос.procid,
		// 			Запрос.mode,
	...
	Если Узел.Свойство("Значение") Тогда
		ПараметрИмяУзла = КнопкиУзла +
			Шаблон.ПолучитьОбласть("ОбластьКнопкаИмяЗначение",
				Новый Структура("ПараметрИдУзла, ПараметрНеактивно, ИдПроцесса, ПараметрРежим, ПараметрКоманда1, ПараметрКоманда2, ПараметрИмя, ПараметрЗначение",
					Узел.Код,
					"disabled",
					Запрос.procid,
					Запрос.mode,
	...
	Иначе
		ПараметрИмяУзла = КнопкиУзла +
			Шаблон.ПолучитьОбласть("ОбластьКнопкаИмя",
				Новый Структура("ПараметрИдУзла, ПараметрНеактивно, ПараметрИдПроцесса, ПараметрРежим, ПараметрКоманда, ПараметрНадписьНаКнопке",
					Узел.Код,
					"",
					Запрос.procid,
					Запрос.mode,

...
Перем Хост, Порт;
Перем Вкладки, ИдВкладки, ТекущаяВкладка, ОбновитьВкладки;
...
Функция ПоказатьПанель()
	...
	Для каждого элВкладка Из Вкладки Цикл
		...
		ПараметрыШаблона.Вставить("ПараметрКоманда", "tabopen");
		...
		КонецЕсли;
	КонецЦикла;
	ПараметрыШаблона.Вставить("ПараметрРежим", "");
	ПараметрыШаблона.Вставить("ПараметрВкладка", "newpage");
...
Функция НоваяСтраница()
	...
	Текст = Шаблон.ПолучитьОбласть("ОбластьНоваяСтраница", Новый Структура("ПараметрИдВкладки", ИдВкладки));
...
Функция НачальнаяСтраница()
	...
	//Текст = Текст + ПоказатьПанель();
	//Текст = Текст + НоваяСтраница();
...
Функция ПоказатьСтруктуруУзла(Данные, Запрос, КодУзла = Неопределено, ПервыйВызов = Истина) Экспорт
	...
	Если НЕ УзелСвойство(Узел, "Дочерний") = Неопределено Тогда
		...
		ПараметрыШаблона.Вставить("ПараметрВкладка", Запрос.tab);
		...
		Иначе
			...
			Иначе
				...
				Если НЕ tab = Неопределено Тогда
					...
					Иначе
						data = Запрос.data;
						Данные = Новый pagedata("" + Запрос.data + ".txt", Шаблон);

...
Функция ПоказатьСтруктуруУзла(Данные, Запрос, КодУзла = Неопределено, этоАтрибут = Ложь) Экспорт
	...
	Если Узел.Свойство("Значение") Тогда
		ПараметрИмяУзла = Шаблон.ПолучитьОбласть("ОбластьКнопкаИмяЗначение",
			Новый Структура("ПараметрИдУзла, ПараметрИдПроцесса, ПараметрРежим, ПараметрИмя, ПараметрЗначение",
				Узел.Код,
				Запрос.procid,
				Запрос.mode,
				УзелИмя,
	...
	Если этоАтрибут Тогда
		Если Узел.Свойство("Соседний") Тогда
			ПараметрИмяУзла = ПараметрИмяУзла + ПоказатьСтруктуруУзла(Данные, Запрос, Узел.Соседний, Истина);
		КонецЕсли;
	...
	Если Узел.Свойство("Дочерний") Тогда
		ПараметрНадписьНаКнопке = ?(Узел.nodeopen, "○" , "◉");
	КонецЕсли;
